...
- Comparative matrix — **do not edit the table**. It is generated by
  ...
  frontmatter. Add that block to the new report with all eleven keys
  (`memory_unit`, `storage`, `retrieval`, `write`, `update_delete`, `scoping`,
  `integration`, `background`, `trust`, `strengths`, `risks`), then run
  `npm run build`. `npm test` fails if the table is out of sync.
