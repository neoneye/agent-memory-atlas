...
**The order is argued from failure modes, not measured, and it has a case where
it does not hold.** It is derived from what breaks in this corpus — extraction
that was wrong with nothing underneath it, scope retrofitted into a store that
already had rows — and no reference implementation exists to adjudicate it,
which is why that absence is listed below rather than buried. The case it does
not fit is the one where **retrieval quality is the product rather than a
property of it**. A tool whose whole value is finding the passage a lexical query
