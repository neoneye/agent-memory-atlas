...
**[KITE](../systems/memoket-kite/) is the family's vector-free member, and the one whose benchmark machinery is worth more than its memory.** Memoket's Apache-2.0 library declares zero runtime dependencies and stores a memory as one XML artifact: an LLM turns each session into dated facts under a controlled topic and entity vocabulary, keeping the raw utterances beside them, and a question is compiled into three candidate JSON plans — `select`, `where`, a `pipe` of sort and head — validated against that vocabulary, and the one returning the most rows is executed over posting lists with a relaxation ladder that records which constraints it dropped and abstains rather than return the corpus. Contradiction is settled at read time and nowhere else: the plan language can sort on event time and take the head, the library's default compile prompt never shows that operator, and the default path orders the evidence pack by date and tells the reader the most recent contradicting row wins. The public API is load, remember, recall and answer, and `grep -rn -i "def forget\|def delete\|def supersede" src/` matches nothing, so a wrong extraction is permanent and an erasure request has no path through the library. What surrounds the claim is unusually rigorous — dataset revisions and SHA-256s pinned in a manifest, judged rows sealed by digest, a verifier that recomputes the published metric from the sealed bytes rather than a re-opened file because "recomputing from a re-opened file would accept any edit to the verdicts that preserved the row count", and a contamination gate that scans every shipped prompt string for terms concentrated in a small fraction of a benchmark corpus. Two things stop a reader using any of it. The judged rows the manifest names are not committed and the release they are meant to be attached to carries no assets, so the verifier runs only for its author; and the two headline scores come from a shared benchmark baseline that turns on eleven switches the library default leaves off, and from per-benchmark bindings with their own prompts, kind vocabularies and seed taxonomies — LoCoMo's enables an inference pass under a comment reading "Its QA set contains no unanswerable questions, so a bounded inference pass can only recover an answer, never invent a refusal" — while the library's own default profile declares none of them and the pipeline reads every one with a `getattr` that defaults it off.
...
**[ThoughtDAG](../systems/thoughtdag/) makes the person's wires the retrieval, and records what the model was shown rather than what it concluded.** An editable context graph — desktop app, DeepSeek Harness plugin, a read-only CLI over MCP — where a generation's input is a deterministic walk of the nodes wired into the question, the same function builds the panel's *will send* preview and the request, every answer version carries the model, the time and a fingerprint of its upstream, and a `commit` event written at dispatch carries the SHA-256 of the exact request into an append-only, metadata-only canvas log that survives undo and travels in backups. A stale answer is one whose upstream fingerprint drifted; it stays in downstream context with a mark, and a decision model that rules the change irrelevant hides the mark from the preview while the request keeps it. The memory a model writes is two profile documents, preferences and identity, admitted by a constitution in code and then merged by a model rewrite the constitution does not re-check, and one dossier per topic whose sentences cite the turns they rest on. Session Atlas mirrors Claude Code, Codex, DeepSeek Harness and Pi sessions read-only; the why layer indexes what each turn did, and recall brings dossiers and excerpts from every project on the machine into an ask. Two marks — the event log, and a hidden-reads case beside a benchmark whose conditions are graph operations and whose status file records its own corrections. Withheld, each a near miss: stale is a label not a gate, archive excludes on the canvas and not in the index recall reads, versions carry record time and no validity, the memory inbox is a gate the judge's topic probability routes around, and a working-directory filter exists that no caller passes.
...
**[basemode](../systems/basemode/) is the family's RDF member, and its source comments audit its own wiring with dates and quad counts.** A Rust binary maps a workspace — projects, decisions, rules, handoffs, and an AST-derived graph of the code's own functions, callers and imports — into oxigraph over one N-Quads file per tier, and injects a slice at each of the four Claude Code hook moments rather than waiting to be called. Correction is a supersession edge with a forward-walking `resolve_head`, a write-time cycle refusal, and a `FILTER NOT EXISTS` spliced inside the GRAPH group of recall, both rules blocks and the prompt neighbourhood. `src/supersede.rs`'s header records that `ops:supersedes` and `ops:supersededBy` had been declared in the ontology for months *"and **nothing has ever written either one** — measured on Chris's store, 2026-09-07: 0 quads of each across both tiers"*, with the only supersession being a sync-ledger JSON field *"which no graph reader can see"*. The module exists to end that, and it is wired — a CLI verb produces the edge and six queries filter on it. The same header separates the edge from the label and justifies distrusting `ops:status` with a census of the author's own store — 44 distinct values against the six the ontology declares, including `Pass` 242, `PASS` 43 and `PASS (no change)` 1. The label is a lifecycle field with no candidate or verified value, which is why `trust_state` is withheld, and it is read: the writer adds `superseded` without removing `active`, so the session-start memory block, which selects `active`, serves a corrected note beside its correction. Two marks, `audit_log` on a `changes.jsonl` appended after the atomic rename lands from the single write funnel, and `negative_eval` on test files whose header states the vacuity rule — *"a filter that excludes everything passes a 'the old one is gone' assertion just as well as a correct one"* — and assert both halves per serving surface, which makes it a third corpus instance of an exclusion asserted about a *corrected* value. Scope is the near-miss: writes route into a per-workspace named graph that no hook query filters on, and the one read-path predicate keeps the working set's projects to the current workspace by a home derived from `ops:path`. Bug #112 is the named-graph difference biting the removal path — a wildcard guard passed a rule sitting in a foreign named graph, *"460 quads of them on the reporting install"*, while the scoped DELETE matched nothing and a hardcoded `Ok(1)` reported success. Licence is the Functional Source License 1.1 with an Apache-2.0 future licence: source-available at this pin.
...
**[Memex](../systems/memex-zettel/) puts its care into a gate that refuses a
secret without quoting it, and leaves correction to a file move.** A Zettelkasten
of markdown cards for coding agents, served through a CLI, an MCP server and
harness plugins, it screens every agent-facing write and every search query for
private keys, token shapes and credential assignments, and tests what the gate
must allow as well as what it must refuse. The gate belongs to the `write`
command rather than the store, so three importers write past it. Archiving moves
a card, records nothing about why, and silently replaces an earlier archived card
of the same slug.
