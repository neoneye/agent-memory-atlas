...
PLACEHOLDER_PATTERN_TOMBSTONE_COUNT systems of PLACEHOLDER_TOTAL_COUNT carry a value-level tombstone, and the corpus is an opportunistic collection rather
...
retrieval assertion is carried by PLACEHOLDER_PATTERN_NEGATIVE_EVAL_COUNT repositories of PLACEHOLDER_TOTAL_COUNT, and they are not all
...
*scope boundary*, and three are recorded asserting it about a value that was
*corrected* — [Verel](../../systems/verel/); [Memora](../../systems/memora/),
whose suite asserts that a superseded memory must not appear under default
search or default list and that the explicit forensic mode still returns it; and
[basemode](../../systems/basemode/), which pairs one test per serving surface
against a fixture holding a note and its correction, and states the reason for
the pairing in the file's own header — *"a filter that excludes everything
passes a 'the old one is gone' assertion just as well as a correct one."*
...
negative retrieval assertion count — PLACEHOLDER_PATTERN_NEGATIVE_EVAL_COUNT systems of PLACEHOLDER_TOTAL_COUNT — is live, checked against report frontmatter on every build. Thirty-seven
...
- **Two axes the seven marks do not carry, named by a survey that coded 435
  ...
  **Recoverability** asks whether a decision taken on a bad record can be traced
  ...
  So rarity is not what keeps recoverability off the list: the 27 of 441 that
