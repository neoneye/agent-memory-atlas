...
Three readers, all expert: a **senior engineer evaluating memory systems**, the **developers of the repository under review**, and a **coding agent navigating the jungle of memory systems** on behalf of one of the first two. Each can act on a finding immediately — the developer opens the exact file and fixes it; the agent quotes the exact slug, path, symbol and capability mark, and cross-references the matrix — and each is ill-served by a gloss that a plain-language rewrite would insert between them and the evidence. So the density is deliberate. Name the file, the symbol, the config key and the constant; do not stop to define `FTS5`, compaction, or a capability mark for a newcomer. Stable, greppable identifiers and checkable claims are what the agent reader keys on and what the human readers verify, so precision is machine-actionable and human-actionable at once. A report that reads as a wall to someone who does not work on memory systems is aimed correctly, not written badly. This is not a beginner tutorial, and it should not be softened into one; the executive summary's job is to let one of these three readers decide whether to keep reading, not to onboard a fourth.
