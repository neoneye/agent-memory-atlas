...
description: A code-grounded comparison of one hundred and thirty-three agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
**Why the two counts differ.** The atlas holds **133 reports across 132
...
*repositories* are 132; neither is a rounding error.
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 72 of 133 systems here have, and is what the mark means. A scope
...
One hundred and thirty-three systems do not fall into one hundred and thirty-three categories. They cluster around
...
`project-golem`, `openyak`, `memento`, `palazzo`, `memex-zero-rag`, `agentrecall-x`, `terse-memory`, `ean-agentos`
...
`neo4j-agent-memory`, `memary`, `m-flow`
...
*4 of 133:* [`daimon`](../systems/daimon/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
*17 of 133:* [`agentic-context-engine`](../systems/agentic-context-engine/), [`agentrecall-x`](../systems/agentrecall-x/), [`aura`](../systems/aura/), [`clio`](../systems/clio/), [`core-memory`](../systems/core-memory/), [`daimon`](../systems/daimon/), [`gini-agent`](../systems/gini-agent/), [`graphify`](../systems/graphify/), [`magic-context`](../systems/magic-context/), [`memledger`](../systems/memledger/), [`npcpy`](../systems/npcpy/), [`rainbox`](../systems/rainbox/), [`terse-memory`](../systems/terse-memory/), [`tigrimosr`](../systems/tigrimosr/), [`tokenmizer`](../systems/tokenmizer/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)

**Bi-temporal validity** — When a fact was true tracked separately from when the system recorded or expired it.

*9 of 133:* [`agent-memory-supabase`](../systems/agent-memory-supabase/), [`atomic-agent`](../systems/atomic-agent/), [`core-memory`](../systems/core-memory/), [`gini-agent`](../systems/gini-agent/), [`graphiti`](../systems/graphiti/), [`memory-engine`](../systems/memory-engine/), [`memvid`](../systems/memvid/), [`neo4j-agent-memory`](../systems/neo4j-agent-memory/), [`verel`](../systems/verel/)
...
*21 of 133:* [`aukora-kernel`](../systems/aukora-kernel/), [`aura`](../systems/aura/), [`csm`](../systems/csm/), [`ctx`](../systems/ctx/), [`daimon`](../systems/daimon/), [`lethe`](../systems/lethe/), [`lorekit`](../systems/lorekit/), [`magic-context`](../systems/magic-context/), [`mem0`](../systems/mem0/), [`mem0sharp`](../systems/mem0sharp/), [`memledger`](../systems/memledger/), [`memora`](../systems/memora/), [`memvid`](../systems/memvid/), [`neko`](../systems/neko/), [`optmem`](../systems/optmem/), [`palazzo`](../systems/palazzo/), [`simplemem`](../systems/simplemem/), [`soul-of-waifu`](../systems/soul-of-waifu/), [`tokenmizer`](../systems/tokenmizer/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
*27 of 133:* [`acontext`](../systems/acontext/), [`agentswarms`](../systems/agentswarms/), [`agno`](../systems/agno/), [`clio`](../systems/clio/), [`core-memory`](../systems/core-memory/), [`cortex`](../systems/cortex/), [`daimon`](../systems/daimon/), [`deer-flow`](../systems/deer-flow/), [`dexto`](../systems/dexto/), [`empryo`](../systems/empryo/), [`engram`](../systems/engram/), [`hermes-agent`](../systems/hermes-agent/), [`juggler`](../systems/juggler/), [`llm-wiki-memory`](../systems/llm-wiki-memory/), [`lorekit`](../systems/lorekit/), [`memanto`](../systems/memanto/), [`memora`](../systems/memora/), [`mercury-agent`](../systems/mercury-agent/), [`npcpy`](../systems/npcpy/), [`openyak`](../systems/openyak/), [`qwen-code`](../systems/qwen-code/), [`rainbox`](../systems/rainbox/), [`risuai`](../systems/risuai/), [`second-me`](../systems/second-me/), [`tigrimosr`](../systems/tigrimosr/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)

**Negative retrieval assertion** — Committed evaluation cases asserting that particular material must *not* be retrieved.

*17 of 133:* [`agent-afk`](../systems/agent-afk/), [`agno`](../systems/agno/), [`aukora-kernel`](../systems/aukora-kernel/), [`crewai`](../systems/crewai/), [`csm`](../systems/csm/), [`daimon`](../systems/daimon/), [`everos`](../systems/everos/), [`graphify`](../systems/graphify/), [`helm`](../systems/helm/), [`lethe`](../systems/lethe/), [`mirix`](../systems/mirix/), [`mnemopi`](../systems/mnemopi/), [`neko`](../systems/neko/), [`open-cowork`](../systems/open-cowork/), [`pydantic-ai-harness`](../systems/pydantic-ai-harness/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
**Negative evidence is almost never tested.** Sixteen repositories of one hundred and thirty-three
...
disputed or forbidden stays gone. Eleven of one hundred and thirty-three is the real figure for the
...
polluting a research agent's recall. Across one hundred and thirty-three systems, almost none can
...
- Re-read 2026-07-31 at [`6f462b8a5a5d8c33c1d624824668aff8ab67ebca`](https://github.com/SyntheticAutonomicMind/CLIO/commit/6f462b8a5a5d8c33c1d624824668aff8ab67ebca): the unset-identity defect this entry reported is fixed in [`7af1d1cf8fd3ec6c5a8f5ddd39ace991d2979d6a`](https://github.com/SyntheticAutonomicMind/CLIO/commit/7af1d1cf8fd3ec6c5a8f5ddd39ace991d2979d6a), whose test file cites this atlas as where it was flagged. The same commit fixed a second defect the atlas had missed and that was blocking the same mechanism from the other side: `add_corroboration`, `promote_entry` and `get_entry_tier` used the singular `entry_type` filter directly as the storage key while entries live under plural keys, so every type-filtered call returned "No entry matching". One defect was visible from a grep and the other only from running the property.
...
- Re-read 2026-08-03 at [`4f995e08349ebb0e0453786d5aad999737ddd239`](https://github.com/jihadkhawaja/mem0sharp/commit/4f995e08349ebb0e0453786d5aad999737ddd239), six commits on and roughly doubled. Nothing published went stale: the history table is still INSERT-only, and it has converged on Mem0's exact column list — `updated_at`, `is_deleted`, `actor_id` and `role` added by migration — so the two implementations now carry the same audit schema. The single `UPDATE` against history in the repository is a one-time backfill inside that migration. Added since: a Qdrant store, four rerankers, and the behaviour switch above.
...
- Re-read 2026-07-31, fifteen commits on: every published claim still holds, and the project shipped a second binary. The `tigrim` CLI scopes skills, persona, settings and chat history to a `.tigrimos` folder in the launch directory — a cleaner boundary than the project-id filter — and never calls `skill_synthesizer::start_cron`, so the mechanism this entry exists for does not run there. The read path also gained a filter that drops assistant turns starting `"Error: "` from the model's view of history, because models were imitating a dead provider's quota message verbatim.
...
- Daimon was re-reviewed on 2026-07-30 at [`3f79a952cf8e7f96b7fbcaa322147a7236dd47d0`](https://github.com/Daily-Nerd/daimon/commit/3f79a952cf8e7f96b7fbcaa322147a7236dd47d0), 29 commits past the previous pin, and the report was rewritten rather than amended. Three claims it had published were no longer true: the tombstone key is canonical rather than literal text, a re-assertion test exists, and committed negative-retrieval cases exist. The mark count moved from five of seven to six. Two of the three stale claims were *criticisms* — the report faulted the system for gaps it had since closed — which is the failure direction a drifting pin produces most often and the one least likely to be reported by a reader.
- Daimon was re-read again on 2026-08-03 at [`3025ee3edecd1958e9e9181fe607a5b1a30309bf`](https://github.com/Daily-Nerd/daimon/commit/3025ee3edecd1958e9e9181fe607a5b1a30309bf), 41 commits on, and this time the outcome was the TigrimOSR shape rather than the July one: the mechanism did not move, no mark changed, and the deletion protocol is byte-identical. **One published claim was stale and it was a detail with a consequence** — the report gave the item id as a 6-hex slice minted in `store.py`. It is now 12 hex through a `(12, 16, 24, 40)` ladder in `policy.stamp_item_ids`, because the project computed that 6 hex collides at ~2.4% over ~2k texts per project, cross-session collisions are undetectable by construction, and the consequence is `forget` silently withholding an unrelated live memory. The atlas had published the narrow width as part of praising the tombstone and had not noticed that the width was the tombstone's weakest link. This is the third re-review in a row where the stale claim was found by reading rather than by any signal a pin comparison could produce.
- **Three entries in the repositories-inspected list above were stale**, found while re-pinning Daimon on 2026-08-03: Daimon, Verel and Swafra each carried a commit older than the one their own report was pinned to, because a re-review updates the report's frontmatter and the list is edited by hand. All three now match. The class of error matters more than the instances — the published list is the atlas's claim about what was actually read, `npm test` validates revision metadata inside reports but never checked it against this list, and nothing would have surfaced the drift except looking. Every stale entry was on a *re-reviewed* system, which is the population where the atlas's own process is the cause.
- TigrimOSR was re-reviewed on 2026-07-31 at [`0813f2eb9f28a07e9adb6231f3de4f09803b66b4`](https://github.com/Sompote/TigrimOSR/commit/0813f2eb9f28a07e9adb6231f3de4f09803b66b4), fifteen commits past the previous pin, and **nothing published about it had gone stale** — the report was extended rather than corrected. Worth recording because it is the opposite outcome from Daimon below and the more common one: a pin drifts, the mechanism does not move, and what changed is elsewhere. Here the elsewhere was a whole second binary. The new `tigrim` CLI scopes skills, persona, settings and chat history to a `.tigrimos` directory in the folder it launches in, and never starts the skill synthesizer, so the propose-stage-approve mechanism this atlas reports on is absent from the mode a new user is most likely to try. A freshness check that compares commit ids would have flagged this repository; only reading it says which of the two outcomes it was.
