...
description: A code-grounded comparison of two hundred and eighty-two agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
3. **Scope is the most implemented mechanism and the shallowest.**
   155 of 282 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 70 of 282 commit a case
...
5. **Trust is usually a number, not a state.** 56 of 282 record a discrete
...
**Why the two counts differ.** The atlas holds **282 reports across 281
...
so a count of *systems* is 282 and a count of *repositories* is 281 — the gap is
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 155 of 282 systems here have, and is what the mark means. A scope
...
Two hundred and eighty-two systems do not fall into one hundred and seventy categories. They cluster around
...
[capability index](../capabilities/) filters all 282 systems by the seven
...
**Negative evidence is almost never tested.** Seventy repositories of two hundred and eighty-two
...
disputed or forbidden stays gone. Twelve of two hundred and eighty-two is the real figure for the
...
polluting a research agent's recall. Across two hundred and eighty-two systems, almost none can
