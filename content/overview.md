...
description: A code-grounded comparison of two hundred and fifty-seven agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
257 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 13 systems of 257 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   140 of 257 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 62 of 257 commit a case
...
5. **Trust is usually a number, not a state.** 50 of 257 record a discrete
...
**Why the two counts differ.** The atlas holds **257 reports across 256
...
tombstones, sixty-two negative-eval suites — are counts *over inspectable code*, and a
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 140 of 257 systems here have, and is what the mark means. A scope
...
Two hundred and fifty-seven systems do not fall into one hundred and sixty-six categories. They cluster around
...
[`daimon`](../systems/daimon/), [`intaris`](../systems/intaris/), [`muninndb`](../systems/muninndb/), [`omniintelligence`](../systems/omniintelligence/)
...
[capability index](../capabilities/) filters all 257 systems by the seven
...
**Negative evidence is almost never tested.** Sixty-two repositories of two hundred and fifty-seven
...
disputed or forbidden stays gone. Twelve of two hundred and fifty-seven is the real figure for the
...
polluting a research agent's recall. Across two hundred and fifty-seven systems, almost none can
