...
description: A code-grounded comparison of two hundred and seventeen agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
217 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 11 systems of 217 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   126 of 217 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 47 of 217 commit a case
...
5. **Trust is usually a number, not a state.** 42 of 217 record a discrete
...
**Why the two counts differ.** The atlas holds **217 reports across 216
...
tombstones, forty-seven negative-eval suites — are counts *over inspectable code*, and a
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 126 of 217 systems here have, and is what the mark means. A scope
...
Two hundred and seventeen systems do not fall into one hundred and sixty-six categories. They cluster around
...
[capability index](../capabilities/) filters all 217 systems by the seven
...
*11 of 217:* [`daimon`](../systems/daimon/), [`memsem`](../systems/memsem/), [`mnemosyne`](../systems/mnemosyne/), [`noosphere`](../systems/noosphere/), [`nova-ai`](../systems/nova-ai/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/), [`wenlan`](../systems/wenlan/)
...
**Negative evidence is almost never tested.** Forty-seven repositories of two hundred and seventeen
...
disputed or forbidden stays gone. Eleven of two hundred and seventeen is the real figure for the
...
polluting a research agent's recall. Across two hundred and seventeen systems, almost none can
