...
description: A code-grounded comparison of two hundred and seventy-seven agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
277 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 14 systems of 277 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   150 of 277 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 68 of 277 commit a case
...
5. **Trust is usually a number, not a state.** 55 of 277 record a discrete
...
**Why the two counts differ.** The atlas holds **277 reports across 276
...
so a count of *systems* is 277 and a count of *repositories* is 276 — the gap is
...
Two consequences worth stating plainly. The atlas's headline counts — fourteen
...
1. **Whether correction is possible at all.** Almost everything can overwrite
   or supersede. Fourteen systems in this entire atlas can record that a *value*
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 150 of 277 systems here have, and is what the mark means. A scope
...
Two hundred and seventy-seven systems do not fall into one hundred and seventy categories. They cluster around
...
[`daimon`](../systems/daimon/), [`intaris`](../systems/intaris/), [`muninndb`](../systems/muninndb/), [`omniintelligence`](../systems/omniintelligence/), [`agent-memory-doctrine`](../systems/agent-memory-doctrine/), [`ouroboros-agent-os`](../systems/ouroboros-agent-os/), [`open-second-brain`](../systems/open-second-brain/)
...
[capability index](../capabilities/) filters all 277 systems by the seven
...
*14 of 277:* [`breadcrumbs`](../systems/breadcrumbs/), [`daimon`](../systems/daimon/), [`memory-compiler`](../systems/memory-compiler/), [`memsem`](../systems/memsem/), [`mnemosyne`](../systems/mnemosyne/), [`noosphere`](../systems/noosphere/), [`nova-ai`](../systems/nova-ai/), [`open-second-brain`](../systems/open-second-brain/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/), [`wenlan`](../systems/wenlan/)
...
a scope key when retrieving, while fourteen systems carry a value-level tombstone.
...
**Negative evidence is almost never tested.** Sixty-eight repositories of two hundred and seventy-seven
...
disputed or forbidden stays gone. Twelve of two hundred and seventy-seven is the real figure for the
...
has published it. Meanwhile fourteen repositories here implement a value-level
...
polluting a research agent's recall. Across two hundred and seventy-seven systems, almost none can
