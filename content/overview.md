...
description: A code-grounded comparison of two hundred and forty-five agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
245 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 12 systems of 245 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   137 of 245 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 52 of 245 commit a case
...
5. **Trust is usually a number, not a state.** 46 of 245 record a discrete
...
**Why the two counts differ.** The atlas holds **245 reports across 244
...
tombstones, fifty-two negative-eval suites — are counts *over inspectable code*, and a
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 137 of 245 systems here have, and is what the mark means. A scope
...
Two hundred and forty-five systems do not fall into one hundred and sixty-six categories. They cluster around
...
[capability index](../capabilities/) filters all 245 systems by the seven
...
*12 of 245:* [`breadcrumbs`](../systems/breadcrumbs/), [`daimon`](../systems/daimon/), [`memsem`](../systems/memsem/), [`mnemosyne`](../systems/mnemosyne/), [`noosphere`](../systems/noosphere/), [`nova-ai`](../systems/nova-ai/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/), [`wenlan`](../systems/wenlan/)
...
**Negative evidence is almost never tested.** Fifty-two repositories of two hundred and forty-five
...
disputed or forbidden stays gone. Twelve of two hundred and forty-five is the real figure for the
...
Repos: `magic-context`, `daimon`, `csm`, `graphify` and `breadcrumbs`; the
procedural analogue is `voyager`; contrast the judgment-based gates in `verel`
and `rainbox`.
...
polluting a research agent's recall. Across two hundred and forty-five systems, almost none can
