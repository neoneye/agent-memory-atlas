...
description: A code-grounded comparison of two hundred and sixty agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
260 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 13 systems of 260 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   142 of 260 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 64 of 260 commit a case
...
5. **Trust is usually a number, not a state.** 50 of 260 record a discrete
...
**Why the two counts differ.** The atlas holds **260 reports across 259
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 142 of 260 systems here have, and is what the mark means. A scope
...
Two hundred and sixty systems do not fall into one hundred and sixty-six categories. They cluster around
...
[capability index](../capabilities/) filters all 260 systems by the seven
...
**Negative evidence is almost never tested.** Sixty-four repositories of two hundred and sixty
...
disputed or forbidden stays gone. Twelve of two hundred and sixty is the real figure for the
...
polluting a research agent's recall. Across two hundred and sixty systems, almost none can
