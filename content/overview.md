...
description: A code-grounded comparison of two hundred and one agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
201 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 11 systems of 201 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   117 of 201 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 43 of 201 commit a case
...
5. **Trust is usually a number, not a state.** 37 of 201 record a discrete
...
**Why the two counts differ.** The atlas holds **201 reports across 200
...
tombstones, forty-three negative-eval suites — are counts *over inspectable code*, and a
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 117 of 201 systems here have, and is what the mark means. A scope
...
Two hundred and one systems do not fall into one hundred and sixty-six categories. They cluster around
...
[capability index](../capabilities/) filters all 201 systems by the seven
...
*11 of 201:* [`daimon`](../systems/daimon/), [`memsem`](../systems/memsem/), [`mnemosyne`](../systems/mnemosyne/), [`noosphere`](../systems/noosphere/), [`nova-ai`](../systems/nova-ai/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/), [`wenlan`](../systems/wenlan/)
...
**Negative evidence is almost never tested.** Forty-three repositories of two hundred and one
...
disputed or forbidden stays gone. Eleven of two hundred and one is the real figure for the
...
polluting a research agent's recall. Across two hundred and one systems, almost none can
