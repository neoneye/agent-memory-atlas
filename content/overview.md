...
description: A code-grounded comparison of one hundred and sixty-three agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
163 open-source memory systems, each read at a pinned commit and judged against
seven mechanisms with strict definitions. Corpus last extended **6 August 2026**.
Five findings, with the counts they rest on:

1. **Correction is the phase that goes unbuilt.** 9 systems of 163 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   94 of 163 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 36 of 163 commit a case
...
5. **Trust is usually a number, not a state.** 30 of 163 record a discrete
...
**Why the two counts differ.** The atlas holds **163 reports across 162
...
so a count of *systems* is 163 and a count of *repositories* is 162 — the gap is
...
tombstones, thirty-six negative-eval suites — are counts *over inspectable code*, and a
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 94 of 163 systems here have, and is what the mark means. A scope
...
One hundred and sixty-three systems do not fall into one hundred and sixty-three categories. They cluster around
...
`project-golem`, `openyak`, `memento`, `palazzo`, `memex-zero-rag`, `agentrecall-x`, `terse-memory`, `ean-agentos`, `memsem`, `cambium`, `perseus-vault`, `provem`, `sovereign`, `memoryops-ai`, `deepcode`, `prime-agent`, `kirocrew`, `engram-alpha`, `mimir`, `brain-md`
...
[capability index](../capabilities/) filters all 163 systems by the seven
...
*9 of 163:* [`daimon`](../systems/daimon/), [`memsem`](../systems/memsem/), [`mnemosyne`](../systems/mnemosyne/), [`nova-ai`](../systems/nova-ai/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
**Negative evidence is almost never tested.** Thirty-six repositories of one hundred and sixty-three
...
disputed or forbidden stays gone. Eleven of one hundred and sixty-three is the real figure for the
...
polluting a research agent's recall. Across one hundred and sixty-three systems, almost none can
