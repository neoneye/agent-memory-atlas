...
description: A code-grounded comparison of two hundred and sixty-seven agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
267 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 13 systems of 267 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   144 of 267 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 66 of 267 commit a case
...
5. **Trust is usually a number, not a state.** 54 of 267 record a discrete
...
**Why the two counts differ.** The atlas holds **267 reports across 266
...
tombstones, sixty-six negative-eval suites — are counts *over inspectable code*, and a
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 144 of 267 systems here have, and is what the mark means. A scope
...
Two hundred and sixty-seven systems do not fall into one hundred and sixty-seven categories. They cluster around
...
[capability index](../capabilities/) filters all 267 systems by the seven
...
**Negative evidence is almost never tested.** Sixty-six repositories of two hundred and sixty-seven
...
disputed or forbidden stays gone. Twelve of two hundred and sixty-seven is the real figure for the
...
polluting a research agent's recall. Across two hundred and sixty-seven systems, almost none can
