...
description: A code-grounded comparison of two hundred and forty-eight agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
248 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 12 systems of 248 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   138 of 248 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 54 of 248 commit a case
...
5. **Trust is usually a number, not a state.** 48 of 248 record a discrete
...
**Why the two counts differ.** The atlas holds **248 reports across 247
...
tombstones, fifty-four negative-eval suites — are counts *over inspectable code*, and a
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 138 of 248 systems here have, and is what the mark means. A scope
...
Two hundred and forty-eight systems do not fall into one hundred and sixty-six categories. They cluster around
...
[capability index](../capabilities/) filters all 248 systems by the seven
...
*12 of 248:* [`breadcrumbs`](../systems/breadcrumbs/), [`daimon`](../systems/daimon/), [`memsem`](../systems/memsem/), [`mnemosyne`](../systems/mnemosyne/), [`noosphere`](../systems/noosphere/), [`nova-ai`](../systems/nova-ai/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/), [`wenlan`](../systems/wenlan/)
...
**Negative evidence is almost never tested.** Fifty-four repositories of two hundred and forty-eight
...
disputed or forbidden stays gone. Twelve of two hundred and forty-eight is the real figure for the
...
polluting a research agent's recall. Across two hundred and forty-eight systems, almost none can
...
- [Daily-Nerd/daimon](https://github.com/Daily-Nerd/daimon) at [`4222243e40352691b957d6e3242b5aed25e8c851`](https://github.com/Daily-Nerd/daimon/commit/4222243e40352691b957d6e3242b5aed25e8c851)
