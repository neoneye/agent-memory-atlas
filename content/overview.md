...
description: A code-grounded comparison of two hundred and fifty-nine agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
259 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 13 systems of 259 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   141 of 259 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 63 of 259 commit a case
...
5. **Trust is usually a number, not a state.** 50 of 259 record a discrete
...
**Why the two counts differ.** The atlas holds **259 reports across 258
...
tombstones, sixty-three negative-eval suites — are counts *over inspectable code*, and a
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 141 of 259 systems here have, and is what the mark means. A scope
...
Two hundred and fifty-nine systems do not fall into one hundred and sixty-six categories. They cluster around
...
[`universal-memory-engine`](../systems/universal-memory-engine/), [`omi`](../systems/omi/), [`mnemory`](../systems/mnemory/), [`vllm-semantic-router`](../systems/vllm-semantic-router/)
...
Beside it sit two failures worth the reading. Its 10,000-dimensional
fading-context reservoir — the component the project is named around — updates as
`decay·state + roll(state)·token`, in which every term is proportional to the
state, so a zero initial vector stays zero and the pronoun resolution it feeds
never fires. And its gate bundles each fact from exactly three components while
bundling the query from all of its tokens, so cosine falls as a question
lengthens against a fixed 0.42 threshold: the same fact passes for a six-token
query and is blocked for an eight-token one. A system whose central claim is
knowing when to refuse has calibrated that refusal against an unstated assumption
about phrasing.
...
[`daimon`](../systems/daimon/), [`intaris`](../systems/intaris/), [`muninndb`](../systems/muninndb/), [`omniintelligence`](../systems/omniintelligence/), [`agent-memory-doctrine`](../systems/agent-memory-doctrine/)
...
is one this atlas had not previously found stated in code: a violation counts as
...
[`second-me`](../systems/second-me/), [`simplemem`](../systems/simplemem/), [`livingfeed`](../systems/livingfeed/), [`mnemopi`](../systems/mnemopi/), [`aeris`](../systems/aeris/), [`sesa`](../systems/sesa/)
...
[capability index](../capabilities/) filters all 259 systems by the seven
...
*73 of 259:* [`agent-memory-doctrine`](../systems/agent-memory-doctrine/), [`agent-mesh`](../systems/agent-mesh/), [`agentmemory`](../systems/agentmemory/), [`alma-memory`](../systems/alma-memory/), [`athena`](../systems/athena/), [`aukora-kernel`](../systems/aukora-kernel/), [`aura`](../systems/aura/), [`brain-md`](../systems/brain-md/), [`breadcrumbs`](../systems/breadcrumbs/), [`clawmem`](../systems/clawmem/), [`core-memory`](../systems/core-memory/), [`cortex-engine`](../systems/cortex-engine/), [`csm`](../systems/csm/), [`ctx`](../systems/ctx/), [`daem0n-mcp`](../systems/daem0n-mcp/), [`daimon`](../systems/daimon/), [`deepcode`](../systems/deepcode/), [`echo-agent`](../systems/echo-agent/), [`empirica`](../systems/empirica/), [`engram-alpha`](../systems/engram-alpha/), [`engram-provable`](../systems/engram-provable/), [`hexis`](../systems/hexis/), [`hindsight`](../systems/hindsight/), [`hippo-memory`](../systems/hippo-memory/), [`icarus`](../systems/icarus/), [`jumbo`](../systems/jumbo/), [`kirocrew`](../systems/kirocrew/), [`klypix-mcp`](../systems/klypix-mcp/), [`lethe`](../systems/lethe/), [`lorekit`](../systems/lorekit/), [`magic-context`](../systems/magic-context/), [`mem0`](../systems/mem0/), [`mem0sharp`](../systems/mem0sharp/), [`memledger`](../systems/memledger/), [`memoir`](../systems/memoir/), [`memora`](../systems/memora/), [`memory-project`](../systems/memory-project/), [`memorybear`](../systems/memorybear/), [`memoryops-ai`](../systems/memoryops-ai/), [`memsem`](../systems/memsem/), [`memvid`](../systems/memvid/), [`midas`](../systems/midas/), [`mnemosyne`](../systems/mnemosyne/), [`muninndb`](../systems/muninndb/), [`neko`](../systems/neko/), [`noosphere`](../systems/noosphere/), [`nornicdb`](../systems/nornicdb/), [`nova-ai`](../systems/nova-ai/), [`octopoda-os`](../systems/octopoda-os/), [`omega-memory`](../systems/omega-memory/), [`omi`](../systems/omi/), [`omniintelligence`](../systems/omniintelligence/), [`optmem`](../systems/optmem/), [`ostk-recall`](../systems/ostk-recall/), [`palazzo`](../systems/palazzo/), [`perseus-vault`](../systems/perseus-vault/), [`plur1bus`](../systems/plur1bus/), [`prime-agent`](../systems/prime-agent/), [`provem`](../systems/provem/), [`shodh-memory`](../systems/shodh-memory/), [`simplemem`](../systems/simplemem/), [`soul-of-waifu`](../systems/soul-of-waifu/), [`superlocalmemory`](../systems/superlocalmemory/), [`token-savior`](../systems/token-savior/), [`tokenmizer`](../systems/tokenmizer/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/), [`vestige`](../systems/vestige/), [`vibe-cognition`](../systems/vibe-cognition/), [`virtual-context`](../systems/virtual-context/), [`wax`](../systems/wax/), [`yantrikdb`](../systems/yantrikdb/), [`yourmemory`](../systems/yourmemory/)
...
**Negative evidence is almost never tested.** Sixty-three repositories of two hundred and fifty-nine
...
disputed or forbidden stays gone. Twelve of two hundred and fifty-nine is the real figure for the
...
polluting a research agent's recall. Across two hundred and fifty-nine systems, almost none can
...
- [roandejager/Hillock](https://github.com/roandejager/Hillock) at [`62f75e0c2b70a92991b47c14b320742b026ad3ce`](https://github.com/roandejager/Hillock/commit/62f75e0c2b70a92991b47c14b320742b026ad3ce) — read only; its benchmark needs a local Ollama model, so the published numbers were read rather than reproduced, and the reservoir and gate-geometry findings were checked by reproducing the arithmetic in separate code
