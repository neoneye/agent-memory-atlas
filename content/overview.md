...
description: A code-grounded comparison of two hundred and sixty-eight agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
268 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 13 systems of 268 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   144 of 268 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 67 of 268 commit a case
...
5. **Trust is usually a number, not a state.** 54 of 268 record a discrete
...
**Why the two counts differ.** The atlas holds **268 reports across 267
...
tombstones, sixty-seven negative-eval suites — are counts *over inspectable code*, and a
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 144 of 268 systems here have, and is what the mark means. A scope
...
Two hundred and sixty-eight systems do not fall into one hundred and sixty-eight categories. They cluster around
...
[capability index](../capabilities/) filters all 268 systems by the seven
...
**Negative evidence is almost never tested.** Sixty-seven repositories of two hundred and sixty-eight
...
disputed or forbidden stays gone. Twelve of two hundred and sixty-eight is the real figure for the
...
polluting a research agent's recall. Across two hundred and sixty-eight systems, almost none can
...
- [roandejager/Hillock](https://github.com/roandejager/Hillock) at [`a0499a55d0e44787dc0df03f4661dd9b0e7c9480`](https://github.com/roandejager/Hillock/commit/a0499a55d0e44787dc0df03f4661dd9b0e7c9480) — read only; its benchmark needs a local Ollama model, so the published numbers were read rather than reproduced, and the reservoir and gate-geometry findings were checked by reproducing the arithmetic in separate code
