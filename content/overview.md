...
description: A code-grounded comparison of two hundred and eighty-four agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
284 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 14 systems of 284 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   156 of 284 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 72 of 284 commit a case
...
5. **Trust is usually a number, not a state.** 58 of 284 record a discrete
...
**Why the two counts differ.** The atlas holds **284 reports across 283
...
so a count of *systems* is 284 and a count of *repositories* is 283 — the gap is
...
tombstones, seventy-two negative-eval suites — are counts *over inspectable code*, and a
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 156 of 284 systems here have, and is what the mark means. A scope
...
Two hundred and eighty-four systems do not fall into one hundred and seventy categories. They cluster around
...
[capability index](../capabilities/) filters all 284 systems by the seven
...
**Negative evidence is almost never tested.** Seventy-two repositories of two hundred and eighty-four
...
disputed or forbidden stays gone. Twelve of two hundred and eighty-four is the real figure for the
...
polluting a research agent's recall. Across two hundred and eighty-four systems, almost none can
