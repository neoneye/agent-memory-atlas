...
description: A code-grounded comparison of one hundred and fifty-nine agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
159 open-source memory systems, each read at a pinned commit and judged against
seven mechanisms with strict definitions. Corpus last extended **6 August 2026**.
Five findings, with the counts they rest on:

1. **Correction is the phase that goes unbuilt.** 9 systems of 159 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   90 of 159 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 33 of 159 commit a case
...
5. **Trust is usually a number, not a state.** 29 of 159 record a discrete
...
**Why the two counts differ.** The atlas holds **159 reports across 158
...
so a count of *systems* is 159 and a count of *repositories* is 158 — the gap is
...
tombstones, thirty-three negative-eval suites — are counts *over inspectable code*, and a
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 90 of 159 systems here have, and is what the mark means. A scope
...
One hundred and fifty-nine systems do not fall into one hundred and fifty-nine categories. They cluster around
...
`project-golem`, `openyak`, `memento`, `palazzo`, `memex-zero-rag`, `agentrecall-x`, `terse-memory`, `ean-agentos`, `memsem`, `cambium`, `perseus-vault`, `provem`, `sovereign`, `memoryops-ai`, `deepcode`, `prime-agent`, `kirocrew`, `engram-alpha`, `mimir`
...
[capability index](../capabilities/) filters all 159 systems by the seven
...
*9 of 159:* [`daimon`](../systems/daimon/), [`memsem`](../systems/memsem/), [`mnemosyne`](../systems/mnemosyne/), [`nova-ai`](../systems/nova-ai/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
**Negative evidence is almost never tested.** Thirty-three repositories of one hundred and fifty-nine
...
disputed or forbidden stays gone. Eleven of one hundred and fifty-nine is the real figure for the
...
polluting a research agent's recall. Across one hundred and fifty-nine systems, almost none can
