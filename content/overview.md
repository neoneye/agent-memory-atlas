...
description: A code-grounded comparison of one hundred and forty-seven agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
1. **Correction is the phase that goes unbuilt.** 5 systems of 147 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   81 of 147 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 17 of 147 commit a case
...
5. **Trust is usually a number, not a state.** 22 of 147 record a discrete
...
**Why the two counts differ.** The atlas holds **147 reports across 146
...
so a count of *systems* is 147 and a count of *repositories* is 146 — the gap is
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 81 of 147 systems here have, and is what the mark means. A scope
...
One hundred and forty-seven systems do not fall into one hundred and forty-seven categories. They cluster around
...
`project-golem`, `openyak`, `memento`, `palazzo`, `memex-zero-rag`, `agentrecall-x`, `terse-memory`, `ean-agentos`, `memsem`, `cambium`, `perseus-vault`, `provem`, `sovereign`
...
[capability index](../capabilities/) filters all 147 systems by the seven
...
*5 of 147:* [`daimon`](../systems/daimon/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
**Negative evidence is almost never tested.** Twenty-one repositories of one hundred and forty-seven
...
disputed or forbidden stays gone. Eleven of one hundred and forty-seven is the real figure for the
...
polluting a research agent's recall. Across one hundred and forty-seven systems, almost none can
