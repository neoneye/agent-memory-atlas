...
description: A code-grounded comparison of one hundred and seventy-two agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
172 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 9 systems of 172 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   100 of 172 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 39 of 172 commit a case
...
5. **Trust is usually a number, not a state.** 32 of 172 record a discrete
...
**Why the two counts differ.** The atlas holds **172 reports across 171
...
tombstones, thirty-nine negative-eval suites — are counts *over inspectable code*, and a
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 100 of 172 systems here have, and is what the mark means. A scope
...
One hundred and seventy-two systems do not fall into one hundred and sixty-six categories. They cluster around
...
[capability index](../capabilities/) filters all 172 systems by the seven
...
*9 of 172:* [`daimon`](../systems/daimon/), [`memsem`](../systems/memsem/), [`mnemosyne`](../systems/mnemosyne/), [`nova-ai`](../systems/nova-ai/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
**Negative evidence is almost never tested.** Thirty-nine repositories of one hundred and seventy-two
...
disputed or forbidden stays gone. Eleven of one hundred and seventy-two is the real figure for the
...
polluting a research agent's recall. Across one hundred and seventy-two systems, almost none can
