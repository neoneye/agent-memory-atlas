...
description: A code-grounded comparison of two hundred and forty-two agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
242 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 12 systems of 242 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   136 of 242 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 51 of 242 commit a case
...
5. **Trust is usually a number, not a state.** 46 of 242 record a discrete
...
**Why the two counts differ.** The atlas holds **242 reports across 241
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 136 of 242 systems here have, and is what the mark means. A scope
...
Two hundred and forty-two systems do not fall into one hundred and sixty-six categories. They cluster around
...
[capability index](../capabilities/) filters all 242 systems by the seven
...
*12 of 242:* [`breadcrumbs`](../systems/breadcrumbs/), [`daimon`](../systems/daimon/), [`memsem`](../systems/memsem/), [`mnemosyne`](../systems/mnemosyne/), [`noosphere`](../systems/noosphere/), [`nova-ai`](../systems/nova-ai/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/), [`wenlan`](../systems/wenlan/)
...
**Negative evidence is almost never tested.** Fifty-one repositories of two hundred and forty-two
...
disputed or forbidden stays gone. Twelve of two hundred and forty-two is the real figure for the
...
polluting a research agent's recall. Across two hundred and forty-two systems, almost none can
