...
description: A code-grounded comparison of one hundred and forty-six agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
1. **Correction is the phase that goes unbuilt.** 5 systems of 146 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   81 of 146 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 17 of 146 commit a case
...
5. **Trust is usually a number, not a state.** 22 of 146 record a discrete
...
**Why the two counts differ.** The atlas holds **146 reports across 145
...
so a count of *systems* is 146 and a count of *repositories* is 145 — the gap is
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 81 of 146 systems here have, and is what the mark means. A scope
...
One hundred and forty-six systems do not fall into one hundred and forty-six categories. They cluster around
...
[capability index](../capabilities/) filters all 146 systems by the seven
...
*5 of 146:* [`daimon`](../systems/daimon/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
**Negative evidence is almost never tested.** Twenty-one repositories of one hundred and forty-six
...
disputed or forbidden stays gone. Eleven of one hundred and forty-six is the real figure for the
...
polluting a research agent's recall. Across one hundred and forty-six systems, almost none can
