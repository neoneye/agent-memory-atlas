...
description: A code-grounded comparison of one hundred and fifty-eight agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
158 open-source memory systems, each read at a pinned commit and judged against
seven mechanisms with strict definitions. Corpus last extended **6 August 2026**.
Five findings, with the counts they rest on:

1. **Correction is the phase that goes unbuilt.** 9 systems of 158 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   89 of 158 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 32 of 158 commit a case
...
5. **Trust is usually a number, not a state.** 29 of 158 record a discrete
...
**Why the two counts differ.** The atlas holds **158 reports across 157
...
so a count of *systems* is 158 and a count of *repositories* is 157 — the gap is
...
tombstones, thirty-two negative-eval suites — are counts *over inspectable code*, and a
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 89 of 158 systems here have, and is what the mark means. A scope
...
One hundred and fifty-eight systems do not fall into one hundred and fifty-eight categories. They cluster around
...
[capability index](../capabilities/) filters all 158 systems by the seven
...
*9 of 158:* [`daimon`](../systems/daimon/), [`memsem`](../systems/memsem/), [`mnemosyne`](../systems/mnemosyne/), [`nova-ai`](../systems/nova-ai/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
**Negative evidence is almost never tested.** Thirty-two repositories of one hundred and fifty-eight
...
disputed or forbidden stays gone. Eleven of one hundred and fifty-eight is the real figure for the
...
polluting a research agent's recall. Across one hundred and fifty-eight systems, almost none can
