...
description: A code-grounded comparison of two hundred and eighty agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
280 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 14 systems of 280 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   153 of 280 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 69 of 280 commit a case
...
5. **Trust is usually a number, not a state.** 56 of 280 record a discrete
...
**Why the two counts differ.** The atlas holds **280 reports across 279
...
so a count of *systems* is 280 and a count of *repositories* is 279 — the gap is
...
tombstones, sixty-nine negative-eval suites — are counts *over inspectable code*, and a
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 153 of 280 systems here have, and is what the mark means. A scope
...
Two hundred and eighty systems do not fall into one hundred and seventy categories. They cluster around
...
[capability index](../capabilities/) filters all 280 systems by the seven
...
**Negative evidence is almost never tested.** Sixty-nine repositories of two hundred and eighty
...
disputed or forbidden stays gone. Twelve of two hundred and eighty is the real figure for the
...
polluting a research agent's recall. Across two hundred and eighty systems, almost none can
