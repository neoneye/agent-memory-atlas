...
description: A code-grounded comparison of two hundred and eighty-one agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
281 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 14 systems of 281 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   154 of 281 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 70 of 281 commit a case
...
5. **Trust is usually a number, not a state.** 56 of 281 record a discrete
...
**Why the two counts differ.** The atlas holds **281 reports across 280
...
so a count of *systems* is 281 and a count of *repositories* is 280 — the gap is
...
tombstones, seventy negative-eval suites — are counts *over inspectable code*, and a
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 154 of 281 systems here have, and is what the mark means. A scope
...
Two hundred and eighty-one systems do not fall into one hundred and seventy categories. They cluster around
...
[capability index](../capabilities/) filters all 281 systems by the seven
...
*31 of 281:* [`agent-memory-doctrine`](../systems/agent-memory-doctrine/), [`agent-memory-supabase`](../systems/agent-memory-supabase/), [`atomic-agent`](../systems/atomic-agent/), [`claude-total-memory`](../systems/claude-total-memory/), [`clawmem`](../systems/clawmem/), [`core-memory`](../systems/core-memory/), [`core-redplanet`](../systems/core-redplanet/), [`daem0n-mcp`](../systems/daem0n-mcp/), [`engram-alpha`](../systems/engram-alpha/), [`engram-provable`](../systems/engram-provable/), [`gini-agent`](../systems/gini-agent/), [`graphiti`](../systems/graphiti/), [`hippo-memory`](../systems/hippo-memory/), [`memory-engine`](../systems/memory-engine/), [`memory-lancedb-pro`](../systems/memory-lancedb-pro/), [`memsem`](../systems/memsem/), [`memv`](../systems/memv/), [`memvid`](../systems/memvid/), [`midas`](../systems/midas/), [`mnemosyne`](../systems/mnemosyne/), [`muninndb`](../systems/muninndb/), [`neo4j-agent-memory`](../systems/neo4j-agent-memory/), [`neurakeep`](../systems/neurakeep/), [`omega-memory`](../systems/omega-memory/), [`omi`](../systems/omi/), [`ostk-recall`](../systems/ostk-recall/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`verel`](../systems/verel/), [`vestige`](../systems/vestige/), [`zep`](../systems/zep/)
...
**Negative evidence is almost never tested.** Seventy repositories of two hundred and eighty-one
...
disputed or forbidden stays gone. Twelve of two hundred and eighty-one is the real figure for the
...
polluting a research agent's recall. Across two hundred and eighty-one systems, almost none can
