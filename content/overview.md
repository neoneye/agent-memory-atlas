...
description: A code-grounded comparison of one hundred and forty-eight agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 82 of 148 systems here have, and is what the mark means. A scope
...
One hundred and forty-eight systems do not fall into one hundred and forty-eight categories. They cluster around
...
`openhuman`, `aukora-kernel`, `helm`, `neko`, `sillytavern`, `risuai`, `soul-of-waifu`, `z-waif`, `virtualwife`, `aura`, `memledger`, `agentic-context-engine`, `deer-flow`
...
`tigrimosr`, `adk-python`, `autogen`, `agno`, `agent-framework`, `dexto`
...
`project-golem`, `openyak`, `memento`, `palazzo`, `memex-zero-rag`, `agentrecall-x`, `terse-memory`, `ean-agentos`, `memsem`, `cambium`, `perseus-vault`, `provem`, `sovereign`, `memoryops-ai`
...
third. Measured against the repository's own milk/lactose example. **1.3.0, two
releases later, adds the half the previous reading said was missing: a human
write-gate tombstone.** A rejected candidate now writes a `memory_suppressions`
row keyed on the normalized value that refuses the write (`rejected: true`) until
explicitly unsuppressed, alongside a discrete trust state and bi-temporal
validity. The distance
...
| `alma-memory` | One of five typed rows — a heuristic, an outcome, a domain fact, an anti-pattern or a preference — each with its own columns and a 384-dimension embedding | Postgres with pgvector in the hosted schema and a local SQLite mirror; five tables plus indexes on `(project_id, agent)` | Per-table SQL with `WHERE project_id = ?` and a confidence floor, plus vector similarity; a verification pass classifies results before returning them | Outcomes recorded per task; heuristics accumulate occurrence and success counts; a file miner extracts heuristics and anti-patterns from a repository | A `ForgettingEngine` prunes by age and by confidence; no supersession pointer and no rejected-value record | `project_id` and `agent` on every one of the five tables, applied as a SQL predicate on the read path and indexed together | An MCP server with retrieval tools, a CLI, and a PyPI package | The forgetting engine's pruning strategies and a file-mining ingestion pass | Four verification states — verified, uncertain, contradicted, unverifiable — computed at retrieval by ground truth, cross-verification or confidence, and never persisted | An anti-pattern table storing the reason something was wrong and the better alternative beside it, which is more than any tombstone in this corpus records | The verification states exist only in the returned object, so nothing accumulates and a contradiction found today is rediscovered tomorrow |
...
| `memsem` | A subject/predicate/object triple with importance, confidence, frequency, tags, theme, trust, evidence, a pinned flag and an archived flag | One SQLite file via `node:sqlite`, with versioned migrations, plus history, edges, episodes, candidates, suppressions, audit and an FTS5 index | Strict lexical by default over an FTS5 index with a `unicode61` tokenizer, ranked by `importance × confidence × recency × frequency`; an opt-in relax mode adds cosine over JSON-stored vectors and two-hop graph propagation; an `asOf` arm reads bi-temporal validity | `memory_add` upserts a triple; a differing object for the same subject and predicate fades every live rival and records a `contradicts` edge; a value under a human suppression is refused outright | Supersession by attenuation — pinned rows exempt, critical rows floored above the archive threshold; a rejected value re-asserted reactivates its archived row at a discount and fades the correction, by design; a human-rejected value is refused at the write gate until explicitly unsuppressed | `project` and `theme` applied as filters on every read path, plus a focus list that attenuates rather than excludes; cross-project reach is opt-in via `crossProject: true` | An MCP server with review, verify, unsuppress and purge tools added to the core set, an opencode plugin, and a CLI with list, edit, forget, doctor and purge | Session-end extraction, consolidation into patterns, and a pairwise-comparison scoring pass — all sub-agents driven by prompts in `plugin.ts` | A discrete `trust` field (`inferred` / `verbatim` / `verified`) with an `evidence` string and provenance; a candidate gate whose `pending` rows are withheld from retrieval until a person approves them | A committed offline benchmark that reproduces exactly, an ablation over its own constants, an audit log carrying a reason, a pass id and a dry-run flag, and now a human write-gate tombstone, a discrete trust state and bi-temporal validity | The automatic supersession path still prices a rejection rather than constraining it — an unpinned correction is archived at the third re-assertion — while the human write-gate tombstone refuses only what a person has explicitly rejected |
...
| `nova-ai` | Two units — an interaction event, and a concept with senses, relations and a per-record audit log | SQLite for events (plus a JSONL mirror and an archive table); one `concepts.json` rewritten whole for knowledge | SQL `LIKE` over the event JSON, `difflib.SequenceMatcher` over the newest 500 rows, and graph walks over `is_a`/`part_of`/`causes` — no embeddings anywhere | Event-bus fan-in with a 50-event/5-second buffer; knowledge writes pass a spoken yes/no confirmation gate | Definitions correctable with old and new value recorded; relations, senses and concepts have no removal path at all; the preference store is the only thing that can be told to forget | None — a single named user, no scope key anywhere | An in-process event bus across roughly 40 modules; a 24/7 loop that decides when to speak unprompted | Six-hourly maintenance — RAM trim, 90-day archival, 365-day gzip, vacuum, backup rotation, health check; four-hourly classifier retraining | Per-relation `source` and `confidence`, a confidence history, and an append-only audit log per concept — but no status field and no rejected state | Corrections quarantined from the curated training set; a confirmation gate before knowledge is written; an audit log on the record itself | Contradiction detection nothing calls, no way to retract a relation, a whole-file non-atomic save, and a licence that forbids reuse |
...
*82 of 148:* [`acontext`](../systems/acontext/), [`adk-python`](../systems/adk-python/), [`agent-framework`](../systems/agent-framework/), [`agent-memory-supabase`](../systems/agent-memory-supabase/), [`agentmemory`](../systems/agentmemory/), [`agentrecall-x`](../systems/agentrecall-x/), [`agentswarms`](../systems/agentswarms/), [`agno`](../systems/agno/), [`ai-memory`](../systems/ai-memory/), [`alma-memory`](../systems/alma-memory/), [`argo`](../systems/argo/), [`aukora-kernel`](../systems/aukora-kernel/), [`basic-memory`](../systems/basic-memory/), [`buzz`](../systems/buzz/), [`claude-mem`](../systems/claude-mem/), [`cognee`](../systems/cognee/), [`cognicore`](../systems/cognicore/), [`core-memory`](../systems/core-memory/), [`cortex`](../systems/cortex/), [`cowagent`](../systems/cowagent/), [`crewai`](../systems/crewai/), [`csm`](../systems/csm/), [`ctx`](../systems/ctx/), [`daimon`](../systems/daimon/), [`deer-flow`](../systems/deer-flow/), [`ean-agentos`](../systems/ean-agentos/), [`ecc`](../systems/ecc/), [`echo-agent`](../systems/echo-agent/), [`elastic-atlas`](../systems/elastic-atlas/), [`empryo`](../systems/empryo/), [`engram`](../systems/engram/), [`everos`](../systems/everos/), [`gini-agent`](../systems/gini-agent/), [`gobii`](../systems/gobii/), [`graphiti`](../systems/graphiti/), [`hindsight`](../systems/hindsight/), [`hippo-memory`](../systems/hippo-memory/), [`honcho`](../systems/honcho/), [`langmem`](../systems/langmem/), [`letta`](../systems/letta/), [`livingfeed`](../systems/livingfeed/), [`llm-wiki-memory`](../systems/llm-wiki-memory/), [`lorekit`](../systems/lorekit/), [`magic-context`](../systems/magic-context/), [`mastra-observational-memory`](../systems/mastra-observational-memory/), [`mateclaw`](../systems/mateclaw/), [`mem0`](../systems/mem0/), [`mem0sharp`](../systems/mem0sharp/), [`memanto`](../systems/memanto/), [`membase`](../systems/membase/), [`memento`](../systems/memento/), [`memmachine`](../systems/memmachine/), [`memobase`](../systems/memobase/), [`memori`](../systems/memori/), [`memory-engine`](../systems/memory-engine/), [`memoryops-ai`](../systems/memoryops-ai/), [`memos`](../systems/memos/), [`mempalace`](../systems/mempalace/), [`memsem`](../systems/memsem/), [`metaclaw`](../systems/metaclaw/), [`mirix`](../systems/mirix/), [`neko`](../systems/neko/), [`neo4j-agent-memory`](../systems/neo4j-agent-memory/), [`nooa-memory`](../systems/nooa-memory/), [`openclaw`](../systems/openclaw/), [`openhuman`](../systems/openhuman/), [`openviking`](../systems/openviking/), [`openworker`](../systems/openworker/), [`openyak`](../systems/openyak/), [`perseus-vault`](../systems/perseus-vault/), [`powermem`](../systems/powermem/), [`promptx`](../systems/promptx/), [`provem`](../systems/provem/), [`pydantic-ai-harness`](../systems/pydantic-ai-harness/), [`qwen-code`](../systems/qwen-code/), [`rainbox`](../systems/rainbox/), [`redis-agent-memory-server`](../systems/redis-agent-memory-server/), [`simplemem`](../systems/simplemem/), [`supermemory`](../systems/supermemory/), [`tigrimosr`](../systems/tigrimosr/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
*29 of 148:* [`aukora-kernel`](../systems/aukora-kernel/), [`aura`](../systems/aura/), [`csm`](../systems/csm/), [`ctx`](../systems/ctx/), [`daimon`](../systems/daimon/), [`echo-agent`](../systems/echo-agent/), [`hippo-memory`](../systems/hippo-memory/), [`lethe`](../systems/lethe/), [`lorekit`](../systems/lorekit/), [`magic-context`](../systems/magic-context/), [`mem0`](../systems/mem0/), [`mem0sharp`](../systems/mem0sharp/), [`memledger`](../systems/memledger/), [`memora`](../systems/memora/), [`memory-project`](../systems/memory-project/), [`memoryops-ai`](../systems/memoryops-ai/), [`memsem`](../systems/memsem/), [`memvid`](../systems/memvid/), [`neko`](../systems/neko/), [`nova-ai`](../systems/nova-ai/), [`optmem`](../systems/optmem/), [`palazzo`](../systems/palazzo/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`simplemem`](../systems/simplemem/), [`soul-of-waifu`](../systems/soul-of-waifu/), [`tokenmizer`](../systems/tokenmizer/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
*34 of 148:* [`acontext`](../systems/acontext/), [`agentswarms`](../systems/agentswarms/), [`agno`](../systems/agno/), [`cambium`](../systems/cambium/), [`clio`](../systems/clio/), [`core-memory`](../systems/core-memory/), [`cortex`](../systems/cortex/), [`daimon`](../systems/daimon/), [`deer-flow`](../systems/deer-flow/), [`dexto`](../systems/dexto/), [`empryo`](../systems/empryo/), [`engram`](../systems/engram/), [`hermes-agent`](../systems/hermes-agent/), [`juggler`](../systems/juggler/), [`llm-wiki-memory`](../systems/llm-wiki-memory/), [`lorekit`](../systems/lorekit/), [`memanto`](../systems/memanto/), [`memora`](../systems/memora/), [`memory-project`](../systems/memory-project/), [`memoryops-ai`](../systems/memoryops-ai/), [`memsem`](../systems/memsem/), [`mercury-agent`](../systems/mercury-agent/), [`nova-ai`](../systems/nova-ai/), [`npcpy`](../systems/npcpy/), [`openyak`](../systems/openyak/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`qwen-code`](../systems/qwen-code/), [`rainbox`](../systems/rainbox/), [`risuai`](../systems/risuai/), [`second-me`](../systems/second-me/), [`tigrimosr`](../systems/tigrimosr/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)

**Negative retrieval assertion** — Committed evaluation cases asserting that particular material must *not* be retrieved.

*22 of 148:* [`agent-afk`](../systems/agent-afk/), [`agno`](../systems/agno/), [`argo`](../systems/argo/), [`aukora-kernel`](../systems/aukora-kernel/), [`crewai`](../systems/crewai/), [`csm`](../systems/csm/), [`daimon`](../systems/daimon/), [`everos`](../systems/everos/), [`graphify`](../systems/graphify/), [`helm`](../systems/helm/), [`lethe`](../systems/lethe/), [`memoryops-ai`](../systems/memoryops-ai/), [`memsem`](../systems/memsem/), [`mirix`](../systems/mirix/), [`mnemopi`](../systems/mnemopi/), [`neko`](../systems/neko/), [`open-cowork`](../systems/open-cowork/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`pydantic-ai-harness`](../systems/pydantic-ai-harness/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
**Negative evidence is almost never tested.** Twenty-two repositories of one hundred and forty-eight
...
- `alma-memory`: a `ForgettingEngine` prunes by age and by confidence, destructively and without recording what went. The interesting half is elsewhere — `alma_anti_patterns` durably stores a pattern with `why_bad` and `better_alternative`, which is more than any tombstone here records, and is read as guidance rather than consulted on the write path.
...
polluting a research agent's recall. Across one hundred and forty-eight systems, almost none can
...
- [RBKunnela/ALMA-memory](https://github.com/RBKunnela/ALMA-memory) at [`164d2e3e3c67f3ce1c33d2b9ccd9acaa65f9ad7a`](https://github.com/RBKunnela/ALMA-memory/commit/164d2e3e3c67f3ce1c33d2b9ccd9acaa65f9ad7a)
...
- [Daily-Nerd/daimon](https://github.com/Daily-Nerd/daimon) at [`3025ee3edecd1958e9e9181fe607a5b1a30309bf`](https://github.com/Daily-Nerd/daimon/commit/3025ee3edecd1958e9e9181fe607a5b1a30309bf)
