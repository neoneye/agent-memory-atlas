...
description: A code-grounded comparison of one hundred and twenty-five agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 69 of 125 systems here have, and is what the mark means. A scope
...
One hundred and twenty-five systems do not fall into one hundred and twenty-five categories. They cluster around
...
`openhuman`, `aukora-kernel`, `helm`, `neko`, `sillytavern`, `risuai`, `soul-of-waifu`, `z-waif`, `virtualwife`, `aura`
...
`project-golem`, `openyak`, `memento`, `palazzo`
...
| `empryo` | A record with a four-value category, summary, details, topics, file references, a unique content hash, and surfaced/acted counters | SQLite per scope with FTS5 over two tokenizers, a 384-dimension embedding column, and a `memory_edges` similarity graph | RRF over five directional signals — unicode FTS, trigram FTS, file affinity, git co-change affinity, semantic rank — plus nine bounded magnitude terms including bm25 and a surfaced/acted feedback term | Agent tool calls plus an optional end-of-session distillation pass; upsert keyed on `content_hash UNIQUE`, with a collision against a hidden row returning a no-op | `superseded_by` and `hidden`, both filtered on the read path; soft delete survives re-saving the same text and is hard-purged by `maintain()` after a 60-day retention window | Separate `writeScope` and `readScope` over `global` and `project`, defaulting writes to project as the safer side | A terminal, desktop and headless agent with memory tools, inline hints on tool results with a 10-turn cooldown and session budget, and a browser with a bulk cleanup queue | None scheduled; similarity edges compute on write, `maintain()` and `autoArchiveIgnored()` are explicit calls, distillation runs once at session end | `source` records user or agent and nothing reads it as trust; surfaced/acted counters feed ranking, not epistemic status | Repo-derived ranking signals with a committed A/B harness and a CI floor gate behind them; a deterministic embedder with measured cosine ranges and the scoring calibrated against those numbers | No audit log and no trust state — a memory the agent invented ranks identically to one the user dictated, and a distilled memory is indistinguishable from a deliberate one after the `auto-distilled` topic is stripped |
...
*5 of 125:* [`daimon`](../systems/daimon/), [`empryo`](../systems/empryo/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
*20 of 125:* [`aukora-kernel`](../systems/aukora-kernel/), [`aura`](../systems/aura/), [`csm`](../systems/csm/), [`ctx`](../systems/ctx/), [`daimon`](../systems/daimon/), [`lethe`](../systems/lethe/), [`lorekit`](../systems/lorekit/), [`magic-context`](../systems/magic-context/), [`mem0`](../systems/mem0/), [`mem0sharp`](../systems/mem0sharp/), [`memora`](../systems/memora/), [`memvid`](../systems/memvid/), [`neko`](../systems/neko/), [`optmem`](../systems/optmem/), [`palazzo`](../systems/palazzo/), [`simplemem`](../systems/simplemem/), [`soul-of-waifu`](../systems/soul-of-waifu/), [`tokenmizer`](../systems/tokenmizer/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
*26 of 125:* [`acontext`](../systems/acontext/), [`agentswarms`](../systems/agentswarms/), [`agno`](../systems/agno/), [`clio`](../systems/clio/), [`core-memory`](../systems/core-memory/), [`cortex`](../systems/cortex/), [`daimon`](../systems/daimon/), [`dexto`](../systems/dexto/), [`empryo`](../systems/empryo/), [`engram`](../systems/engram/), [`hermes-agent`](../systems/hermes-agent/), [`juggler`](../systems/juggler/), [`llm-wiki-memory`](../systems/llm-wiki-memory/), [`lorekit`](../systems/lorekit/), [`memanto`](../systems/memanto/), [`memora`](../systems/memora/), [`mercury-agent`](../systems/mercury-agent/), [`npcpy`](../systems/npcpy/), [`openyak`](../systems/openyak/), [`qwen-code`](../systems/qwen-code/), [`rainbox`](../systems/rainbox/), [`risuai`](../systems/risuai/), [`second-me`](../systems/second-me/), [`tigrimosr`](../systems/tigrimosr/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)

**Negative retrieval assertion** — Committed evaluation cases asserting that particular material must *not* be retrieved.

*18 of 125:* [`agent-afk`](../systems/agent-afk/), [`agno`](../systems/agno/), [`aukora-kernel`](../systems/aukora-kernel/), [`crewai`](../systems/crewai/), [`csm`](../systems/csm/), [`daimon`](../systems/daimon/), [`empryo`](../systems/empryo/), [`everos`](../systems/everos/), [`graphify`](../systems/graphify/), [`helm`](../systems/helm/), [`lethe`](../systems/lethe/), [`mirix`](../systems/mirix/), [`mnemopi`](../systems/mnemopi/), [`neko`](../systems/neko/), [`open-cowork`](../systems/open-cowork/), [`pydantic-ai-harness`](../systems/pydantic-ai-harness/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
**Negative evidence is almost never tested.** Sixteen repositories of one hundred and twenty-five
...
disputed or forbidden stays gone. Eleven of one hundred and twenty-five is the real figure for the
...
polluting a research agent's recall. Across one hundred and twenty-five systems, almost none can
