...
description: A code-grounded comparison of two hundred and forty-seven agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
247 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 12 systems of 247 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   138 of 247 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 54 of 247 commit a case
...
5. **Trust is usually a number, not a state.** 47 of 247 record a discrete
...
**Why the two counts differ.** The atlas holds **247 reports across 246
...
so a count of *systems* is 247 and a count of *repositories* is 246 — the gap is
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 138 of 247 systems here have, and is what the mark means. A scope
...
Two hundred and forty-seven systems do not fall into one hundred and sixty-six categories. They cluster around
...
can be re-asserted tomorrow as a fresh card that nothing recognises.
...
[capability index](../capabilities/) filters all 247 systems by the seven
...
*12 of 247:* [`breadcrumbs`](../systems/breadcrumbs/), [`daimon`](../systems/daimon/), [`memsem`](../systems/memsem/), [`mnemosyne`](../systems/mnemosyne/), [`noosphere`](../systems/noosphere/), [`nova-ai`](../systems/nova-ai/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/), [`wenlan`](../systems/wenlan/)
...
**Negative evidence is almost never tested.** Fifty-four repositories of two hundred and forty-seven
...
disputed or forbidden stays gone. Twelve of two hundred and forty-seven is the real figure for the
...
polluting a research agent's recall. Across two hundred and forty-seven systems, almost none can
