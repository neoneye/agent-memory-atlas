...
description: A code-grounded comparison of one hundred and sixty-six agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
166 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 9 systems of 166 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   95 of 165 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 37 of 165 commit a case
...
5. **Trust is usually a number, not a state.** 31 of 165 record a discrete
...
**Why the two counts differ.** The atlas holds **166 reports across 165
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 95 of 166 systems here have, and is what the mark means. A scope
...
One hundred and sixty-six systems do not fall into one hundred and sixty-five categories. They cluster around
...
[capability index](../capabilities/) filters all 166 systems by the seven
...
*9 of 166:* [`daimon`](../systems/daimon/), [`memsem`](../systems/memsem/), [`mnemosyne`](../systems/mnemosyne/), [`nova-ai`](../systems/nova-ai/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
**Negative evidence is almost never tested.** Thirty-seven repositories of one hundred and sixty-six
...
disputed or forbidden stays gone. Eleven of one hundred and sixty-six is the real figure for the
...
polluting a research agent's recall. Across one hundred and sixty-six systems, almost none can
