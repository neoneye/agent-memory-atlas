...
description: A code-grounded comparison of one hundred and fifty-four agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
154 open-source memory systems, each read at a pinned commit and judged against
seven mechanisms with strict definitions. Corpus last extended **6 August 2026**.
Five findings, with the counts they rest on:

1. **Correction is the phase that goes unbuilt.** 9 systems of 154 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   87 of 154 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 29 of 154 commit a case
...
5. **Trust is usually a number, not a state.** 27 of 154 record a discrete
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 87 of 154 systems here have, and is what the mark means. A scope
...
One hundred and fifty-four systems do not fall into one hundred and fifty-four categories. They cluster around
...
*9 of 154:* [`daimon`](../systems/daimon/), [`memsem`](../systems/memsem/), [`mnemosyne`](../systems/mnemosyne/), [`nova-ai`](../systems/nova-ai/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
**Negative evidence is almost never tested.** Twenty-nine repositories of one hundred and fifty-four
...
disputed or forbidden stays gone. Eleven of one hundred and fifty-four is the real figure for the
