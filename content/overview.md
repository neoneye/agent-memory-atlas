...
description: A code-grounded comparison of two hundred and eighty-three agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
283 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 14 systems of 283 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   156 of 283 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 71 of 283 commit a case
...
5. **Trust is usually a number, not a state.** 57 of 283 record a discrete
...
**Why the two counts differ.** The atlas holds **283 reports across 282
...
so a count of *systems* is 283 and a count of *repositories* is 282 — the gap is
...
tombstones, seventy-one negative-eval suites — are counts *over inspectable code*, and a
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 156 of 283 systems here have, and is what the mark means. A scope
...
Two hundred and eighty-three systems do not fall into one hundred and seventy categories. They cluster around
...
[capability index](../capabilities/) filters all 283 systems by the seven
...
**Negative evidence is almost never tested.** Seventy-one repositories of two hundred and eighty-three
...
disputed or forbidden stays gone. Twelve of two hundred and eighty-three is the real figure for the
...
polluting a research agent's recall. Across two hundred and eighty-three systems, almost none can
...
- [Krilliac/Sonder-runtime](https://github.com/Krilliac/Sonder-runtime) at [`2a11324cbb131c76572724ac1fe47362a4aedf83`](https://github.com/Krilliac/Sonder-runtime/commit/2a11324cbb131c76572724ac1fe47362a4aedf83) — read only; no auto-run surface, no dependency surface inside the cooldown, two build-time execution points (`conftest.py`), two unpinned dev/train requirement ranges. Nothing installed or run; the quarantine base-rate and attribution guards and the outcome-provenance enforcement were read from `retriever.py`, `memory_store.py` and `lesson_decay.py` against the committed tests
