...
description: A code-grounded comparison of one hundred and fifty-seven agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
157 open-source memory systems, each read at a pinned commit and judged against
seven mechanisms with strict definitions. Corpus last extended **6 August 2026**.
Five findings, with the counts they rest on:

1. **Correction is the phase that goes unbuilt.** 9 systems of 157 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   88 of 157 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 31 of 157 commit a case
...
5. **Trust is usually a number, not a state.** 28 of 157 record a discrete
...
**Why the two counts differ.** The atlas holds **157 reports across 156
...
so a count of *systems* is 157 and a count of *repositories* is 156 — the gap is
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 88 of 157 systems here have, and is what the mark means. A scope
...
One hundred and fifty-seven systems do not fall into one hundred and fifty-seven categories. They cluster around
...
[capability index](../capabilities/) filters all 157 systems by the seven
...
*9 of 157:* [`daimon`](../systems/daimon/), [`memsem`](../systems/memsem/), [`mnemosyne`](../systems/mnemosyne/), [`nova-ai`](../systems/nova-ai/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
**Negative evidence is almost never tested.** Thirty-one repositories of one hundred and fifty-seven
...
disputed or forbidden stays gone. Eleven of one hundred and fifty-seven is the real figure for the
...
polluting a research agent's recall. Across one hundred and fifty-seven systems, almost none can
