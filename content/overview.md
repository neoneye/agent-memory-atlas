...
description: A code-grounded comparison of one hundred and forty-four agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
1. **Correction is the phase that goes unbuilt.** 4 systems of 144 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   79 of 144 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 17 of 144 commit a case
...
5. **Trust is usually a number, not a state.** 21 of 144 record a discrete
...
**Why the two counts differ.** The atlas holds **144 reports across 143
...
so a count of *systems* is 144 and a count of *repositories* is 143 — the gap is
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 79 of 144 systems here have, and is what the mark means. A scope
...
One hundred and forty-four systems do not fall into one hundred and forty-four categories. They cluster around
...
`project-golem`, `openyak`, `memento`, `palazzo`, `memex-zero-rag`, `agentrecall-x`, `terse-memory`, `ean-agentos`, `memsem`, `cambium`, `perseus-vault`
...
Python checks — for corpora maintained by LLM agents, and it ships no corpus:
the repository is *intentionally uninstantiated*, with no profile selected and no
composed vocabulary. Two of its checks are built so that **a run which examined
...
`neo4j-agent-memory`, `memary`, `m-flow`, `nova-ai`
...
[capability index](../capabilities/) filters all 144 systems by the seven
...
| `memsem` | A subject/predicate/object triple with importance, confidence, frequency, tags, theme and an archived flag | One SQLite file via `node:sqlite`, with versioned migrations, history, edges, episodes, audit tables and a trigger-maintained FTS5 table | Strict lexical by default — Unicode FTS5 narrows candidates before the 50% word-match floor and ranking by `importance × confidence × recency × frequency`; an opt-in relax mode adds cosine and two-hop graph propagation | `memory_add` upserts a triple with case-insensitive subject/predicate matching; a differing object fades every live rival and records a `contradicts` edge | Supersession by attenuation — confidence × 0.6, or × 0.9 above the critical threshold — archiving below 0.25; archived rows are kept and excluded from reads, and nothing is ever deleted | `project` and `theme` applied as filters on every read path, plus a focus list that attenuates rather than excludes | An MCP server with fourteen tools, an opencode plugin, and a CLI with list, edit, forget and doctor | Session-end extraction, consolidation into patterns, and a pairwise-comparison scoring pass — all sub-agents driven by prompts in `plugin.ts` | Importance, confidence, frequency and a pinned flag; provenance text; no discrete epistemic status | A committed offline benchmark that reproduces exactly, an ablation over its own constants, and an audit log carrying a reason and a dry-run flag | The lexical path is indexed and Unicode-safe, but optional cosine ranking still scans stored vectors; trust remains a confidence float and background safety rules remain prompt-driven |
...
*5 of 144:* [`daimon`](../systems/daimon/), [`memsem`](../systems/memsem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
*27 of 144:* [`aukora-kernel`](../systems/aukora-kernel/), [`aura`](../systems/aura/), [`csm`](../systems/csm/), [`ctx`](../systems/ctx/), [`daimon`](../systems/daimon/), [`echo-agent`](../systems/echo-agent/), [`hippo-memory`](../systems/hippo-memory/), [`lethe`](../systems/lethe/), [`lorekit`](../systems/lorekit/), [`magic-context`](../systems/magic-context/), [`mem0`](../systems/mem0/), [`mem0sharp`](../systems/mem0sharp/), [`memledger`](../systems/memledger/), [`memora`](../systems/memora/), [`memory-project`](../systems/memory-project/), [`memsem`](../systems/memsem/), [`memvid`](../systems/memvid/), [`neko`](../systems/neko/), [`nova-ai`](../systems/nova-ai/), [`optmem`](../systems/optmem/), [`palazzo`](../systems/palazzo/), [`perseus-vault`](../systems/perseus-vault/), [`simplemem`](../systems/simplemem/), [`soul-of-waifu`](../systems/soul-of-waifu/), [`tokenmizer`](../systems/tokenmizer/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
*32 of 144:* [`acontext`](../systems/acontext/), [`agentswarms`](../systems/agentswarms/), [`agno`](../systems/agno/), [`cambium`](../systems/cambium/), [`clio`](../systems/clio/), [`core-memory`](../systems/core-memory/), [`cortex`](../systems/cortex/), [`daimon`](../systems/daimon/), [`deer-flow`](../systems/deer-flow/), [`dexto`](../systems/dexto/), [`empryo`](../systems/empryo/), [`engram`](../systems/engram/), [`hermes-agent`](../systems/hermes-agent/), [`juggler`](../systems/juggler/), [`llm-wiki-memory`](../systems/llm-wiki-memory/), [`lorekit`](../systems/lorekit/), [`memanto`](../systems/memanto/), [`memora`](../systems/memora/), [`memory-project`](../systems/memory-project/), [`memsem`](../systems/memsem/), [`mercury-agent`](../systems/mercury-agent/), [`nova-ai`](../systems/nova-ai/), [`npcpy`](../systems/npcpy/), [`openyak`](../systems/openyak/), [`perseus-vault`](../systems/perseus-vault/), [`qwen-code`](../systems/qwen-code/), [`rainbox`](../systems/rainbox/), [`risuai`](../systems/risuai/), [`second-me`](../systems/second-me/), [`tigrimosr`](../systems/tigrimosr/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)

**Negative retrieval assertion** — Committed evaluation cases asserting that particular material must *not* be retrieved.

*19 of 144:* [`agent-afk`](../systems/agent-afk/), [`agno`](../systems/agno/), [`aukora-kernel`](../systems/aukora-kernel/), [`crewai`](../systems/crewai/), [`csm`](../systems/csm/), [`daimon`](../systems/daimon/), [`everos`](../systems/everos/), [`graphify`](../systems/graphify/), [`helm`](../systems/helm/), [`lethe`](../systems/lethe/), [`memsem`](../systems/memsem/), [`mirix`](../systems/mirix/), [`mnemopi`](../systems/mnemopi/), [`neko`](../systems/neko/), [`open-cowork`](../systems/open-cowork/), [`perseus-vault`](../systems/perseus-vault/), [`pydantic-ai-harness`](../systems/pydantic-ai-harness/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/)
...
**Negative evidence is almost never tested.** Nineteen repositories of one hundred and forty-four
...
disputed or forbidden stays gone. Eleven of one hundred and forty-four is the real figure for the
...
polluting a research agent's recall. Across one hundred and forty-four systems, almost none can
