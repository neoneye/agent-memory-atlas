...
description: A code-grounded comparison of two hundred and sixty-four agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
264 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 13 systems of 264 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   143 of 264 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 64 of 264 commit a case
...
5. **Trust is usually a number, not a state.** 52 of 264 record a discrete
...
**Why the two counts differ.** The atlas holds **264 reports across 263
...
tombstones, sixty-four negative-eval suites — are counts *over inspectable code*, and a
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 143 of 264 systems here have, and is what the mark means. A scope
...
Two hundred and sixty-four systems do not fall into one hundred and sixty-six categories. They cluster around
...
[capability index](../capabilities/) filters all 264 systems by the seven
...
**Negative evidence is almost never tested.** Sixty-four repositories of two hundred and sixty-four
...
disputed or forbidden stays gone. Twelve of two hundred and sixty-four is the real figure for the
...
polluting a research agent's recall. Across two hundred and sixty-four systems, almost none can
