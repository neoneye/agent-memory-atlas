...
description: A code-grounded comparison of one hundred and eighty-seven agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
187 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 10 systems of 187 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   109 of 187 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 42 of 187 commit a case
...
5. **Trust is usually a number, not a state.** 35 of 187 record a discrete
...
**Why the two counts differ.** The atlas holds **187 reports across 186
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 109 of 187 systems here have, and is what the mark means. A scope
...
One hundred and eighty-seven systems do not fall into one hundred and sixty-six categories. They cluster around
...
[capability index](../capabilities/) filters all 187 systems by the seven
...
*10 of 187:* [`daimon`](../systems/daimon/), [`memsem`](../systems/memsem/), [`mnemosyne`](../systems/mnemosyne/), [`nova-ai`](../systems/nova-ai/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/), [`wenlan`](../systems/wenlan/)
...
**Negative evidence is almost never tested.** Forty-two repositories of one hundred and eighty-seven
...
disputed or forbidden stays gone. Eleven of one hundred and eighty-seven is the real figure for the
...
polluting a research agent's recall. Across one hundred and eighty-seven systems, almost none can
