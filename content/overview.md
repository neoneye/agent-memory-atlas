...
description: A code-grounded comparison of two hundred and eighty-nine agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
289 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 17 systems of 289 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   158 of 289 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 75 of 289 commit a case
...
5. **Trust is usually a number, not a state.** 60 of 289 record a discrete
...
**Why the two counts differ.** The atlas holds **289 reports across 288
...
so a count of *systems* is 289 and a count of *repositories* is 288 — the gap is
...
Two consequences worth stating plainly. The atlas's headline counts — seventeen
tombstones, seventy-five negative-eval suites — are counts *over inspectable code*, and a
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 158 of 289 systems here have, and is what the mark means. A scope
...
Two hundred and eighty-nine systems do not fall into one hundred and seventy categories. They cluster around
...
[capability index](../capabilities/) filters all 289 systems by the seven
...
*17 of 289:* [`breadcrumbs`](../systems/breadcrumbs/), [`daimon`](../systems/daimon/), [`memmy-agent`](../systems/memmy-agent/), [`memory-compiler`](../systems/memory-compiler/), [`memsem`](../systems/memsem/), [`mnemosyne`](../systems/mnemosyne/), [`noosphere`](../systems/noosphere/), [`nova-ai`](../systems/nova-ai/), [`open-second-brain`](../systems/open-second-brain/), [`perseus-vault`](../systems/perseus-vault/), [`plur1bus`](../systems/plur1bus/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`sonder-runtime`](../systems/sonder-runtime/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/), [`wenlan`](../systems/wenlan/)
...
**Negative evidence is almost never tested.** Seventy-five repositories of two hundred and eighty-nine
...
disputed or forbidden stays gone. Twelve of two hundred and eighty-nine is the real figure for the
...
polluting a research agent's recall. Across two hundred and eighty-nine systems, almost none can
