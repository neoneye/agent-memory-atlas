...
description: A code-grounded comparison of one hundred and ninety-six agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
196 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 11 systems of 196 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   114 of 196 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 42 of 196 commit a case
...
5. **Trust is usually a number, not a state.** 37 of 196 record a discrete
...
**Why the two counts differ.** The atlas holds **196 reports across 195
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 114 of 196 systems here have, and is what the mark means. A scope
...
One hundred and ninety-six systems do not fall into one hundred and sixty-six categories. They cluster around
...
[capability index](../capabilities/) filters all 196 systems by the seven
...
*11 of 196:* [`daimon`](../systems/daimon/), [`memsem`](../systems/memsem/), [`mnemosyne`](../systems/mnemosyne/), [`noosphere`](../systems/noosphere/), [`nova-ai`](../systems/nova-ai/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/), [`wenlan`](../systems/wenlan/)
...
**Negative evidence is almost never tested.** Forty-two repositories of one hundred and ninety-six
...
disputed or forbidden stays gone. Eleven of one hundred and ninety-six is the real figure for the
...
polluting a research agent's recall. Across one hundred and ninety-six systems, almost none can
