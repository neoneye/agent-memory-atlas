...
description: A code-grounded comparison of two hundred and sixty-nine agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
269 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 13 systems of 269 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   145 of 269 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 68 of 269 commit a case
...
5. **Trust is usually a number, not a state.** 54 of 269 record a discrete
...
**Why the two counts differ.** The atlas holds **269 reports across 268
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 145 of 269 systems here have, and is what the mark means. A scope
...
Two hundred and sixty-nine systems do not fall into one hundred and sixty-nine categories. They cluster around
...
[capability index](../capabilities/) filters all 269 systems by the seven
...
**Negative evidence is almost never tested.** Sixty-eight repositories of two hundred and sixty-nine
...
disputed or forbidden stays gone. Twelve of two hundred and sixty-nine is the real figure for the
...
polluting a research agent's recall. Across two hundred and sixty-nine systems, almost none can
