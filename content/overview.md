...
description: A code-grounded comparison of two hundred and fifty-five agent memory architectures, their retrieval mechanics, trust models, and operational tradeoffs.
root: ..
...
255 memory systems with publicly readable source, each read at a pinned commit
...
1. **Correction is the phase that goes unbuilt.** 12 systems of 255 carry a
...
3. **Scope is the most implemented mechanism and the shallowest.**
   140 of 255 apply a scope key as a filter on the read path. It is also
   the mark most often satisfied by a single predicate that a background job
   then ignores.
4. **Negative evidence is almost never tested.** 60 of 255 commit a case
...
5. **Trust is usually a number, not a state.** 50 of 255 record a discrete
...
**Why the two counts differ.** The atlas holds **255 reports across 254
...
tombstones, sixty negative-eval suites — are counts *over inspectable code*, and a
...
3. **Whether scope is identity or decoration.** Three levels, and the atlas's
   ...
   path* is what 140 of 255 systems here have, and is what the mark means. A scope
...
Two hundred and fifty-five systems do not fall into one hundred and sixty-six categories. They cluster around
...
[capability index](../capabilities/) filters all 255 systems by the seven
...
*12 of 255:* [`breadcrumbs`](../systems/breadcrumbs/), [`daimon`](../systems/daimon/), [`memsem`](../systems/memsem/), [`mnemosyne`](../systems/mnemosyne/), [`noosphere`](../systems/noosphere/), [`nova-ai`](../systems/nova-ai/), [`perseus-vault`](../systems/perseus-vault/), [`provem`](../systems/provem/), [`rainbox`](../systems/rainbox/), [`universal-memory-engine`](../systems/universal-memory-engine/), [`verel`](../systems/verel/), [`wenlan`](../systems/wenlan/)
...
**Negative evidence is almost never tested.** Sixty repositories of two hundred and fifty-five
...
disputed or forbidden stays gone. Twelve of two hundred and fifty-five is the real figure for the
...
polluting a research agent's recall. Across two hundred and fifty-five systems, almost none can
