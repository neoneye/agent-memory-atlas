...
The page makes two kinds of claim. The first half is reported practice: host runtimes that mount memory behind one lifecycle contract are common in this atlas, from Hermes and OpenClaw to ADK, AutoGen, Agno and Strands. The second half is argued: carrying **scope and deletion across that contract**. ADK and MateClaw declare scope; the Pydantic AI Harness declares a targeted delete; no contract read here declares both. That half rests on one or two instances, and the page is arguing for it rather than reporting it.
...
[Hermes Agent](../../systems/hermes-agent/) has the most explicit contract: a `MemoryProvider` ABC with more than twenty members covering identity and availability, initialization, prompt blocks, prefetch and its queued form, recall status, turn start and per-turn sync, tool schemas and dispatch, session end and switch, pre-compression extraction, delegation observation, config, backup paths, and shutdown, with a `MemoryManager` that mounts exactly one external provider. First-party adapters ship in-tree for Honcho, Mem0, Hindsight, Supermemory, OpenViking, ByteRover, RetainDB, and the built-in Holographic plugin — so the adapters are reviewable even when a backing service is not. The contract has **no deletion hook and no per-call tenant scope**: `initialize(session_id, **kwargs)` documents `user_id` among optional keyword arguments, and `prefetch` and `sync_turn` take a `session_id`, so a provider learns its user once at startup rather than on each read or write.
...
[TencentDB Agent Memory](../../systems/tencentdb-agent-memory/) sidesteps the host contract altogether: at its pin it is a reverse proxy in front of the model API, reached by pointing an agent's `base_url` at it, with SDKs and an OpenClaw plugin shipped beside it. Because it owns the traffic rather than a slot in a host's interface, deletion and scope are its own to define, and it defines both — `atomic/delete`, `scenario/rm` and a clear-memory path, each journalled to `memory_audit`, with the team/user/agent/task key composed into the delete predicates as well as the reads.
...
Outside the corpus, and read without a pinned report, [LightAgent](https://github.com/wanxingai/LightAgent) shows the pattern at its smallest — a 196-line `shared_memory.py` with swappable adapters shipped as examples (`clawmem_memory_adapter.py`, `vector_memory_adapter.py`). The shape survives the scale reduction; so does the missing deletion path.

**[SmythOS SRE](../../systems/smythos-sre/) is the degenerate case, and it is deliberately not counted in the tally above.** `LLMMemoryConnector` is an abstract class extending the runtime's secure-connector base, declaring one method — `load(messages)` — and it is exported from the package index at `packages/core/src/index.ts`. There is no subclass anywhere in the tree, no registration in the connector service, and no call site. Nothing mounts it, so it is not a provider contract that anyone has had to satisfy; it is the seam without the socket. It earns a mention here because the failure it illustrates is cheap to reach: the same repository ships a working connector pattern for cache, storage, vector search, LLM providers and accounts, each with real implementations behind it, and a reader encountering `MemoryManager` reasonably assumes memory has one too. An exported abstract class is a promise the type system will not keep, and the distance between "we defined the interface" and "a provider exists" is the entire pattern.
