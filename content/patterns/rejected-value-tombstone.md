...
> **This is not an established best practice.** Thirty-nine systems of four hundred and thirty-eight
...
**Thirty-nine systems of 438 in the atlas have this.** That is still the most
...
That makes the negative result stronger rather than weaker. Two of four hundred and thirty-eight
would suggest a hard idea that a few teams reach independently. One of four hundred and thirty-eight, plus one adoption by a reader who went looking, suggests an idea
...
**[Memory Garden](../../systems/memory-garden/) consults its rejection and keys it
on the wrong thing.** A person who denies a proposed change in their own views
leaves a reviewed discovery that every later scan checks before writing the same
pair again — the consulted form this page argues for. The key is the pair of
source-atom ids, and an atom's id hashes its note's whole-content revision, the
importer version and its position. Editing any line of either note reissues every
id in the file, and the denied pair is eligible again. A living notes vault is
edited constantly, so the rejection decays exactly where it is needed. Keying the
check on the normalised text of the two statements would make it the value.
...
**[GrayMatter](../../systems/graymatter/) builds the durable half and skips the value-keyed half.** Its supersession marker excludes a fact from recall immediately and unconditionally, before any signal is scored, and the update path latches it — when the stored fact is retired and an incoming snapshot is not, the write is dropped, so a consolidation cycle holding a pre-retirement snapshot cannot resurrect a corrected value. That is more retirement durability than most systems here manage. What is absent is anything keyed on the value: the write path performs no exact-text lookup, no similarity query and no history check, so a forget followed by the same sentence stores a second live fact at full weight. The pattern's distinction is visible in one repository — a record-keyed retirement engineered carefully, and the re-assertion it cannot see.
