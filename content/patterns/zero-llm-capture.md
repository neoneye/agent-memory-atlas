...
*without* it. Its hooks write the file map, the action log and the bug index
synchronously, with no model call anywhere on the path — so a memory is durable
the instant a tool returns and nothing is lost when a provider is down. The store
it leaves out of that arrangement is the one holding beliefs, which no hook
writes; until release 2.5 it was also rewritten wholesale by a weekly model call,
since removed. The contrast is the argument for this pattern in one repository:
the mechanically captured half never fails, and the half that depends on a model
obeying an instruction is the half that goes stale.
...
append-only hash-chained journal of mutations is enough to rebuild the store
exactly, which is what makes the record auditable. Its journal sits at the
store boundary *after* extraction on purpose, so replay stays deterministic even
for the configurations where an LLM did produce the content — the model's
nondeterminism happened upstream of the line.

Two cautions come with it, and both generalise. **The reproducibility claim is
only worth what its exceptions say**, and this one enumerates them: embeddings
are re-derived rather than journaled, a synthesized record replays with its
content vector rather than the original recombined one, access statistics are
excluded — all stated in the module docstring *"rather than discovered."* And
**a replayable log is an undelete**. A deleted memory's text stays in the
journal and `replay_journal(until_seq=N)` before the delete rebuilds it, offered
as a rollback feature; nothing in that repository redacts or compacts the log.
