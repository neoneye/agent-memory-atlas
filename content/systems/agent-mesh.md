...
capability_evidence:
  trust_state: "the decision status, guarded on the way in by a quorum and read out through an allowlist a caller must name to widen | src/agent_mesh/store/rebuild.py:3728-3741, :4207-4221, :384, :629, src/agent_mesh/core/decision_applicability.py:198-230, src/agent_mesh/core/decision_context.py:54-65, src/agent_mesh/adoption.py:322-326, src/agent_mesh/cli/q.py:2200, :2213 | `_project_decision_event` promotes to `accepted` or `in_force` only through `decision_accepted` and only when `_decision_quorum_reached` passes, returning without promotion otherwise; `_ensure_supersede_target_valid` refuses a supersession target that is not accepted or in force; and a revision that folds `status` back to `proposed` clears `accepted_utc` and `in_force_utc` together in the same statement, so the timestamps cannot outlive the standing they record. The read is the half the first record left out, and it is an allowlist rather than a denylist: `evaluate_decision_applicability` — the engine that decides which decisions apply to a set of paths — builds a status set defaulting to `accepted` and `in_force` and skips every row outside it, so a value nobody anticipated is withheld instead of admitted. Widening is explicit and named: `include_proposed` adds one value, `lifecycle_statuses` replaces the set outright, and both default to the narrow set through `build_decision_context`; the only caller that widens is the CLI, behind a flag a person types. The adoption report applies the same pair in SQL | tests/public/test_public_contract.py::test_invalid_decision_transition_never_reaches_the_log — the `accepted_utc` clear itself is untested"
