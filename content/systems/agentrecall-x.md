...
revision: 5d3389d23b0dd38dda49e75ea91892e624df1c99
revision_url: https://github.com/Goldentrii/AgentRecall-X/commit/5d3389d23b0dd38dda49e75ea91892e624df1c99
analyzed_at: 2026-09-19
capabilities: "trust_state, scope_enforced"
capability_evidence:
  trust_state: "the correction store — a stored authoritative flag and severity decide whether a record may veto a plan or only advise it, and a cached precision withdraws the veto without withdrawing the record | packages/core/src/tools-logic/check-action.ts:369-384, packages/core/src/storage/corrections.ts:94 (the cached precision), :109-115 (kept separate from `predict_precision`) | `rec.authoritative !== false && rec.severity === p0 && !isNoiseCandidate(rec)` decides the verdict, which is the two-value `blocked` or `advisory` on the next line — so the state decides whether the store overrides the model or merely warns it, rather than adjusting a score. The withdrawal is the part worth copying: a record whose precision is under 0.3 across three or more retrievals is a noise candidate and stops blocking while staying retrievable, under a comment naming the risk it answers — a stale p0 that keeps firing without being heeded should not veto legitimate plans. `precision` is heeded over retrieved, cached on the record and recomputed when an outcome arrives, and it is held strictly separate from `predict_precision` rather than blended into one number | scripts/eval"
