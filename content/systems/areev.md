...
revision: 5b6b29f3924ae02ecc2a366d7dbf16d56a7f77d6
revision_url: https://github.com/AreevAI/areev/commit/5b6b29f3924ae02ecc2a366d7dbf16d56a7f77d6
...
capability_evidence:
  ...
  human_review: "the recommendation lifecycle, with the transition table enforcing it and the reason required to record it | crates/areev-loop/src/recommendation.rs:685-700, crates/areev-loop/src/engine.rs:1388, :1419-1433, crates/areev-mcp/src/lib.rs:1093, :1116-1131 | `RecStatus` is `Pending / Approved / Rejected / Applied / RolledBack / Expired` and `can_transition_to` is a real table, not a suggestion: `Pending → Approved` and `Pending → Rejected` are open, `Approved → Applied` is open, and `Pending → Applied` is permitted only when `by_policy` is set — *\"the auto-apply actor, the only one permitted the reasonless pending → applied jump\"*, which names the one path that skips a person and marks it. `ObserverType` distinguishes `Human` from `Agent`, the MCP server lists pending recommendations and accepts transitions by name, and every transition persists an `AuditRecord` whose `because` field is not optional. A model-authored proposal is stamped `analyzer: \"loop.llm/1\"` with no manifest family, which `grants_auto_apply` cannot grant, and a code revision is refused an apply without the run id of an evaluation that executed the tool's own pinned evalset. The producer cannot clear its own review, and the refusal is structural rather than a role flag: `Engine::review` (`engine.rs:1388`) raises `Error::SelfApproval` on a transition to `Approved` when the actor is the recommendation's creator, and again when the actor is the `co_creator` that `triggered the run that authored this recommendation` (`:1419-1433`). The MCP handler cannot dodge either clause by renaming itself — its actor is the literal `\"agent:mcp\"` (`areev-mcp/src/lib.rs:1093`), not a caller argument — and `Scope::Review` is required before any of it. The limit to state: this is separation of duties by actor identity, not a human check, so a recommendation from a run the agent did not trigger can be approved by an agent, with `ObserverType::Agent` recorded on the audit row rather than refused | crates/areev-conformance/, crates/areev-loop/src/integration.rs:2536-2572"
...
governed learning loop on top. 208,493 lines of Rust across 306 files,
dual-licensed MIT or Apache-2.0, 635 commits since 16 August 2026.
...
**The benchmark harness ships; the receipts no longer do.**
`crates/areev-bench/` still holds the harness, and it still gates CI. What it no
longer holds is the evidence. `crates/areev-bench/results/` contains one file, a
README titled "Results moved", which says the per-trial records, governance
ledgers, meters, statistical workings and manifests "now [live] in [their] own
repository" at `AreevAI/areev-benchmark`, on the reasoning that the evidence
"only ever grows, so it was most of this repository's weight and none of its
build".

That destination does not resolve. `https://github.com/AreevAI/areev-benchmark`
returns 404 to an unauthenticated request, the contents API returns Not Found,
and the `AreevAI` organisation lists four public repositories — `areev`,
`dejadb`, `flowcat` and `memorygrain.org` — with no benchmark repository among
them. So the receipts are checkable from neither location: they are gone from
the repository that published them and not yet public at the address that
replaced it. The design they served is still visible in the harness, and the
figures the project reports are still stated; what a reader has lost is the
ability to verify them.

That matters here more than it would elsewhere, because publishing the receipts
was the project's own stated reason for doing it — *"the category has a history
of unreproducible claims; we publish the receipts"* — and what is quoted below
describes the apparatus that reason produced, read at the previous pin when the
files were present. The `*.transcripts*.jsonl` rows carried one line per
question or per model call, with the question, the gold answer, the model's
answer and the judge's verdict.

The verification apparatus went with the evidence. Each run directory carried a
`MANIFEST.md` generated by `scripts/verify_run.py --write-manifest`, recording
the exact command, the git rev and a SHA-256 of every file; the script's own
docstring gave the reason, that it had caught an overwrite of published evidence
here before, *"when the single-seed pilot's transcripts were renamed and
overwritten by the three-seed run."* Neither the script, nor any `MANIFEST.md`,
nor the pinned digests in `tests/golden/reproducibility.txt` are in this
repository at this pin. `RESULTS.md` remains, and states the numbers.

**And a published run could declare itself no longer comparable** — the part of
this apparatus most worth carrying into a design of your own, wherever it now
lives. The manifest body was compared byte-for-byte on regeneration with one
exception — a blockquote
...
The figures above are the project's own. They were read from files it committed
at the previous pin; at this one they are stated in `RESULTS.md` and the files
behind them are not in this repository.
