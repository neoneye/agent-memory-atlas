...
capability_evidence:
  ...
  trust_state: "the candidate ladder and the record status, the second composed by one shared clause builder that has no parameter for switching it off | argos_plugin/store_core.py:96-176, argos_plugin/store_write.py:1326-1352, :1371-1390, :1962-1978, argos_plugin/store_retrieval.py:66-96, :925-940, argos_plugin/api_facade.py:404-411, argos_plugin/provider_session.py:431-437 | a candidate is `pending`, `reviewed_approved`, `pending_user_confirmation`, `approved`, `rejected`, `quarantined` or `deduplicated`; only `approved` and `reviewed_approved` create a `memory_records` row, and the storage layer raises when an automatic review tries to write `approved` — the docstring says why it lives there: the invariant is *enforced here, at the storage boundary, not by prompt instructions*. External-origin approvals are downgraded to `pending_user_confirmation`, promotion is capped at a `grounding` ceiling of observed, extracted, inferred or speculative, and an error path is kept out of the ladder's terminal state on purpose: when reconciliation fails to produce a memory the candidate returns to `pending_user_confirmation` rather than `rejected`, because rejecting it *would permanently burn the candidate and block paraphrased retries via the one-way ladder*. On the record side every read composes its WHERE clause through one builder whose documented clause order begins with status, appended unconditionally and commented *always present* — the temporal and expiry gates each have a widening parameter and the status gate has none. Where a widening flag does exist, on the by-id fetch, it is in `FORBIDDEN_CLIENT_FLAGS`: an external caller that passes `include_quarantined` is refused with code `forbidden` rather than quietly ignored | argos_plugin/tests/test_approval_invariant.py (`test_auto_review_cannot_set_approved`, `test_tool_path_can_set_approved`) and argos_plugin/tests/test_api_facade.py::test_forbidden_client_flags_rejected, which asserts the refusal for `include_quarantined`, `include_archived` and `include_expired`"
...
**2026-09-19** — [`755f652a5d1cff21b1a38c371f5790f79feb87af`](https://github.com/bobaba76/Argos/commit/755f652a5d1cff21b1a38c371f5790f79feb87af) — `trust_state` re-tested, read from the archive fork since the upstream is still 404. The pin is unchanged, so the additions are the atlas's own. Three of them, all on the question of how hard the predicate is to switch off. Every read of `memory_records` composes its WHERE clause through one builder in `store_retrieval.py:66`, whose docstring fixes the clause order — status, then temporal, expiry, user scope, project, namespace, client scope, category, tier — and appends the status clause first with the comment *"Status — always present"*. The temporal gate has `as_of` and `include_closed`, the expiry gate has `include_expired`; the status gate has no parameter at all. The one place a status escape exists, `get_memories_by_ids(include_quarantined=...)`, is covered from the other side: `include_quarantined`, `include_archived`, `include_expired`, `include_closed` and `suppress_retrieval` are listed in `api_facade.py` as `FORBIDDEN_CLIENT_FLAGS` — *"internal-only and must not be set by API clients"* — and an external caller that passes one is refused with code `forbidden`, asserted by `test_forbidden_client_flags_rejected`. Its single internal user reads a record's `valid_from` and `valid_to` to fill in an update's temporal fields, not its content. On the candidate side, two sentences the record should have quoted. The approval invariant explains its own location — automatic review may not write `approved`, and this is *"enforced here, at the storage boundary, not by prompt instructions"*. And a failed reconciliation deliberately does **not** reject the candidate, because on a one-way ladder that *"would permanently burn the candidate and block paraphrased retries"*; it returns to `pending_user_confirmation` instead. Nothing was installed and no suite was run.
