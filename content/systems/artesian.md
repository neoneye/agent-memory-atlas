...
capability_evidence:
  ...
  trust_state: "a three-value lifecycle field whose two non-default values withhold a record from recall, tested before anything else in the chain | crates/aquifer/src/types.rs:17-35, :160-163, crates/aquifer/src/files.rs:155-161, :294-336, :502-505, crates/aquifer/src/vector_memory.rs:1543, :2006-2013, crates/aquifer/src/eviction.rs:85-109, crates/artesian-cli/src/main.rs:3319 | `MemoryState` is `Active`, `Archived` or `Retracted` on the record itself, not derived. `retract` sets `Retracted`, stamps `retracted_at`, and links a `superseded_by` relation to a stored retraction record; eviction sets `Archived`, and only considers records that are still `Active`. Both backends apply the same clause — admit the record when the caller asked for archived rows *or* its state is `Active` — and on the files backend it is the *first* filter in the chain, ahead of node, tag and tenancy matching and ahead of scoring, so a withheld record is gone before relevance is computed rather than scored and discarded. `confidence` exists as a separate `Option<f32>` and is used for ranking, which is the distinction the mark turns on. Two details belong beside it. The single escape is named for one state and governs both: `--include-archived` re-admits `Retracted` records as well, which the type documentation makes discoverable but the flag and the comment beside it do not. And `Active` is written as the *absence* of the field — `#[serde(default)]` with `skip_serializing_if = \"is_active_state\"` — so a record missing or losing that key reads as retrieval-eligible; the type doc says why, that records written before the field existed must round-trip | crates/aquifer/tests/memory_backend_contract.rs:583-639 (retract sets the state, the record leaves default recall, and the same query with `include_archived` returns it)"
...
**2026-09-19** — [`14e4f2d9494d396695f79350b9e42bacd82d953d`](https://github.com/aquifer-labs/artesian/commit/14e4f2d9494d396695f79350b9e42bacd82d953d) — `trust_state` re-tested at the unchanged pin. Every claim in the record holds and three things are added. The filter's *position* is the best of them: on the files backend the state clause is the first link in the chain (`files.rs:157-161`), ahead of node, tag and tenancy matching and ahead of `score_record`, so a withheld record never reaches the scorer — the shape this atlas keeps distinguishing from a rank that merely sinks a retired row. Eviction is symmetrical: it selects only records that are still `Active`, so nothing already archived is re-archived. Second, the escape hatch is named for one state and governs two — `--include-archived` re-admits `Retracted` records as well as `Archived` ones, since the predicate tests equality with `Active` rather than inequality with `Archived`. The `MemoryState` doc comment makes that discoverable; the flag name and the comment beside the filter (*"Exclude Archived records unless --include-archived is set"*) do not, and `Retracted` is the epistemically loaded value, the one carrying `retracted_at` and a `superseded_by` link to a stored retraction record. Third, `Active` is persisted as the *absence* of the field: `state` is `#[serde(default)]` with `skip_serializing_if = "is_active_state"`, so a record whose key is missing, hand-edited away or truncated reads as retrieval-eligible. That is deliberate and documented — *"the serde default is `Active` so that records written before this field existed round-trip correctly"* — and it is the opposite polarity from the one that protects an unconstrained column elsewhere in this corpus, so it is worth a reader knowing which way it falls. Nothing was installed and no suite was run.
