...
revision: 8a85f3b5cc3d925fd1b65cc6606dcf7265c20ebc
revision_url: https://github.com/MaxMiksa/Auto-Company/commit/8a85f3b5cc3d925fd1b65cc6606dcf7265c20ebc
analyzed_at: 2026-09-17
capabilities: "human_review"
capability_evidence:
  human_review: "the consensus baton — the two human-governed sections of `memories/consensus.md` | scripts/core/consensus-guard.sh:70-86, :140-142, :153-177, :179-192, scripts/core/consensus-format.py:98 | `## Human Overrides` is captured to a baseline by `consensus-guard.sh begin` and compared byte for byte by `verify_cycle` after the cycle; a cycle that changed or deleted it has the pre-cycle consensus restored, a pause flag written with the reason `human_override_mutated`, and every subsequent cycle refused. An unchecked `- [ ] P1:` item under `## Priority Issues` returns 41 from `preflight`, which runs *before* the engine is invoked, so the loop stops without a model call until a person resolves the item. `reset` refuses to run with `AUTO_COMPANY_CYCLE=1` — `consensus reset is human-only` — and preserves both human sections and the pause flags while leaving the prior consensus under `memories/resets/` | tests/test_governance_boundaries.py:445 `test_unresolved_p1_blocks_real_loop_until_human_resolves_it` starts the real loop against a fake engine, waits for the state file to read `unresolved_p1`, asserts no engine call was recorded at all, then checks the item off and asserts exactly one call; :428 `test_human_override_pause_survives_process_restart`; :253 asserts a reset keeps the human sections, the pause and a recoverable original"
stack_storage: "files"
