...
capability_evidence:
  trust_state: "a supersession stamp that withholds a fact from the default read on three of the four retrieval methods — the fourth being the one the other three fall back to | src/beever_atlas/models/domain.py:74-78, src/beever_atlas/stores/weaviate_store.py:865 and :904, :954 and :1012, :1042 and :1087 (the three that filter), :916-944 (`bm25_search`, which does not), src/beever_atlas/capabilities/memory.py:196, :303, :398 (its callers) | `supersede_fact` sets `invalid_at` and `superseded_by` on the old fact and marks a conflicting pair `potential_contradiction=True`. `semantic_search`, `true_hybrid_search` and `pseudo_hybrid_search` each take `include_superseded: bool = False` and skip any fact whose `invalid_at` is set, so the default read drops a retired fact rather than ranking it down. The exclusion is applied in Python rather than in the query for a stated reason — the store cannot express an is-null filter, which a regression test pins by asserting no `is_none` reaches Weaviate. Two consequences follow from that placement. A retired fact still consumes a slot in the top-k before being dropped. And `bm25_search`, which returns objects straight from the query with no post-processing stage, never got the exclusion — while its three callers are all fallbacks, entered when the hybrid search fails, so the guarantee is lost on the degraded path rather than the healthy one | tests/agents/tools/test_hybrid_no_nullstate_filter.py:74-126 and :159-210 assert the default read returns only the active fact and that flipping `include_superseded` returns both; the four tests cover `true_hybrid_search` and `semantic_search` only, so neither the keyword path nor `pseudo_hybrid_search` is covered"
...
**2026-09-19** — [`7d791af27ef4aa19644cb385c7ca24619bc4f848`](https://github.com/Beever-AI/beever-atlas/commit/7d791af27ef4aa19644cb385c7ca24619bc4f848) — `trust_state` re-tested at an unchanged pin. Every claim held, including the one worth re-checking: `bm25_search` (`stores/weaviate_store.py:916-944`) still takes no `include_superseded` argument and still returns `[self._obj_to_fact(obj) for obj in result.objects]` with no `invalid_at` test, while the other three methods carry the parameter and the skip. What the re-read adds is where that lands. `bm25_search` has three production callers (`capabilities/memory.py:196`, `:303`, `:398`) and all three are fallbacks, entered under *hybrid search failed, falling back to bm25*. So the exclusion is not missing from a secondary surface someone might reach deliberately — it is missing from the path the system takes when the primary one has already failed, which is when a superseded fact is least likely to be noticed and most likely to matter. A missing predicate on a fallback is worse than one on a main path for exactly that reason. The four regression tests cover `true_hybrid_search` and `semantic_search`; `pseudo_hybrid_search` carries the filter and is untested, and the keyword path is neither. Re-read from a fresh clone; nothing was installed and no suite was run.
