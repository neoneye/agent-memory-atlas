...
revision: bc31e5e0f0ea17761b30a18514b1892d32f18775
revision_url: https://github.com/Bitterbot-AI/bitterbot-desktop/commit/bc31e5e0f0ea17761b30a18514b1892d32f18775
analyzed_at: 2026-09-16
capabilities: "trust_state, bitemporal, negative_eval"
capability_evidence:
  trust_state: "chunks carry an indexed lifecycle status excluded from retrieval, and canonical facts carry an active/retired status that gates unconditional injection | src/memory/chunk-writer.ts:46-108, src/memory/manager-embedding-ops.ts:1119-1120, src/memory/canonical-facts.ts:654-661, src/memory/memory-schema.ts:134 | `LifecycleState` is `active | archived | consolidated | forgotten` and `Lifecycle` is `generated | activated | frozen | consolidated | archived | expired`, both stored on `chunks` and indexed; the retrieval query excludes rows with `lifecycle_state <> 'forgotten'` and `lifecycle <> 'expired'`. Producers are `setChunkLifecycle` — documented as \"the single place that reconciles the two columns\" — plus consolidation, governance and the manager, which write `lifecycleState: \"forgotten\"` directly. Separately `CanonicalFacts.retire` sets `status = 'retired'` to \"[d]emote a fact out of injection (kept queryable; nothing deleted)\", and `epistemic-directives.ts:66` stores `open | answered | expired` filtered as `status = 'open'` on the injection read | src/memory/consolidation.purge-expired.test.ts; src/memory/canonical-facts.test.ts"
  bitemporal: "relationships carry a valid-time interval separate from when the edge was recorded, and a committed as-of read returns the belief as it stood at a timestamp | src/memory/temporal-filter.ts:106-131, src/memory/knowledge-graph.ts:495-567 | relationships store `valid_from` / `valid_until` while rows are ordered by their own `created_at`, and `buildRelationshipTemporalWhereClause` filters `valid_from <= ?` and `valid_until IS NULL OR valid_until > ?`. `beliefHistory(entityId, { validAt })` passes `includeClosed: true` so \"the universal `valid_until IS NULL` guard used elsewhere is deliberately dropped here\", letting callers \"answer 'what did I believe about X as of T?'\", and `beliefAsOf(entityId, ts)` is the wrapper. Supersession sets `valid_until` rather than deleting, and `conflict-resolver.ts:126` does the same for chunks with `valid_time_end` | src/memory/knowledge-graph.sabm.test.ts:115-151"
...
**2026-09-16** — [`bc31e5e0f0ea17761b30a18514b1892d32f18775`](https://github.com/Bitterbot-AI/bitterbot-desktop/commit/bc31e5e0f0ea17761b30a18514b1892d32f18775) — first reading, at a commit dated 15 September 2026. Screened before opening, from a shallow clone; a dependency surface had changed inside the seven-day cooldown. Nothing was installed, built or run.
