...
revision: 1d0acee29e3b5121f9097ea856e2e6017be8af69
revision_url: https://github.com/Bitwarelabscom/bwmem/commit/1d0acee29e3b5121f9097ea856e2e6017be8af69
...
capability_evidence:
  ...
  trust_state: "a four-value status constrained in the schema, filtered on every recall read and deliberately not on the as-of read | src/db/migrations/001_core.sql:49-50, :65, src/memory/facts.service.ts:306, :397, :441-458 (`getFactsAsOf`), :543, :648, :689, src/memory/quality-scorer.service.ts:402, src/memory/fact-collision.service.ts:421, src/memory/contradiction.service.ts:230 | `fact_status` is `VARCHAR(20) DEFAULT active` under `CHECK (fact_status IN (active, overridden, superseded, expired))`, so the vocabulary is closed by the database rather than by convention. Seven read sites filter to `fact_status = active` — retrieval, search, collision, quality scoring, contradiction detection — and the partial index `ON facts(user_id, category) WHERE fact_status = active` is built for exactly that predicate, a second and structural statement that the active set is the read set. The eighth read omits it on purpose: `getFactsAsOf` answers what was believed at one time about the world at another, and uses `recorded_at`, `superseded_at` and the validity window instead — the status would contradict the question. An overridden, superseded or expired fact keeps its row, its lineage through `supersedes_id` and its place in that history while dropping out of what the bot is given, and the retrieval clause doubles the expiry case so a temporary fact past its window is withheld before any sweep marks it | tests/unit/temporary-fact-expiry.test.ts:80 asserts the expiry sweep only ever touches temporary, active rows"
...
constrained to `active`, `overridden`, `superseded` and `expired`, and the
partial index is built `WHERE fact_status = 'active'` — so the active set is the
read set in the planner as well as in the predicate. A superseded fact keeps its
row, its lineage and its place in the bitemporal history while leaving what the
bot is handed.

Seven of the eight reads of the facts table carry the predicate — retrieval
(`facts.service.ts:306`), full-text search (`:397`), the mention-count lookup
(`:543`), the relevance pass (`:648`), the similarity check (`:689`), quality
scoring (`quality-scorer.service.ts:402`), collision detection
(`fact-collision.service.ts:421`) and the contradiction correlation
(`contradiction.service.ts:230`). The eighth leaves it out on purpose, and the
omission is the interesting part: `getFactsAsOf` (`:441-458`) answers what was
believed at one time about the world at another, and composes `recorded_at`,
`superseded_at`, `valid_from` and `valid_until` instead. Filtering that query to
`active` would answer the question wrong — a fact superseded last week was the
belief the week before, which is exactly what the caller asked for. This atlas
more often finds the opposite: a predicate that holds on the main read and
silently lapses on a second one. Here the one read without it is the one that
should not have it.
