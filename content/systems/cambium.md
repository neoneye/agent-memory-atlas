...
matrix:
  ...
  trust: "Four independent status axes that must not be merged and an evidence-maturity ladder from signal to validated, all validated against a controlled vocabulary; the one axis any tool reads is the coverage disposition, and it decides what enters the authoring queue rather than what may be believed"
...
**2026-09-19** — [`6ed60b8cbacf121fa11f6b0753746e4b6d9b7216`](https://github.com/KimGLee/Cambium/commit/6ed60b8cbacf121fa11f6b0753746e4b6d9b7216) — **`trust_state` is withdrawn**, at the unchanged pin, on the same narrower reading that took the mark off this atlas's other documentation corpus. The record rested on the vocabulary and its checker, and both are excellent: `check_vocab.py` refuses an empty, unparseable or profile-less artifact rather than treating an absent vocabulary as a pass, and `test_a_truncated_vocabulary_does_not_pass_the_same_page` is a genuinely discriminating control. But that is a test on the validator, not a use of the state. Asking which axis any tool reads back gives a clear answer. `evidence_maturity` — the epistemic ladder, the one carrying `contested` and `superseded` — has exactly one mention anywhere in `Tools/` outside the test suite, and it is inside `check_vocab.py`'s own docstring: nothing consults it. `lifecycle: retired` and `merged` reach `check_links.py`, which reports a link to a retired page as `result=candidate`, a suggestion to repoint, explicitly not a failure — advice, not exclusion. `learning_status` has no non-test consumer at all. The axis that *is* load-bearing is `coverage_disposition`, tested against `"required"` as an allowlist in the queue compiler, the queue runtime, the amendment policy and the audit planner — but that decides which pages enter the authoring backlog, not which may be treated as true. And `Tools/mcp_server.py`, the agent-facing surface, contains no mention of status, lifecycle, superseded or deprecated. So the status axes are a validated contract and a planning input; no page is withheld from being believed. `audit_log` stands, and the receipt machinery described above is untouched. Two stale citations are retired with the mark: `Tools/vocab.yaml` does not exist at this pin — the composed artifact is written to `.cambium/derived/vocab.yaml` from `kernel/K08 Metadata and Status/vocabulary-base.yaml` — and `Tools/check_vocab.py` is one of the shims a previous reading already noted, with the implementation at `Tools/knowledge/metadata/check_vocab.py`. Nothing was installed and no suite was run.

**2026-09-18** — [`6ed60b8cbacf121fa11f6b0753746e4b6d9b7216`](https://github.com/KimGLee/Cambium/commit/6ed60b8cbacf121fa11f6b0753746e4b6d9b7216) — re-read at the same commit; `main` has not moved, so the correction is this report's. **Human review withdrawn**, and the project's own help text is the clearest statement of why. The record rested on *"the applier that only an Integrator may run"*, and the Integrator is not established anywhere — it is asserted by the caller. `apply_metadata_transition.py:249-250` declares the argument as `choices=("integrator", "worker")` with the help string **"declared caller role; only integrator may write"**, and the guard at `:69-70` compares that declared string. `record_gate_attestation.py` is the same shape one layer over: `--actor-role` and `--statement` are both required CLI arguments, and `manual_attestation.require_statement` checks only that the statement is a non-empty string. So the human attestation is a sentence the caller types beside a role the caller names.

The machinery around it is genuinely careful and none of this touches it: the compare-and-swap across Profile, K00, the metadata contract, the page and Coverage under a shared writer lock, the typed producer receipt validated before acting, and the restoration of both Coverage and the exact page before-image on a pre-commit failure. That is what keeps `audit_log`. What it is not is an approver the writer cannot impersonate.

**2026-09-13** — [`6ed60b8cbacf121fa11f6b0753746e4b6d9b7216`](https://github.com/KimGLee/Cambium/commit/6ed60b8cbacf121fa11f6b0753746e4b6d9b7216) — re-read, 140 commits past the previous pin across 1,008 files. All three marks stand, and every path they rest on had to be re-resolved: the `Tools/` scripts the evidence cited are now 22-line shims whose docstrings read *"Stable public command; implementation lives at…"*, with the implementations moved under `Tools/platform/common/`, `Tools/knowledge/metadata/` and `Tools/execution/evidence/`. `kblib.py` and `metadata_gate_runtime.py` no longer exist at the cited paths at all; the records now name where the code is. The audit writer gained a distinction worth recording: `write_receipts_observed` returns an outcome rather than raising, and separates a writer error *after* durable bytes — reported `present` with the error attached — from a partial or truncated record, reported `uncertain`, because a canonical transaction caller uses exactly that difference to decide whether a rollback is closed or must keep its recovery lock. Screened again first; nothing was installed and no suite was run.
