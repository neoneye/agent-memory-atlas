...
**2026-09-19** — re-read at the same pin [`61561508a926c889487a5fcabe54ba9771100863`](https://github.com/Dicklesworthstone/cass_memory_system/commit/61561508a926c889487a5fcabe54ba9771100863), still the tip, as the first of a corpus re-test of `trust_state` against the line this atlas drew during the `human_review` sweep: a state answers *"may this be acted on"* and is used for filtering, while a number answers *"how sure"* and is used for ranking. **The mark holds on the filtering side.** `getActiveBullets` (`src/playbook.ts:517-523`) drops a bullet whose `state` is `retired`, whose `maturity` is `deprecated`, or which carries the `deprecated` flag, and `exportToMarkdown` — the function that renders the playbook an agent reads — starts from it at `:537`, as do `getBulletsByCategory`, `rule-validation.ts:226` and the count in `info.ts:89`. Nothing multiplies a score by a tier here. The caveat already on the record was re-checked and stands: `draft` is not in the exclusion set, so a draft rule reaches the agent while a retired one does not. `tombstone` and `negative_eval` were not re-derived in this pass and keep their existing evidence. Screened again first; nothing installed, built or run.
