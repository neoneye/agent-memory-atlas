...
revision: c7da9d5e762adeb431d4f1f6e01fd6530edb4ad1
revision_url: https://github.com/caura-ai/caura/commit/c7da9d5e762adeb431d4f1f6e01fd6530edb4ad1
...
capability_evidence:
  ...
  human_review: "a staged-candidate inbox where a person approves, rejects, quarantines, defers or edits before anything becomes active | core-api/src/core_api/routes/skills_inbox.py:1-30, :71-90, :502-556, :682, :818, :977, :1035, :1108 | the module names itself *\"Skills Inbox — HITL endpoints\"*: `GET /` lists staged candidates and five POST routes land status transitions — `approve` (staged to active, with a pre-apply rescan), `reject` (staged to rejected, plus the poison-table write), `quarantine` (staged to quarantined, for security review), `defer` (stamps `deferred_at` so the distiller can revise) and `edit` (revises content, rehashes, rescans, stays staged). A candidate that is not approved never reaches an agent's palette | tests/test_skills_inbox*.py and the skill-factory suites; the routes are gated on `org_settings.skills_factory.enabled` and return 403 `SKILLS_FACTORY_DISABLED` when the flag is off"
