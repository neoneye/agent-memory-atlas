...
revision: c4406e4c97ecdca253ed1552c528c46f659e2df4
revision_url: https://github.com/caura-ai/caura/commit/c4406e4c97ecdca253ed1552c528c46f659e2df4
...
capability_evidence:
  ...
  trust_state: "an eight-value status of which exactly three are live, applied as a set-membership filter on the read path | common/enrichment/constants.py:235-247, common/constants.py:25, common/models/memory.py:87-90, core-storage-api/src/core_storage_api/routers/memories.py:950, :1114, core-api/src/core_api/services/crystallizer_service.py:666, :922 | `MEMORY_STATUSES` is `active`, `pending`, `confirmed`, `cancelled`, `outdated`, `conflicted`, `archived`, `deleted`, and the enrichment LLM may downgrade a write to `cancelled` or `conflicted` at capture. `LIVE_MEMORY_STATUSES` is the three-value subset `(active, confirmed, pending)`; reads filter to it, so five of the eight withhold a memory from recall entirely. `confidence` is a separate nullable float documented as confidence in the claim, and `is_inferred` marks a memory the system materialised rather than one that was stated — so ranking, provenance and admissibility are three different fields | tests/test_a69_degraded_extraction_is_visible.py, tests/test_p6_1_type_decay.py, the crystallizer suites"
  bitemporal: "a validity window on the memory row queried by a `valid_at` parameter, distinct from the row's creation time | common/models/memory.py:83-84, core-storage-api/src/core_storage_api/services/postgres_service.py:2781-2810, :3202-3213 | a memory carries `ts_valid_start` and `ts_valid_end` beside `created_at` and `updated_at`. A search taking `valid_at` applies a hard filter on the start side at day granularity — `ts_valid_start IS NULL OR ts_valid_start::date <= valid_at::date` — with the day cast explained in place, because a strict timestamp comparison *\"excludes same-day memories written a few hours after the query was asked\"*. On that path the end side is deliberately soft: a past `ts_valid_end` multiplies a `currency_factor` rather than excluding the row, so *\"an over-eager enrichment date can't silently hide a semantically strong memory from historical-question queries\"*. `memory_find_successors` (:3639) answers the same parameter differently, hard-filtering both bounds at timestamp granularity — so a true as-of window exists, on the successor path rather than on search | tests/test_a55_contradiction_model.py and the temporal suites; the `valid_at` path is exercised through the search API tests"
  scope_enforced: "tenant, fleet and a three-tier visibility key, applied through one shared clause builder | core-api/src/core_api/constants.py:217-225, common/models/memory.py:92-96, core-storage-api/src/core_storage_api/services/postgres_service.py:262-294, tests/test_c27_strict_fleet_scoping.py:95-107 | `MEMORY_VISIBILITIES` is `scope_agent`, `scope_team`, `scope_org`, stored on the row with a `scope_team` default, beside `tenant_id`, `fleet_id` and `agent_id`. `_fleet_scope_clause` builds the fleet disjunction plus the org-visibility escape in one place, and its docstring says why: *\"A54 established what happens otherwise — the identical predicate lived in several queries, one was fixed, and the leak simply moved to the next copy. Every fleet-scoped read builds its clause here so 'strict' cannot mean different things in different queries\"*. A committed structural test enforces that centralisation rather than trusting it: `test_the_multi_fleet_predicate_is_built_in_exactly_one_place` reads the module's own source with `inspect.getsource` and asserts `fleet_id.in_(fleet_ids)` occurs exactly once and `_fleet_scope_clause(` exactly five times — the definition and four fleet-scoped reads — so a sixth hand-rolled copy fails the suite | tests/test_a13_fleet_less_visible_to_fleet_scoped_search.py:119-260 (four cases including fleet X visible to X and not visible to Y), tests/test_cross_tenant_auth.py, tests/test_cross_tenant_audit_surfaces.py, tests/test_entity_relations_scope.py"
  ...
  human_review: "a staged-candidate inbox where a person approves, rejects, quarantines, defers or edits before anything becomes active | core-api/src/core_api/routes/skills_inbox.py:1-30, :146-182, :720, :856, :1015, :1073, :1146, core-api/src/core_api/auth.py:24-34, :235 | the module names itself *\"Skills Inbox — HITL endpoints\"*: `GET /` lists staged candidates and five POST routes land status transitions — `approve` (staged to active, with a pre-apply rescan), `reject` (staged to rejected, plus the poison-table write), `quarantine` (staged to quarantined, for security review), `defer` (stamps `deferred_at` so the distiller can revise) and `edit` (revises content, rehashes, rescans, stays staged). A candidate that is not approved never reaches an agent's palette. The producer cannot clear its own inbox: all five mutating handlers call `_require_inbox_admin`, whose docstring records why the check is centralised — *\"a missed handler is a privilege-escalation bug\"* — and admin status is `is_admin or org_role == \"admin\"`, an axis the gateway emits only for user principals and deletes on the API-key path, because *\"keys carry no org membership, and admin surfaces stay human-only\"*. The limit to state is OSS path 3: in standalone mode `auth.py:31` gives every caller `org_role=\"admin\"`, so the gate is an enterprise-deployment property rather than an unconditional one | tests/test_skills_inbox*.py and the skill-factory suites; the routes are gated on `org_settings.skills_factory.enabled` and return 403 `SKILLS_FACTORY_DISABLED` when the flag is off"
...
**The scope clause exists because the leak moved.** `_fleet_scope_clause`
...
- **Scope clause.** `_fleet_scope_clause`
  (`core-storage-api/.../postgres_service.py:262-294`) → `fleet_id IN (...)`,
...
- **Read.** hybrid search → tenant and fleet predicates, status in
  ...
  on `ts_valid_start` at day granularity (`:2781-2810`) → vector and full-text
...
`memory_find_successors` (`:3639`) `valid_at` hard-filters *both* bounds at
...
**The reference clock for recency is now itself a parameter.** A search's
freshness decay and its `temporal_boost` — the 1.3× multiplier that answers
*last month* — both measured from `now()` against the row's `created_at`, which
is record time. `postgres_service.py:2591-2602` adds a `freshness_reference`
setting: at `1`, and only when the request also carried a `valid_at`, both the
decay and the window anchor to that instant instead, and the window compares the
row's event time rather than its ingest time (`:3116-3122`). The reason is
stated where the switch is: *"a corpus ingested in one sitting still has a 'last
month'."* That is the single-clock failure in one sentence — load a decade of
documents on a Tuesday and every one of them is equally fresh, so a recency
term ranks nothing. The gating is the part to copy: the knob alone changes
nothing, `valid_at` alone changes nothing, and only the conjunction retargets
the clock, so the new behaviour cannot reach a caller who did not ask for it.
It stays opt-in, which means the default search still ranks an as-of question by
how recently the answer was ingested.
...
**2026-09-19** — re-pinned to [`c4406e4c97ecdca253ed1552c528c46f659e2df4`](https://github.com/caura-ai/caura/commit/c4406e4c97ecdca253ed1552c528c46f659e2df4), 8 commits on. All seven marks stand. `human_review`'s anchors all moved and were re-verified, and the record gained the producer test it had been asserted without — which matters here, because the corpus-wide re-test of that mark has been withdrawing it from systems whose approve verb turns out to sit on the agent's own surface. It does not here, and the code says why in its own words: every one of the five mutating handlers calls `_require_inbox_admin`, centralised because *"a missed handler is a privilege-escalation bug"*, and admin status reads `is_admin or org_role == "admin"` — a header the gateway emits only for user principals and deletes on the API-key path, since *"keys carry no org membership, and admin surfaces stay human-only."* The docstring is unusually careful about what that does and does not buy, and the report now carries the honest limit beside it: OSS standalone mode gives every caller `org_role="admin"`, so the gate is a property of the enterprise deployment rather than of the code alone. Screened again first; nothing was installed and no suite was run.

**2026-09-16** — [`c7da9d5e762adeb431d4f1f6e01fd6530edb4ad1`](https://github.com/caura-ai/caura/commit/c7da9d5e762adeb431d4f1f6e01fd6530edb4ad1) — re-read after 73 commits across 188 files. All seven marks hold. Three anchor sets were byte-identical at both commits and needed no re-reading — the poison module and the Forge service that consults it, the audit model, and the fleet-visibility test file — and the rest were re-derived: `LIVE_MEMORY_STATUSES` is still the same three-value subset and still applied as a set-membership filter rather than a literal `active`, the validity window still hard-filters the start side on search and both sides on the successor lookup, and the inbox still lands five status transitions behind the org flag. One correction carried over from the first reading: the shared scope-clause builder is named `_fleet_scope_clause`, not `_fleet_visibility_clause` — the earlier name appears nowhere in the tree at either commit, and it had propagated into the report's evidence record, its implementation-paths list, its matrix and the overview. Found alongside it, and now recorded, is the structural test that protects the centralisation: `tests/test_c27_strict_fleet_scoping.py:95-107` reads the module's own source and asserts the multi-fleet predicate is built in exactly one place. The retrieval change is written up in section 6 — a `freshness_reference` setting that re-anchors recency decay and the *last month* window from `now()` to the request's `valid_at`. Re-screened at this commit from a full clone: three auto-run surfaces, four build-time execution paths, ten floating versions, twelve manifests inside the cooldown. Nothing was installed, built or run.
