...
capability_evidence:
  ...
  human_review: "the challenge and adjudication path, role-gated, with the parked clause withheld while it waits | smart-app-service/main.py:12318, :12340 (`_require_memory_curator`), :12156-12176, smart-app-service/auth.py:204-212, smart-app-service/clause_store.py:65, :70-83 | `challenge_clause` is *\"a supervisor stopped this judgement pending adjudication\"* — a `challenged` status outside `LIVE_STATUSES`, so the clause stops firing until a named person decides. The design states why it is not a score: *\"The fix is deliberately NOT a trust tier. Weighting officers by seniority would encode org hierarchy into an audit trail a regulator reads, and seniority is not correctness. This is a ROLE-held stop: it does not rank anyone, it parks one clause and forces a named human to decide.\"* Quarantine, retire and SOP-conflict resolution are separate endpoints behind `_require_memory_curator`, each recording actor and cause. The gate is an identity check rather than a parameter: `_require_memory_curator` calls `_can_edit_app` with the caller's user id, tenant, org, departments, roles and SA-admin set, every one of them read from the request's authenticated state rather than from the body — `get_secure_user_id` returns `request.state.authenticated_user_id` or raises 401 — and the docstring draws the line the mark needs: the memory surface *\"is a curation/owner surface, NOT an end-user one: a plain audience member who can merely open the app must not read or change it\"*. `challenged` sits outside `LIVE_STATUSES` (`clause_store.py:65`), so the clause stops matching while it waits | smart-app-service/tests/test_clause_store.py:1247 — `test_dismissing_a_challenge_cannot_lift_an_admin_quarantine`, a regression test for a privilege-escalation hole in this exact path"
...
**2026-09-19** — audited at the unchanged pin [`6f200f2992291eda45c23d95c5e231b82e8f1cf3`](https://github.com/Trustedwear-Tech/citra-decision-system/commit/6f200f2992291eda45c23d95c5e231b82e8f1cf3); nothing upstream moved. All five marks stand, and `human_review`'s anchors were corrected — the challenge endpoint is at `main.py:12318`, not the line range the previous record cited — and the record gained its producer test. It passes on the strongest of the available grounds: `_require_memory_curator` (`:12156`) resolves owner, dept-admin, org-admin and super-admin from the request's authenticated state, and `get_secure_user_id` returns `request.state.authenticated_user_id` or raises 401, so none of it can be supplied in a call. The parked clause really is withheld while it waits: `challenged` is outside `LIVE_STATUSES`, which is `("active", "candidate", "dissented")`. The comment explaining why this is a role-held stop rather than a trust tier remains the clearest statement of that distinction in this corpus. Screened again first; nothing was installed and no suite was run.
