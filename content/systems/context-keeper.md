...
capability_evidence:
  ...
  trust_state: "a three-value status and a three-value origin, both stored, with the strongest filters on the summary and the write path rather than on the tool named for context | server.py:1003-1009, :1105-1112, :1255-1261, :1688-1696, :1851-1853, :2069, :2170, :2295-2299, :2505-2521, :2927 | `status` is `active`, `superseded` or `deprecated`; `_VALID_ORIGINS` is `user`, `agent` or `import`, and an unrecognised origin is coerced to `agent` rather than trusted. The clearest uses are allowlists: `get_project_summary` reads decisions, pipelines and constraints and keeps only `status == \"active\"` in each, and the constraints-rules writer does the same before emitting a file per scope. The write path is guarded the same way — the near-duplicate check ignores entries that are deprecated or superseded, and only an `active` entry may be superseded, so the ladder cannot be re-entered from below. Recording an entry that supersedes another sets the old one to `superseded` in place; `deprecate_entry` sets `deprecated` with a required reason. Two read paths are weaker and the difference is worth carrying: `get_context` and its related-entry expansion exclude `deprecated` only, so an entry the system itself retired by supersession is still assembled into context, and `query_entries` returns every status unless asked otherwise — which its own tool description states, *\"Includes superseded/deprecated unless you filter status\"* | tests/test_server.py:1872-1882 (a deprecated constraint is absent from `get_context` after `query_entries` was added — a guard the file names as such), tests/test_supersession_survey.py"
...
matrix:
  ...
  trust: "`status` gates the project summary and the constraints file, and the write path, though `get_context` excludes only deprecated entries; `origin` records whether a user, an agent or an import wrote it and an unknown value is coerced to `agent`; `hardness` grades a constraint. `get_context` annotates a response with `no_confident_match` and `top_relevance` when the top entry's tag-and-text overlap falls below a 0.20 floor — chosen as the highest value with zero false abstentions on the eval set — while still returning the results. Capture-time conflict labelling separates a restatement from a contradiction"
...
**2026-09-19** — [`d08355a46548d5a20d9170d3271e2054c51a4db9`](https://github.com/jarmstrong158/context-keeper/commit/d08355a46548d5a20d9170d3271e2054c51a4db9) — `trust_state` re-tested at the unchanged pin; the mark holds and one sentence in the record was wrong. It said *"`get_context` drops non-active entries"*. It does not: both status tests inside `handle_get_context` (`:2069` on the assembled set, `:2170` on the related-entry walk) compare against `"deprecated"` alone, so a `superseded` entry is assembled into context — and `superseded` is the value the system writes *itself* when a newer entry supersedes an older one, while `deprecated` is the one a caller must ask for with a reason. The allowlist the record was describing is real but lives elsewhere: `handle_get_project_summary` keeps `status == "active"` across decisions, pipelines and constraints, and the rules writer does the same before emitting a file per scope. The write path is guarded the same way — the near-duplicate scan ignores deprecated and superseded entries, and only an `active` entry may be superseded — so the mark rests on those. Two things found alongside are worth a reader's time. `query_entries` returns every status by default and says so in its own tool description, which is the honest version of a default this atlas has seen claimed the other way round. And the abstention comment at `:2199` is the narrowed reading of this mark, arrived at independently: the composite score *"is inflated by recency/status/origin, so even a totally irrelevant active entry ranks ~55 and looks confident"*, so the honesty signal was re-keyed to tag-and-text overlap alone — a project noticing that folding a state into a number stops the number answering how sure, and separating them. Nothing was installed and no suite was run.
