...
**And that move is larger than passing the argument.** `Database::memories` does carry a scope predicate, `(?2='' OR scope=?2)`, so the enum can reach SQL; the two keys the handoff actually matches on cannot. `project` appears in that statement only inside the substring haystack `instr(lower(title || ' ' || body || ' ' || tags || ' ' || project), …)`, and `agents` is a JSON array serialised into a text column that the statement never mentions at all — an agent-scoped filter would need a JSON containment test, not a column compare. The project-path half already exists a few hundred lines away: `database.rs:262` counts a project's memories with `WHERE deleted_at IS NULL AND scope='project' AND project=?1`, which is exactly the predicate the builder re-implements in TypeScript. The fix is not hypothetical; half of it is written down elsewhere in the same file tree.
...
**2026-09-18** — [`77d0acbdeda5769d181e7398dcea32be4e70edc9`](https://github.com/Ste2027/ContextMeld/commit/77d0acbdeda5769d181e7398dcea32be4e70edc9) — re-read at the same commit; nothing upstream moved. Both marks re-verified against the code and both hold: the handoff filter is still the only scope predicate on the path that reaches an agent, and `Insights.test.tsx` still asserts the foreign project's memory absent beside a global one present. The report's own recommendation is sharpened, because it was cheaper-sounding than it is. `Database::memories` does carry a scope predicate — `(?2='' OR scope=?2)`, live whenever `MemoryLibrary` passes a scope — so the enum can reach SQL; but `project` appears in that statement only inside the substring haystack, and `agents` is a serialised JSON array the statement never mentions, so neither of the two keys the builder actually matches on can be expressed there as written. The project-path half of the predicate already exists at `database.rs:262`, counting a project's memories with `scope='project' AND project=?1` — the same test the handoff re-implements in TypeScript.
