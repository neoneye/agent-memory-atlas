...
revision: 1e58077c1e80cae83618b6511f383bdc813f2f2c
revision_url: https://github.com/cdeust/Cortex/commit/1e58077c1e80cae83618b6511f383bdc813f2f2c
analyzed_at: 2026-09-16
capabilities: "trust_state"
capability_evidence:
  trust_state: "supersession is a stored link exposed as a view that the recall paths join against, so a corrected memory keeps its row and leaves retrieval | mcp_server/infrastructure/pg_schema.py:81, :95-97, mcp_server/hooks/auto_recall.py:146-151, mcp_server/hooks/session_start.py:155-162 | `memories.superseded_by_id` references another memory, and `CURRENT_MEMORIES_VIEW_DDL` publishes `SELECT * FROM memories WHERE superseded_by_id IS NULL` as `current_memories`. Both recall paths join it — automatic recall and session start each `JOIN current_memories cm ON cm.id = m.id` and repeat `AND m.superseded_by_id IS NULL` — so the superseded row survives with its link to what replaced it while dropping out of what an agent is given. The wiki layer carries three further status machines with `CHECK` constraints: concepts (`candidate`, `saturating`, …, with a partial index on the first two), drafts (`pending | approved | rejected | published`), and pages (`proposed | accepted | rejected | deprecated | superseded`, with their own `superseded_by`) | mcp_server/infrastructure/pg_schema.py:198-199, :232-233, :256-265"
...
**2026-09-16** — [`1e58077c1e80cae83618b6511f383bdc813f2f2c`](https://github.com/cdeust/Cortex/commit/1e58077c1e80cae83618b6511f383bdc813f2f2c) — first reading, at a commit dated 16 September 2026. Screened before opening, from a shallow clone: twenty-five files scanned, four auto-run surfaces, four build-time execution points, no unpinned surfaces and four dependency files inside the seven-day cooldown. Nothing was installed, built or run.
