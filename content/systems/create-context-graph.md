...
description: "A Neo4j Labs CLI that scaffolds an agent application around a three-tier context graph, with thirteen connectors importing SaaS and session history into it."
root: ../..
page_kind: system
...
licence: "Apache-2.0"
size: "16,962 lines of Python in 48 files and 13,421 lines of Jinja2 templates in 65 files under src/create_context_graph"
activity: "242 commits on main by 11 author names, one of them a bot, 21 March – 4 September 2026"
tests: "1,125 pytest functions in 28 files (19,839 lines), 41 parametrized"
capabilities: "scope_enforced, negative_eval"
capability_evidence:
  scope_enforced: "the generated app's REST read paths, self-hosted bolt backend, over nodes the generated seeder writes | src/create_context_graph/templates/backend/shared/generate_data.py.j2:68-73, src/create_context_graph/templates/backend/shared/context_graph_client.py.j2:293-305, :311-319, :329-345, :371, :422-429, src/create_context_graph/templates/backend/shared/routes.py.j2:341, :354, :378, :398, :424-427 | `make seed` runs `generate_data.py`, which MERGEs every entity on `{name, domain}` with `domain` from `settings.domain_id`. Nine read paths put `(x.domain IS NULL OR x.domain = $domain)` in their Cypher with `settings.domain_id` bound: `search_entities`, `get_entity_graph`, `get_schema`, `get_schema_visualization` and `expand_node` in the client, and `list_documents`, `get_document`, `list_traces` and `get_entity_detail` in the routes. What narrows it: the predicate is null-permissive; the CLI's own bolt ingest through neo4j-agent-memory (at the library's python-v0.5.0 tag) and the generated `make import` write no `domain` node property, so their nodes are visible from every domain; `expand_node` filters neighbours and not the node it is handed; no bundled agent tool's Cypher names `$domain`; the front end's schema-node click sends an unscoped query through `POST /cypher`; and on the default NAMS backend no domain property is written at all. The nearest test, tests/test_integration.py:309-334, queries the graph directly with `n.domain = $domain`, not through any of the nine read paths, and asserts zeros without a positive control | unknown"
  negative_eval: "the NAMS document read path, search fallback branch | tests/test_memory_adapter.py:135-149, :176-196 | `test_skips_typed_non_document_entities` feeds OBJECT, PERSON and ORGANIZATION records to `list_documents_nams` and asserts the returned title set is exactly `{\"DocA\"}`, so the excluded records are named and the kept one is present. `test_strips_ccg_edges_block_from_preview` asserts one result, that neither `ccg-edges` nor `_pole_type` appears in its preview, and that `chest pain` does. Both build the client as a `MagicMock` with only `long_term.search_entities` set, so awaiting `client.query.cypher` raises and the function takes its fallback; the preferred Cypher branch has no behavioural test. Both run in the default `pytest tests/` invocation the CI test job uses on every push and pull request | the same file"
...
matrix:
  ...
  update_delete: "Nothing on the default backend — the CLI and `make reset` report that no delete endpoint exists rather than pretending. On bolt the designed eraser is `MATCH (n) DETACH DELETE n`, and the free-form Cypher route and agent tool accept any write, a targeted delete included. No per-memory delete, TTL, supersession or expiry is designed anywhere in the tree"
  scoping: "A `domain` property MERGEd into the node key by the generated seeder on bolt and filtered as `n.domain IS NULL OR n.domain = $domain` in nine REST read paths. The CLI's library ingest and the generated importer write no node-level domain, no bundled agent tool's Cypher names it, and on the hosted backend no domain property is written at all, so the boundary there is the API key's workspace"
  ...
  risks: "On the default backend every relationship is encoded into a text field nothing in the tree reads back, entity properties collapse into prose, and nothing can be deleted; on bolt the agent's Cypher tool is described to the model as read-only and accepts writes; the generated document browser returns any OBJECT-typed entity as a document; and the domain-scoping test's assertion is `len(...) >= 0`"
---

## 1. Executive Summary

Create Context Graph is a Neo4j Labs CLI — `uvx create-context-graph` or `npx create-context-graph` — whose interactive wizard writes out a complete agent application: a FastAPI backend, a Next.js and Chakra UI frontend with an NVL graph view, a domain ontology, Cypher schema, fixtures, and an agent against one of eight frameworks. Its import machinery is careful work: per-connector watermarks that advance only after a clean batch, a drainable deadletter, and a parity test over two deliberately duplicated write paths. The memory it scaffolds cannot correct or delete a single memory, encodes relationships on its default backend into text nothing reads back, and hands the agent a Cypher tool described to the model as read-only that writes freely on a self-hosted graph. Version 0.14.0 at the pinned commit of 4 September 2026.
...
**An entity** is a name plus a POLE+O type plus one description string. On bolt, an entity written by `make seed` also carries typed attributes and a `domain`; the CLI's library branch packs both into one metadata string. On the hosted backend `add_entity` is called with three keyword arguments and nothing else (`ingest.py:351-355`), so the type system is the description's `_pole_type: X_` suffix and a markdown attribute list. Identity is the name: re-ingest MERGEs onto it, which is the only deduplication in this tree.
...
Nothing moves between states. A memory is created and then is permanent: no `valid_from`/`valid_until`, no status column, no archival, no TTL — searches for all of them are in the appendix. The closest thing to a rejection record is the Claude Code connector's correction detector (`connectors/_claude_code/decision_extractor.py:162-269`), which reads a user message matching one of ten correction patterns, writes the preceding assistant turn as an `Alternative` with `wasChosen: False` and `reason: "User corrected this approach"`, and links it `REJECTED` from a `Decision` node. That is a durable record of something a person rejected.

Its name is `alt-{decision_id}-original`, where `decision_id` is twelve hex digits of a SHA-256 over the session id, the message index and `correction` (`:211`, `:489-492`). The key is where the turn sat in a transcript rather than the value, so a later import of the same bad suggestion produces a fresh node and walks straight past it. `tombstone` is withheld on exactly that.

Agency sits with the ingest pipeline and the library by design. The generated agent has no memory verb; it queries a graph someone else populated, and its own turns are captured for it. On a self-hosted scaffold that boundary is a sentence in a tool description rather than a session mode, as [section 8](#8-agent-integration) sets out.
...
flowchart TD
  ...
  BK -->|"bolt, --self-hosted"| B1["make seed MERGEs on {name, domain}; CLI library ingest and make import write no node-level domain"]
  B1 --> B2["relationships as native edges via Cypher MERGE"]
  B2 --> B3["Document nodes plus MENTIONS edges"]
  B3 --> BR["read: agent tools and run_cypher unfiltered and writable; nine REST paths filter n.domain, null-permissive"]
  ...
  NR --> DEL
  DEL -->|"bolt"| W["designed eraser: MATCH n DETACH DELETE n, the whole store"]
...
- **Scaffold-time write, bolt.** `_ingest_with_memory_client` (`:556-730`) applies the generated Cypher schema, calls `add_entity` *with* `attributes={**item, "domain": domain_id}` (`:609-615`), MERGEs each relationship as a native edge behind `_require_safe_cypher_identifier` (`:126-130`), MERGEs `:Document` nodes and links `MENTIONS` by substring containment of an entity name in the document body (`:675-684`), and writes `:DecisionTrace`/`:TraceStep`. `_ingest_with_driver` (`:733-901`) is the same without the library, and MERGEs entities on `{name, domain}` (`:787`). The CLI takes the library branch whenever `neo4j_agent_memory` imports, which the optional `ingest` extra installs, and the driver branch otherwise (`ingest.py:1059-1077`, `pyproject.toml:41-43`).
- **What the library does with those attributes.** At neo4j-agent-memory's `python-v0.5.0` tag ([`8bd50eff9debbe7211a46b2d66ac74c94422f625`](https://github.com/neo4j-labs/agent-memory/commit/8bd50eff9debbe7211a46b2d66ac74c94422f625)), `add_entity` MERGEs `(e:Entity {name: $name, type: $type})`, adds only the POLE type as a label, and serializes `attributes`, `domain` included, into one `e.metadata` string (`memory/long_term.py:504-525`, `graph/query_builder.py:270-291` there). The ontology label and a `domain` node property never reach an entity written this way. The CLI's `uv.lock` resolves that extra to 0.0.5, which has no tag in the library's repository and which I did not read.
- **Bolt writes in the generated project.** `make seed` runs `generate_data.py`, which MERGEs every entity on `{name, domain}` with ontology labels (`templates/backend/shared/generate_data.py.j2:68-73`). `make import` runs `_ingest_via_bolt` (`templates/backend/connectors/import_data.py.j2:428-`), which MERGEs entities on `{name}` alone (`:485`) and documents on `{title}` alone (`:547`) and stamps no `domain` on either.
...
- **Retrieval.** `execute_cypher` (`context_graph_client.py.j2:253-287`) dispatches to `client.query.cypher` on NAMS and to a driver session on bolt. `search_entities` (`:290-305`), `get_entity_graph` (`:308-322`), `get_schema` (`:325-352`), `get_schema_visualization` (`:354-`) and `expand_node` (`:419-432`) are the domain-filtered helpers behind the REST routes, and `list_documents`, `get_document`, `list_traces` and `get_entity_detail` in `routes.py.j2` (`:327-440`) filter the same way; `memory_adapter.py.j2` holds the NAMS equivalents, each cypher-first with a REST search fallback.
...
- **Redaction.** `connectors/_claude_code/redactor.py:23-61` — fourteen patterns for Anthropic, OpenAI, GitHub, Slack and AWS credentials, bearer headers, generic key and password assignments, credentialed connection strings and `.env`-style variables — called at `claude_code_connector.py:252`, `:306`, `:382` and `:411`.
...
| --- | --- | --- |
| Entity identity | `MERGE (n:Label {name, domain})` from `make seed` and the CLI's driver branch; `{name}` alone from `make import`; `(e:Entity {name, type})` from the CLI's library branch | `add_entity(name=…)`, server-side merge by name |
| Properties | native node properties, plus `domain` from `make seed`; one serialized `e.metadata` string on the library branch | one markdown block inside `description` |
| Relationships | native typed edges via `MERGE` | a fenced ```ccg-edges``` block inside the source entity's `description` |
| Documents | `:Document` MERGEd on `{title}` with `content`, `template_id`, `template_name`, and `domain` everywhere but `make import`, plus `MENTIONS` | an OBJECT entity *and* a `short_term` message |
...
**Scoping** is the `domain` string, and its writers decide what it protects. `make seed` MERGEs it into the node key (`generate_data.py.j2:73`), so two seeded domains sharing one Neo4j instance cannot collide on a name. The CLI's library branch packs it into `e.metadata`, and `make import` writes none, so imported entities of one label merge across domains by name.

It is read back under `(x.domain IS NULL OR x.domain = $domain)` on nine paths: five helpers in `context_graph_client.py.j2` (`:296`, `:314`, `:331`, `:371`, `:427`) and four handlers in `routes.py.j2` (`:341` and `:354`, `:378`, `:398`, `:424`). The predicate is permissive, so every node written without the property, library-written entities included, is visible from every domain. `expand_node` filters neighbours and not the node it is handed by `elementId`. The front end's schema-node click sends `MATCH (n:<label>)-[r]-(m)` through `POST /cypher` with no predicate (`ContextGraphView.tsx.j2:238-243`).

None of the 193 agent tools across the bundled domains names it; the search is in the appendix. On the hosted backend the property is never written, so the boundary there is the workspace behind the API key — a physical partition, which is a real boundary and a different mark.
...
**The vector index has no reader.** `main.py.j2:71-75` calls `create_vector_index()` at startup, creating `entity_embeddings` on `(:Entity).embedding` at 1536 dimensions. No code in this tree writes that property; the library does, setting `e.embedding` on `:Entity` in `add_entity` at its `python-v0.5.0` tag whenever an embedder is configured. The bolt default embedder here is local `all-MiniLM-L6-v2` (`memory.py.j2:120-132`), which emits 384 dimensions. Neither `vector_search` nor `hybrid_search` in `vector_client.py.j2` has a caller anywhere; both searches are in the appendix.

The ontology's own vector index is emitted as commented-out Cypher with a note that dimensions must match the embedder. So the generated app's retrieval is graph traversal plus substring matching, and the vector file is scaffolding an adopter has to finish.
...
**Deduplication in this tree is MERGE on a name key and nothing more.** On bolt, the seeder's `MERGE (n:Label {name, domain}) ON CREATE SET … ON MATCH SET …` overwrites in place, so a re-import replaces properties with no record that they changed. On NAMS the service merges by name. The library's `add_entity` adds embedding-similarity deduplication when an embedder is configured, which is its mechanism rather than this repository's. Two different people called "J. Smith" are one entity in both. There is no conflict detection, no contradiction pass and no consolidation anywhere in the tree.

**Deletion.** `reset_memory_store` (`ingest.py:962-975`) branches: bolt runs `MATCH (n) DETACH DELETE n`, NAMS runs `_reset_nams`, which counts and refuses. The generated Makefile mirrors both — `make reset` on the hosted backend prints "NAMS exposes no delete API (neo4j-agent-memory 0.5.x) — reset is not available from the CLI." A test asserts the refusal path calls no delete (`tests/test_ingest_nams.py:437`, `delete_entity.await_count == 0`). No designed operation narrower than the whole store exists on either backend; on bolt, `POST /cypher` and the agent's Cypher tool run a targeted `DETACH DELETE` as readily as a read ([section 9](#9-reliability-safety-and-trust)).
...
**Eight frameworks, one shape.** PydanticAI, Claude Agent SDK, OpenAI Agents, LangGraph, CrewAI, Strands, Google ADK and a hand-rolled Anthropic tool loop. Each template differs only in how tools are registered and how streaming is collected; the memory calls, the session handling and the HTTP layer are identical. That uniformity is the scaffolder's real product and it is executed cleanly. `tests/test_matrix.py` scaffolds every bundled domain against every framework, 216 combinations at this commit, in the `full-suite` CI job; the README says 176 and the test's docstring 184.

**The agent has no memory verbs, and its read tool is not read-only on bolt.** It has no tool to save, forget or correct a memory; its affordance is Cypher over the graph and a message history the harness injects. Every framework template registers a free-form Cypher tool described to the model as read-only (`agents/pydanticai/agent.py.j2:80-91`, and the same in the other seven). On bolt it reaches `execute_cypher`, which runs the string in a default session (`context_graph_client.py.j2:273-287`), so a `MERGE`, `SET` or `DETACH DELETE` the model writes is executed. On NAMS, the project's changelog records the service rejecting write queries (`CHANGELOG.md:45`). Capture is automatic and invisible to the model.

**Sessions.** `--session-strategy` picks `per_conversation` (default), `per_day` or `persistent`, passed to the library's `SessionStrategy` at construction (`memory.py.j2:261-282`). The browser keeps the id in `sessionStorage` under `ccg-session-id-${DOMAIN.id}` (`ChatInterface.tsx.j2:51`, `:130`, `:192`), so it dies with the tab, as does the chat history kept beside it under `ccg-chat-history-${DOMAIN.id}` (`:50`, `:191`). On the hosted backend the server-id mapping is a module-level dict, so a backend restart silently starts a new conversation for a returning session id. There is no compaction-boundary handling; nothing bounds how much history `get_context(max_items=10)` returns beyond that default.
...
**Adapting it to another stack** is easy in one direction and not the other: the memory surface is four functions in one module, so swapping frameworks is trivial, but the whole design assumes a Neo4j-shaped store behind `neo4j-agent-memory` and nothing abstracts that.
...
**`POST /cypher` and the agent's Cypher tool on bolt accept writes.** The route's docstring says "On NAMS, only read queries are accepted (server enforces)", and the bolt branch passes the query to `session.run` with no access mode and no keyword filter (`routes.py.j2:274-292`, `context_graph_client.py.j2:273-287`). Searches for `execute_read`, `default_access_mode` and a write-keyword guard are in the appendix and return nothing. The endpoint is CORS-exposed to a local frontend, so a `DETACH DELETE` reaches a self-hosted store through the same door the graph view uses.

The agent's tool reaches the same function under a description that says read-only. Imported Slack messages, Jira descriptions and Gmail bodies come back to the model as tool results, so an instruction planted in imported content has a write path to the graph on a self-hosted scaffold.

**Trust is a float, and it is not consulted.** A correction decision carries `confidence: 0.75`, and the other three detectors assign between 0.5 and 0.9 (`decision_extractor.py:203`, `:295`, `:372`, `:446`). An explicit preference starts at 0.6 and climbs 0.15 per repeat to a 0.95 cap; a package-frequency preference scores 0.4 plus 0.1 per session up to 0.9; anything under 0.4 is dropped at extraction (`preference_extractor.py:112-121`, `:154`, `:170`). No read path consults either, and no bundled tool's Cypher names `confidence`. The `Alternative` node's `wasChosen: False` is the nearest thing to an epistemic state and it describes a past decision, not the status of a memory. `trust_state` is withheld; the closest field in the whole tree is the data-journalism demo ontology's `Claim.verdict` enum of true/mostly_true/…/unverified, which is a schema for the demo's *subject matter*, with nothing filtering on it.

**Privacy.** Claude Code redaction is well-wired — four call sites covering message content, tool output, error text and document bodies, and the generated connector template drops the toggle entirely and redacts unconditionally (`templates/backend/connectors/claude_code_connector.py.j2:87-97`). The other twelve connectors have no equivalent: a Slack channel or a Gmail thread is imported as-is. And the output lands in tracked paths — the generated `.gitignore` ignores `.env` and `data/documents/*.json` but not `data/fixtures.json` or `.context-graph/`, so an imported corpus and a deadletter carrying the records that failed to write are committed by default.
...
- `human_review` — the frontend displays entities, documents and traces and cannot alter any of them; the only POSTs it issues are `/chat/stream`, `/expand` and one fixed `MATCH` through `/cypher`, and the backend has no PUT, PATCH or DELETE route at all.
...
The suite is 1,125 `test_` functions across 28 files, with 41 `parametrize` decorators. CI runs a `test` job (`pytest tests/ -v` on Python 3.11 and 3.12) and a `lint` job on every push and pull request, a `full-suite` job adding `--slow --functional`, and a `smoke-test` job that is `main`-push-only *and* gated behind a `SMOKE_TESTS_ENABLED` repository variable. I ran none of it.

**The contract test is the best thing in the suite.** `tests/test_nams_ingest_parity.py` exists because `run_nams_ingest` and the rendered `import_data.py` are duplicated on purpose. It loads the rendered template into a namespace and drives both against one canonical fixture chosen to hit every branch: an entity with no body, one with a body, one with outbound edges, a document, a trace. It captures every client call from each and diffs the sequences, including splitting both descriptions on the `ccg-edges` fence and comparing the blocks (`:354-376`). It is the right instrument for the risk it was built for.

**The domain-scoping test cannot fail.** `tests/test_security.py:165-186`, parametrized over all twenty-seven domains, collects every agent tool whose Cypher contains `$domain` or `domain:` and then asserts `assert len(domain_scoped_tools) >= 0`. A list's length is never negative. The docstring and the failure message both describe a check the assertion does not perform — and the check would fail if it were written, because none of the 193 tool bodies in the bundled domains contains either substring; the parse is in the appendix. The whole class of "at minimum, the domain should have some tools with domain scoping" is unverified at this commit for every domain, and the repair is one character.

**What backs the marks.** `negative_eval` rests on `tests/test_memory_adapter.py:135-149`, which is the shape the mark asks for: three records in, PERSON and ORGANIZATION named as excluded, and the kept `DocA` asserted by set equality, so a function returning nothing fails. `:176-196` adds a preview exclusion with a positive control (`assert "chest pain" in result[0]["preview"]`). Both run unconditionally in the CI test job. Both hand `list_documents_nams` a `MagicMock` client with only `long_term.search_entities` set, so awaiting `client.query.cypher` raises and the function takes its search fallback; the Cypher branch the live service uses has no behavioural test.

**The scope-boundary case cannot fail on an empty graph.** `tests/test_integration.py:309-334` seeds financial-services and healthcare fixtures under two test domains and asserts zero nodes in one domain carrying any of the seven labels unique to the other. Nothing asserts those labels exist under the other domain. The query is its own `n.domain = $domain`, not one of the app's null-permissive read paths. It seeds through `ingest_data`, and the CI job installs every extra from the lockfile, so it takes the library branch at 0.0.5. Against the library's `python-v0.5.0` tag that branch writes no ontology label, and every count would be zero whatever the isolation. It is also doubly gated, and its `neo4j_driver` fixture calls `pytest.skip` when Neo4j is unreachable.
...
- **Encoding structure into a text field before the reader exists.** The lossy write is defensible when the target API cannot hold the shape. Shipping it while the docs claim a parser renders it is not, and it is the failure mode this pattern always takes: the encoder is easy and gets written, the decoder is deferred, and the round trip is never closed. Write the reader first, or state in the README that the data is quarantined until one exists.
...
- **Creating an index nothing queries.** A vector index over a property a dependency fills, at a dimension the default embedder does not produce, beside two uncalled search functions, reads from outside exactly like a working vector arm.
- **A read-only promise made in a tool description.** Telling the model a tool is read-only constrains nothing the model or an injected instruction writes. Open the session in read mode so the database refuses the write, and keep the free-form query tool off any surface that reads imported text.

### Fit

This is a demo generator and it is an unusually good one — twenty-seven ontologies, eight frameworks, a 216-combination render matrix in CI, and a frontend that makes a graph legible in about five minutes. Take it if you want to show someone what an agent over a knowledge graph looks like, or if you want a running skeleton to gut.

Do not take the generated app as a memory architecture to build a product on. It has no way to correct or delete a single memory on either backend, and its self-hosted agent can rewrite the graph through a tool it is told is read-only. Its default backend cannot store a relationship in a form anything reads back, and its agent never sees the long-term tier unless it writes the Cypher itself. The people best served here will replace `memory.py` and `context_graph_client.py` within a week and keep the ontology pipeline, the connectors and the import machinery, which is the reusable third of the repository.
...
- What the CLI's library branch writes at neo4j-agent-memory 0.0.5, the version its lockfile resolves and the integration job installs, which has no tag in the library's repository.
...
python3 - <<'PY'   # 193 tool bodies parsed by indentation, 0 containing '$domain' or 'domain:', the test's two substrings
import glob, re
tools, hits = 0, []
for p in sorted(glob.glob('src/create_context_graph/domains/*.yaml')):
    lines, i = open(p).read().split('\n'), 0
    while i < len(lines):
        m = re.match(r'^(\s*)cypher:\s*(\|[-+]?|>[-+]?)?\s*(.*)$', lines[i])
        if not m: i += 1; continue
        tools += 1; ind, body, j = len(m.group(1)), [m.group(3)], i + 1
        while j < len(lines) and (not lines[j].strip() or len(lines[j]) - len(lines[j].lstrip()) > ind):
            body.append(lines[j]); j += 1
        if '$domain' in '\n'.join(body) or 'domain:' in '\n'.join(body): hits.append(p)
        i = j
print(tools, hits)
PY
...
grep -rn "SET n.embedding\|\"embedding\":" --include='*.j2' --include='*.py' src/                                                               # 2 hits, both query parameters in vector_client.py.j2: no writer in this tree
...
grep -rniE 'detach|forbid|blocklist|denylist|_WRITE_|write_keyword|is_read_only' src/create_context_graph/templates/backend/shared/routes.py.j2 src/create_context_graph/templates/backend/shared/context_graph_client.py.j2 src/create_context_graph/templates/backend/agents/*/agent.py.j2   # 2: a docstring and reset_database's DETACH DELETE, no write guard
grep -rn 'read-only Cypher' src/create_context_graph/templates/backend/agents/                                                                  # 9 lines across all 8 templates
grep -rn 'domain IS NULL OR' src/create_context_graph/templates/                                                                                # 16: 9 in five client helpers, 6 in four route handlers, 1 in the seeder's MENTIONS link
grep -n 'MERGE (n:\|MERGE (d:Document' src/create_context_graph/templates/backend/connectors/import_data.py.j2                                   # :485 on {name}, :547 on {title}: make import stamps no domain
grep -n 'query.cypher' tests/test_memory_adapter.py                                                                                             # 0: the document tests never configure the Cypher branch
grep -rn 'confidence' src/create_context_graph/domains/ | grep -vE 'name: confidence|_template|action:|Cover known|confidence scores'           # 0: no tool Cypher names confidence
grep -rn 'method: "POST"' src/create_context_graph/templates/frontend/                                                                          # 3: /chat/stream, /cypher, /expand
grep -rn 'localStorage' src/create_context_graph/templates/                                                                                     # 0: chat history and session id are both in sessionStorage
...
**2026-09-26** — [`707b168d9509cbf1b919b82940285d1df4c6810d`](https://github.com/neo4j-labs/create-context-graph/commit/707b168d9509cbf1b919b82940285d1df4c6810d) — audit at an unchanged pin; upstream `main` has not moved. The screen matches the first reading. Both marks hold on narrower evidence. Corrected: the domain predicate sits on nine read paths, not four, and neither the CLI's library ingest nor `make import` stamps a node-level domain ([section 5](#5-memory-data-model)). The scope integration test has no positive control, and the negative tests reach only the search fallback ([section 10](#10-tests-evals-and-benchmarks)). The agent's Cypher tool, described as read-only, writes on bolt, so "no tool that writes memory" was wrong ([section 8](#8-agent-integration)). The library writes `e.embedding`, so the vector index is unqueried rather than empty. Smaller fixes: ten correction patterns, fourteen redaction patterns, 216 matrix combinations, `sessionStorage`, a latent traces N+1. Nothing installed, built or run.
