...
revision: 26b95700646b22265e696ae27df7e1a240eebc43
revision_url: https://github.com/markhuangai/dense-mem/commit/26b95700646b22265e696ae27df7e1a240eebc43
...
capability_evidence:
  ...
  trust_state: "an eight-value status and a three-tier ladder, with the invariant that a candidate can never be active enforced by a database CHECK | migrations/postgres/v2_4/2026071704_v2_semantic_ledger.sql:239-246 | `status IN ('pending_evidence','active','needs_review','quarantined','superseded','disputed','retracted','rejected')` and `tier IN ('candidate','validated_claim','fact')`, with `relationship_records_active_tier_check` rejecting any row that is both candidate and active, and the partial indexes the read path uses built `WHERE status = 'active' AND tier IN ('validated_claim','fact')` so a non-promoted row is not merely filtered but absent from the index | migrations/postgres/v2_4/2026071704_v2_semantic_ledger.sql:272-280"
