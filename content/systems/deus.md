...
capability_evidence:
  ...
  human_review: "a detected contradiction is queued for a person and never auto-resolved, with the outcome and its time recorded write-once | scripts/memory_indexer.py:477-499, :2582-2590, :3355-3380, :4700-4710 | the indexer's contradiction detection ends in `INSERT OR IGNORE INTO pending_conflicts` under a comment that states the policy — `Log to pending_conflicts for user review — never auto-invalidate`. The table holds both texts, the two atom ids, `resolved`, `resolution` and `resolved_at`. The producer of a resolution is the operator: `--resolve-conflicts` and `--dismiss-conflict <id>` are CLI flags, and the dismissal path is guarded by `resolved = 0` so `resolved_at` is write-once and re-dismissing cannot move it | the migration that added `resolved_at` refuses to invent one for rows resolved before it existed — `we never observed WHEN they were resolved, so inventing a timestamp would fabricate data` — so the review cadence it measures starts empty rather than wrong"
