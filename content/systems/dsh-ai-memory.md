...
**2026-09-18** — [`9208281522612ebb349dff2e1b7e07a9c7683def`](https://github.com/zzjzzb/ai-memory/commit/9208281522612ebb349dff2e1b7e07a9c7683def) — re-read at the same commit; `main` has not moved since the first reading. Every claim was re-tested and every one held, so nothing here is a correction. The headline finding is exact: `Policy::chat()` sets `retention.working` to `Duration::from_secs(60 * 60)` and does not touch `working_to_episodic_after`, whose default is the same `60 * 60`, and `consolidate` checks expiry before promotion. `recall` still opens `WHERE m.project_id = ?` before any scoring, with the same predicate on get, list, pin and forget, and `multi_project_recall_never_crosses` still asserts the alpha result non-empty, all-alpha, and free of beta's phrase. `tier` is a checked enum used as a read predicate with live promotion writers, but the three values are durability classes rather than judgements about the content, which is why it earns no trust state. One detail added: `pinned_skip_expiry` defaults to true, so pinning is the documented way past the preset's TTL trap.
