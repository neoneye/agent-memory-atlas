...
revision: 763859dc034771a8219afe08b198f23100372d89
revision_url: https://github.com/EmpiricaAI/empirica/commit/763859dc034771a8219afe08b198f23100372d89
analyzed_at: 2026-09-19
capabilities: "trust_state, scope_enforced, audit_log"
capability_evidence:
  trust_state: "a resolved flag on findings and unknowns, filtered by five read paths — one of which is there because it was once missing | empirica/data/resolution_kind.py:21 and :25-34, empirica/core/memory_manager.py:143-159, empirica/core/epistemic_brief.py:172, empirica/core/bootstrap/situation.py:198 and :214, empirica/core/bootstrap/circles.py:174, empirica/core/derived_confidence.py:89 | `ResolutionKind` is a closed four-value `stale / superseded / retracted / mistyped`, each with a one-line gloss surfaced in `--help` so the choice is made at the point of resolving rather than looked up, and it sits beside the boolean `is_resolved` that the reads actually test. The hot cache, the epistemic brief, the situation assembly and the circles view all narrow with `is_resolved = 0`, and `derived_confidence` reads the kind alongside the flag. The comment above the hot-cache filter records the bug that put it there: the findings predicate omitted `is_resolved = 0` while the sibling predicates for unknowns and goals both filtered their closed state, so a finding marked superseded still entered *the one surface whose whole job is what is currently true* | tests/test_artifact_visibility.py"
...
**2026-09-19** — [`763859dc034771a8219afe08b198f23100372d89`](https://github.com/EmpiricaAI/empirica/commit/763859dc034771a8219afe08b198f23100372d89) — `trust_state` re-tested. The mark holds and the record was one line long: it named the vocabulary and said the read paths filter on it without naming one. There are five — the hot cache (`empirica/core/memory_manager.py:149-159`), the epistemic brief (`core/epistemic_brief.py:172`), two in the situation assembly (`core/bootstrap/situation.py:198`, `:214`) and the circles view (`core/bootstrap/circles.py:174`) — plus `core/derived_confidence.py:89`, which reads the kind alongside the flag. The hot-cache filter is the one worth quoting, because the comment above it records the bug that put it there: the findings predicate omitted `is_resolved = 0` while the sibling predicates for unknowns and goals both filtered their closed state, so a finding marked superseded still entered *the one surface whose whole job is what is currently true*. Three predicates built side by side, one missing the clause — the failure this atlas has found by hand in several systems, found here by the project and kept in the source as a comment rather than quietly patched. The record now carries the anchors and that sentence. Screened again first; nothing was installed and no suite was run.

**2026-09-16** — [`763859dc034771a8219afe08b198f23100372d89`](https://github.com/EmpiricaAI/empirica/commit/763859dc034771a8219afe08b198f23100372d89) — re-read after 45 commits, through 1.13.47. `empirica/data/resolution_kind.py` is byte-identical, so the four-value vocabulary behind `trust_state` is exact; the derived-confidence reader, the breadcrumb repository and the visibility test moved and were re-derived, and `empirica/core/derived_confidence.py:89` still reads `is_resolved` and `resolution_kind` together at the same line. All three marks hold. The addition to section 9 is the project's own measurement of how often its trust vocabulary is used — 1,344 of 1,604 resolutions unclassified, `mistyped` used zero times across 4,833 findings, and a second practice with not one classified resolution in seven hundred and twenty-six — together with a repair that refuses both making the field required and defaulting it, and instead names the omission in the response. The evidence record's shorthand path for the derived-confidence reader is corrected to its real location. Nothing was installed, built or run.
