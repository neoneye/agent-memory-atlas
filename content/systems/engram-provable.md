...
capability_evidence:
  ...
  trust_state: "the quarantine binding, which is a value of the same enum the read paths filter on | migrations/023_firewall_quarantine_schema.up.sql:8-25, internal/store/memory.go:383, :409, :426 | `binding` is a stored discrete value — `canon`, `private`, `anchored`, `session`, `quarantine` — held by a CHECK constraint, and the read paths select on it: the canon reads are `WHERE tenant_id = $1 AND binding = 'canon' AND is_archived = FALSE` and the session read is `binding = 'session' AND ...`, so a row whose binding is `quarantine` is outside every recall by construction rather than by a flag someone must remember. `quarantine_reason` and `quarantined_at` record why and when, and the partial index `WHERE binding = 'quarantine'` exists to serve the review queue | the state is the binding rather than a separate status, so a quarantined row loses the binding it would have had — the migration comment explains that anchor and session ids are preserved precisely so the real binding can be recomputed on release. Nothing expresses doubt about a memory that is already in canon"
