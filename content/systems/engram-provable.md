...
capability_evidence:
  bitemporal: "event_date beside created_at, written by the API and filtered on the read | migrations/009_event_date.up.sql, internal/api/handlers/memory.go:39, :130-133, :515, internal/store/memory.go:79 | `event_date` is a column added in its own migration with two indexes, carried in the INSERT beside `created_at` and `updated_at`, and it has both a writer and a reader on the HTTP surface: the request body accepts `event_date` as a string that the handler parses and assigns, and the list endpoint accepts `event_date_from` as a query filter. So a memory can be recorded today as having been true last year, and read back on either clock | the column is nullable and omitted by default, so a write that does not supply `event_date` records no validity date at all and that row has only the record axis; nothing backfills it"
  trust_state: "the quarantine binding, selected against positively by the scoped reads and excluded explicitly by the five that span bindings | migrations/023_firewall_quarantine_schema.up.sql:8-25, internal/domain/memory.go:109, internal/store/memory.go:383, :409, :426, :447-449, :503, :643, :750, :875, :1087, :1118-1128, :1160-1161 | `binding` is a stored discrete value — `canon`, `private`, `anchored`, `session`, `quarantine` — held by a CHECK constraint, and the read paths reach it two ways that suit their shapes. The scoped reads name the binding they want: the canon reads are `WHERE tenant_id = $1 AND binding = \"canon\" AND is_archived = FALSE` and the session read names `session`, so a quarantined row is outside them by construction rather than by a flag someone must remember. The similarity reads cannot name one binding, because they span several, so each of the five carries `binding <> \"quarantine\"` written out — the vector search, the threshold search, the join, the embedding scan and the candidate scan. `quarantine_reason` and `quarantined_at` record why and when, the partial index `WHERE binding = \"quarantine\"` exists to serve the review queue, and release is a single statement that sets the new binding and clears both fields, guarded by `WHERE binding = \"quarantine\"` so only something quarantined can be released | the state is the binding, so nothing can carry it without also changing which reads can see it"
  scope_enforced: "tenant_id as a predicate on every statement in the memory store | internal/store/memory.go:90, :177, :180, :198, :249, :252, :261, :264, :383, :409 | the key is on the row and in the SQL rather than carried as a tag: the single get is `WHERE id = $1 AND tenant_id = $2 AND is_archived = FALSE`, the anchor read and the canon reads carry it, and so do the deletes and the derived-row cascade — the same predicate appears on the write path and on the destructive path, not only on recall. Per-tenant audit chain heads (`audit_chain_heads WHERE tenant_id = ...`) make the tenant the unit of the tamper-evident log as well | the tenant is supplied by the caller through the API layer, so the mark certifies that the key reaches the query and not that the boundary is authenticated; there is no row-level security behind it, so a direct database connection sees everything"
  audit_log: "a per-tenant hash-chained mutation log enforced by database triggers, with a verify function that names the break | migrations/003_learning_loop.up.sql:6, migrations/016_audit_chain.up.sql, migrations/019_audit_chain_complete.up.sql:9, :92, migrations/029_audit_exclude_decay.up.sql | `mutation_log` records memory mutations, and later migrations add `prev_hash`, a per-tenant `audit_chain_heads` row, and a trigger that assigns `seq` and `prev_hash` inside the database rather than in application code — so a writer that bypasses the service still links into the chain. `verify_audit_chain(p_tenant UUID)` walks the chain and, per migration 019's own header, was fixed so that it compares the walked chain against the recorded head rather than trusting it | the chain is a hash chain rather than a signature, so a writer with database privileges can rewrite both the rows and the heads consistently; and migration 029 deliberately excludes decay events from the chain, so the log is a record of deliberate mutations and not of everything that changed a row"
  human_review: "the Provenance Firewall's review queue, behind an admin scope the agent's own credentials do not carry | internal/api/router.go:367, :399-403, internal/api/handlers/memory.go (ListQuarantine, ReleaseQuarantine, RejectQuarantine) | an untrusted write lands with `binding = 'quarantine'`, outside every recall path, and the only way out is `POST /quarantine/{id}/release` or `/reject` — both inside `r.Route('/quarantine', ...)` with `r.Use(mw.RequireScope('admin'))`, and the listing is `r.With(mw.RequireScope('admin')).Get('/quarantine', ...)`. The decision is a separate endpoint under a separate scope rather than a parameter on the write, which is what distinguishes this from a proposal whose approval rides with the proposer | the gate is a scope claim, so anything holding an admin token can release — the code does not distinguish a person from an operator script; and release recomputes the binding rather than recording who decided, so the queue is adjudicated but the adjudicator is not named on the row"
...
**2026-09-19** — [`4a3d20487a370d0cca6eaaf97861a6d5d0bcbe37`](https://github.com/Harshitk-cp/engram/commit/4a3d20487a370d0cca6eaaf97861a6d5d0bcbe37) — `trust_state` re-tested at the unchanged pin. The mark is right and the record showed half the mechanism. It cited the three reads that *name* the binding they want — the two canon reads and the session read — where a quarantined row is excluded by construction. The other half is the five reads that cannot name one binding because they span several: the vector similarity search, the threshold search, the join, the embedding scan and the candidate scan each carry `binding <> 'quarantine'` written out in the SQL (`memory.go:503`, `:643`, `:750`, `:875`, `:1087`). So the codebase uses an allowlist where the query is scoped to one binding and a denylist where it is not, which is the right choice in each place and worth recording because the denylist is the half that has to be remembered. Two details complete it: the partial index `WHERE binding = 'quarantine'` exists to serve the review queue rather than the recall path, and release is one statement that sets the new binding and clears `quarantine_reason` and `quarantined_at` together, guarded by `WHERE binding = 'quarantine'` so nothing else can be released into canon by that route. Nothing was installed and no suite was run.

**2026-09-18** — [`4a3d20487a370d0cca6eaaf97861a6d5d0bcbe37`](https://github.com/Harshitk-cp/engram/commit/4a3d20487a370d0cca6eaaf97861a6d5d0bcbe37) — re-read at the same commit. Nothing upstream had moved and nothing needed correcting; all five marks were re-derived from the source and now carry evidence records. The 91.4% LongMemEval figure is still in `README.md` and still the only place it appears — no harness, no result file, no configuration anywhere in the tree.

The details worth keeping. `binding` turns out to be the trust state rather than a separate status column: the CHECK constraint holds `canon`, `private`, `anchored`, `session` and `quarantine`, and because the read paths select `binding = 'canon'` or `binding = 'session'`, a quarantined row is outside recall by construction rather than by a flag a query must remember — with the migration comment explaining that anchor and session ids are preserved precisely so the real binding can be recomputed on release. `human_review` rests on `r.Use(mw.RequireScope("admin"))` wrapping the release and reject endpoints, which is the shape that distinguishes a review gate from a parameter the proposer can set. `tenant_id` appears as a predicate on the destructive statements as well as the reads. And the audit chain is assigned by a database trigger rather than in application code, so a writer that bypasses the service still links into it — with migration 029 deliberately excluding decay events, so the log records deliberate mutations rather than everything that touched a row. `stack_source` goes from seeded to reviewed.
