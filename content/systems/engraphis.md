...
capability_evidence:
  trust_state: "one named predicate deciding what may enter a model's context, registered as a SQL function so the query and the application cannot disagree | engraphis/core/poisoning.py:18-19 (REVIEW_PENDING, REVIEW_APPROVED), :483-498, :531-542, :545-553, engraphis/service.py:638-655, :316-318, :1209, :9919, :4779, :8266, :8494, :11829, engraphis/core/store.py:305-318 | every write is stamped `pending` unless it came from a local agent source that did not arrive over http, mcp or remote ingress, in which case it is `approved`, and the comment above that line names the attack it exists for: *transport labels are not capabilities*, since otherwise a remote caller *could simply submit source=agent and self-approve prompt-visible content*. The read side is one function, `prompt_eligible`, whose docstring states the distinction the mark turns on — *inspection visibility and prompt eligibility deliberately differ* — and which requires four things at once: explicit trust, an explicit approval state, metadata that is not quarantined, and text that is not unverified LLM derivation. Its approval test is an equality against `REVIEW_APPROVED` rather than an inequality against pending, so an unrecognised state fails closed. It has nine call sites and is also registered with SQLite as `_engraphis_prompt_eligible`, so the SQL read appends `AND _engraphis_prompt_eligible(provenance, metadata)=1` and runs the same implementation the Python callers run rather than a second copy of the rule. Merging two provenance envelopes cannot launder a restriction: `trusted` false, `quarantined` true and any non-approved review state each survive the merge by construction | eval/adversarial_memory_security.py:221-223 asserts a caller-asserted trusted flag is downgraded to trusted false, review_state pending, trust_downgraded true"
...
**2026-09-19** — [`ca790261f499e0d124cdc7131235ffff89637248`](https://github.com/Coding-Dev-Tools/engraphis/commit/ca790261f499e0d124cdc7131235ffff89637248) — `trust_state` re-tested at the unchanged pin. The mark is right and the record pointed at the wrong line for it. `service.py:7011-7014` is not the packer: it is the review queue, and its test *keeps* the quarantined, pending and conflicted rows rather than dropping them — the comment two lines down says so, *"Pending/quarantined content is evidence for a human reviewer, not model context."* The packer's rule is `prompt_eligible` in `core/poisoning.py:531`, and it is the strongest form of this mechanism the sweep has found. One function answers *"whether a record may enter agent/model context"*, conjoining four tests — explicit trust, an explicit approval state, untainted metadata, and not-unverified-LLM-derivation — and its approval test is an equality with `REVIEW_APPROVED`, so an unrecognised value fails closed rather than slipping through a denylist. It has nine call sites, a sibling for graph edges with a documented legacy rule, and, decisively, it is registered as a SQLite user-defined function at `service.py:316-318` and `:1209`, so the SQL path appends `AND _engraphis_prompt_eligible(provenance, metadata)=1` and executes the same Python the application calls. There is one implementation of the rule, not a query copy and an application copy that drift. Two more details belong on the record: the write-time stamp is preceded by a comment naming its threat model — *"Transport labels are not capabilities… otherwise a remote caller could simply submit source=agent and self-approve prompt-visible content"* — and `_merge_provenance_envelopes` is written so that merging two trust envelopes cannot lose a restriction, with `trusted` false, `quarantined` true and any non-approved review state each surviving by construction. Nothing was installed and no suite was run.
