...
revision: 1ab2e249b9c85111446b70785eb69360bb4de5ed
revision_url: https://github.com/EMI-Group/genesis/commit/1ab2e249b9c85111446b70785eb69360bb4de5ed
...
**2026-09-19** — re-pinned to [`1ab2e249b9c85111446b70785eb69360bb4de5ed`](https://github.com/EMI-Group/genesis/commit/1ab2e249b9c85111446b70785eb69360bb4de5ed), 22 commits and 35 files on. **Both marks hold.** The first reading was made on 2026-09-17, a day before the `human_review` wording narrowed, so the mark was re-tested against the current rubric and the record now cites the call graph rather than the review page alone. `merge_branch/2` and `merge_branch/3` are defined in `apps/evo_git/lib/evo_git/review.ex:235` and `:301`, and their only caller is `apps/evo_dash/lib/evo_dash_web/live/review_live.ex:1490-1492` — the LiveView that renders the per-file diff. Nothing on the agent's side merges a branch, so a completed task's `CONTEXT.md` edit sits on that branch, out of the target, until a person clicks; `set_review_status/3` then persists `:merged` or `:rejected` beside the base and final shas. That is a memory waiting in a state for an actor the producer cannot be. Screened again first; nothing installed, built or run.

**2026-09-17** — [`7df3fd5b7f1b6f33efe2792d8113c3ec766707dc`](https://github.com/EMI-Group/genesis/commit/7df3fd5b7f1b6f33efe2792d8113c3ec766707dc) — re-read after 695 commits. Three of the four anchored files are byte-identical at both commits: `context_node.ex`, which computes the ancestor walk and reads only the `CONTEXT.md` files along it, `context_integration.ex`, which drives the tool surface from the same walk, and the hierarchical skills test. `context_builder.ex` gained 31 lines and deleted none; the cited line still calls `ContextNode.build_context(state.node_path, state.repo_path)`, so the scope key still reaches the assembly. Both marks hold. Nothing was installed, built or run. Later the same day the pinned commit stopped being reachable from the project's default branch: `ab8f6f6d` is a merge commit dated 2026-09-16T13:19:11Z, `main` moved to `1ab2e249` — *"Add changelog for v0.13.0"*, 2026-09-17T07:14:25Z — and the two lines have diverged, 433 ahead and 411 behind, with `main` the only branch the repository now lists. The commit is not gone: GitHub still serves it by sha, so every link in this report resolves, and the atlas's fork `agent-memory-atlas-archive/EMI-Group--genesis` holds it at the same root tree `91497efb64d6`. The pin was first left on `ab8f6f6d` for that reason — moving it without re-reading would attach these findings to code nobody examined. Checked again later the same day, the move turns out to cost nothing: `main` carries [`7df3fd5b7f1b6f33efe2792d8113c3ec766707dc`](https://github.com/EMI-Group/genesis/commit/7df3fd5b7f1b6f33efe2792d8113c3ec766707dc), whose root tree is the same `91497efb64d6`, with the same author date and a merge parent under a different sha — the repository rebuilt those merges rather than changing their content. So the pin now names a commit that is reachable from `main` and whose tree is byte-identical to the one read, which is the only condition under which a pin may move without a re-read. How it was established, because the obvious commands mislead here: `merge-base --is-ancestor` reported *not an ancestor* in a shallow clone whose grafted boundary made the pin look parentless, and a second run compared the pin with itself because a bare `git fetch origin HEAD` had overwritten `FETCH_HEAD`. The answer came from `git rev-list refs/remotes/origin/head2`, 2,435 commits deep: `ab8f6f6d` does not appear in it and `7df3fd5b` does, and the tree match was found by comparing `<commit>^{tree}` across that list.
