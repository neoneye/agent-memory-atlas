...
revision: 3a9f7f528ce8ea481a11d87ab3454044401de9e1
revision_url: https://github.com/Morephine/feltstate/commit/3a9f7f528ce8ea481a11d87ab3454044401de9e1
analyzed_at: 2026-09-19
capabilities: "trust_state, bitemporal, audit_log, human_review, negative_eval"
capability_evidence:
  trust_state: "one named predicate on the recall paths, deliberately absent from the two that answer about the past | feltstate/memory/canon.py:481-483, :628, :673-676, :711-717, :825, :837-838, :1031, :1091-1097, :1174-1181, :1228-1240, :880, :932, :946-957, :959-984, :986-1010 | `_is_active(entry)` is a single method returning `not (entry.get(\"_retracted\") or entry.get(\"_superseded_by\"))`, and every recall surface goes through it — ordinary recall, the keyword walk, the tier view over both the live file and the archive, and the id-or-text lookups. Supersession sets `_superseded_by` on the old entry and retraction sets `_retracted` with `_retracted_at`, both under a write lock that finds, mutates and rewrites in one transaction. The two reads that *omit* the predicate are the interesting half and neither is a leak. `history` loads without it on purpose and labels each version `retracted`, `superseded` or `active`, which is what a timeline is for. `as_of` omits it too, but retraction also writes `invalid_at = now` — the code comments it *this belief stopped being true now* — so the temporal query excludes a retracted fact for instants after the retraction and still returns it for instants before, which is the correct bitemporal answer rather than a missing filter. Separate again from all of this is the `confidence` float, and the grey-zone pending file a `confirm` promotes | tests/test_canon.py, test_ask_is_grey_zone_and_confirm_promotes and test_retract_hides_fact_but_keeps_record"
  ...
  human_review: "the skill region inside canon, gated on *independent observation* rather than on a person as such | feltstate/memory/skill.py:79, :200-232, :430-460, :510-530 | a 1/2/3 rating folds into a skill and may promote it out of the grey zone or retire it, and the gate is one line: `if source not in OBSERVED_SOURCES: return {}`, with the comment *\"independently-observed gate: a self-reported source moves nothing.\"* The rule is narrower than *human* and sharper for it — `OBSERVED_SOURCES` is `human`, `user_correction`, `tool_exit`, `verifier`, `harness`, so a tool's exit status or a verifier counts and the reply model does not; the docstring names the excluded case outright, *\"NOT the agent grading itself.\"* `review_skills` exposes merge, retire and ratify for an explicit review pass, and `RatingGate` rate-limits *the ask* rather than the answer — its docstring describes an overlay calling `allow()` before showing the 1/2/3 widget and `stamp()` after, with the cooldown persisted in a JSON sidecar so it survives a restart. Two limits: `source` is a caller-supplied string defaulting to `\"human\"`, so the gate rejects an *honest* self-report rather than detecting a dishonest one; and the library ships no tool surface at all, so what calls `record_rating` is the integrating application | tests/test_skill_region.py"
...
**2026-09-19** — [`3a9f7f528ce8ea481a11d87ab3454044401de9e1`](https://github.com/Morephine/feltstate/commit/3a9f7f528ce8ea481a11d87ab3454044401de9e1) — `trust_state` re-tested at the unchanged pin; the mark holds and the record was a four-clause shorthand for something worth stating properly. `_is_active` is one method, `not (_retracted or _superseded_by)`, and it is called at nine sites covering every recall surface — ordinary recall, the keyword walk, the tier view over both the live file and the archive, and the id-or-text lookups. What makes this a good example is the two reads that deliberately do without it. `history` loads the confirmed set unfiltered and labels each version `retracted`, `superseded` or `active`; that is the labelling path, and it is kept apart from the recall paths rather than mixed into them. `as_of` also omits the predicate, and the first reading of that looks like the omission this sweep keeps finding — but `retract` writes `invalid_at = now` alongside `_retracted`, commented *"M3: this belief stopped being true now"*, so the temporal query reaches retraction through the temporal axis: a fact retracted at an instant *T* is excluded from an as-of read of any instant after *T* and still returned for instants before it, which is the right answer and not an accident. Three paths, three different correct treatments of the same state, in one file. Nothing was installed and no suite was run.

**2026-09-18** — [`3a9f7f528ce8ea481a11d87ab3454044401de9e1`](https://github.com/Morephine/feltstate/commit/3a9f7f528ce8ea481a11d87ab3454044401de9e1) — re-pinned to 0.2.0a4; the diff from the previous pin is a version-string correction, a `.gitignore` line and a removed scheduler sidecar, with no change under `feltstate/`, so the marks' subject code is identical and the findings here are this report's. The `human_review` record is rewritten around what the code actually gates on. It said *"human 1/2/3 ratings"*; the check is `if source not in OBSERVED_SOURCES: return {}`, and that set is `human`, `user_correction`, `tool_exit`, `verifier`, `harness` — so a tool exit status and a verifier both count, and the one thing excluded is the reply model grading its own output, which the docstring says in those words. That is the atlas's own producer test implemented as a data rule, and it is a better mechanism than the record described. The limits are stated with it: `source` is a caller-supplied string defaulting to `"human"`, so the gate refuses an honest self-report rather than detecting a dishonest one, and the library ships no tool surface, so the caller is the integrating application rather than a model.
