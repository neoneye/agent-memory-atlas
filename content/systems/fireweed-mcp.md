...
revision: b37747b0c8f2ed7f1b6b05309a6fb660a82d8605
revision_url: https://github.com/Starksood/fireweed-mcp/commit/b37747b0c8f2ed7f1b6b05309a6fb660a82d8605
...
**The certificate also declares what it does not cover, and it names the
adversary it loses to.** Its text closes with a scope statement rather than a
guarantee: the erasure *"certifies one substrate held by one operator. It says
nothing about copies taken before the erasure, and the salt and content keys are
held by that same operator -- a party who holds both the store and the keys is
not the adversary this defends against."* Writing down the attacker your design
does not stop, inside the artefact that attests to the design, is unusual.

The residual that made such a statement necessary has been closed on the served
path. An entity id used to be `"ent_" + mention.lower().replace(" ", "_")`, and
the code says what that cost: an identifier like `ent_jane_doe` would still sit
in the append-only ledger after the subject's content was shredded. Ids are now
`"ent_" + sha256(salt + "\x00" + mention)[:20]`, with the salt generated once
per install by `id_salt()` — sixteen bytes from `secrets`, written `O_EXCL` at
mode `0600` beside the keys rather than in the store — and assigned to the graph
when the MCP server starts. One seam is worth knowing: `Graph._id_salt` defaults
to the empty string, and with no salt the linker keeps the legacy readable form
so existing stores stay loadable, while the certificate's wording that *"[e]ntity
identifiers are opaque, salted per install"* is a fixed string rather than a
conditional one. Through the server that is always true; for a library embedder
who constructs a `Graph` directly and never sets the salt, the certificate would
claim more than the store delivers.
...
**2026-09-19** — [`b37747b0c8f2ed7f1b6b05309a6fb660a82d8605`](https://github.com/Starksood/fireweed-mcp/commit/b37747b0c8f2ed7f1b6b05309a6fb660a82d8605) — `trust_state` re-tested at an unchanged pin, with the negative half re-checked first because that is where this atlas gets things wrong. It holds: the only writes to `memory_state` in the tree are `superseded` (`src/fireweed/pipeline.py:251`, `src/fireweed/consolidation_ops.py:419`) and `disputed` (`pipeline.py:291`). Nothing writes `quarantined` or `frozen` — the `frozen` matches elsewhere in the source are a dataclass decorator, frozensets of pronouns and the consolidator's own set of frozen node ids, none of them the memory state. The enforcement side is wider than the record listed: beyond the retrieval admissions, three edge-field gates test `node.status.memory_state != "active"` before following a relation (`src/fireweed/field_edges.py:43`, `:98`, `:129`), so a superseded claim is dropped from graph traversal as well as from recall. Those anchors are added. Re-read from a fresh clone; nothing was installed and no suite was run.

**2026-09-16** — [`b37747b0c8f2ed7f1b6b05309a6fb660a82d8605`](https://github.com/Starksood/fireweed-mcp/commit/b37747b0c8f2ed7f1b6b05309a6fb660a82d8605) — re-read at a commit dated 28 August 2026, four commits past the previous pin. All five marks re-tested and held. The substantive change is to the erasure certificate, and it runs toward the subject: the residual it used to disclose — that a structural identifier such as `ent_jane_doe` persisted in the append-only ledger after the content was shredded, so the name was recoverable from the id — is closed by hashing the mention under a per-install salt, generated by `id_salt()` and held beside the keys rather than in the store. In its place the certificate now states its own scope limits and names the adversary it does not defend against. The producer test found the seam worth recording: the salting is conditional on `Graph._id_salt`, which the server sets and the bare library leaves empty, while the certificate's claim of opacity is a fixed string. Screened before reading, from a full clone: one auto-run surface, no build-time execution point, one unpinned dependency surface and none inside the seven-day cooldown. Nothing was installed, built or run.
