...
capability_evidence:
  ...
  human_review: "gbrain takes propose --accept / --reject and takes resolve | src/commands/takes.ts:640-650, src/core/take-proposals.ts:167 acceptProposal, :237 rejectProposal, src/cli.ts:84 CLI_ONLY, src/core/operations.ts:286-288, src/core/postgres-engine/takes.ts:537-560, src/core/cycle/grade-takes.ts:1-25 | the propose_takes phase never writes a take; a person lists the pending queue, accepts a proposal into the page's fence or rejects it with `acted_by` stamped, and records a bet's resolution as correct, incorrect, partial or unresolvable, immutably; the grade_takes phase writes judge verdicts to a cache and applies none unless the operator opts in and the verdict clears 0.95. The producer has no accept verb to reach: the operation layer — what the remote and MCP surfaces dispatch — exposes `takes_list`, `takes_search`, `think`, `takes_scorecard` and `takes_calibration`, every one a read, while `takes` itself is in `CLI_ONLY`, the set of commands that *bypass the operation layer*. Two limits belong with it: `acted_by` defaults to the literal `'cli'` and is otherwise whatever `--acted-by` supplies, so the stamp records a claim rather than a verified principal; and a general shell can run the CLI, which this atlas treats as distinct from an approve verb on the tool surface | test/take-proposals.test.ts:128-283, test/takes-propose-drain.test.ts:70 (the phase inserts a pending proposal and the CLI lists, accepts and stamps the promoted row)"
...
the write fails), `rejectProposal` (`:237-250`). CLI in `src/commands/takes.ts:640-650`.
...
**2026-09-19** — re-pinned to [`d13aa742fd68b71bfd6c98be3dda5813791f1d6c`](https://github.com/garrytan/gbrain/commit/d13aa742fd68b71bfd6c98be3dda5813791f1d6c), 1 commit on. All five marks stand. `human_review` was producer-tested against the dispatch layers rather than carried forward, and holds: `takesOperations` contributes `takes_list`, `takes_search`, `think`, `takes_scorecard` and `takes_calibration` to the operation layer and nothing that accepts, rejects or resolves, while `takes` sits in `CLI_ONLY` — described in `cli.ts:83` as the commands that bypass the operation layer altogether. So the surface a remote or MCP client dispatches through cannot reach `acceptProposal` or `rejectProposal`. The record now carries the two limits that reading the accept path turns up: `acted_by` defaults to the literal `'cli'` and is otherwise a caller-supplied string, and a general shell can run the CLI. Screened again first; nothing was installed and no suite was run.
