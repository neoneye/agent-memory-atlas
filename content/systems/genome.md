...
capability_evidence:
  ...
  audit_log: "the journal, wrapping the store | genome/journal.py (`Journal`, `JournalingStore`, `verify_journal_integrity`, `replay_journal`) | an append-only JSONL of store mutations — `add`, `update`, `delete`, `add_edge`, `delete_edge`, `delete_edges_touching` — each line carrying a monotonic `seq` and a `line_hash` chained to its predecessor from a fixed `GENESIS_LINE_HASH`. The line is made durable BEFORE the inner store mutates — written and `os.fsync`'d, because *\"flush() only hands the line to the OS page cache\"* — and a mutation that raises after its line is durable is cancelled by a second, reversing line, so a replay still reproduces live state. The sequence is assigned under both an in-process lock and an OS advisory lock on a sidecar file, `fcntl.flock` on POSIX and `msvcrt.locking` on Windows, chosen over create-with-`O_EXCL`-and-spin because that variant is broken by any holder slower than the spin count and an advisory lock is released by the kernel. With a key the chain is HMAC-SHA256 rather than SHA-256, and the module states the difference it buys: an unkeyed chain *\"can be recomputed by anyone… a determined attacker edits a line and rechains the rest.\"* Opt-in — passing `journal=` to `Memory` wraps the store; without it nothing is recorded | tests/test_journal.py; `verify_journal` compares a replay against live state, and `verify_journal_integrity` walks the chain"
...
matrix:
  ...
  update_delete: "`delete` by id, `reset` by scope, an MCP `forget` that refuses below a `min_score` cosine floor and names the best candidate instead, and an opt-in conflict resolver returning ADD, UPDATE, DELETE or NONE. No tombstone: a deleted value can be added again, and the journal retains its text"
  ...
  integration: "A Python API, a CLI, a FastAPI server that also exposes the trust firewall over HTTP, a fully local MCP server exposing `remember`/`recall`/`forget`/`reset_memories`, LangChain and LlamaIndex adapters, and a TypeScript SDK with the same reach"
...
Test functions across roughly forty files, about ten thousand lines, running in
public CI, plus an install canary workflow. I did not run them.

`tests/test_journal_durability.py` is the one to read for how the project tests a
durability claim rather than asserting it. Its cases cover the fsync itself, the
write-ahead ordering, the cancellation line that reverses a mutation which raised
after its journal line was durable, a cross-process check that no sequence number
is duplicated, and — the case that names the defect it was written for — that a
slow holder is never stolen from. The commit that added it reports reproducing
the stolen lock in a Linux container before clearing it, which is the harder half
of fixing a race.
...
**2026-09-19** — re-read at the same pin [`ea76cf924b2382499a158b8c7ad8a815913c962f`](https://github.com/NORTHTEKDevs/genome/commit/ea76cf924b2382499a158b8c7ad8a815913c962f), still the tip, as part of a corpus re-test of `trust_state` against the filtering-versus-ranking line. **The mark holds on the filtering side and the record gains the half that matters most.** `is_quarantined` (`genome/firewall.py:89-92`) is a boolean test — `trust_of(record, policy) < policy.recall_min_trust` — not a weight, and it is applied on three arms rather than one. What the previous record did not cite is the write half of the same concern: `add()` pops the reserved provenance key out of caller-supplied metadata before building the record, under a comment that names the attack it prevents — a caller passing `metadata={"_provenance": {"trust": 999}}` to *"self-assign authority, defeating quarantine and origin-bound authority"*. So the tier a read filters on cannot be set by the writer asking to be trusted, which is the producer question this atlas spent the preceding sweep asking of `human_review` and which applies just as sharply here. The two existing qualifications stand: the firewall is off unless `recall_min_trust` is set, and the tier records where content came from rather than whether anyone checked it. Screened again first; nothing installed or run.

**2026-09-13** — [`ea76cf924b2382499a158b8c7ad8a815913c962f`](https://github.com/NORTHTEKDevs/genome/commit/ea76cf924b2382499a158b8c7ad8a815913c962f) — re-read, six commits past the previous pin. Screened again first: two auto-run surfaces and two unpinned dependency surfaces, unchanged in kind; nothing was installed and no suite was run. All five marks hold and the producer check was re-run on each. The substantive change is to the mechanism `audit_log` rests on. The journal line was appended after the inner store committed and only flushed; it is written and `os.fsync`'d before the store mutates, and a mutation that raises after its line is durable is cancelled by a second reversing line so a replay still reproduces live state. The sidecar mutex was a create-with-`O_EXCL`-and-spin lock that broke a stale lock after a fixed spin count, which on Linux let a holder slower than that count lose the mutex mid-critical-section; it is an OS advisory lock, `fcntl.flock` or `msvcrt.locking`, with the module stating why. MCP `forget` gained a `min_score` cosine floor, defaulting to 0.5, and refuses below it while naming the best candidate — before, a query with no relevance floor deleted its nearest neighbour. The trust firewall gained an HTTP surface and matching SDK reach. None of the four was a finding this atlas published; the repository attributes them to an external review and ships tests for each, including a Linux-container reproduction of the stolen lock. The report's standing criticism is unaffected: the journal still makes deletion reversible, because a purged memory's text stays in the log.
