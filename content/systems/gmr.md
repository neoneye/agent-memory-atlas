...
revision: 7bee2a081a455e9d48331af7594ee50508588d77
revision_url: https://github.com/Anchorstate-Lab/GMR/commit/7bee2a081a455e9d48331af7594ee50508588d77
analyzed_at: 2026-09-19
capabilities: "trust_state, audit_log, negative_eval"
capability_evidence:
  trust_state: "terminality is per-anchor data rather than a fixed vocabulary, and a memory's currency is read at the journal sequence it was bound at | crates/gmr-core/src/anchor.rs:79, :90-94, :188-206, :294-304, crates/gmr-core/src/journal.rs:100, :372, crates/gmr-runtime/src/read.rs:229-245, crates/gmr-runtime/src/memory.rs:61-67, :165-186, crates/gmr-runtime/src/link.rs:140, crates/gmr-runtime/src/edges.rs:316, crates/gmr-core/src/probe.rs:91-99 | `State` is an open JSON object and `state.status()` reads one reserved slot, returning `Option<StatusId>`; which of those ids are terminal is not fixed in code but declared per anchor as `Anchor.terminal`, a set of status ids amendable through a `Reterminal` journal entry, with `is_terminal` testing membership. Two committed cases pin the edges: terminality is decided by the status slot alone, so no other field of the state can make it terminal, and a state with no status is never terminal. Once reached, closure is monotonic — the journal folds closure as a logical *or* of its previous value and `is_terminal(state)`, so a later move off a terminal status does not reopen it. A memory records `bound_at_seq` as the journal head at bind time and its currency is resolved against that sequence rather than the latest entry, so a memory bound under a state the anchor has since left is not silently carried forward. The admission test in the runtime is `Footing::is_current`, an equality against exactly one of nine values: the other eight — `Unverified`, `Rewritten`, `NoBefore`, `Gone`, `NoProvider`, `Unreachable`, `NeverAsked` — each name a different reason the basis could not be confirmed, and the traversal reports the non-current hop rather than dropping it silently. Beside it `Verifiability` is `Closed` or `Open` over a named set, carrying *what* the probe is open over. The line the design holds is that none of this says whether the memory is *correct*: it says whether its basis moved and whether the basis can be checked | crates/gmr-runtime/tests/grounding.rs and state_machine.rs"
...
**2026-09-19** — [`7bee2a081a455e9d48331af7594ee50508588d77`](https://github.com/Anchorstate-Lab/GMR/commit/7bee2a081a455e9d48331af7594ee50508588d77) — re-pinned 40 commits and 79 files on, and the `trust_state` mechanism was redesigned rather than adjusted. The record described `State` as an enum carrying the anchor's status with terminal variants named `Close` and `Superseded`. It is now `pub struct State(pub Value)` — an open JSON object with one reserved status slot read through `state.status() -> Option<StatusId>` — and terminality has moved out of the type system into per-anchor data: `Anchor.terminal` is a `BTreeSet<StatusId>`, amendable at runtime through a `Reterminal` journal entry. So the vocabulary of retirement is a property of each anchor, not of the codebase. Two committed cases pin the edges that shape invites, both worth a reader's time: `terminal_is_decided_by_the_status_slot_alone`, so no other field of an open object can make a state terminal, and `a_state_with_no_status_is_never_terminal`, which fixes the fail-open direction explicitly rather than leaving it to be discovered. Closure is monotonic — `journal.rs:372` folds `closed = closed || is_terminal(state)` — so moving off a terminal status does not reopen what was closed. The claim the record turned on survives intact: a memory stores `bound_at_seq` as the journal head at bind time and its currency is resolved at that sequence. And the runtime's admission test is sharper than the record's two-value reading: `Footing::is_current` is an equality against one of nine variants, the other eight naming *why* the basis could not be confirmed — rewritten, gone, no provider, unreachable, or never asked because the budget was spent — and `link.rs:140` records the non-current hop in its `Reached` list instead of dropping it. `audit_log` and `negative_eval` both still stand at this pin: the two `journal_no_update` / `journal_no_delete` triggers are in `gmr-store`'s schema, with a comment beside a table that deliberately has none because *"changing one settles nothing"*, and both named cases exist, one line reference nudged from `:956` to `:962`. The new pin is the `v0.6.6` tag; the previous one was `v0.6.4`, and the inspected-list entry's claim that it was v0.3.2 is corrected with it. Screened again first — one agent surface, three build-time execution paths, one floating dependency range and two notes, the same shape as before; the FRESH count rises from 2 to 22 only because this screen ran on a `--depth 1` clone, which dates every file to the tip, so that number is not comparable with the previous reading. Nothing was installed and no suite was run.
