...
capability_evidence:
  ...
  human_review: "an operator approves or rejects each installed-host candidate in the Inspector before it becomes durable memory | src/install/hostReviewQueue.ts:11-49, src/inspector/adminApi.ts:202-214, src/install/hostWritebackRuntime.ts:306-311, src/install/hostMcpServer.ts:159-680 | candidates captured in review mode are, in the file's own words, \"extracted but NOT committed, waiting for an operator to approve (promote to durable memory) or reject via the Inspector\"; the queue stores the redacted statement rather than the transcript, approval is reserved before the durable write so two approvals cannot replay it, and a failed durable write releases the reservation. The producer cannot approve: the only caller of `approveCandidate` is the Inspector admin API, and the host MCP server registers ten tools — `goodmemory_get_context`, `_inspect_memory`, `_trace_recall`, `_search_index`, `_timeline`, `_get_records`, `_read_artifacts`, `_stats`, `_remember` and `_write_note` — with no approve, reject or release verb among them. Two limits belong with the mark. The queue exists only when the host's writeback `mode` is `review`, one of four values defaulting to `off` (`hostWritebackRuntime.ts:173`); and it covers hook-captured candidates, not the agent's own `goodmemory_remember`, which writes durable memory directly subject to classification and dedupe | tests/unit/inspector-review-mode.test.ts:78 'queues durable candidates for review without writing memory'; tests/unit/inspector-candidate-review.test.ts:91, :253"
...
**2026-09-19** — audited at the unchanged pin [`416c15b479e189039709468fb319bb50c0fbe50e`](https://github.com/hjqcan/GoodMemory/commit/416c15b479e189039709468fb319bb50c0fbe50e); nothing upstream moved. All six marks stand. `human_review` was producer-tested rather than carried forward, and passes on the shape this corpus has found hardest to fake: there is no approve verb to guard. The host MCP server registers ten tools and every write among them is `goodmemory_remember` or `goodmemory_write_note`; `approveCandidate`'s only caller is the Inspector admin API. Its record now also carries the two limits that reading the writeback runtime turns up — the queue exists only when the host's mode is `review`, which is one of four values defaulting to `off`, and it covers hook-captured candidates rather than the agent's own explicit writes. The anchor `src/api/memoryAdminOps.ts` in the previous record does not contain the approve path and has been replaced by the file that does. Screened again first; nothing was installed and no suite was run.
