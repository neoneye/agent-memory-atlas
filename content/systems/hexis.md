...
capability_evidence:
  ...
  trust_state: "two CHECK-constrained axes on the claims table, both required by the one function that builds agent context | db/00_tables.sql, db/85_functions_connector_cognition.sql:251-448, :530-589, :590-616, :617-660, :288-290, :393-397, :268-269, :327 | `status` carries the lifecycle and `review_status` the approval, and `get_approved_user_model_context` — the only read in this file that assembles context for an agent — selects `WHERE status = \"active\" AND review_status = \"approved\"`, so a claim in `pending_review` or `rejected` is absent rather than ranked below. The other reads of the table are unfiltered and each is correctly so: they sit inside `upsert_user_model_claim`, inside `list_user_model_claims` where the status is an optional parameter for a reviewer, and inside `review_user_model_claim`, which is the writer. An unrecognised review value coerces to `pending_review` rather than being trusted, so the permissive value is never the fallback. One asymmetry belongs on the record: a re-upsert of an already-approved claim deliberately keeps the approval — the CASE reads *when approved and the incoming value is pending_review, keep it* — while `claim` is overwritten with the incoming text unconditionally. The match is on `claim_key`, a caller-supplied string independent of the claim, so the approval is bound to the key rather than to the sentence a reviewer approved | tests/db/test_memory_triage.py, tests/db/test_cognition_v2.py"
...
**2026-09-19** — [`7423622a276382f617196b5d8925bdcd7c8311ff`](https://github.com/quixiai/hexis/commit/7423622a276382f617196b5d8925bdcd7c8311ff) — `trust_state` re-tested at the unchanged pin. The predicate is where the record said and the mark is clean; what the record could not show is *why the other reads are allowed to omit it*. `user_model_claims` is read at nine places in `85_functions_connector_cognition.sql` and only one carries the pair, which looks like a leak until the function boundaries are drawn: every other read sits inside `upsert_user_model_claim` (the write path's dedup, supersession and contradiction resolution), inside `list_user_model_claims` where `p_review_status` is an optional filter a reviewer chooses, or inside `review_user_model_claim`, the writer itself. `get_approved_user_model_context` is the only one that assembles context for an agent, and it requires both `status = 'active'` and `review_status = 'approved'`. The fallback polarity is right too: an unrecognised review value coerces to `pending_review`, so the permissive value is never what an unexpected input lands on. One asymmetry is worth carrying, because it is the shape that has bitten a sibling system in this corpus from the other direction. Re-upserting an already-approved claim deliberately preserves the approval rather than returning it to the queue, while `claim` is overwritten with the incoming text unconditionally — and the row is matched on `claim_key`, a caller-supplied string derived from a parameter rather than from the claim. So the approval attaches to the key, and the sentence under that key can be replaced without a second review. Nothing was installed and no suite was run.
