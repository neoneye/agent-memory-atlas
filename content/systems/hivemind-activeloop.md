...
description: "A hook layer storing coding-agent turns in hosted Deeplake tables, mining them into team skills, and rewriting any skill an LLM judge marks failed."
root: ../..
page_kind: system
source_name: "activeloopai/hivemind"
source_url: https://github.com/activeloopai/hivemind
archive_name: "activeloopai--hivemind"
revision: ce30de7ca94115cb73fa991538c6d2595ac2622a
revision_url: https://github.com/activeloopai/hivemind/commit/ce30de7ca94115cb73fa991538c6d2595ac2622a
analyzed_at: 2026-09-26
licence: "Apache-2.0"
size: "56,112 lines of TypeScript in 271 files under src/; package @deeplake/hivemind 0.7.159"
activity: "2,219 commits reachable from main by 18 author names, one of them a release bot, 3 April – 17 September 2026"
tests: "5,394 it() cases in 314 test files under tests/"
capabilities: "scope_enforced, negative_eval"
capability_evidence:
  scope_enforced: "code-docs search — the `hivemind_docs_search` MCP tool and the `<memory>/docs/find/` VFS path | src/shell/grep-core.ts:468-469,:492-499,:514-517, src/mcp/server.ts:145-147, src/docs/vfs-handler.ts:75-84, src/commands/docs.ts:719 | every docs row stores `project` = `deriveProjectKey(cwd).key`, a sha1 of the normalised git origin URL; `searchDocs` appends `AND (project = '<key>' OR project = '')` to both the lexical and the semantic arm of its UNION, and the MCP tool sets the key from its own cwd with the reason in the comment — *\"a shared org table must not leak another repo's docs into this one's search\"*. A second project predicate, keyed on the cwd basename rather than the git remote and paired with `author`, reaches the resume brief on the summaries table (`src/notifications/sources/resume-brief.ts:275,:303-306`). It does not reach memory retrieval: `searchDeeplakeTables` carries no project or author predicate, and the boundary there is the workspace in the request URL | tests/claude-code/grep-core.test.ts:54-65, tests/claude-code/resume-brief.test.ts:261-271"
...
matrix:
  memory_unit: "One row per hook event — the prompt, tool call with full input and output, or assistant turn, as JSONB with a 768-dimension vector; beside it an LLM-written session summary, a mined `SKILL.md`, a team rule, a goal, and a per-source-file doc page"
  storage: "No store of its own: seven tables in a Deeplake workspace reached by a POST to the workspace's tables/query endpoint with SQL in the body, plus local files — the skills themselves under `.claude/skills/`, including skills mined from local transcripts before sign-in, a pull manifest, a JSONL event cache, an optimizer meta log and a code-graph snapshot"
  retrieval: "Grep over `~/.deeplake/memory/` intercepted and compiled to one UNION ALL of an ILIKE arm scoring a constant 1.0 and a cosine arm, so any literal hit outranks every semantic hit, and lexical-only while embeddings are off, which is the default; a docs search with a project predicate; no per-prompt recall on any host, and at session start the model gets team rules while the resume brief goes to the user only"
  write: "Every hook event embeds locally and INSERTs on the turn path, after secret masking on every host except pi; a detached worker shells out to the host agent's own CLI for the summary; a second worker mines the last ten sessions in scope and asks Haiku for KEEP, SKIP or MERGE"
  update_delete: "`hivemind sessions prune` hard-deletes your own session rows and their summaries; `rm` under the memory mount is a `DELETE FROM`; skills, rules and docs append a new version row; goals are updated in place and the `version` column is vestigial"
  ...
  background: "Detached workers on Stop and SessionEnd: summary, skill mining, code-graph rebuild, doc refresh, skill auto-pull at SessionStart, an event-driven skill optimizer armed by each org-skill invocation, and a one-shot install-time mining and backfill of past local sessions"
...
**Secret masking is per capturer, and one of seven skips it.** Six capture paths pass the serialized turn through `redactSecrets` before it is embedded or queued. The pi extension ships as a zero-dependency raw `.ts` file with its own INSERT, and its `writeSessionRow` sends `JSON.stringify(entry)` unmasked (`harnesses/pi/extension-source/hivemind.ts:1139,:1154-1158`). A pi user's pasted token reaches a table every workspace member can read. Cowork ingest had the same gap until 17 September 2026 ([`fd927c76126e5bf3f2f83a407d4d4907b687e010`](https://github.com/activeloopai/hivemind/commit/fd927c76126e5bf3f2f83a407d4d4907b687e010)).

## 2. Mental Model

Four kinds of thing are remembered and they sit at different epistemic levels. A **trace row** is a fact about what happened and cannot be wrong; it is immutable through the agent's own surfaces — `rm` on a session path throws `EPERM` (`src/shell/deeplake-fs.ts:1036`) — and removable only by the owner through `hivemind sessions prune`. A **summary** is one LLM's reading of a session, rewritten in place on each checkpoint. A **skill** is a claim about how to do something, and it is the only object with a lifecycle. A **rule**, **goal** or **doc page** is human- or generator-authored state with its own small table.
...
Scope is the one field that moves on its own, in one direction. A `MERGE` whose editor is not the row's author sets `scope` from `me` to `team` — *"one-directional promotion"* (`src/skillify/skillify-worker.ts:488-489`, `src/skillify/scope-promotion.ts:34-41`) — and the SkillOpt publish promotes unconditionally, on the argument that an optimizer edit is inherently shared. The `scope` column is written on every row and carried forward, and no query filters on it: the pull selects it and the optimizer's publish copies it before overwriting it with `team` (`src/skillify/pull.ts:139`, `src/skillify/skill-org-publish.ts:86,:136`).
...
**Nothing runs as a server here.** The unit of execution is a short-lived Node process spawned by a host agent's hook, and every one of them talks to `https://api.deeplake.ai` by `POST /workspaces/<workspaceId>/tables/query` with `{query: "<SQL>"}` in the body, a bearer token, and an `X-Activeloop-Org-Id` header (`src/deeplake-api.ts:304-314`). The client composes all the SQL — DDL, healing `ALTER TABLE`, inserts, the retrieval UNIONs — so the mechanism is fully readable at this commit even though the engine behind the endpoint is not.

Seven tables are declared in one place (`src/deeplake-schema.ts`): `memory` for summaries and virtual-filesystem files, `sessions` for per-event traces, plus `skills`, `hivemind_rules`, `hivemind_goals`, `hivemind_docs` and a `codebase` graph snapshot table. Schema drift is handled by `healMissingColumns`, which reads `information_schema.columns` for the workspace, diffs against the declaration, and `ALTER TABLE ADD COLUMN`s only what is missing, tolerating one "already exists" race and re-verifying it. A module-load lint refuses any `NOT NULL` column without a `DEFAULT`, because the backfill on a populated table would fail.

**Local state is small but real**: `~/.deeplake/credentials.json` at mode 0600, `~/.deeplake/state/skillify/{config,pulled}.json`, `skillopt/meta.jsonl`, a per-session event cache the summary worker reads instead of re-scanning the fat `message` column, a `query-cache` directory materialising intercepted `Read` results as real files, and the skills themselves as `SKILL.md` directories under `.claude/skills/` or `~/.claude/skills/`.

### Deployment and ergonomics

Capture, search and sharing need an account and a token; with no credentials session start prints *"Not logged in to Deeplake; memory search is unavailable this session."* Install is one command that detects every supported assistant and wires its hooks.

Before sign-in, install does two local things in the background: it mines up to ten recent Claude Code sessions into `SKILL.md` files under the global skills root, and it stages summaries of past sessions on disk for a later `hivemind memory flush` (`src/cli/index.ts:289-293,:394`, `src/commands/mine-local.ts:1-20`, `src/skillify/pending-memory-manifest.ts:8-17`). The mined skills stay local; the `push-local` upload their manifest names has no implementation in the tree.

Semantic search is opt-in: `hivemind embeddings install` fetches nomic-embed-text-v1.5 (768 dimensions, matryoshka-truncated) and a local daemon, a ~600 MB dependency the README states up front. Until then memory search is keyword-only, and a one-time session-start banner says so (`src/notifications/rules/embeddings-nudge.ts:24-29`). The summary, doc and skill-mining workers shell out to the host agent's own CLI (`claude -p`, `codex exec`, `pi --print`), so no second API key is needed and the token cost lands on the user's existing plan. The store is not repairable by hand — it is rows behind an HTTP endpoint — but the virtual filesystem makes it browsable with `ls` and `cat`, and BYOC points the bytes at the adopter's own GCS, Azure or S3 bucket.
...
- **Retrieval.** `src/hooks/pre-tool-use.ts` intercepts the tool call; `src/shell/grep-core.ts:308-412` builds the hybrid UNION; `src/shell/grep-core.ts:681-768` re-applies the real regex to the returned rows and formats grep-shaped output. `src/mcp/server.ts` exposes the same searches as `hivemind_search`, `hivemind_docs_search`, `hivemind_read` and `hivemind_index`.
...
- **Deletion.** `src/commands/session-prune.ts` via `hivemind sessions prune`; `DeeplakeFs.rm` (`src/shell/deeplake-fs.ts:1034-1110`) for everything under the memory mount.
...
Nothing is recalled per prompt. At session start the model gets the active team rules and one-line notes for the graph and docs wiki; the resume brief built from the user's own recent summaries goes to the user alone, flagged `userVisibleOnly` so its summary-derived prose stays out of the model's context (`src/notifications/sources/primary-banner.ts:256-261`). A `UserPromptSubmit` hook that injected one cosine-ranked teammate summary per prompt was removed on 16 September 2026, because with embeddings off by default its gate returned nothing on every prompt ([`99008d8996ab0dd3c95ed0b80ec71d076dfd5b03`](https://github.com/activeloopai/hivemind/commit/99008d8996ab0dd3c95ed0b80ec71d076dfd5b03)). Everything else waits for the agent to run `grep`.

**Failure modes.** Under-recall is the default state: nothing surfaces unless the agent searches, and without the embedding daemon the search is keyword-only, so a paraphrase finds nothing. The index the agent reads first, `/index.md`, is capped at the 50 most recent rows per section. Tool output is normalised on the way out — `formatToolCall` drops boilerplate, and any `<recalled-memories>` block injected by a previous turn is stripped greedily from first open to last close, so recalled text cannot be re-retrieved as if it were fresh evidence (`src/shell/grep-core.ts:248-269`). The code graph under `<memory>/graph/` is substring search over node ids with a one-hop neighbourhood, read from a local snapshot with zero network calls; it is a browsable surface, not an arm of memory retrieval, and the README's claim that *"Search and recall walk this graph"* is not true of `searchDeeplakeTables`, the only memory search.

## 7. Write Mechanics

Capture is on the turn path. Each hook event does one local embed and one INSERT before the hook returns; a table-missing error triggers a create and one retry, and the whole `main` is wrapped so a failure exits 0 and surfaces nothing mid-session. The refusal to surface anything is a deliberate rule worth quoting: a user-facing message can only reach a mid-session hook through `additionalContext`, which is the model's prompt, and *"Writing a user-facing 'credits exhausted, top up at `<url>`' notice there is a prompt-injection pattern… Hard rule: nothing user-facing is ever written into the model/agent prompt"* (`src/hooks/capture.ts:302-308`). The same rule keeps LLM-derived banner prose out of `additionalContext` at session start.
...
Noisy input is filtered by one module, called from each capturer rather than from a shared writer. `src/hooks/shared/redact.ts` masks before the text is embedded or written: PEM and OpenSSH key blocks, about thirty provider token schemes with the scheme prefix kept as a hint, `Authorization` headers, URL basic-auth, Sentry DSNs and Slack webhooks, `KEY=VALUE` where the key is secret-ish, and a Shannon-entropy backstop for unlabelled tokens with guards that exempt UUIDs, hex hashes and dated model identifiers. It runs on six of the seven capture paths, on every generated summary, and again on upload (`src/hooks/capture.ts:177`, `src/mcp/cowork-ingest.ts:421`, `src/hooks/upload-summary.ts:130`). It does not run on the pi extension's own capture writer, on a `SKILL.md` a person pushes with `hivemind skillify push`, or on skills mined from raw local transcripts at install.
...
**Provenance is tracked and never consulted.** A skill row records who authored it, who has edited it, which session ids it came from, which agent produced it and which plugin version wrote it, and the pulled file carries all of it in frontmatter so a later gate can see whose skill it is. No query filters on any of it. There is no epistemic state anywhere: `trust_state` is withheld because no field distinguishes a candidate from a verified skill, and the closest thing — `docs.status` moving `active` → `archived` when a branch overlay is promoted — is archival, which the rubric excludes.

**`tombstone` is withheld.** No record of a rejected value exists in `src` or `tests`, and no read path consults one. Deletion is unrecorded. `hivemind sessions prune` hard-deletes your own trace rows and the matching summary, and `rm` under the mount is a `DELETE FROM … WHERE path = …`. Neither touches the skills those sessions were mined into — a skill's `source_sessions` can name rows that no longer exist, and the knowledge distilled from a session survives the session's deletion with no marker anywhere.
...
**The trust boundary is stated plainly by the product and is the thing to weigh.** Everyone in the workspace reads every trace, and a skill mined from one engineer's session is executed as instructions in another's. A `.hivemind` file travels with a repository and can route a clone's traces and reads to a different org; the defence is that it cannot carry a token, so it can only target orgs the existing login already authorises, and that the session-start banner always prints the effective org, workspace and the file that routed it.

One user-facing notice does reach the model. When `HIVEMIND_WORKSPACE_ID` or a `.hivemind` names a workspace the org does not have, session start writes a warning into the model's context. Because the file is committed content from a cloned repository, each value is flattened of control characters and cut to 64 characters first (`src/commands/auth.ts:352-358,:389`, `src/hooks/session-start.ts:355`).

The command interception is defended with unusual care. `isSafe` rejects `$( )`, backticks, process substitution and `$'…'` ANSI-C quoting, splits on pipes and separators, and requires every stage's first token to be in the allowlist. The allowlist's comments record what was removed and why — `sed` for `-e '1e <cmd>'`, `awk` for `system()`, `xargs`, `tar --to-command`, `env`, `timeout` and `time` as wrappers that run an arbitrary child. Against that, the summary prompt instructs the model to preserve *"exact token/key/secret-name values"* verbatim into a summary every workspace member can read, with a worked example that writes a staging token into the wiki; the redactor masks on both sides of that instruction, so what survives is whatever the masker does not recognise.
...
What that gate does not cover is named in the same file. `src/skillify/skillify-worker.ts` and its spawner are excluded from coverage because they *"need a live Deeplake workspace + a real agent CLI to exercise meaningfully"*. The worker whose verdict decides what becomes a team skill is therefore checked only by a bundle-scan test asserting the built artifact exists and contains the right entry strings, and the end-to-end matrix script is recorded as living *"at /tmp/skillify-e2e-matrix.mjs in the author's worktree, not committed"*.

The evaluation claims are the weakest part. The README reports a LoCoMo run — $8.94 to $6.65 per 100 QA, 1,700 to 1,008 tokens per question, 8.9 to 6.2 turns — and no harness, result file or dataset for it exists in the tree. No path matches `locomo` or `benchmark`, and the numbers appear only as prose plus a hardcoded 1.7x multiplier used to render a per-session "tokens saved" banner (`src/dashboard/data.ts:57-58`, `vitest.config.ts:449`).

What the repository does carry is the residue of that run as regression tests. `tests/claude-code/pre-tool-use-baseline-cloud.test.ts` rebuilds 272 session rows matching the real `locomo_benchmark/baseline` workspace and pins the three QAs the run got wrong. `tests/claude-code/output-cap.test.ts` records the cause behind that run's losses — *"11 of 14 losing QAs that hit this path never recovered the persisted file"* when Claude Code spilled a large tool result to disk and showed the model a 2 KB preview. Turning a benchmark loss into a committed fixture is better practice than most published numbers, and it is not the number.

The cited tests can fail. The resume-brief exclusions each assert the replacement is present against a populated fixture, so none of them can pass on an empty result; the docs project-scope test asserts the predicate is present when a project is set and absent when it is not, so it fails in both directions. The Cowork masking fix landed with four cases asserting a masked OpenAI, GitHub and Anthropic token beside an unchanged plain message (`tests/claude-code/cowork-ingest.test.ts:180-235`).

Two absences follow from the design. The pi extension's source test pins its INSERT shape, summary thresholds and daemon protocol against the canonical modules, and has no case for masking, the one drift it would have caught (`tests/pi/pi-extension-source.test.ts`). The pull carries no project predicate to test (`src/skillify/pull.ts:128-136`). No test asserts anything about the optimizer's effect on a skill's later success — there is no signal to assert it with.
...
- **Filtering a read path with a parameter every caller omits.** A `users` list hardcoded empty at the only automatic call site and a `scope` column no query filters on are two shapes of the same defect: the boundary exists in the code and not in any request.
...
- **Copying the write path once per host.** The pi extension re-implements the sessions INSERT to stay dependency-free and is kept in step by hand, and the masking every other capturer applies did not come across; Cowork's copy lacked it until 17 September 2026. Put redaction inside the one function every writer must call, or test each copy for it.

### Fit

This suits an organisation that has already decided its agent transcripts are a shared asset and is willing to keep them in a vendor's store — the data-collection notice is blunt about what is captured and who can read it, and that is the first question, not the last. Inside that decision it is a strong fit for a small-to-mid engineering team on one codebase who all use coding agents daily and want the propagation: the install is one command, the per-agent adapters are real, and nothing needs to be operated. It is a poor fit in three cases:

- anyone who needs local-only memory, since capture and search need an account and the only writes before sign-in are a one-shot mining of past local transcripts;
- consultancies and anyone working across client boundaries, where the `.hivemind` routing is the only separation and the default is one shared pool;
- teams that would need to review what enters a shared skill library, because the repository's position on that is explicit and the roadmap entry for pre-release review is a roadmap entry.

Adopt the mounted-filesystem interface and the reaction judge as ideas regardless — they transfer to a store you run yourself.

## 12. Open Questions

- How often does the optimizer actually fire and publish in a real team, and how often is the resulting edit an improvement? Nothing in the tree measures it: the meta log records proposals and no outcome.
...
- **Write path:** `src/hooks/capture.ts`, `src/hooks/{codex,cursor,hermes}/capture.ts`, `harnesses/pi/extension-source/hivemind.ts`, `src/hooks/shared/redact.ts`, `src/hooks/shared/session-insert-sql.ts`, `src/hooks/summary-state.ts`, `src/hooks/session-event-cache.ts`.
- **Summaries:** `src/hooks/spawn-wiki-worker.ts`, `src/hooks/wiki-worker.ts`, `src/hooks/upload-summary.ts`, `src/hooks/wiki-offset.ts`.
- **Retrieval:** `src/shell/grep-core.ts`, `src/shell/grep-interceptor.ts`, `src/shell/deeplake-fs.ts`, `src/hooks/pre-tool-use.ts`, `src/hooks/memory-path-utils.ts`, `src/hooks/virtual-table-query.ts`.
- **Skills:** `src/skillify/skillify-worker.ts`, `skill-writer.ts`, `skills-table.ts`, `pull.ts`, `auto-pull.ts`, `push.ts`, `unpull.ts`, `manifest.ts`, `scope-config.ts`, `scope-promotion.ts`, `gate-{parser,runner}.ts`; pre-sign-in mining and backfill in `src/commands/mine-local.ts`, `src/skillify/spawn-mine-local-worker.ts`, `src/skillify/pending-memory-manifest.ts`, `src/commands/flush-memory.ts`.
...
- **Integration and deletion:** `src/mcp/server.ts`, `src/mcp/cowork-ingest.ts`, `src/hooks/session-start.ts`, `src/notifications/sources/{resume-brief,primary-banner}.ts`, `src/notifications/rules/embeddings-nudge.ts`, `src/commands/auth.ts`, `src/dir-config.ts`, `src/commands/session-prune.ts`, `harnesses/`.
- **Tests:** `tests/claude-code/{grep-core,resume-brief,skillify-pull,deeplake-fs,pre-tool-use-baseline-cloud,output-cap}.test.ts`, `tests/claude-code/cowork-ingest.test.ts`, `tests/pi/pi-extension-source.test.ts`, `tests/shared/{docs,success-judge,skillopt-*}.test.ts`, `vitest.config.ts`.

**Searches recorded for the negative claims**

```sh
ls src/hooks/recall.ts src/hooks/shared/recall-gate.ts                    # both absent: the per-prompt recall hook is gone
grep -n 'UserPromptSubmit' -A6 harnesses/claude-code/hooks/hooks.json      # capture.js only
grep -n 'recall' harnesses/openclaw/src/index.ts                          # two comments recording the removal; the pi input handler only captures
grep -rln 'redactSecrets(' src harnesses/openclaw/src                      # six capturers, five summary workers, upload-summary; no pi capturer
grep -c 'redactSecrets' harnesses/pi/extension-source/hivemind.ts          # 0: the pi writer masks nothing
grep -ci 'redact' tests/pi/pi-extension-source.test.ts                     # 0: the drift test has no masking case
grep -rn 'push-local' src                                                  # comments only; no command uploads locally mined skills
...
grep -rnE "scope\s*(=|IN)\s*'" src/skillify src/shell                       # 0: no query filters on a skill's scope
grep -rniE 'tombstone|deleted_value|rejected_value' --include='*.ts' src tests         # 0
...
grep -n 'author\|project' src/shell/grep-core.ts                           # project appears only in searchDocs; memory search has neither predicate
...
**2026-09-26** — [`ce30de7ca94115cb73fa991538c6d2595ac2622a`](https://github.com/activeloopai/hivemind/commit/ce30de7ca94115cb73fa991538c6d2595ac2622a) — 51 commits on. Both marks held; the correction loop, scope predicates and cited tests are byte-identical. Upstream removed the per-prompt recall hook and the KPI table on 16 September 2026, so the summary's recall finding is replaced ([section 6](#6-retrieval-mechanics)). Cowork capture gained masking; pi capture has none at either pin ([section 7](#7-write-mechanics)). Corrected, wrong at the first pin: install writes skills and staged summaries before sign-in ([section 3](#deployment-and-ergonomics)); the scope column is selected, just never filtered; the resume brief keys on a cwd basename; the matrix counted seven tables where eight were declared. Screened first from a full clone: one auto-run surface, two build-time execution paths, one unpinned manifest, none in the cooldown. Nothing was installed, built or run.
