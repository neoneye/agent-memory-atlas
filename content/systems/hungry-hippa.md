...
revision: e9254e8942bc7c04f15e40141588fc28907c5193
revision_url: https://github.com/TRUE-BLUE-INDUSTRIES/hungry-hippa/commit/e9254e8942bc7c04f15e40141588fc28907c5193
analyzed_at: 2026-09-16
capabilities: "trust_state, scope_enforced"
capability_evidence:
  trust_state: "a stored row status partitions retrieval before ranking, with a quarantine the caller opts into explicitly and every exclusion returned with its reason | src/hungry_hippa/schema.py:56, src/hungry_hippa/retrieval.py:323-338, src/hungry_hippa/policy.py:126-138 | `episodes.status` is `active | archived | compressed | purged` with its own index, and step six of the recall pipeline partitions on it: an item whose status is not `active` goes to `excluded` with the status as its reason and never reaches the ranker. The pipeline docstring names the stage — \"actor/status policy partition (quarantine + sensitivity)\" — and quarantined rows return only when a caller passes `include_quarantined`. Relationship reads carry the same predicate (`WHERE … AND status = 'active'`), and the context compiler applies an active-over-superseded preference on top | src/hungry_hippa/graph.py:148, :167, :228"
