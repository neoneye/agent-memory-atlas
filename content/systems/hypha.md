...
capability_evidence:
  trust_state: "the managed record's status, applied as an admission test on every retrieve | packages/memory/src/record-contract.ts:52-61, :188, packages/memory/src/retrieval.ts:464-474, packages/memory/src/foundation.test.ts:226-243 | `memoryStatusSchema` is an eight-value stored enum \u2014 pending, active, dormant, superseded, invalidated, deletion_pending, deleted, failed \u2014 written on every record and set to `active` by the native write path. The retrieval gate drops `deleted` and `deletion_pending` unconditionally and `superseded` and `invalidated` unless the request sets `includeSuperseded` or `includeInvalidated`, so four of the eight withhold a record from a default read rather than ranking it down. It is a state rather than a score standing in for one: `confidence`, `importance`, `strength` and `salience` are separate unit intervals that feed the ranking, and `indexStatus` is a separate seven-value machine so \"not yet searchable\" never borrows a value from \"not believed\" | packages/memory/src/foundation.test.ts:179-243, where a record moved to `invalidated` disappears from a retrieve that returned it moments earlier"
