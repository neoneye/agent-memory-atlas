...
capability_evidence:
  trust_state: "a four-value verified status gated by a legal-transition table, excluded by default at every public entry point, and widened only by a separately named function | src/icarus_memory/validation.py:22-28, src/icarus_memory/retrieval.py:15-26, :44-52, :59-64, :82, :100-117, :140, src/icarus_memory/__init__.py:259, :296 | `_LEGAL_TRANSITIONS` is an explicit mapping, so `rolled_back` is terminal by having no outgoing set, `contradicted` can only move to `rolled_back` and never back to `verified`, and an illegal move raises rather than landing. The status is a stored field distinct from the two-value freshness lifecycle and from any score, and it reaches the read path three ways that do different jobs: `_VERIFIED_ORDER` ranks, `_MIN_VERIFIED_THRESHOLD` is a floor expressed as the set of values each threshold admits, and `_matches_status_filter` is the admission test. `status_filter` defaults to `safe` at all four public entry points, and `safe` excludes `contradicted` and `rolled_back` — the two values that mean *not believed*. The escape is not a flag on the ordinary search: `audit_search` is a separate function whose docstring says it *includes contradicted, rolled-back, and superseded entries*, so the audit view cannot be reached by accident | the `verifier` is a free-text string defaulting to `manual`, so the record cannot distinguish a person from the process that wrote it"
...
**2026-09-19** — [`6e348708dcddb7cf1ad47726cb287cd4c9183c40`](https://github.com/esaradev/icarus-memory-infra/commit/6e348708dcddb7cf1ad47726cb287cd4c9183c40) — `trust_state` re-tested at the unchanged pin; the mark stands and the mechanism is tidier than the record showed. Three structures do three separate jobs and none of them is doing another's work: `_VERIFIED_ORDER` ranks, `_MIN_VERIFIED_THRESHOLD` expresses a floor as the set of values each threshold admits, and `_matches_status_filter` decides admission. The default matters and it is consistent: `status_filter` is `"safe"` at all four public entry points — both `search` signatures in `retrieval.py` and both in `__init__.py` — and `safe` excludes exactly the two values that mean *not believed*. Best of all is the shape of the escape. There is no `include_rolled_back` flag on the ordinary search to be passed by accident; the widening is a differently named function, `audit_search`, whose one-line docstring is *"Raw audit search that includes contradicted, rolled-back, and superseded entries."* A caller reaches the audit view by asking for it by name. The transition table is exact as recorded: `rolled_back` is terminal by having an empty outgoing set, and `contradicted` can only move to `rolled_back`, so nothing walks back to `verified`. Nothing was installed and no suite was run.
