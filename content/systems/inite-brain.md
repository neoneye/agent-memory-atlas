...
revision: e57752a047902fcb9fd9331a5701c5a94682b836
revision_url: https://github.com/inite-ai/inite-brain-service/commit/e57752a047902fcb9fd9331a5701c5a94682b836
...
**2026-09-19** — [`e57752a047902fcb9fd9331a5701c5a94682b836`](https://github.com/inite-ai/inite-brain-service/commit/e57752a047902fcb9fd9331a5701c5a94682b836) — `trust_state` re-tested. The mark holds and the record gains the thing that makes it worth reading. Anchors corrected: the status field is at `src/db/schema.surql:64-65`, not `:46`, and carries its vocabulary as an `ASSERT` rather than a convention, with indexes at `:73` and `:75`; the helper file is `src/entities/entity-read.helpers.ts` with a dot, and `activeFactWhere` is at `:86`. `where-builder.ts:85-86` held exactly, and shows the pair the search path applies — `retractedAt IS NONE` beside `status != 'competing'`, a timestamp and a status doing adjacent jobs. `activeFactWhere` composes the bitemporal window with `status != 'compacted'` and `status != 'corroborating'`, hiding the audit rows of a second claim so the incumbent carrying the corroboration counter is the row that surfaces. The addition is what happened to the duplicated predicates. The MCP code-memory tool (`src/mcp/code-memory-tools.ts:71`) and the summarize-entity service (`src/summarize-entity/summarize-entity.service.ts:178`) each carry a comment stating that they no longer filter status locally, because the shared helper applies the full rule. This sweep has repeatedly found the opposite — a predicate copied per query and forgotten in one of them — so a project that consolidated its copies into one helper and left a note where each copy used to be is worth naming. Screened again first; nothing was installed and no suite was run.

**2026-09-16** — [`e57752a047902fcb9fd9331a5701c5a94682b836`](https://github.com/inite-ai/inite-brain-service/commit/e57752a047902fcb9fd9331a5701c5a94682b836) — first reading, at a commit dated 11 September 2026. Screened before opening, from a shallow clone: two auto-run surfaces, three build-time execution points, five unpinned surfaces, seven dependency files inside the cooldown across fifteen scanned files, and `AGENTS.md` read as data. Nothing was installed, built or run.
