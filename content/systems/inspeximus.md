...
revision: 3b9223d8d1b43c68e46b538c135b67844510a5bd
revision_url: https://github.com/DanceNitra/inspeximus/commit/3b9223d8d1b43c68e46b538c135b67844510a5bd
...
capability_evidence:
  ...
  trust_state: "a stored status filtered on recall by an allowlist, with the bug that motivated the allowlist recorded beside it | inspeximus/core.py:176, :239-257, :11107-11160 | `_WITHHELD = frozenset({\"provisional\", \"discarded\", \"candidate\"})` and `_serving_class` answers \"will this record ever reach a reader?\"; `recall` filters status through an allowlist rather than a denylist, and the comment says why: \"a denylist only knows the statuses somebody remembered to add. `discard_provisional()` set status='discarded' and that fell straight through to `return include_superseded` — a REJECTED record, surfaced by a flag named for supersession, minutes after the denylist above it was written.\" `include_superseded` is the caller's explicit widening | tests/test_core.py:20"
