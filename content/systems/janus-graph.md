...
flowchart TD
  ...
  S --> W["Graphiti.add_episode, reference_time =<br/>the sweep clock, since created_at is unset"]
...
- **Sweep.** `run_cron_sweep` (`pipeline/cron.py:29-136`) reaps rows stuck in `processing` past the timeout (`queue.py:310-370`), claims a batch (`:207`), and runs `EpisodeWorker.process_record` under a semaphore. `_execute_ingestion` (`pipeline/worker.py:48-81`) builds a fresh Graphiti instance per episode and calls `add_episode` with `reference_time` taken from `record.created_at` — which `claim_next_batch` neither selects nor passes to the record it builds (`queue.py:217-218`, `:248-259`), so the fallback `datetime.now()` at `worker.py:66` is always the value used.
...
**2026-09-25** — [`5a66516fd795ca178870b90253bedb0a6a0e5aac`](https://github.com/Maple-Aikon/janus-graph/commit/5a66516fd795ca178870b90253bedb0a6a0e5aac) — same pin, one correction. The `bitemporal` evidence record and the 2026-09-18 entry below said the worker prefers the enqueue time as Graphiti's reference time. It does not: `worker.py:66-71` reads `record.created_at`, and `claim_next_batch`, the only source of the records the sweep processes, neither selects that column (`queue.py:217-218`) nor passes it to the `EpisodeRecord` it builds (`:248-259`), so the sweep clock is always used — as section 4 said. The evidence record, the section 2 diagram and two queue anchors in section 4 are corrected to that. No mark moved; nothing was installed or run.

**2026-09-18** — [`5a66516fd795ca178870b90253bedb0a6a0e5aac`](https://github.com/Maple-Aikon/janus-graph/commit/5a66516fd795ca178870b90253bedb0a6a0e5aac) — re-pinned from `987c34c`; 32 files and +4,936 lines, re-screened at the new pin. **One claim corrected and it was wrong when written.** The bitemporal record said *"the reference time forwarded is the sweep's clock rather than the enqueue time"*; `worker.py:66-71` initialises `ref_time` to `now()` and then overwrites it from `datetime.fromisoformat(record.created_at)`, so the enqueue time is preferred and the sweep clock is the fallback for a missing or unparseable field — and the block is byte-identical at the previous pin. **The engine pin moved and is no longer readable at its version.** The declared constraint is `graphiti-core[falkordb]>=0.20` and `uv.lock` resolves 0.30.1, which has no git tag: the repository's tags step from `v0.30.0` to `v0.30.2`. The anchors were re-verified against `v0.30.0` and hold — `invalid_at` parsing at `:255-308`, the contradiction close at `:548`, the already-superseded case at `:822-838`. New at this pin: a Phase 3.1 BFS search endpoint (`daemon/search_engine.py`, `engine/search_memory.py`, a 395-line HTTP handler and a 705-line test file) that carries `invalid_at IS NULL` into the Cypher traversal and filters again after the cosine rerank, kept deliberately separate from `search_memory` — its module docstring records the reason as an anti-pattern: *"semantic MMR rerank != structural traversal; mixing degrades both"*.
