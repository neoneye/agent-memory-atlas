...
root: ../..
page_kind: system
source_name: "jumbocontext/cli"
source_url: https://github.com/jumbocontext/cli
archive_name: "jumbocontext--cli"
revision: 30e7947a8a0afac627c82755be0a6c8ff71106a2
revision_url: https://github.com/jumbocontext/cli/commit/30e7947a8a0afac627c82755be0a6c8ff71106a2
analyzed_at: 2026-09-18
capabilities: "audit_log, negative_eval"
capability_evidence:
  audit_log: "the filesystem event store, which is the memory rather than a record beside it | src/infrastructure/persistence/FsEventStore.ts:24-41 | `append` writes one JSON file per event into a per-aggregate stream directory as `000001.GoalApproved.json`, to a temp path then `fs.move`; no code path mutates or deletes a written event — the only `fs.remove` calls in `src/` are in the project scaffolder. The limit is in how the sequence is chosen: `const files = await fs.readdir(streamDir); const nextSeq = files.length + 1;` with no lock, mutex or compare-and-swap anywhere in the persistence layer, and the write is `fs.move(tmpPath, filepath, { overwrite: true })`. Two appends to one aggregate that interleave between the `readdir` and the `move` compute the same `nextSeq`, and the second overwrites the first. The comment above the temp-then-rename says it prevents *\"corruption from concurrent processes\"*, and it does prevent a torn file; it does not prevent a lost event, and `overwrite: true` is what turns the race into silence rather than an error. Within one awaited call chain in one process the sequence is safe, which is the CLI's normal shape | tests/infrastructure/persistence/FsEventStore.test.ts"
