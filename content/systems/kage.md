...
capability_evidence:
  trust_state: "a pending packet is written to a different directory, and the approved loader filters on the status as well | mcp/kernel.ts:95, :2515-2525, :4775-4780, :3104, :3799, :4172, :4230, :7978, :9595-9602, :4615, :384, :432 | the status is one of `pending`, `approved`, `deprecated` or `superseded`, validated on write. The withholding is structural before it is conditional: `.agent_memory/packets` holds approved memory and `.agent_memory/pending` is the inbox, so `loadApprovedPackets` reads the packets directory *and* filters `status === \"approved\"` while `loadPendingPackets` reads the other directory entirely — a pending packet is not a row the recall path skips, it is a file the recall path never opens. The same equality guards the access-entry read and the supersession-candidate scan, and two paths refuse a deprecated or superseded packet outright, one of them returning the reason as a string. Capture routes to pending on either an explicit request or an ungrounded utterance, and an ungrounded capture is withheld regardless of status. The single denylist in the file is deliberately not a recall path: the stale-findings scan skips deprecated and superseded while keeping pending, and the comment says why with a count — flagging end-state packets as stale *dominated the stale list*, on the project's own tally 292 of the findings in the audit that prompted the change, and a pending packet is the one a person can still act on | mcp/kernel.test.ts"
...
**2026-09-19** — [`e7cc087666fd3d01a5727f8a67e7b9e745fca904`](https://github.com/kage-core/kage/commit/e7cc087666fd3d01a5727f8a67e7b9e745fca904) — `trust_state` re-tested at the unchanged pin. The record described the status as a field the read paths test, which is true and is the second half of the mechanism. The first half is that a pending packet lives somewhere else: `.agent_memory/packets` holds approved memory and `.agent_memory/pending` is the inbox, and `loadApprovedPackets` reads the packets directory then also filters `status === "approved"`, while `loadPendingPackets` reads the other directory outright. A pending packet is not a row recall skips — it is a file recall never opens, with the predicate as a second line over the partition. Two more sites are worth the record: a deprecated or superseded packet is refused by two paths rather than merely filtered, one of them returning the reason as a string a caller can print. And the one denylist in the file is on a surface where the opposite polarity is correct and justified with a measurement: the stale-findings scan skips deprecated and superseded but keeps pending, because flagging end-state packets as stale *"dominated the stale list"* — 292 of the findings in the audit that prompted the change — while a pending packet is exactly the thing a person can still act on. Nothing was installed and no suite was run.
