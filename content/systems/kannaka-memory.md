...
revision: 2b35b46d05ceb3b84bedce9d7a520618b204effe
revision_url: https://github.com/kannaka-labs/kannaka-memory/commit/2b35b46d05ceb3b84bedce9d7a520618b204effe
analyzed_at: 2026-09-19
capabilities: "trust_state"
capability_evidence:
  trust_state: "a stored hallucination flag that excludes a memory from consolidation while protecting it from dampening, which the single write chokepoint refuses to let the wire set or clear | src/absorb_gate.rs:1-16 (the module contract), :138, :147, :167-169 (the forcing), :222 (the ignored wire parameter), src/consolidation.rs:1192-1197 (bridge protection), :1651 (the deliberate writer), :1673 and :1812 (the weaving filters), src/medium/chiral_persistence.rs:32, :71, :113 | `HyperMemory` carries a `hallucinated` boolean persisted through every chiral-persistence layer, and the two weaving sites filter `!m.hallucinated && m.amplitude > 0.01`, so a flagged memory cannot become part of a consolidated belief. The flag is not only a defence marker: consolidation Stage 8 sets it deliberately on memories it synthesises by combining clusters (`:1651`), and destructive dampening skips them because they serve as cross-cluster integration bridges — so the flag marks material that bridges without becoming belief. The producer boundary is the rest of it. `absorb_gate` is the single write-side chokepoint every wire-to-store path routes through, its sanitizer returns `hallucinated: false` unconditionally under a SECURITY comment carrying the incident id, and the absorb signature takes `_wire_hallucinated` with a comment saying it is deliberately ignored — a parameter named for the thing it refuses to read | src/absorb_gate.rs:167-169, :222"
stack_storage: "files"
