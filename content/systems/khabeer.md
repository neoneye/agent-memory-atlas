...
capability_evidence:
  human_review: "the staged-write approval queue on the Memory page | app/src/main/java/com/termux/app/AiMemoryStore.java:291-346 (`stageWrite`, `approvePending`, `rejectPending`), AiRuntimeService.java:3638-3641 and :4184-4189, AiActivity.java:1732-1733 and :2020-2040 | with the `Require approval before memory writes` toggle on, both the agent's foreground `memory` call and the background review's operation batch are written to `pending/memory/<id>.json` instead of the files, and a person approves or rejects each one from an Approve/Reject pair on the Memory page; the page also carries editors for all three files that save through the same gates. The toggle defaults to off (`getBoolean(KEY_MEMORY_WRITE_APPROVAL, false)`), approval re-applies the write with both stores forced enabled regardless of the toggles, and the reviewer sees a 140-character one-line rendering of the arguments JSON. The producer test passes on the verb and not on the files: `AiMemoryStore.approvePending` has exactly one caller in the tree, `AiActivity.java:2027`, so the approve route is a button in the app's own Activity and no agent tool reaches it — but `dataRoot()` is `TERMUX_HOME_DIR_PATH/.khabeer`, the agent's own shell home, and the runtime registers a `terminal` tool that runs an arbitrary command through `mToolExecutor.runCommand`. The queue governs the `memory` tool; `MEMORY.md`, `USER.md`, `SOUL.md` and `pending/memory/` are ordinary files one redirect away, and the terminal's own approval is a separate setting with a `\"never\"` value that skips the prompt entirely | app/src/test/java/com/termux/app/AiMemoryStoreTest.java:163-192 (`stagedWritesCanBeApprovedOrRejected`)"
...
**2026-09-18** — [`cd7526219892d097646434e6802f105cc796d87b`](https://github.com/bakka22/khabeer/commit/cd7526219892d097646434e6802f105cc796d87b) — re-read at the same commit; `main` has not moved since the v0.118.0 release of 7 September 2026. The mark holds and the record gains the half it was missing. The approve verb passes the producer test cleanly: `AiMemoryStore.approvePending` has one caller in the whole tree, `AiActivity.java:2027`, so approval is a button in the app's own Activity and no agent tool reaches it. What the record did not say is where the files are. `dataRoot()` returns `TERMUX_HOME_DIR_PATH/.khabeer` — the agent's own shell home — and the runtime registers a `terminal` tool running arbitrary commands through `mToolExecutor.runCommand`, under an approval policy separate from the memory toggle and with a `"never"` value that skips the prompt. So the queue guards the `memory` tool while `MEMORY.md`, `USER.md`, `SOUL.md` and `pending/memory/` sit one shell redirect away. In a product that *is* a terminal emulator, that is worth stating plainly rather than filing under the corpus's usual general-shell caveat.
