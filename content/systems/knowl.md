...
revision: f2252db376fb42f76f5795b3e38d89ed9970a080
revision_url: https://github.com/dat999zx/knowl/commit/f2252db376fb42f76f5795b3e38d89ed9970a080
analyzed_at: 2026-09-17
archive_name: "dat999zx--knowl"
capabilities: "trust_state, bitemporal, audit_log, scope_enforced, negative_eval"
capability_evidence:
  trust_state: "a five-value status on the item, defaulted to `active` on every read that does not name one | src/store/schema.ts:7, src/store/queries.ts:192-197, :26, :56, src/store/tier.ts:56-66 | `status` is `active | deprecated | rejected | archived | superseded`, and the query builder's else branch is the whole mechanism: `By default query active unless status is specified`. So four of the five values withhold an item from an ordinary retrieval rather than ranking it down, and `rejected` and `superseded` are reachable states rather than decoration — `supersededById` points at the successor. It is kept apart from the three axes that would otherwise absorb it: `confidence` is a real, `freshness` is `fresh | stale | needs_review` for a separate drift check, and `tier` is `asserted | verified` standing earned by confirmed use, with `tierSince` so a reset `genuinely restarts the climb instead of inheriting the history that a correction or a rewording just invalidated` | the supersession ablation in benchmarks/memoryagentbench/results measures the difference the status makes: stale leaks fall from 62 to 2 at 6k and 28 to 5 at 262k"
