...
capability_evidence:
  trust_state: "a three-value field named for a score that nothing ever sorts by — it is verified, it gates automatic admission, and it withholds | mygraph/validator.py:6, :92-102, mygraph/review.py:14, :111, mygraph/export_context.py:55, mygraph/audit.py:58 | `confidence` is `high`, `medium` or `low`, and the validator makes the top value a checkable claim rather than a self-report: a node asserting `high` must substring-match its excerpt against the source markdown, and when it does not the validator writes `node[\"confidence\"] = \"low\"`. So `low` is the value a failed verification produces, not merely a weaker guess. Two consumers act on it and neither is a ranking. Review admission is keyed on it — `--auto-accept-high` accepts a node when the value is `high` and otherwise records `skip`, leaving it for a person — and the context export drops a decision whose value is `low` rather than ordering it lower. Nothing in the tree sorts by the field; the ideas section is ordered by incoming edge count instead, with the value rendered beside it as a marker. The name is the only thing about it that suggests a score | tests/test_deep_dive.py::test_validation_report_records_demotions_for_bad_high_confidence_excerpt"
  ...
  human_review: "ingest, interactive by default with three named opt-outs | mygraph/review.py:110-137, mygraph/ingest.py:97-104, :140-141, mygraph/validator.py:95-107 | each extracted node is printed and a person answers accept, reject, edit, skip or quit; an edit is re-validated against the source before it can be accepted, and a failed re-validation becomes a skip rather than a pass. The opt-outs are explicit CLI flags, never a default: `--auto-accept-high`, `--auto-accept-all`, and `--non-interactive`, which turns on auto-accept-high by itself under the comment *\"Default headless behavior: be conservative — accept only `high`\"*. What makes the first and third defensible is that the label they trust is not a self-report by the time review sees it: the extractor asks the model for a `confidence` of high, medium or low and instructs it to use `high` only with a direct quote, and then `validator.py` enforces the rule — a `high` node with no excerpt, or whose excerpt does not normalise to a substring of the source, is demoted to `low` with `no_excerpt` or `excerpt_not_in_source` recorded in the manifest. `--auto-accept-all` is the one route that bypasses that reasoning, since it takes `medium` and `low` as well | no test covers the review gate itself; `tests/test_benchmarks.py::test_b5_weak_claim_detection_feeds_review_queue` exercises the separate weak-claim audit queue"
stack_storage: ""
stack_retrieval: ""
stack_source: "seeded"
matrix:
...
**2026-09-19** — [`bbb46379be2a8c45b20627fba30ffb8c98d1adc1`](https://github.com/rahulmranga/knowledge-worker/commit/bbb46379be2a8c45b20627fba30ffb8c98d1adc1) — `trust_state` re-tested at the unchanged pin, and the first reading of the re-test nearly withdrew it. A three-value field called `confidence`, cited on one line, filtering one section of one export under a comment reading *"(high confidence only to keep it tight)"*, looks exactly like the ranking axis this mark is supposed to exclude. Following the field through the tree reverses that. Its producer is a check, not a guess: `validator.py` requires a node claiming `high` to substring-match its excerpt against the source markdown and writes `node["confidence"] = "low"` when it does not, so `low` is the value a failed verification assigns. Its consumers are both admission decisions. `review.py:111` accepts a node into the graph automatically only when the value is `high` and otherwise records `skip`, leaving it to a person; `export_context.py:55` drops a `low` decision from the exported context. And the thing a confidence score is for — ordering — never happens: nothing in `mygraph/` sorts by the field, the ideas section is ranked by incoming edge count, and the value appears beside a node as a rendered marker. The mark holds, and the record now says why the name is misleading rather than resting on the one line the previous record cited. Nothing was installed and no suite was run.

**2026-09-18** — [`bbb46379be2a8c45b20627fba30ffb8c98d1adc1`](https://github.com/rahulmranga/knowledge-worker/commit/bbb46379be2a8c45b20627fba30ffb8c98d1adc1) — re-read at the same commit; `main` has not moved since the 0.8.3 release of 4 September 2026. The mark holds and the record was missing both halves of the interesting part. It did not mention that review has three opt-outs — `--auto-accept-high`, `--auto-accept-all`, and `--non-interactive`, which enables auto-accept-high on its own — all explicit flags rather than defaults. And it did not say why two of those three are defensible: the `confidence` the auto path keys on is a field in the model's own output schema, but `validator.py:95-107` checks it before review ever runs, demoting a `high` node to `low` when it carries no excerpt or when the excerpt does not normalise to a substring of the source, with `no_excerpt` or `excerpt_not_in_source` recorded in the manifest. Provenance-or-bust is enforced, not prompted. `--auto-accept-all` is the single route that skips that reasoning. The test field said *unknown*; no test covers the review gate, and the benchmark case that mentions a review queue is about the weak-claim audit rather than ingest.
