...
revision: 11e869f5f7bfe1aca31deeb3dd7a54f831e43ebe
revision_url: https://github.com/JanYork/llm-wiki-cli/commit/11e869f5f7bfe1aca31deeb3dd7a54f831e43ebe
analyzed_at: 2026-09-19
capabilities: "trust_state, bitemporal, audit_log, negative_eval"
capability_evidence:
  trust_state: "every recall result carries a `latest_known` or `superseded` state beside an unresolved-conflict flag, and a superseded event is dropped from the result in favour of its replacement unless the caller passes an explicit flag | src/store/temporal_memory.rs:1035-1075 (`current_visible_memory_id`), :768-787 (the result fields), :859, :886 (the two walk sites), :897-913 (the conflict probe and the tagging) | `current_visible_memory_id` walks the `supersedes` edges in `memory_relations` from a lexical hit to the newest event that replaced it, under the same event-time window as the search itself. The recall loop then keeps the original hit only when the caller asked to include superseded events or when the walk found no replacement, and always keeps the successor, tagging it `matched_via: \"superseded_event\"`. Each result reports `state` as `latest_known` or `superseded`, beside `unresolved_conflict` — a SQL `EXISTS` over `memory_relations` for a `contradicts` edge with no matching `resolves` edge — and a `live_verified` field held at `false`, which the workflow documentation states as a standing disclaimer rather than a placeholder: the latest unsuperseded memory is not a claim about current code. The state is derived per call but not derived-only: the `supersedes` rows are durable and carry an index on `(target_event_id, relation_type, event_id)`, so replaced events are directly queryable rather than replayable, which is what separates this from the read-time ladders the corpus withholds the mark from | tests/temporal_memory.rs:814-901 asserts a one-result recall returning the replacement instead of the superseded hit, then the same query with the override returning three with the old event marked `superseded`"
...
matrix:
  ...
  trust: "Three separate vocabularies. The wiki has a four-value ordered provenance — `source-grounded`, `user-provided`, `agent-observed`, `hypothesis` — rejected at the boundary if unrecognised, decorated onto every search result, and filtered on nowhere. Memory events have `latest_known` versus `superseded`, resolved from the relation graph on every recall and used to drop the replaced event, beside an `unresolved_conflict` flag and a `live_verified` field pinned to `false`. And `memory_feedback` records a `useful` or `not-useful` signal with a mandatory reason, aggregated into an integer clamped to plus or minus three that adjusts rank by five per cent and never excludes anything"
...
is stamped `latest_known` or `superseded`, and a hit reached through the chain is
...
**Trust state — awarded.** `latest_known` versus `superseded`, resolved on every
...
- **Label the result with the state you filtered on.** `state: "latest_known"`
  plus `matched_via: "superseded_event"` tells a reader that a redirect happened.
  A filter that leaves no trace is indistinguishable from a retrieval miss.
- **Name the state for what you can actually check.** The same field used to read
  `"current"`, and the rename is the whole lesson: a memory store can establish
  that nothing has superseded a row, and cannot establish that the row is still
  true of the world. Beside it sit `unresolved_conflict`, computed per result
  from a `contradicts` edge with no `resolves` edge, and `live_verified`, held at
  `false` and documented as a standing disclaimer rather than a placeholder —
  *"it is not a claim about current code."* A constant `false` is usually a
  defect in this corpus; it is the honest value when the verification it names
  has not been built, and saying so in the payload beats leaving the reader to
...
**2026-09-19** — [`11e869f5f7bfe1aca31deeb3dd7a54f831e43ebe`](https://github.com/JanYork/llm-wiki-cli/commit/11e869f5f7bfe1aca31deeb3dd7a54f831e43ebe) — `trust_state` re-tested at an unchanged pin. Every anchor held: `current_visible_memory_id` at `src/store/temporal_memory.rs:1035`, the result fields — `live_verified`, `unresolved_conflict`, `matched_via` — at `:768-787`, and the recall loop's conflict probe and tagging at `:897-913`. The one addition is a second call site for the supersession walk at `:859` beside the one at `:886`, so the record now names both. The point the record already makes is the one this sweep keeps confirming from the other direction: `live_verified` is held at `false` as a standing disclaimer rather than a placeholder — the latest unsuperseded memory is not a claim about current code — and the state that decides what comes back is the durable `supersedes` edge, not that field. A system that reports a trust field it refuses to compute, and filters on something it can actually check, is making the same distinction the mark turns on. Re-read from a fresh clone; nothing was installed and no suite was run.

**2026-09-16** — [`11e869f5f7bfe1aca31deeb3dd7a54f831e43ebe`](https://github.com/JanYork/llm-wiki-cli/commit/11e869f5f7bfe1aca31deeb3dd7a54f831e43ebe) — re-read after 17 commits, through v0.18.5. Three of the seven anchored files are byte-identical, including `src/sync.rs` and `src/store/checkpoints.rs` behind the audit mark. All four marks hold. The change worth recording is in the recall result and is a rename plus two fields: the state formerly reported as `current` is now `latest_known`, and every result carries `unresolved_conflict` — a SQL `EXISTS` for a `contradicts` edge with no matching `resolves` edge — beside `live_verified`, pinned to `false` and documented in `docs/agent-workflow.md` as a standing disclaimer rather than an unwired field. The report's trust_state evidence, its mental-model paragraph, its section 9 and its build advice are updated to the new vocabulary, and the anchors in `temporal_memory.rs` moved with the file. `parse_memory_capsule` also moved from a bare `serde_json::from_str` onto a shared contract parser that returns structured error details. Re-screened at this commit: no auto-run surface, two build-time execution paths, one floating version, three manifests inside the cooldown. Nothing was installed, built or run.
