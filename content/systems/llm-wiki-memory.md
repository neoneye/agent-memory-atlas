...
capability_evidence:
  ...
  human_review: "the self-improvement write gate, whose deterministic layer reads the real transcript | scripts/hooks/pretooluse-gate-memory-writes.mjs:150-180, src/webapp/shared/contract.mjs:189, src/webapp/server/edit.mjs:40, docs/write-gate.md | a `self_improvement` lesson is propose-then-confirm, and the layer that makes it a gate rather than an instruction is the Claude Code `PreToolUse` hook: it opens `transcript_path`, tests the last user turn against a save-phrase pattern, returns `allow` on a match and `ask` otherwise — a one-click prompt — and enforces per-lesson consent by counting how many gated writes have already consumed this turn's approval, so a batch flush cannot ride one yes. The model cannot forge that: the hook reads the conversation off disk rather than taking the model's word. Two limits belong with the mark and the project documents both in its own table. The hook is Claude Code only and is switched by `gate.claudeHookEnabled`; on every other MCP client the remaining deterministic layer is the server's check that `gate.userRequested` is `true`, and that is a `z.boolean().optional()` in the request schema which the *model* sets — so `docs/write-gate.md`'s claim that the model *\"can't forge `userRequested`\"* is the one sentence in that document the code does not support. The Lexical editor the earlier record cited is a person editing stored rows, which is curation rather than review | src/webapp/server/edit.test.mjs, test/hardening-pretooluse-gate-hook.test.mjs"
stack_storage: "files"
...
**2026-09-18** — [`4e7f98f3851ca51a8159852aff57a2512794a877`](https://github.com/ctxr-dev/llm-wiki-memory/commit/4e7f98f3851ca51a8159852aff57a2512794a877) — re-read at the same commit; `main` has not moved. The mark holds and its record is re-anchored, because it was pointing at the wrong mechanism. The Lexical editor is a person editing rows that are already stored — curation, which the [narrowed rubric](../../methodology/atlas-rubric/#human-review-surface) no longer counts. What does count is the self-improvement write gate, and specifically its `PreToolUse` hook: it opens `transcript_path`, tests the last user turn against a save-phrase pattern, returns `allow` on a match and `ask` otherwise, and counts prior gated writes in the turn so a batch cannot ride one approval. Reading the conversation off disk is what the model cannot forge.

Two limits are now stated with it, both of which the project's own layer table is honest about. The hook is Claude Code only and switchable through `gate.claudeHookEnabled`; on Cursor, Codex or a generic client the remaining deterministic layer is the server checking `gate.userRequested === true`, and `contract.mjs:189` declares that as `z.boolean().optional()` on the request — a field the model fills in. `docs/write-gate.md` calls the server layer *"the airtight bottom layer for hook-less clients"* and says the model *"can't forge `userRequested`"*; the first is true against a missing flag and the second is the one sentence in that document the code does not support.
