...
revision: 462df8b7843f71fad0789b637e02d74370afaffb
revision_url: https://github.com/lobu-ai/lobu/commit/462df8b7843f71fad0789b637e02d74370afaffb
...
capability_evidence:
  ...
  scope_enforced: "every content recall seam — search_memory, read_knowledge, query_sql and the SDK list path | packages/server/src/authz/scope.ts:10-24, packages/server/src/authz/connection-visibility.ts:35-52, packages/server/src/authz/resource-visibility.ts:61-198, packages/server/src/utils/content-search/visibility.ts:99-176, packages/server/src/utils/content-search/search-path.ts:429-460 | one `AuthzScope { organizationId, principal, agentId }` compiles three ANDed SQL fragments onto a query that reads `current_event_records`: `buildOrgScopeWhere` binds the tenant, `compileConnectionFkVisibility` admits a connection-sourced row only when `vc.visibility = 'org' OR vc.created_by = $principal`, and `compileResourceVisibility` requires that the principal's `$member` entity hold a live `member_of` edge to *every* `$resource` entity in the row's `entity_ids` — a `NOT EXISTS (… WHERE NOT satisfied)` over the linked set, so a summary drawn from two repositories is readable only by a member of both — with each edge backed either by a `manual` ownership claim or by a connection whose ACL graph is `acl_support='full'` and `freshness_state='fresh'` within `ACL_STALE_AFTER_MINUTES`. A row that links no resource takes the legacy path only when its connection was never graphed; on a graphed connection an unstamped row fails closed, and a stamped row on a stale or ungraphed authority is dropped rather than served | packages/server/src/__tests__/integration/authz/github-repo-visibility.test.ts:175-189, :311-319"
  audit_log: "tool-invocation audit events in the same append-only log the memories live in | packages/server/src/tools/audit.ts:191-199, :229-260, packages/server/src/tools/execute.ts:409-433, packages/server/src/tools/constants.ts:17 | `executeTool` calls `recordToolInvocationAudit` on both the success and the throw path of every dispatched tool, and it appends an `events` row with `semantic_type = 'audit'`, `origin_type = 'tool_invocation'`, a SHA-256 of the sanitised arguments, a credential-redacted preview, the duration, the outcome and the acting `agent_id`. That covers the memory mutations — `save_memory`, `client.knowledge.delete` and the `run_sdk` script that wraps `manage_entity` — for every OAuth or PAT caller, which is every MCP agent. The gap is stated in the code: a generic tool called on a browser session token returns `null` and writes nothing | packages/server/src/__tests__/integration/mcp/tool-invocation-audit.test.ts:106-127, which asserts the row exists with its hash and that the probe string was replaced by the redaction sentinel"
  human_review: "the entity-mutation gate over the typed entity graph | packages/server/src/authz/entity-mutation-gate.ts:1-38, packages/server/src/authz/approval-interceptor.ts:1-19, packages/server/src/utils/entity-field-merge.ts:17-24, :77-130, packages/server/src/tools/admin/manage_operations/handlers/approvals.ts:940-977, :1455, :1811, :2022, :2113, packages/server/src/tools/admin/entity-field-approval.ts:1137-1210 | an agent or automation write to an entity runs `runMutationGate`; the approval interceptor turns a denied or escalated field into a `DeferredMutation` queued post-commit as an approval run instead of a write. `entities.field_controls` marks a field human-owned, and `computeFieldMerge` refuses an automation's write to an owned field, returning it in `blocked` for the card. `requireHumanApprovalContext` rejects any context carrying `agentId`, `clientId` or `mcpSessionId` and requires a `userId`, so no agent can resolve its own proposal, and `applyEntityFieldChangeProposal` skips a field whose live value drifted since the card was built | packages/server/src/__tests__/integration/authz/entity-approval-atomic-apply.test.ts:188-238, :281-328"
  negative_eval: "search_memory content recall under the generic resource gate | packages/server/src/__tests__/integration/authz/github-repo-visibility.test.ts:175-189, :311-319 | two events are linked to two repo entities and the ACL graph is built; a member who collaborates on repo A asserts `ids.has(eventAId)` true and `ids.has(eventBId)` false in the same result, and a requester with no `$member` entity asserts both false against the same populated fixture. The positive control is inside the first case rather than beside it: the same result that must not contain event B must contain event A, so a predicate that hid everything fails that assertion rather than passing quietly. The no-graph case now asserts the opposite of what it once did — dropping the ACL state hides *both* events, because stamped content never falls back to the legacy fence. Neither assertion can pass over an empty result set, and the suite runs under a real Postgres in CI behind a guard that fails the job when zero tests ran | the same file, plus scripts/assert-vitest-report-clean.mjs"
...
**The resource gate now quantifies over the whole linked set, and covers what the server itself wrote.** The predicate used to admit a row when the caller was `member_of` *one* of the `$resource` entities the event linked to. It now compiles `NOT EXISTS (SELECT 1 FROM (linked resources) lr WHERE NOT (satisfied))` — universal, not existential — with the consequence stated in the header: *"A derived summary of two repos is readable only by a member of both — membership in one is not enough."* That is the right shape for a memory that is a *derivation*: an any-of gate over a summary hands the whole summary to someone entitled to half its inputs, and there is no way to redact the half they are not. The gate also stopped being about connectors. It is now connection-independent, so a server-authored `save_content` summary carrying resource stamps and a `NULL` connection is gated by those stamps exactly as a synced GitHub row is — which closes the seam where the system's own inferences would otherwise have escaped the fence its sources sit behind.

The fail-closed edges moved the same direction. An unattributed row on a graphed connection no longer takes the legacy path — *"a missing stamp is not proof of workspace visibility"* — and every `member_of` edge must now carry an ownership claim, either `manual` or a connection whose ACL graph is currently enforced, with `db/migrations/20260827174500_index_live_relationship_claims.sql` tombstoning the legacy claimless edges so the *claim is absent* case cannot arise from history. The one arm that is deliberately freshness-independent is the approval card: an interaction event is readable by any organization member in every graph state, because hiding an approval when an ACL sync has stalled wedges the operation the approval gates. The header records the production incident that established it, and connector sync never writes `interaction_type`, so no synced content can ride that exemption.

The gate's composition requirement is now written down as a warning rather than left to be inferred: soft-deleted connections are not fenced by this compiler, so it must always be composed behind `compileConnectionFkVisibility` — *"never use this compiler standalone."*
...
**2026-09-19** — re-pinned to [`462df8b7843f71fad0789b637e02d74370afaffb`](https://github.com/lobu-ai/lobu/commit/462df8b7843f71fad0789b637e02d74370afaffb), 12 commits on across 133 files, of which two touch `authz` and neither is a file any mark rests on. All five marks stand with their anchors re-verified. `human_review`'s producer test was re-read rather than carried: `requireHumanApprovalContext` is still at `approvals.ts:940`, still refuses a context carrying `agentId`, `clientId` or `mcpSessionId`, still requires a `userId`, and is still the first thing all four of the approve and reject handlers call (`:1455`, `:1811`, `:2022`, `:2113`). In a sweep that has withdrawn this mark from two dozen reports for an approve verb sitting on the producer's own surface, this is the shape that forecloses it by refusing the caller rather than by hiding the verb. Screened again first; nothing was installed and no suite was run.

**2026-09-16** — [`54e49072e4b2937f2ca0a6599269bdd3b959e7f9`](https://github.com/lobu-ai/lobu/commit/54e49072e4b2937f2ca0a6599269bdd3b959e7f9) — re-read after 102 commits. Twelve of the seventeen anchored files were byte-identical at both commits and needed no re-reading — the baseline schema, the `save_memory` contract, the write path, the search parameters, the scope compiler, the connection-visibility compiler, the visibility assembly, the tool dispatcher and its constants, the mutation gate, the approval interceptor and the field-merge rule. The five that moved were re-read in full. All five marks hold, and the resource gate is materially stronger; that change is written up in section 9. Three corrections follow from it. The `compileResourceVisibility` evidence described an any-of membership test, which is now an all-of one, and described the gate as covering connector-sourced events, which it no longer is. And the `negative_eval` record cited the wrong positive control: the no-graph case it named as proving the predicate does not hide everything now asserts the opposite — stamped content fails closed without a fresh authority — so the control is the `toBe(true)` sitting beside the `toBe(false)` in the enforced case, which is the stronger form anyway because both run on the same result. Line anchors in sections 6 and 9 moved with their files. Re-screened at this commit: five auto-run surfaces, two build-time execution paths, 34 floating versions, twelve manifests inside the cooldown. Nothing was installed, built or run, and the `packages/owletto` submodule was again left uninitialised.
