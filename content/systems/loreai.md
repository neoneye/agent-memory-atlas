...
revision: 620b824820ff76f2f6237d244ce40d5dc03654c1
revision_url: https://github.com/BYK/loreai/commit/620b824820ff76f2f6237d244ce40d5dc03654c1
...
capability_evidence:
  ...
  human_review: "a person approves an entry into team scope and picks the survivor of a contradiction, and neither is done for them | packages/core/src/ltm.ts:627, :688-700, packages/core/src/sync-data.ts:1742, :1761, packages/gateway/src/cli/team-cmd.ts:523-524, packages/core/src/contradiction.ts:1-24 | under a `manual` promotion policy a new entry in a team-bound project lands `pending` and is shared to the team only once someone sets `approved` with `approved_by`; `listPendingTeamPromotions` is the queue, and the approval is copied forward by `appendVersion` so a later content edit does not silently re-share it; the contradiction worker is documented as \"Detection ONLY — never merges, never deletes. The user picks the survivor (or keeps both) on the dashboard\". The withholding is real on the read side — the two team-scope sync queries require `k.approval_status = 'approved'` — and the producer cannot supply the approval: `approveForTeam` has exactly one caller, a CLI command that passes `(await getCurrentUser())?.user_id` rather than an argument, and the three agent-facing packages (`hermes`, `opencode`, `pi`) contain no reference to `approveForTeam` or `approval_status` at all | packages/core/test/scope-selection.test.ts:175 'approve/reject transitions + listPendingTeamPromotions'; :209 'approval survives a content edit'"
...
**2026-09-19** — re-pinned to [`620b824820ff76f2f6237d244ce40d5dc03654c1`](https://github.com/BYK/loreai/commit/620b824820ff76f2f6237d244ce40d5dc03654c1). All five marks stand. `human_review`'s anchors were re-resolved — `approveForTeam` is at `ltm.ts:688` and the pending-queue predicate at `:627` — and the record gained the producer test it had been asserted without, which matters because this sweep has withdrawn the mark from two dozen reports whose approve verb turned out to sit on the agent's own surface. Here it does not, on two independent grounds: `approveForTeam` has exactly one caller, `team-cmd.ts:524`, which passes `(await getCurrentUser())?.user_id` rather than an argument the caller names; and the three agent-facing packages carry no reference to `approveForTeam` or `approval_status` anywhere. The withholding was checked on the read side too — both team-scope sync queries require `k.approval_status = 'approved'` (`sync-data.ts:1742`, `:1761`) — so a pending entry really is absent from team scope rather than merely labelled. Screened again first; nothing was installed and no suite was run.
