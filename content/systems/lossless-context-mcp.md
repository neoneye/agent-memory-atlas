...
capability_evidence:
  audit_log: "the archive — a per-writer append-only event log beside a content-addressed blob store | src/archive.ts | every read, edit and write is appended as an `ArchiveEvent` carrying timestamp, session, path, repo, content SHA-256, git blob sha1, byte and token counts, source and op; each process appends only to its own `events/<writer>.jsonl`, named `${process.pid}-${Date.now().toString(36)}`, so the log is multi-process safe with no locking — the sequence problem is dissolved rather than guarded — and a denied path still gets its event with `excluded: true`. Two horizons belong with the mark, both enforced by `enforceBudget` at `src/archive.ts:197-237`, which `index.ts:71` runs on every server start. Blobs are evicted oldest-mtime-first until the store fits `LOSSLESS_ARCHIVE_BYTES`, 512 MB by default, so an event can outlive the content it names and leave a hash pointing at nothing; and a whole writer's `.jsonl` is unlinked once its mtime passes `LOSSLESS_EVENTS_DAYS`, 30 by default, with only the current writer's file exempt. The two run on different clocks — one on size, one on age — so which of the pair survives the other is not fixed | test/archive.test.ts"
...
**2026-09-18** — [`1ef5bf9e1168b3717a4e08e2e0424721f652da51`](https://github.com/NORTHTEKDevs/lossless-context-mcp/commit/1ef5bf9e1168b3717a4e08e2e0424721f652da51) — re-read at the same commit; `main` has not moved since the 2.0.2 notes of 4 September 2026. Both marks hold and the exclusion test was re-verified in code rather than from its test name: `isExcluded` matches the literal path against the deny patterns, then resolves with `realpathSync.native` and matches again, so an innocuously named symlink pointing at a secret is denied by its target — and the `catch {}` around the resolution carries its own justification, that a path which cannot be resolved cannot be read either. The `audit_log` record gains the two horizons it was missing. `enforceBudget`, run from `index.ts:71` on every server start, evicts blobs oldest-first until the store fits `LOSSLESS_ARCHIVE_BYTES` (512 MB default) and unlinks any writer log older than `LOSSLESS_EVENTS_DAYS` (30 default) except the live one. So an event can outlive the blob it names, a whole writer's log can vanish wholesale, and because one horizon is a size and the other an age, neither reliably outlasts the other. Worth reading beside [Jumbo](../jumbo/), whose event store takes its sequence from a directory listing and writes with `overwrite: true`: one writer file per process is how that race is avoided rather than locked.
