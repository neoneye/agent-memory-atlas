...
revision: 8805036a353f96b41fc0f02de9e8d24b0a851bdf
revision_url: https://github.com/cortexkit/magic-context/commit/8805036a353f96b41fc0f02de9e8d24b0a851bdf
analyzed_at: 2026-09-19
capabilities: "trust_state, scope_enforced, audit_log"
capability_evidence:
  trust_state: "the memory row — a lifecycle status the reads filter on, beside a verification status that is measured against the files a memory describes and gates nothing | packages/plugin/src/features/magic-context/storage-db.ts:1145 (the column), packages/plugin/src/features/magic-context/project-embedding-registry.ts:2551, packages/plugin/src/features/magic-context/storage-primers.ts:419, packages/plugin/src/features/magic-context/search.ts:1581, packages/plugin/src/features/magic-context/context-authority.ts:1383, :1419, :1889-1891 | a memory carries `verification_status` and `verified_at` alongside `status`, so how it is doing and whether it has been checked are two fields rather than one, and the mark rests on the first: ten reads of the memories table narrow with `status = active`. `verification_status` is the more interesting field and does not filter — it appears in no WHERE clause anywhere in the tree, only in the column list, the projection and the update. What makes it worth reading is that it is empirical: `verified_at=0` means mapped, with the backing files known but the content not yet checked, and a memory re-enters verification when git reports one of its mapped files changed since that memory own last verification — a state a measurement writes rather than a judgement about the claim, surfaced to a reader and left out of the decision | the verification tests in packages/plugin"
...
**2026-09-19** — [`8805036a353f96b41fc0f02de9e8d24b0a851bdf`](https://github.com/cortexkit/magic-context/commit/8805036a353f96b41fc0f02de9e8d24b0a851bdf) — `trust_state` re-tested against the narrowed line, and the record rested on the wrong one of the two fields. `verification_status` appears in no `WHERE` clause anywhere in the tree — only in the column definition (`storage-db.ts:1145`), the projection and the update (`context-authority.ts:1383`, `:1419`, `:1889-1891`). What filters is `status`, at ten reads of the memories table, among them the embedding backfill (`project-embedding-registry.ts:2551`), the primer read (`storage-primers.ts:419`) and the full-text search (`search.ts:1581`). The mark holds on the lifecycle field. That leaves `verification_status` as the field worth describing rather than the field the mark rests on, and it is worth describing: `verified_at=0` means mapped — the backing files are known and the content is not yet checked — and a memory re-enters verification when git reports one of its mapped files changed since that memory's own last verification. It is a state a measurement writes, not a judgement about the claim, surfaced to a reader and deliberately outside the decision. This sweep found the same separation in flair, where the non-gating field was the only candidate and the mark went; here the lifecycle field does gate, so it stays. Screened again first; nothing was installed and no suite was run.

**2026-09-16** — [`8805036a353f96b41fc0f02de9e8d24b0a851bdf`](https://github.com/cortexkit/magic-context/commit/8805036a353f96b41fc0f02de9e8d24b0a851bdf) — re-read after 111 commits. Every file behind a mark is byte-identical at both commits: the status ladder's store, the memory storage and its types, the visibility predicate, the mutation log and `migrations.ts`. All three marks hold on unchanged code. Of the 24 files that moved inside the feature directory, the memory-bearing ones did not; the churn is in the dreamer's task scheduling and classification, the compaction marker, and `context-authority.ts`, which grew 69 lines of prepared statements — the one cited line moved from 1308 to 1374 and its column list is identical. Re-screened at this commit. Nothing was installed, built or run.
