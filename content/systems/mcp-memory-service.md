...
revision: 742f50b19abc3cb89fd0f142a4642485c67f4672
revision_url: https://github.com/doobidoo/mcp-memory-service/commit/742f50b19abc3cb89fd0f142a4642485c67f4672
...
capability_evidence:
  ...
  trust_state: "a four-value belief status, applied as an indexed SQL equality filter at every call site, with confidence kept beside it as a separate floor and ordering | src/mcp_memory_service/consolidation/belief.py:92-99, src/mcp_memory_service/consolidation/belief_service.py:160-180, :196-236, :245-257, src/mcp_memory_service/consolidation/quarantine.py:51-79, src/mcp_memory_service/server_impl.py:2509-2515, src/mcp_memory_service/server/handlers/memory.py:260-276, :966-975 | the `beliefs` table carries `belief_hash`, `content`, `confidence`, `status`, `derived_from` and `contradicted_by`, with `idx_beliefs_status` over the column — an index is what a filtered column gets, not a displayed one. `status` is `candidate`, `active`, `superseded` and `disputed`, and every transition runs through one setter: `should_promote` requires the confidence to clear a floor *and* the supporting count to clear a provenance floor before a candidate becomes active, `should_supersede` demotes an active belief once confidence falls back under the floor, and `challenge_belief` marks a belief disputed and immediately re-derives it, so `disputed` is a marker held during re-derivation rather than a resting state — it survives only if the derivation raises between the two writes. `get_beliefs` selects `WHERE status = ? AND confidence >= ? ORDER BY confidence DESC` with `active` as the default, which is the split itself: the state is the equality, the number is the floor and the sort. Four consumers read it and all four carry the predicate, including the session bootstrap that assembles starting context. What differs between them is the number, not the state: the bootstrap asks for `min_confidence=0.6` to present a belief, while the on-store quarantine gate asks for `0.35` to act on one, so the acting set is deliberately wider than the presented set | tests/test_belief_store.py:193-194 asserts `should_promote(0.9, PROVENANCE_FLOOR - 1) is False` — a confidence of 0.9 with one supporter does not promote, so the number alone cannot buy the state"
...
matrix:
  ...
  trust: "Two layers with different answers. A belief carries a typed, indexed `status` of `candidate`, `active`, `superseded` or `disputed` in its own table, promoted only when confidence clears a floor and the supporting count clears a provenance floor, demoted when confidence falls back, and read with `WHERE status = ? AND confidence >= ?` — so a candidate, superseded or disputed belief is excluded. A memory carries no status: the contradiction detector quarantines one that disagrees with an active belief by writing a metadata flag and a tag, with a threshold defaulting to three, and no retrieval path consults either, so a quarantined memory continues to be returned by search"
...
`candidate` → `active` → `superseded` status with a fourth value, `disputed`,
used while a challenged belief is re-derived; promotion requires both a
...
`candidate`, `active`, `superseded` or `disputed`, stored in a typed and indexed
column, moved by rules that require a confidence floor *and* a provenance floor
to promote and a fall below the floor to demote, and filtered in SQL with
`active` as the default. The confidence rides alongside as an ordering and a
floor. That is the mark exactly — and the clearest demonstration is a boundary
test: `should_promote(0.9, PROVENANCE_FLOOR - 1)` is `False`, so a belief the
system is 90% sure of still cannot enter the acting set on one witness. The
number answers how sure; the state answers whether it may be used.

**The two floors are different numbers on the same predicate.** Every consumer
filters on `status = "active"` — including the session bootstrap that assembles
starting context, which is the read most likely to be written separately from
the search path and so to be written without it. What varies here is the
confidence: the bootstrap asks for `0.6` before it will *show* a belief, while
the on-store quarantine gate asks for `0.35` before it will *act* on one. So the
set a belief must join to suppress an incoming memory is wider than the set it
must join to be printed as context, and the difference is expressed in the
number while the state stays constant. Quarantine then closes the loop: a belief
that quarantines three memories is itself challenged and re-derived, so the
state that decides what may be acted on is revised by what it acted against.
...
**2026-09-19** — [`742f50b19abc3cb89fd0f142a4642485c67f4672`](https://github.com/doobidoo/mcp-memory-service/commit/742f50b19abc3cb89fd0f142a4642485c67f4672) — `trust_state` re-tested, and the whole `consolidation/` subtree is byte-identical to the previous pin, so every correction below is the atlas's own. Three of them. The vocabulary is four values, not three: `disputed` is written by `challenge_belief` (`belief_service.py:205`) and was present at the first reading too — though it is a marker rather than a resting state, since the same call re-derives the belief and overwrites it fourteen lines later, and it survives only if the derivation raises in between. There are four consumers, not two, and the one the record missed is the strongest: `quarantine.check_beliefs_on_store` reads only `status="active"` beliefs before letting any of them suppress an incoming memory, and a belief that quarantines three memories is itself challenged — the state that decides what may be acted on is revised by what it acted against. And the two call sites differ in the number while agreeing on the state: the session bootstrap wants `min_confidence=0.6` to *show* a belief, the quarantine gate `0.35` to *act* on one, so the acting set is wider than the presented set. The split the mark tests for is also asserted directly — `tests/test_belief_store.py:193-194` fixes `should_promote(0.9, PROVENANCE_FLOOR - 1)` at `False`, so a 90%-confident belief with one witness cannot enter the acting set. The report's spine is untouched: the memory row's quarantine flag is still read by no retrieval path. Screened again first — the surface is unchanged at seven build-time execution paths, four auto-run surfaces, eight floating dependency ranges and two auto-run agent-harness hook scripts; the FRESH count rises from 4 to 12 only because this screen ran on a `--depth 1` clone, which dates every file to the tip, so that one number is not comparable with the previous reading. Nothing was installed and no suite was run.
