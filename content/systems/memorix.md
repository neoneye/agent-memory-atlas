...
revision: f7ed62180137bfc7c1dfdece1d0cd299c12c9bb1
revision_url: https://github.com/AVIDS2/memorix/commit/f7ed62180137bfc7c1dfdece1d0cd299c12c9bb1
...
capability_evidence:
  ...
  human_review: "a person advances a memory from candidate to qualified to approved from the CLI, giving a reason, and nothing reaches a task until they do | src/cli/commands/memory.ts:572-582, src/memory/long-term.ts:123, :429, :433, src/server.ts:1858, :1999-2022 | `qualifyLongTermMemory` and `approveLongTermMemory` are reachable only through the `memorix memory` CLI, each requires a `reason` that is stored on the transition event, and the store refuses to advance a record that carries no evidence rows; a freshly created candidate returns zero results from `selectLongTermMemoriesForTask` until the person acts, because the eligibility predicate at `long-term.ts:123` admits only a state of `qualified` or `approved`. The mark covers that path and not the other one in this tree: `claim_review` is an action on the knowledge MCP tool and takes `claimReviewState: z.enum(['approved', 'rejected'])` as a caller parameter (`src/server.ts:1858`), writing it straight to the claim's `reviewState` with no actor check, after which the reply says the approved claim *\"can now be considered by knowledge compilation\"* | tests/memory/long-term.test.ts:44"
...
**2026-09-19** — re-pinned to [`f7ed62180137bfc7c1dfdece1d0cd299c12c9bb1`](https://github.com/AVIDS2/memorix/commit/f7ed62180137bfc7c1dfdece1d0cd299c12c9bb1). All four marks stand, and `human_review` is now scoped rather than asserted whole. The long-term path holds and its withholding was verified rather than assumed: `qualifyLongTermMemory` and `approveLongTermMemory` are reached only from `src/cli/commands/memory.ts`, and the eligibility predicate at `long-term.ts:123` admits only a state of `qualified` or `approved`, so a candidate is genuinely absent from task selection. What the previous record did not find is the second review path in the same tree. The knowledge MCP tool's `claim_review` action takes `claimReviewState: z.enum(['approved', 'rejected'])` as a caller parameter and writes it to the claim's `reviewState` with nothing checking who asked, and the handler then reports that the approved claim *"can now be considered by knowledge compilation"*. That is now the stated limit on the mark. Screened again first; nothing was installed and no suite was run.
