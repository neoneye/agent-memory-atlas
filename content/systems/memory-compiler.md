...
capability_evidence:
  tombstone: "TOMBSTONES.md as an add-only table of rejected values, scanned on the close | memory_check.py:285-312, :735-738, examples/TOMBSTONES.md | each row records the fact key, the rejected value, a pointer to the replacement, the date and the reason — the rejected value itself, not a row id, is what the guard keys on. `tombstone_collision_check()` parses the table, collects the rejected values, and searches every other canonical file for each one verbatim; a hit becomes an `ask` finding, and `cmd_close` returns 1 with the message that the ledger entry is left OPEN, so a reasserted rejected value blocks the session from sealing rather than being logged | the scan skips any value shorter than twelve characters — `if len(val) >= 12:  # ignore short/generic strings, too noisy` — and both rejected values in the project's own worked example are ten characters (`2026-09-01`, `hex 1E3A5F`), so neither would be caught by it. The match is a verbatim substring, so a reformatted or reworded reassertion passes"
  human_review: "the close gate and the recovery-candidate resolution, both run by a person with a named resolver | memory_check.py:697-760, :778, :825 | nothing is extracted automatically — every canonical row is a human or agent edit to Markdown — and `--close` is the adjudication point: validation, the tombstone collision scan and `memory_tests.yaml` all run, and any `ask` finding returns 1 and leaves the ledger entry OPEN. Recovery-sourced facts arrive tagged `candidate` and unresolved candidates are themselves an `ask` that blocks the close; clearing one requires `--resolve-recovery <id>` with `--resolved-by`, so the decision carries a name | the approver is whoever runs the command, and nothing authenticates the `--resolved-by` string; the gate is on sealing a session rather than on an individual memory entering the store, and an agent wired to the same hook can run every one of these commands"
  negative_eval: "must_not_return as a first-class assertion type in a committed test file, enforced on the close | examples/memory_tests.yaml:1-26, memory_check.py:751-754 | the file documents the contract in its own header — 'Negative assertions (must_not_return): a tombstoned value that must NEVER reappear verbatim' — and ships three cases: one `must_reference` asserting the current go-live date is still findable in CONTEXT.md, and two `must_not_return` cases asserting the superseded date and the superseded palette value are absent from the same file. Positive control and refusals in one file over one corpus, and `run_tests()` is called by `--close`, where any `ask` returns 1 | the two negative values are exactly the ones the collision scan's twelve-character floor excludes, so the per-value assertions are doing work the generic scan cannot — which means the guarantee holds only for values somebody remembered to write a case for. Assertions are verbatim substring checks against one named file"
stack_storage: "files"
stack_retrieval: ""
...
**2026-09-18** — [`e79ee179a70688b349fc4f1bd957aafc54d0c224`](https://github.com/KTVSUN/memory-compiler/commit/e79ee179a70688b349fc4f1bd957aafc54d0c224) — re-read at the same commit. All three marks were re-derived and now carry evidence records, and the headline was verified down to the character count: `memory_check.py:297` is `if len(val) >= 12:  # ignore short/generic strings, too noisy`, and the two rejected values in `examples/TOMBSTONES.md` are `2026-09-01` and `hex 1E3A5F` — ten characters each.

The blocking behaviour is real: `cmd_close` returns 1 on any `ask` finding and prints that the ledger entry is left OPEN, so a reasserted rejected value stops the session sealing rather than being noted.

The relationship between the two guards is what this reading adds. `examples/memory_tests.yaml` makes `must_not_return` a first-class assertion type — its header states the contract, *"a tombstoned value that must NEVER reappear verbatim"* — and its two negative cases name exactly the values the collision scan's floor excludes, `2026-09-01` and `1E3A5F`, beside a `must_reference` positive control. So the per-value assertions cover precisely what the generic scan cannot, and the suite runs on the same close. The cost is that the guarantee then holds only for values somebody remembered to write a case for, which is now the `negative_eval` record's limit.

Worth one more line because it is rare: the close prints `[ok] audit: not implemented in this reference build — no audit trail was written`. A build that states its own missing subsystem in its own output is easier to trust than one that leaves the reader to find out.
