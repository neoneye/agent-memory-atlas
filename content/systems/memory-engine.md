...
capability_evidence:
  bitemporal: "a validity range stored per memory as a constrained tstzrange and queried through six independent predicates, separate from the row's own created and updated stamps | packages/database/space/migrate/incremental/001_memory.sql:8, :36-48, packages/database/space/migrate/idempotent/001_memory.sql:139-148, packages/database/space/migrate/idempotent/002_search.sql:9-27 | `memory.temporal` is a `tstzrange` describing when the memory is true of the world, and `created_at`/`updated_at` describe the row. The `temporal_bounds_convention` CHECK is what makes it constrained rather than conventional: the range must be null, or a point in time with both bounds equal and inclusive, or a period with `lower < upper` inclusive-exclusive — so a half-specified or inverted window is refused by the database rather than stored and mis-queried. `search_memory` and `hybrid_search_memory` each take `_temporal_within`, `_temporal_overlaps`, `_temporal_before`, `_temporal_after` and `_temporal_contains` beside the range itself, so the validity axis is a filter with five shapes rather than a column that renders. The range is carried onto every `memory_event` row as well, so the interval a memory claimed at each version is recoverable | packages/database/space/migrate/migrate.integration.test.ts"
  ...
  negative_eval: "two committed integration cases that seed distractors and pin the returned set by exact equality, each with the passing rows asserted in the same result | packages/engine/space/db.integration.test.ts:330-351, :378-405 | `minSimilarity drops below-threshold vectors, preserving rank order` seeds three memories with hand-set embeddings at cosine 1, ~0.71 and 0 to the query, searches at a 0.5 floor, and asserts the far one absent while the other two are asserted present and in order — an empty result fails on the first `toContain` before the absence check is reached. `bm25 search returns only genuine matches, never zero-score non-matches` is the stronger of the pair: two documents carry a term unique to the test, eight distractors do not, `limit` is ten so the page has room for all of them, and the assertion is `expect(ids).toEqual([a, b].sort())` plus every score above zero. Its comment records the regression it closes — the previous `ORDER BY <@> LIMIT` backfilled the page with zero-score non-matches on both the index path and a sequential scan — so the case exists because the failure happened | run against a real Postgres, beside 139 test files across the workspace"
...
- **Retrieval exclusions asserted against a populated page**, with the
  regression each one closes written into its comment.
...
Two of those integration cases are retrieval *exclusion* tests of the kind this
atlas looks for, and they are why the negative-evaluation mark is awarded.
`minSimilarity drops below-threshold vectors, preserving rank order` hand-sets
three embeddings at cosine 1, ~0.71 and 0 to the query and asserts the far one
absent with the other two present and in rank order. `bm25 search returns only
genuine matches, never zero-score non-matches` seeds eight distractors beside
two real matches, leaves the page large enough to hold all ten, and pins the
result by exact equality — its comment recording that the previous
`ORDER BY <@> LIMIT` used to backfill the page with zero-score rows on both the
index path and a sequential scan. A test written because the failure happened is
worth more than one written because the shape looked right.
