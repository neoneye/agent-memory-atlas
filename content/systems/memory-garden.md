...
capability_evidence:
  ...
  human_review: "the review surfaces over candidates, answers and memory | src/memory_garden/web.py:492 (/api/verdict), :548 (/api/discover/react), :558 (/api/discover/review), :474 (/api/memory/{memory_id}/revoke), src/memory_garden/cli.py:72-75 (review), src/memory_garden/memory.py:112 record_verdict | a scan proposes candidates and a person reviews each from the web workspace or the CLI; only a saved user verdict can produce a long-term memory, and the person can revoke one from the workspace. The producer cannot stand in for the reviewer, checked against the registries rather than the prose: `record_verdict` is the sole writer of a `memory_items` row, the three statements that insert a `verdicts` row are `cognitive.py:76` and `:272` — reached only from `web.py:495`, `:551`, `:561` and `cli.py:369` — plus an eval harness that wipes the table first, and the MCP server (`src/memory_garden/mcp_server.py:22-119`) declares nine tools of which every one is a read: `search_sources`, `read_source`, `get_topic_timeline`, `find_change_candidates`, `discover_cognitive_shifts`, `find_interval_events`, `search_hypothesis_evidence`, `get_user_verdicts` and `ask_garden`. The agent can read a verdict and cannot write one | tests/test_cognitive.py, tests/test_memory_lifecycle.py:36"
...
**2026-09-19** — audited at the unchanged pin [`d7fdb1c7c354e3eafdead896ed64941a049755ac`](https://github.com/drephantom/memory-garden/commit/d7fdb1c7c354e3eafdead896ed64941a049755ac); nothing upstream moved. All three marks stand and every anchor is exact. `human_review` was producer-tested against the tool registry rather than carried forward, because the corpus-wide re-test of that mark has been withdrawing it wherever the approve verb turns out to sit on the agent's own surface. It does not here, and the shape is the strongest of the four this atlas has found: there is no write verb to guard. The MCP server declares nine tools and every one reads; `record_verdict` is the only writer of a long-term memory row; and the three statements that insert a verdict are reached from the web workspace, the CLI, and an eval harness that clears the table first. The record now carries that trace. Screened again first; nothing was installed and no suite was run.
