...
capability_evidence:
  trust_state: "review_state on derived procedural memory, withholding it from recommendation and from the counter that would justify one | backend/core/procedural_engine.py:9-34, :63-66, :119, :149-150, :324-330, :375, :442, :480 | a synthesised procedure is persisted with `review_state=ProceduralReviewState.DRAFT` — the module header states the rule outright — and the vocabulary is `draft`, `human_reviewed`, `rejected`, validated on construction so an unknown value raises. It is a stored discrete field rather than a score, and `recommend_for_trigger` returns only `human_reviewed` rows whose trigger matches; the header says it plainly, that drafts are never recommended. The second half is the one worth copying: `increment_success` refuses to operate on anything but a `human_reviewed` row, so a draft cannot accumulate the track record that would later argue for promoting it — the state withholds the memory from the decision *and* from the evidence for the decision. Approving an already-approved draft is refused rather than being a no-op | a caution for a reader: the forgetting engine carries a `review_state` column of its own on archive rows, with a different vocabulary including `auto_generated`, recording whether a *forgetting* decision was reviewed — the same column name on a different table"
...
**2026-09-19** — [`56c9bed39957f615da0b66b5e1459281d8fd1fef`](https://github.com/agi-is-going-to-arrive/memory-palace/commit/56c9bed39957f615da0b66b5e1459281d8fd1fef) — `trust_state` re-tested at the unchanged pin. Everything the record said is exact — the draft default, the three-value vocabulary, the recommend path restricted to `human_reviewed` — and one thing it did not have is the property most worth copying from this engine. `increment_success` refuses to operate on anything that is not `human_reviewed`. So a draft procedure is withheld from recommendation *and* from the success counter that would later be cited as a reason to promote it: the state blocks the decision and the evidence for the decision at once, which closes the loop a reviewer would otherwise have to watch for. Approving an already-approved draft is refused rather than silently repeated, and an unknown `review_state` raises on construction. One caution belongs on the record for a reader grepping this tree: `forgetting_engine.py` has a `review_state` column of its own on archive rows, with a different vocabulary that includes `auto_generated`, and it records whether a *forgetting* decision was human-reviewed — the same column name on a different table, not a second writer of the procedural field. Nothing was installed and no suite was run.
