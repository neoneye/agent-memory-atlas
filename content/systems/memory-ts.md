...
capability_evidence:
  trust_state: "the five-value status on the memory record, dropped from both retrieval entry points — except a superseded row, which is redirected to its replacement rather than discarded | src/types/memory.ts:106, src/core/retrieval.ts:148-160, :432, :486-493, :812, src/server/index.ts:477 | `status` is one of `active`, `pending`, `superseded`, `deprecated` or `archived` on the stored record — a field, not a score, with `importance_weight` and `confidence_score` as separate columns — and both retrieval entry points drop anything that is not active. The supersession case is deliberately not a drop and the code says so twice, once in the filter's doc comment and once inside it, so a later reader does not tidy it away: a row carrying `superseded_by` or `resolved_by` is redirected in the first phase to the memory it names, so a corrected fact still answers the question that found the stale one instead of the query silently returning nothing. The replacement and linked-memory lookups separately skip `archived` and `deprecated`. One polarity is worth knowing: the test is `memory.status && memory.status !== \"active\"`, so a record with no status at all passes | the redirect is what makes the drop safe — the shape most systems reach for here is deletion, which loses the answer along with the stale row"
stack_storage: "files"
...
**2026-09-19** — [`8fcadf6d8783869878a64d42aec3ed88f7f91a70`](https://github.com/rlabs-inc/memory-ts/commit/8fcadf6d8783869878a64d42aec3ed88f7f91a70) — `trust_state` re-tested at the unchanged pin; the mark holds and the detail the record led with is the one worth leading with. Both retrieval entry points drop a non-active record, but `superseded_by` is deliberately *not* filtered there — it is redirected in the first phase to the memory it names, and the code says so twice, in the doc comment above the filter and again inside it, which is a note written to stop a future reader from "fixing" it. That is the difference between a store that forgets a corrected fact and one that forwards the question to the correction. Two details are added: `resolved_by` takes the same path as `superseded_by`, and the predicate is written `memory.status && memory.status !== 'active'`, so a record carrying no status at all passes — the permissive value is the absent one. Nothing was installed and no suite was run.
