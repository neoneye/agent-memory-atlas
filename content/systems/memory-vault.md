...
capability_evidence:
  trust_state: "a `forgotten` flag set by the forget tool and excluded by default on six reads, with the one place the widening could have been silently overridden identified and avoided | src/memory_vault/mcp/server.py:500-525, :576-590, :679, :741, :777, src/memory_vault/services/search.py:297, src/memory_vault/api/routers/chunks.py:54-84, src/memory_vault/api/routers/spaces.py:29, src/memory_vault/migrations/007_live_graph_views.sql:30, src/memory_vault/cli.py:203-230 | `forget` refuses a chunk that is already forgotten, then sets `metadata->>forgotten` with a `forgotten_at` timestamp and drops the importance to zero — the row stays. Every read excludes it: the shared search clause builder puts `(c.metadata->>forgotten)::boolean IS NOT TRUE` in first, three further MCP queries repeat it, the spaces router carries it, and the chunks router defaults `include_forgotten=False`. The part worth copying is what the chunks router does *not* do. An entity filter is written as an `EXISTS` against `entity_mentions` rather than against the `live_entity_mentions` view, and the comment gives the reason: the view hardcodes the exclusion, which would quietly override `include_forgotten=true` for entity-filtered queries only, so the two parameters keep agreeing. A predicate-drift bug identified and declined rather than shipped. Destruction is a separate later step: `purge-forgotten` deletes only memories forgotten more than N days ago, default thirty | tests/test_space_filter_security.py:75-80 asserts the exclusion is present for all three shapes of the space argument; tests/test_purge_forgotten.py:49, :62, :72 and :81 pin the window, the purge, the zero-day case and the rejection of a negative window"
...
**2026-09-19** — [`84987337c1418d85468c4dd09c28e7a75252c378`](https://github.com/MihaiBuilds/memory-vault/commit/84987337c1418d85468c4dd09c28e7a75252c378) — `trust_state` re-tested at an unchanged pin. The exclusion clause is at six sites, `forget` still refuses a chunk already forgotten and keeps the row, and the purge tests are better named than the previous line ranges suggested — `test_recent_forget_survives_the_default_window`, `test_old_forget_is_purged`, `test_zero_days_purges_every_forgotten_memory` and `test_negative_days_is_rejected`, the last being the guard against a negative window quietly meaning everything. One correction, and it reverses the sense of the sentence. The record said an entity-filtered query goes through a view that hardcodes the exclusion. It does not, and the comment explains the choice (`api/routers/chunks.py:74-79`): the filter is written as an `EXISTS` against `entity_mentions` rather than against `live_entity_mentions` precisely because *the view hardcodes `forgotten IS NOT TRUE`, which would quietly override `include_forgotten=true` for entity-filtered queries only*, so the two parameters keep agreeing. That is not an honest admission of a gap; it is a gap identified and declined, with the reasoning left for whoever reaches for the convenient view next. It is the inverse of what this sweep found in beever-atlas, where a fallback path lost the same kind of guarantee without anyone noticing. Screened again first; nothing was installed and no suite was run.
