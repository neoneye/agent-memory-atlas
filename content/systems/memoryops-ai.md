...
capability_evidence:
  trust_state: "an admission predicate on the repository interface, so the vector index scores and the repository decides | services/api/app/db/repository.py:128, services/api/app/db/memory_repo.py:31, :188, :259, :263-265, :268-282, services/api/app/db/postgres_repo.py:44, :472-473, :475-505, services/api/app/routes/memories.py | `status` is a discrete field whose values include `pending`, and admission can hold sensitive content there rather than storing it active; the PATCH route moves it through approve, reject and archive. `retrieve_active` is declared on the storage protocol rather than written per call site, and both shipped implementations honour it — the in-memory repo compares against `_ACTIVE`, the Postgres repo delegates to a `status=active` query — so a pending record reaches no prompt. The semantic path carries it too, and the in-memory implementation states the rule as an invariant: similarity is delegated to a pluggable vector index while *the repository stays authoritative for which rows are candidates*. The Postgres candidate search puts `status = active` and `deleted_at IS NULL` in the base `SELECT` before the vector clause is added. What happens when the ranker fails is the proof: with no query embedding, active rows are still returned at similarity 0.0 so the caller degrades to keyword ranking — the filter survives the loss of the score | services/api/tests/test_memory_route_authorization.py and the governance suite"
  ...
  human_review: "the memory record, before it is active | services/api/app/routes/memories.py | the PATCH route carries edit, approve, reject and archive as separately authorized actions — `approve` deliberately has no self permission, because requiring only one would let `memory:approve:tenant` grant a content edit and an approver could rewrite the text in the request approving it — `derive_patch_actions` therefore decomposes one PATCH into every action its body requests and authorizes each separately, each recording its own witness. The producer test turns on where the permission comes from, and it is not the caller: `permissions_for` maps roles, `parse_roles` maps *a token claim*, and the JWT provider verifies the signature against a JWKS url or key with audience and issuer checked when configured, dropping unknown role names silently so *\"a typo must not escalate\"*. An agent whose token carries no `memory:approve:*` cannot mint one. The caveat is the second provider: `TrustedHeaderProvider` takes the identity from a request header, which is an identity only as far as the proxy in front of it is trusted to set that header and to strip a client-supplied one | services/api/tests/test_memory_route_authorization.py"
...
**2026-09-19** — [`b357e90be763824494ca4eaa279803d26561b6af`](https://github.com/patibandlavenkatamanideep/memoryops-ai/commit/b357e90be763824494ca4eaa279803d26561b6af) — `trust_state` re-tested at the unchanged pin. The record carried file-level anchors and the mechanism deserves line-level ones, because it is one of the cleaner statements of this mark's distinction in the corpus. `retrieve_active` is declared on the repository *protocol*, so the admission rule is part of the storage contract rather than a predicate each caller remembers, and both shipped implementations honour it. The semantic path carries it as well, and the in-memory repository writes the principle out: similarity is delegated to a pluggable vector index, *"the repository stays authoritative for which rows are candidates"*. The Postgres implementation shows the same shape structurally — `status = 'active'` and `deleted_at IS NULL` are in the base `SELECT`, and the pgvector distance clause is added on top of it. The best evidence is what happens when the ranker fails: with no query embedding the search returns active rows at similarity `0.0` so the caller degrades to keyword-only ranking, so the filter outlives the score rather than being part of it. The write path uses the same predicate — deduplication only matches active rows. Nothing was installed and no suite was run.

**2026-09-18** — [`b357e90be763824494ca4eaa279803d26561b6af`](https://github.com/patibandlavenkatamanideep/memoryops-ai/commit/b357e90be763824494ca4eaa279803d26561b6af) — re-read at the same commit; `main` has not moved. The `human_review` mark was re-tested against the [narrowed rubric](../../methodology/atlas-rubric/#human-review-surface), which nineteen of twenty-four marks failed elsewhere in the corpus, and **it holds** — the approving authority is established by an issuer rather than asserted by the caller. `parse_roles` maps a *token claim*, `permissions_for` maps roles to permissions, and the JWT provider verifies the signature against a JWKS url or a key with audience and issuer checked when configured; unknown role names are dropped silently so a typo cannot escalate. An agent whose token carries no `memory:approve:*` cannot mint one.

Two things are added to the record. The separation of duties is sharper than it was described: `derive_patch_actions` decomposes a single PATCH into every action its body requests and authorizes each one separately, each recording its own witness — which is what stops `{"content": …, "status": "active"}` from editing under the approve permission. And the caveat that belongs beside the mark is the second auth provider: `TrustedHeaderProvider` reads the identity from a request header, which is an identity only as far as the proxy in front of it is trusted to set that header and to strip a client-supplied one.
