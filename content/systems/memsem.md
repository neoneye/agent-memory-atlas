...
capability_evidence:
  trust_state: "the candidate ladder — a proposal is a row in a different table, and approval is what creates the memory | src/db.ts:1585-1601, :1265, :1620-1648, :1660-1680, :2128-2145, :35-40, :1423, :565, :618 | `memory_candidates.status` is `pending`, `approved` or `rejected`, and the withholding is structural before it is conditional: a pending candidate is not a row in `memories` that a read skips, it is a row in another table that no recall touches. Approving one calls the ordinary write path and *creates* the memory, returning its new id; rejecting one writes a `memory_suppressions` row keyed on subject, predicate, object and project, and a later add of the same fact is refused with *candidat bloqué par une suppression active*, so a rejection is a standing decision rather than a discarded queue entry. The listing takes an optional status filter and the health count reports pending candidates separately. The `trust` field on a memory — `inferred`, `verbatim`, `verified` — is a different axis and does not carry this mark: `verified` is reachable only through `memory_verify` with non-empty evidence and an audit line, and `TRUST_RANK` ratchets it so a re-add or a resurrection keeps the higher level, but no read filters, orders or excludes by it | src/test/governance-test.ts:37-43, :96-97"
...
**2026-09-19** — [`8332a23620d88dd9755dba2b9ac954d34598d67e`](https://github.com/WindSeries83/memsem/commit/8332a23620d88dd9755dba2b9ac954d34598d67e) — `trust_state` re-tested at the unchanged pin and **moved to a different field**. The record rested on the `trust` level — *"`normalizeTrust` admits the three levels, the add tools' schemas refuse `verified`, and only `verify(id, evidence)` writes it"* — and that is producer-side from end to end. It is well built: `verified` is reachable only through the verify tool with non-empty evidence and an audit line, and `TRUST_RANK` gives the field a ratchet, so re-adding an existing memory or resurrecting an archived one keeps the higher of the two levels. What it does not have is a reader — grepping `src/` for a predicate on `trust` finds one write and a row projection in every read, so an `inferred` memory is retrieved exactly like a `verified` one. The axis that does withhold is `memory_candidates.status`, and it withholds structurally: a `pending` candidate is not a row in `memories` that a read skips, it is a row in a different table that no recall touches, and approving one calls the ordinary write path and *creates* the memory. The rejection side is the part worth studying, and it reaches past this mark into the `tombstone` one this report already carries: rejecting a candidate writes a `memory_suppressions` row keyed on subject, predicate, object and project, and a later attempt to add the same fact is refused with *"candidat bloqué par une suppression active"* — a standing decision rather than a discarded queue entry. Nothing was installed and no suite was run.
