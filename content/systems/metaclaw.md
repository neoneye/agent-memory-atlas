...
capability_evidence:
  ...
  trust_state: "a three-value lifecycle status whose every writer unlinks the row from the full-text index on the next line | metaclaw/memory/models.py:21-24, metaclaw/memory/store.py:464-477, :480-506, :1447-1460, :369-379, metaclaw/memory/manager.py:1183-1186, :1243-1246, :1348-1351, :1501-1504 | `MemoryStatus` is `active`, `superseded` or `archived`, stored in the `status` column and filtered in `list_active`, which is the only read the retriever uses, so a superseded or archived unit cannot reach a prompt. The mechanism is doubled, and the second half is the part worth reading: every one of the nine writers that moves a unit off `active` calls `_remove_fts` immediately afterwards — `supersede` and `expire_stale` in the store, four archival passes in the manager — so the unit is deleted from `memories_fts` in the same breath as the status change. Both keyword arms then intersect with `list_active` anyway (`_search_fts` comments the step as *\"Filter to active-only from the main table\"*, and the manual fallback scans `list_active` directly), so the predicate is present on the fallback path too rather than only on the fast one. The writers are live rather than declarative: five supersession call sites in the consolidator and the manager dedup, and an expiry pass that compares `expires_at` to the clock rather than testing it for presence. Nothing in the tree writes the status back to `active`, so the machine is one-way — and nothing but the repeated idiom keeps the eviction attached to the status write, since no single method does both | tests/test_memory_system.py asserts stats by status and that an expired unit leaves the active set"
...
matrix:
  ...
  trust: "A three-value `status` filtered in the read query and evicted from the full-text index by each of its nine writers, plus a source session and turn range; reinforcement is kept apart from confidence, and confidence itself defaults to 0.7 with no verification path"
...
**2026-09-19** — [`922caf3a1cd093fb316e95183a8acc8aa47b3b21`](https://github.com/aiming-lab/MetaClaw/commit/922caf3a1cd093fb316e95183a8acc8aa47b3b21) — `trust_state` re-tested at the unchanged pin, so the corrections are the atlas's own. The `list_active` predicate is exactly where the record said, and the record stopped one line too early. Every writer that moves a unit off `active` calls `_remove_fts` on the following line — `supersede` (`store.py:1457`), `expire_stale` (`:502`) and four archival passes in the manager (`:1186`, `:1246`, `:1351`, `:1504`) — so the row leaves `memories_fts` in the same call that changes its status. The filter is the second line of defence, not the only one. Nine writers, nine evictions, none missed; but no single method does both, so what holds the invariant is a repeated two-line idiom rather than an abstraction, and a tenth writer would have to remember. Two further observations. The keyword path intersects the FTS hit set with `list_active` rather than trusting the index — the code comments the step as *"Filter to active-only from the main table"* — and the manual fallback scans `list_active` directly, so the fallback carries the predicate that fallbacks usually lose. And a limit worth recording beside the retrieval fields rather than the mark: both keyword arms call `list_active(scope_id, limit=500)`, whose SQL is `ORDER BY updated_at DESC LIMIT ?`, so keyword search can only ever see the 500 most recently updated active units in a scope — and the expiry filter runs in Python *after* that limit, so expired rows consume slots before being dropped. An older active memory is unreachable by keyword search however well it matches. Nothing was installed and no suite was run.
