...
capability_evidence:
  bitemporal: "recall(as_of=) as a second reference clock, with supersession resolved at that time | midas/memory.py:600-616, :645, :991, :1087-1092, tests/test_bitemporal.py | `as_of` replaces the query's reference time and adds a predicate excluding records whose `created_at` is later, and the comment states the distinction the mark is about: `now` asks what is current, `as_of` asks 'what did memory say on date X'. The second half is what makes it real — each retired belief carries a `superseded_at` validity bound, and the context filter swaps `_resolve_head` for `_resolve_at(r, as_of)`, so a historical query returns the version that was current then rather than today's head | `created_at` is the caller-supplied event time — `remember(created_at=…)` at midas/memory.py:298-315 and the batch importer at :369 both take it and fall back to the wall clock — so an importer can record a fact as true before the row was written, and `superseded_at` is set to the revising record's `created_at` (:995), making the window an event-time interval. The limit is that the axis collapses onto record time whenever a caller omits `created_at`, and the as-of read walks this one axis. `as_of` defaults to None and the predicate is dropped entirely when no filter is active, for a stated performance reason"
  scope_enforced: "an equality metadata predicate applied on recall and re-applied on neighbour expansion | midas/memory.py:618-627, :1078-1085 | `metadata_filter` is applied inside the recall predicate as an all-keys equality check over the stored metadata — the comment names the keys it is for, 'namespace / user / agent / project' — so the key reaches the query rather than being available as a tag. The part worth crediting is the second site: neighbour expansion passes `recall_kwargs.get('metadata_filter')` through, with the reason written above it — 'Neighbour expansion must respect the recall scope, or a same-thread record from another namespace would leak past the filter' — which is the exact drift this atlas has recorded in five other systems, anticipated and closed here | the predicate is dropped when no filter is supplied so the no-filter fast path can be taken, so a caller that omits the namespace reads everything; and the scope is a metadata convention rather than a column, so nothing constrains which keys a writer stamps"
  audit_log: "a hash-chained log of every SQLite mutation, with four committed cases on the chain itself | midas/memory.py, tests/test_audit_chain.py:1, :17-27, :34, :46, :62, :78, :104 | the module docstring states the scope — 'every SQLite mutation appends a hash-chained entry (no memory content)' — and the committed cases assert the properties that matter rather than the shape: entries link (`log[1]['prev_hash'] == log[0]['hash']`), supersession and clear are audited, tampering breaks the chain, deleting an entry breaks the chain, the chain survives reopen and cross-instance use, and a receipt never leaks memory content (`'PostgreSQL' not in json.dumps(out)`). It records mutations, not retrievals | it is a hash chain rather than a signature, so a writer able to rewrite the log consistently is not caught; and because entries carry no memory content, the log proves that something changed and cannot say what it said"
  negative_eval: "a superseded belief asserted absent from assembled context, and a forgotten secret asserted absent from recall | tests/test_memory_supersede.py:456-471, tests/test_mcp_server.py:270-282 | the supersession case stores two events in one store, queries for the older wording, and asserts all three halves together — `retrieved_event_ids == ['new-event']`, the new text present, and `'old launch' not in result.context`. The forget case remembers a badge PIN and a standup time, dry-runs the deletion and asserts nothing was removed, then deletes and asserts the PIN's digits are absent from three recalls while the store still holds the other record | both are library-level cases rather than an eval harness, and the forget assertion is a disjunction — an empty result also passes it, though the surviving second record keeps the store non-empty"
...
**2026-09-25** — [`ee9953c15a977343eb783de0b9f217aaf46e5b4e`](https://github.com/vornicx/Midas/commit/ee9953c15a977343eb783de0b9f217aaf46e5b4e) — same pin, one correction. The `bitemporal` evidence record and the 2026-09-18 entry below said there is no caller-supplied event date, so a fact could not be recorded as true before it was written. That was wrong at this commit, and section 5 had it right: `remember` and the batch importer both take `created_at` as the event time (`midas/memory.py:298-315`, `:369`), commented *"`created_at` is the EVENT time (when the fact was true/stated), distinct from ingest order"*, and `superseded_at` is set from the reviser's `created_at`. The record now states that, with the limit that the axis falls back to the wall clock when a caller omits it. No mark moved; nothing was installed or run.

**2026-09-18** — [`ee9953c15a977343eb783de0b9f217aaf46e5b4e`](https://github.com/vornicx/Midas/commit/ee9953c15a977343eb783de0b9f217aaf46e5b4e) — re-read at the same commit. Nothing needed correcting; all four marks were re-derived from the source and now carry evidence records, and two of them are among the better instances in this corpus.

`scope_enforced` is the one worth singling out. `metadata_filter` is an all-keys equality predicate inside recall, and neighbour expansion re-applies it with the reason written above the call: *"Neighbour expansion must respect the recall scope, or a same-thread record from another namespace would leak past the filter."* That is precisely the drift this atlas has recorded in five other systems, anticipated and closed in the source with the failure named.

`audit_log` rests on `tests/test_audit_chain.py`, which asserts the properties rather than the shape: that entries link by `prev_hash`, that supersession and clear are audited, that **tampering breaks the chain**, that **deleting an entry breaks the chain**, that the chain survives reopen and cross-instance use, and that a receipt never leaks memory content. Most audit logs in this corpus are asserted to exist; this one is asserted to detect.

The `bitemporal` limit is now stated: the two clocks are the record clock and the query clock. `as_of` genuinely resolves supersession chains to the version valid at that time — `_resolve_head` is swapped for `_resolve_at(r, as_of)` — but there is no caller-supplied event date, so the axis is when memory believed something rather than when it was so. `stack_source` goes from seeded to reviewed.
