...
capability_evidence:
  trust_state: "a decision status the analyzer validates before assigning, filtered at every point where candidates are gathered and at none where they are ranked | mindcache/Database/decision_analyzer.py, and the seven occurrences of the admission predicate across the embedding job, the tree-cache queries, the client retrieval query and the embedder type map | `DecisionStateAnalyzer.analyze_cluster` checks each value an LLM returns against the five-name set before assigning it and drops an unrecognised one rather than storing it, so the vocabulary cannot be widened by a model. The predicate is `status.in_([\"active\", \"conditional\"])` — an allowlist of two out of five — and it appears seven times, including in the fetch that builds the similarity candidate set, so a decision superseded after it was embedded stops being retrievable without anything deleting or re-embedding its vector. The assembly path itself carries no status term — `active_path.py` never mentions `status` — and does not need one, because every id it fetches by came from a query that filtered. That is sound and it is also load-bearing: a future candidate source that skips the filter would leak straight through an assembler that has no reason to check | the status is written by an LLM handed a semantic cluster of related decisions, with a one-sentence context recorded for each verdict"
...
**2026-09-19** — [`45b904a7d6d29d0f6ac1b15cfd9b61508a5e33b4`](https://github.com/faisalhussain-devs/MindCache/commit/45b904a7d6d29d0f6ac1b15cfd9b61508a5e33b4) — `trust_state` re-tested at the unchanged pin; every count in the record is still exact. `status.in_(["active", "conditional"])` appears seven times, and `active_path.py` contains no mention of `status` at all, which is the pair of facts the record's argument rests on and both hold. Two things are worth restating now that the sweep has a comparison set. The predicate is an allowlist of two values out of five, so a sixth value — or a model that invents one — is withheld rather than admitted, and the analyzer closes that door on the write side too by checking each value an LLM returns against the five-name set and dropping an unrecognised one instead of storing it. And the shape of the assembly path is the one this corpus keeps finding broken elsewhere: `active_path.py` has no status term because every id it is given came from a query that already filtered. That is correct today and it is a convention rather than a constraint — the same convention that, in several other systems read this week, held everywhere except the one read somebody added later. Nothing was installed and no suite was run.
