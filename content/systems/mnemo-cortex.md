...
revision: fec22efd46c83b62899cc76d4e9bc9b8e12df84b
revision_url: https://github.com/GuyMannDude/mnemo-cortex/commit/fec22efd46c83b62899cc76d4e9bc9b8e12df84b
analyzed_at: 2026-09-19
capabilities: "trust_state, audit_log, negative_eval"
capability_evidence:
  trust_state: "the facts store — a three-rung ladder whose lowest rung is excluded from the default read | agentb/facts_store.py:26 (`CONFIDENCE_LEVELS`), :232-242 (`get(..., include_false=False)`), :330 (`save`), :469 (`demote`), :28-40 (`AUTHORITY_LEVELS`) | confidence is `false`, `high_probability` or `verified` — discrete values, not a score — and `get` excludes `false` unless a caller opts in, so the state withholds on the read path. The write path enforces the ladder in one direction: a lower-ranked assertion cannot overwrite a higher-ranked one, and `demote()` exists specifically because *\"the promotion ladder otherwise blocks verified→false\"*. A second axis arrived at the 2026-09-19 pin and belongs beside the ladder: `AUTHORITY_LEVELS` is `open`, `probe`, `declared`, and *\"a tier says WHO may change a slot, not what it is about\"* — a locked slot accepts a write only from evidence of its own kind, a machine for `probe` and the operator's word for `declared`, and anything else *\"is recorded as a proposal — never as the value, and never as an error\"*, with the incident that produced it named in the same comment. Every change writes a `fact_history` row | tests/ cover the store; not run, the screen reports four dependency files changed the day of the pin, inside the cooldown"
...
**2026-09-19** — re-pinned to [`fec22efd46c83b62899cc76d4e9bc9b8e12df84b`](https://github.com/GuyMannDude/mnemo-cortex/commit/fec22efd46c83b62899cc76d4e9bc9b8e12df84b), ten commits and 22 files on, read as part of a corpus re-test of `trust_state` against the filtering-versus-ranking line. **All three marks hold, and the confidence record gains a second axis the range added.** The ladder is unchanged in substance — `CONFIDENCE_LEVELS` is still `false`, `high_probability`, `verified`, and `get` still drops `false` unless a caller opts in — but every cited line moved and was re-mapped: `get` to `:232-242`, `save` to `:330`, `demote` to `:469`. New in v4.23 is `AUTHORITY_LEVELS` (`open`, `probe`, `declared`) at `:28-40`, described as *"a tier says WHO may change a slot, not what it is about"*: a locked slot accepts a write only from evidence of its own kind, and anything else *"is recorded as a proposal — never as the value, and never as an error"*, with the specimen named in the comment — an agent's loose *"Tailscale is the only path"* sentence auto-captured and served as topology on 2026-09-15. **This does not add `human_review`**, and the reason is worth recording: `fact_proposals` is drained by `POST /facts/proposals/{id}/resolve` (`agentb/server.py:2474-2478`), whose `by` is a field on the request body, so the approver is whatever string the caller sends. The queue is real and the actor is not checked. Screened again first; nothing installed or run.
