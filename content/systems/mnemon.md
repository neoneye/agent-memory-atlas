...
revision: b0661c0bcdb8e7c08e9239b6942118e18dd288ad
revision_url: https://github.com/mnemon-dev/mnemon/commit/b0661c0bcdb8e7c08e9239b6942118e18dd288ad
...
**2026-09-18** — [`b0661c0bcdb8e7c08e9239b6942118e18dd288ad`](https://github.com/mnemon-dev/mnemon/commit/b0661c0bcdb8e7c08e9239b6942118e18dd288ad) — re-pinned from `6c371ec`; 59 files and +2,721 lines, re-screened at the new pin. **Human review withdrawn**, and the evidence the record cited is what withdraws it. `mnemon gc` really does return the non-immune insights below the threshold as full records with the two verdicts beside them, and `--keep` really does adjudicate by boosting retention — the mechanism is as described. The question is whose hand is on it, and the answer is in the file the record pointed at: `internal/memory/setup/assets/openclaw/SKILL.md` is the skill an agent is given, its **Commands** block lists `mnemon forget <id>`, `mnemon gc --threshold 0.4` and `mnemon gc --keep <id>` among the commands the agent may run, and line 88 instructs it outright — *"To retire a superseded memory, store and verify the new fact, then explicitly run `mnemon forget <old-id>`."* Both verdicts are handed to the writer. A CLI is only a gate when the agent has not been told to use it. `audit_log` and `negative_eval` hold.
