...
revision: 3676311e73ad95ca650957d44ccb46a627cc74da
revision_url: https://github.com/takecchi/mnemora/commit/3676311e73ad95ca650957d44ccb46a627cc74da
analyzed_at: 2026-09-16
capabilities: "trust_state, bitemporal, negative_eval"
capability_evidence:
  trust_state: "a five-value status where recall admits exactly two, so superseded, archived and forgotten memories stay stored and stop being recalled — and contested is deliberately one of the two admitted | packages/core/src/memory.ts:5-13, packages/postgres/src/memory-store.ts:1044, :1050 | `MemoryStatus` is `active | superseded | contested | archived | forgotten`, declared once as a TypeScript union and a Zod enum together. The recall scan filters `WHERE status IN ('active', 'contested')`, and the same predicate governs the count beside it, so a withheld row does not move the number either. Including `contested` is the decision worth noting: a memory under dispute is still surfaced rather than suppressed, which is the opposite of resolving a conflict at write time. Status transitions are guarded by an `expectedStatus` compare-and-set | packages/testkit/src/memory-store-conformance.ts:1710-1760"
