...
revision: 9fa38c2de9c9f072f73c89665e8d752ef8955098
revision_url: https://github.com/team-monet/monet/commit/9fa38c2de9c9f072f73c89665e8d752ef8955098
analyzed_at: 2026-09-19
capabilities: "trust_state, scope_enforced, audit_log, negative_eval"
capability_evidence:
  trust_state: "the concept lifecycle, not the disputed flag the previous record named — that one is marked DISCLOSURE ONLY in its own comment | packages/core/src/engine.ts:5863-5873, :5996, :7281, :8016-8019, :8163, :8185, :9063, :3993-4002, packages/core/src/gates.ts:1564-1583 | `concepts.status` carries `retired`, and `status != \"retired\"` is written into the read paths that matter — the circle listing and the global listing, the orphan query, the dirty scan the checkpoint walks, the native search `WHERE` clause and the counts — so a retired concept is excluded from retrieval rather than ranked below. Retirement is a status write rather than a delete, and a comment at the retire site records that the readers filter it out explicitly instead of relying on a cascade. The flag the previous record cited does something different and says so: `parentDisputed` is set exactly when a derivation parent is a principle whose status is `disputed`, is never `false` because absence is the signal, and is documented as *DISCLOSURE ONLY* — *delivery, severity and firing are unchanged by it*, because a rule *stands on its own evidence, so a doubtful parent does not silently withdraw a rule an agent is relying on*. It is computed at delivery and never cached, so it cannot keep announcing doubt after a human has resolved it. That is a good design and it is disclosure, not admission | a superseded rule is deliberately different again: it stays `status=\"active\"` and keeps its binding, because it is history"
...
**2026-09-19** — [`9fa38c2de9c9f072f73c89665e8d752ef8955098`](https://github.com/team-monet/monet/commit/9fa38c2de9c9f072f73c89665e8d752ef8955098) — re-pinned one commit on, both files in `packages/cli` (a repair command and its test), so every anchor in this report is untouched. `trust_state` re-tested and **moved to a different field**. The record rested on `parentDisputed`, and that flag's own comment is the reason it cannot carry the mark: *"DISCLOSURE ONLY. Delivery, severity and firing are unchanged by it: the rule was born of a correction or a projection and stands on its own evidence, so a doubtful parent does not silently withdraw a rule an agent is relying on. What it changes is what the agent knows."* Nothing is withheld; the agent is told. It is a well-made piece of design — never `false` so absence is the signal, computed at delivery and never cached so it cannot keep announcing doubt after a human resolved it, and carrying the disputed parents' identities because the flag alone *"advertised a recovery step no MCP caller could take"* — and it is disclosure rather than admission. The field that withholds is `concepts.status`: `status != 'retired'` appears in the circle and global listings, the orphan query, the dirty scan, the native search `WHERE` clause and the counts, so a retired concept is out of retrieval, and the retire path sets the status rather than deleting, with a comment noting the readers filter explicitly instead of relying on a cascade. Screened again first — one build-time execution path, two floating dependency ranges and a note, unchanged in shape; the FRESH count of 5 is an artefact of screening a `--depth 1` clone, which dates every file to the tip, and is not comparable with the previous reading's 0. Nothing was installed and no suite was run.
