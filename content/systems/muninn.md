...
revision: b95d5044b7acc585773390142bec2b59b2785c39
revision_url: https://github.com/RuneLind/muninn/commit/b95d5044b7acc585773390142bec2b59b2785c39
...
capability_evidence:
  ...
  human_review: "the gardener's proposal queue | src/dashboard/routes/wiki-gardener-routes.ts, src/db/wiki-proposals.ts | drafted pages are listed with a rendered preview and approved or rejected over HTTP, with the draft-to-approved transition written as a compare-and-swap and a comment naming the regression it pins — *\"approve 403s, row stuck in `approved` forever\"*. Reject is *\"deliberately NOT guarded — it flips a DB status and mutates no wiki\"*, which is the correct asymmetry: the destructive-to-the-wiki direction is the guarded one. The draft really is held: `markWikiProposalApplied` (`src/db/wiki-proposals.ts:246`) only fires `WHERE status = 'approved'`, so nothing reaches the wiki without the approval, and `revertWikiProposalToDraft` at `:283` is the way back. The producing agent has no verb for it either — the MCP server registers exactly one tool, `research_knowledge`, a read (`src/research/mcp-server.ts:197`). Two limits belong with the mark: no equivalent surface exists for `memories`, which the dashboard only displays, and the dashboard itself is an *\"auth-less loopback\"* by design (`:115-117`), stated there as *\"containment of accidents, not defence against a hostile author\"* | src/gardener/apply.test.ts, src/dashboard tests"
