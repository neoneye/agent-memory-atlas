...
revision: c1f34a27f53fc75530620a50d61fc9b0aa20e759
revision_url: https://github.com/The-Geek-Freaks/NEOTH/commit/c1f34a27f53fc75530620a50d61fc9b0aa20e759
analyzed_at: 2026-09-16
capabilities: "trust_state"
capability_evidence:
  trust_state: "a six-value fact state on the ground-truth table where recall surfaces only `verified` by default, with an explicit widening argument and a separate revocation condition | SRC/neothd/src/memory/groundtruth.rs:1-14, :158-165, :635-653, SRC/neothd/src/memory/contradiction.rs:1-8 | `FactState` is `Raw | Candidate | Verified | Superseded | Contradicted | Deprecated`, persisted in `idx_groundtruth.fact_state`. `surface_for_recall` builds `WHERE revoked_at IS NULL` plus `AND fact_state = 'verified'` unless the caller passes `include_unverified`, so the default is the narrow set and widening is a deliberate argument. Contradiction detection is what populates the withholding value: it \"flags the LOWER-credibility fact (fewer corroborating sources …) as `FactState::Contradicted` so it drops out of recall (`surface_for_recall` already gates on `verified`)\". Ground truth is also decay-immune by construction — \"no Hebbian decay, no FORGET_FLOOR sweep, no consolidation pass\" — and surfaced \"in every recall hit BEFORE any episodic row so a stale Hebbian-decayed memory cannot overwrite an operator ground truth\" | SRC/neothd/src/memory/groundtruth.rs:1081"
...
**2026-09-16** — [`c1f34a27f53fc75530620a50d61fc9b0aa20e759`](https://github.com/The-Geek-Freaks/NEOTH/commit/c1f34a27f53fc75530620a50d61fc9b0aa20e759) — first reading, at a commit dated 16 September 2026. Screened before opening, from a shallow clone: thirty-four files scanned, no auto-run surfaces, two build-time execution points, no unpinned surfaces and twenty-eight dependency files inside the seven-day cooldown. Nothing was installed, built or run.
