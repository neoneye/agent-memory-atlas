...
revision: 5f99b7af3142bf9fc562bd35d127b852a99bc113
revision_url: https://github.com/no-human-ai/no_human/commit/5f99b7af3142bf9fc562bd35d127b852a99bc113
analyzed_at: 2026-09-19
capabilities: "tombstone, trust_state, scope_enforced, audit_log, human_review, negative_eval"
capability_evidence:
  tombstone: "the archive that keeps its dedupe key | src/no_human/learning/queue.py:91-100,1246-1313, src/no_human/core/db.py:3407-3560,3938-3947, src/no_human/learning/retire.py:246-279 | a rejected proposal from a batch-driven origin — supervisor, history, curator, escalation, review_fail, tamper — is archived with `archived = 1` and its `file_path` dedupe key intact, so the next harvest's `memory_dedupe_key_exists` check finds the key and never re-proposes the lesson; `add_memory` returns None on a key hit; a proposal that fails an auto-activation screen is archived the same way; a rejection on any other origin — the outcome path's NULL, review and reply — deletes the row and its key, on the stated ground that those producers cannot regenerate without new evidence; `nh rules remove`, `nh skills remove` and the matching API routes call `delete_memory` on any row an id prefix resolves, outside the queue and without an audit row | tests/test_learning.py:240-330 (every origin declares a reject verb; reject uses the right verb per origin), tests/test_memory_retirement.py:148 (the sweep is idempotent and reversible)"
  ...
  human_review: "the Second brain pane, nh learnings and the learnings API | web/src/Settings.jsx:913-1100, web/src/learningCard.js, web/src/learningRetire.js, src/no_human/api/app.py:4258, :4296, :4308, src/no_human/cli/commands.py:6385, :6527, src/no_human/context/sessions.py:90, src/no_human/intake/mcp_bridge.py | a person confirms, rejects, pauses, deletes, retires and restores individual learnings, sees each one's origin task, usage count and archived count, and gets a suggest-only list of active rules unused for ninety days that nothing but a human click can retire; the same surface adds rules and skills, and its remove commands hard-delete any row an id prefix resolves. The confirmation is what admits, not merely what labels: recall reads `SELECT type, title, content FROM memories WHERE confirmed = 1` (`context/sessions.py:90`), so an unconfirmed learning reaches no session. And the producing side has no confirm verb — the only MCP surface in the tree is `intake/mcp_bridge.py`, a task-intake bridge whose payload is a board, an external id and a repo path, and which contains the string `confirm` zero times | tests/test_learning.py:81,133 (confirm promotes, reject removes), web/src/learningRetire.test.mjs, web/src/secondBrainPanel.test.mjs"
...
**2026-09-19** — re-pinned to [`5f99b7af3142bf9fc562bd35d127b852a99bc113`](https://github.com/no-human-ai/no_human/commit/5f99b7af3142bf9fc562bd35d127b852a99bc113), 20 commits on. All six marks stand. `human_review`'s anchors moved and were re-resolved — `GET /api/learnings` is now at `api/app.py:4258`, the retire-candidates route at `:4296`, `POST /api/learnings/{mem_id}/confirm` at `:4308`, and the CLI's `learnings` and `learnings_curate` at `cli/commands.py:6527` and `:6385` — and the record gained the two checks it was missing. The confirmation admits rather than labels: recall is `SELECT type, title, content FROM memories WHERE confirmed = 1` (`context/sessions.py:90`), so an unconfirmed learning reaches no session. And the producing side has no confirm verb: the only MCP surface in the tree is `intake/mcp_bridge.py`, a task-intake bridge carrying a board, an external id and a repo path, which contains the string `confirm` zero times. Screened again first; nothing was installed and no suite was run.
