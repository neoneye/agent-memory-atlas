...
capability_evidence:
  trust_state: "a four-state lesson ladder where the middle states are injected into a deterministic half of runs, so the other half is a control group | src/memory/lessons.ts:15-19, :91, :120, :138-160, :183, :226-249, :252-262 | `LessonStatus` is `draft`, `canary`, `active`, `disabled`. `select` applies three admission tests before it computes any score: a `disabled` lesson is skipped, a lesson under the confidence floor is skipped, and a lesson that is not `active` is skipped unless `inCanarySlice(runId, lessonId)` admits it. That third test is the design — `CANARY_SHARE` is 0.5 and the slice is an FNV-1a hash of the run id and the lesson id rather than a random draw, so the same run always decides the same way about the same lesson and, as the comment says, *the runs without it are the control group*. Promotion is then keyed on what those runs produced: `applyLifecycle` reads only concrete signals — two accepts and a confidence of at least 0.6 promote a canary to active, three rejects or a confidence under 0.2 disable it as *contradicted by outcomes*, and six injected runs with zero matches disable it as *never matched — expired as noise*. `disabled` is terminal, checked first and returned unchanged. Where a lesson enters is itself a judgement: one distilled from an explicit correction starts `active`, one distilled from a failure starts `draft` and has to earn its way in | the confidence floor sits in the same guard as the state test and does a different job, which is the distinction this mark turns on: one asks how sure, the other asks whether this run may see it"
...
**2026-09-19** — [`a90396cff12b1e2fbb8a14f74eef6c6c89105b4f`](https://github.com/klairtech/one-agent-many-hats/commit/a90396cff12b1e2fbb8a14f74eef6c6c89105b4f) — `trust_state` re-tested at the unchanged pin, and the middle of the ladder turns out to be the point. The record said `select` skips `disabled`, which it does, but that is the least of the three admission tests in that loop. The third is `if (lesson.status !== 'active' && !inCanarySlice(opts.runId, lesson.id)) continue` — a draft or canary lesson is injected into a deterministic half of runs, chosen by an FNV-1a hash of the run id and the lesson id rather than at random, and the comment gives the reason: *"so the runs without it are the control group."* The lesson is withheld from half the runs in order to measure whether it helps. Promotion then reads only what those runs produced — two accepts with confidence at or above 0.6 promote, three rejects or confidence under 0.2 retire it as *"contradicted by outcomes"*, six injected runs with no match retire it as *"never matched — expired as noise"* — and `disabled` is terminal, returned unchanged before any other rule runs. Two details complete it: all three admission tests run before the relevance score is computed, and the confidence floor sits in the same guard as the state test while doing a different job, which is this mark's distinction stated in three consecutive lines of code. Nothing was installed and no suite was run.
