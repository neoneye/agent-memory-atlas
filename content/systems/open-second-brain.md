...
capability_evidence:
  ...
  trust_state: "a preference's status, which partitions the active set rather than ordering it | src/core/brain/types.ts:89-102, src/core/brain/active.ts:141-158, :162-168 | `BRAIN_PREFERENCE_STATUS` is three discrete values — `unconfirmed`, `confirmed` and `quarantine` — and the last is a probation state for a previously-confirmed rule whose recent evidence is dominantly negative. It is a state rather than a score because of what is done with it: the active digest reads every preference once and splits it, `confirmed` into the rule list and `quarantine` into a separate list of its own, each sorted afterwards. An `unconfirmed` preference lands in neither, so it does not reach the model as an active rule at all. The ordering is a different axis applied inside the partition — confidence then id, and optionally a provenance re-rank of stated over deduced over inferred, which is stable so the confidence order survives within a band and is off by default so enabling it is a visible change. Beside it, the guardrails loader throws rather than falling back to defaults when the operator's config is present but unreadable, on the stated ground that a broken config *must not quietly resolve to defaults on a surface the agent reads as its rule set* | a person moves a preference between the states; the system proposes"
...
**2026-09-19** — [`54bb28d9b760758446e494e3c6473f6534dfbdee`](https://github.com/itechmeat/open-second-brain/commit/54bb28d9b760758446e494e3c6473f6534dfbdee) — `trust_state` re-tested at the unchanged pin. The record was right that the status filters; the shape is more precise than a filter. `active.ts` reads the preference set once and *partitions* it — `confirmed` into the rule list, `quarantine` into a list of its own — and sorts each afterwards, so an `unconfirmed` preference is in neither output rather than being ordered last in one. The ordering that does happen is deliberately a separate axis inside the partition: confidence then id, with an optional provenance re-rank of stated over deduced over inferred that is stable, so the confidence order survives within a trust band, and off by default so that turning it on is a visible change rather than a silent one. Worth recording beside the mark: the guardrails loader throws when the operator's config exists but cannot be read, rather than falling back to defaults, and the comment gives the reason — a config someone wrote and broke *"must not quietly resolve to defaults on a surface the agent reads as its rule set."* That is the same instinct as the status itself, applied one level up, to the configuration of the trust surface. Nothing was installed and no suite was run.

**2026-09-18** — [`54bb28d9b760758446e494e3c6473f6534dfbdee`](https://github.com/itechmeat/open-second-brain/commit/54bb28d9b760758446e494e3c6473f6534dfbdee) — re-read at the same commit; `main` has not moved, so the correction is this report's. **Human review withdrawn.** The record's claim was that *"nothing a session captures becomes a preference on its own: signals wait for the nightly dream pass, which proposes, and a person moves a candidate to `confirmed`"*. The dream pass and the `o2b brain feedback` verb are both real; what the record did not check is that the verb has an MCP twin the writing agent holds. `src/mcp/brain/feedback-tools.ts:942` registers `brain_feedback`, `src/mcp/registry-guard.ts:329` classes it *"write; returns a small fixed-shape ack"*, and the project states the CLI/MCP relationship as a design goal elsewhere — *"the CLI mirror and the MCP tool cannot drift"*.

The tool's own schema then names the bypass. Its description reads *"With `force_confirmed: true`, create the preference directly (skips the dream trial window)"*, and the `force_confirmed` property is a boolean documented as *"When true, also creates an immediately-active confirmed `pref-*` alongside the inbox signal, skipping the dream-pass promotion step."* One flag in the model's own call turns a proposal into an active rule, which is both shapes the [narrowed rubric](../../methodology/atlas-rubric/#human-review-surface) rejects at once — an approve verb on the producer's surface, and a flag the caller sets. The other five marks hold, and the trial window remains the default path for a signal recorded without the flag.
