...
capability_evidence:
  trust_state: "the claim lifecycle column, not the review status the previous record named — that one is a term in a score | api/db/init/001_schema.sql:28, :38, api/app/schemas.py:7, :19, :85, api/app/retrieval.py:310, :332, :380, :448, :511, :434-442 | `claims.status` is `pending`, `accepted` or `rejected`, held by a CHECK constraint, indexed, and defaulting to `pending` in the database. Every retrieval filters on it — the vector query, the ranked query and the expansion query each carry `c.status = ANY(%(statuses)s)` — and when a caller names no status the list defaults to `[\"accepted\"]`, so a pending or rejected claim is absent from a search rather than ranked below one. Beside it, and separately, `claim_review_status` is the ranking axis: `unreviewed`, `confirmed`, `disputed`, `superseded` and `merged` appear as a `CASE` adding +0.14 to −0.42 inside the same sum as the role weight and the confirm and dispute counts, so a superseded claim is penalised and still returned if it scores well enough. Two axes, and only the first withholds | the weakening is at the boundary: the API write schema defaults `status` to `accepted`, so a claim submitted through it is admitted on arrival and the database's `pending` default is reached only by a writer that omits the column"
...
**2026-09-19** — [`6c916d9aac6198de0912a97739ff95d439a9b382`](https://github.com/szara7678/OpenAkashic/commit/6c916d9aac6198de0912a97739ff95d439a9b382) — `trust_state` re-tested at the unchanged pin and **moved to a different column**. The previous record put the mark on `claim_review_status` and described it accurately: *"the public API adds a per-state term inside its ranking SQL"*, and the closed-web server multiplies by a trust multiplier. That is the ranking axis, and reading the SQL settles it — `confirmed` adds 0.14, `disputed` subtracts 0.18, `superseded` 0.42 and `merged` 0.30, in the same sum as the claim-role weight and the confirm and dispute counts. A superseded claim is penalised and still returned if it scores well enough, which is the definition this mark excludes. The column that does withhold is `claims.status`, which the record did not mention: `pending`, `accepted` or `rejected`, CHECK-constrained, indexed, defaulting to `pending` in the schema, and filtered by every retrieval path — the vector query, the ranked query and the expansion query all carry `c.status = ANY(%(statuses)s)`, and a caller who names no status gets `["accepted"]`. One weakening belongs with it: the API's own write schema defaults `status` to `accepted`, so a claim arriving through the API is admitted immediately and the database's `pending` default only applies to a writer that omits the column. Nothing was installed and no suite was run.
