...
revision: 561d5697a4c52057fee9d49bf3c62a0472537590
revision_url: https://github.com/asuramaya/osiris/commit/561d5697a4c52057fee9d49bf3c62a0472537590
analyzed_at: 2026-09-19
capabilities: "trust_state, audit_log"
capability_evidence:
  trust_state: "a stored object status maintained as a projection of an append-only event log, filtered on by the reads alongside the merge pointer, and reversible by a compensating action that keeps the original event as a witness | src/actions/core.py:7-12 (the model), :610 (`merge_objects`), :680-700 (`unmerge`), :789-812 (`set_status`), src/mcp_server.py:3062, :4204, src/cli.py:4852 | the actions layer states the model — merges are event-sourced, `object_events` is the source of truth and `objects.status` and `merged_into` are a projection updated alongside. The predicate is applied widely rather than in a few places: `status = active` appears at over three hundred sites and `merged_into IS NULL` at thirty-six, so a merged object stays in the graph and stops being returned. Two details make the state reversible rather than terminal. `set_status` takes a `justification` and an `actor` as required arguments, writes them into an append-only `object_events` row, and only then updates the projection — a transition that cannot be made without recording why. And unmerge is a first-class compensating act whose docstring cites the constitution — heal with events, never delete — notes that the original merge event and its same_as link stay as witnesses of the era, and names the defect it was first used for | tests/"
