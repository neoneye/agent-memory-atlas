...
stack_source: "seeded"
capability_evidence:
  trust_state: "seven claim states of which two are current, applied as the same SQL literal in eight queries and as a Rust method in one place, with nothing binding the two | crates/store/src/claims.rs:20-64, and the eight occurrences of the predicate in that file, crates/pipeline/src/lib.rs:905 | `ClaimState` is `Active`, `Disputed`, `Unsupported`, `Superseded`, `Retracted`, `Suppressed`, `Expired`, and `is_current()` admits only the first two — so five of the seven withhold a claim, and `Disputed` is deliberately not one of them, since a contested claim is still the current answer rather than a retired one. The parser errors on a value outside the seven rather than defaulting, so an unrecognised state fails closed at the boundary. The predicate reaches the reads as the literal `state IN ('active','disputed')`, written out eight times. The maintenance hazard is that this is the same rule in two forms: the Rust `is_current()` has exactly one caller in the whole workspace, and the eight SQL strings share no constant with it, so deciding that `Disputed` is no longer current would take nine coordinated edits and nothing in the build would catch a missed one | crates/store/src/claims.rs::split_is_atomic_replayable_and_keep_false_suppresses_parent"
...
**2026-09-19** — [`4c75f9204eec9bf52ccad52b61a1d1c1adf4e1ab`](https://github.com/os-tack/ostk-recall/commit/4c75f9204eec9bf52ccad52b61a1d1c1adf4e1ab) — `trust_state` re-tested at the unchanged pin; every figure in the record checks out, including the count — the literal `state IN ('active','disputed')` appears exactly eight times. Three things are added. `ClaimState::parse` returns an error on a value outside the seven rather than defaulting to anything, so an unrecognised state fails closed where it enters. `Disputed` being *current* is a position rather than an oversight: a contested claim is still the current answer to its question, and the five states that withhold are the ones that mean retired, unsupported or suppressed. And the hazard worth a reader's attention is duplication: the rule exists twice, as `is_current()` — which has exactly one caller in the workspace — and as eight hand-written SQL strings sharing no constant with it. Changing the definition of current means nine coordinated edits with nothing in the build to catch a missed one. A sibling system in this corpus solves exactly that by registering its admission predicate with the database so the query and the application run one implementation; the contrast is the useful part. Nothing was installed and no suite was run.
