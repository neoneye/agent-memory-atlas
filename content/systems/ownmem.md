...
capability_evidence:
  ...
  trust_state: "a nine-state machine with explicit transitions, three terminal states, and one state defined by having no path into a delivered context | lib/memory-lifecycle.mjs:3-13, :23-28, :31-41, :73, lib/memory-candidates.mjs:124-126, lib/memory-promotion-policy.mjs:63, :336, :356-359, lib/memory-trust-store.mjs:89, :143-144, :154, schemas/promotion/decision.schema.json:20-32 | the lifecycle is `observed`, `candidate`, `shadow`, `advisory`, `active`, `stale`, `deprecated`, `rejected`, `superseded`, with a frozen transition map and `deprecated`, `rejected` and `superseded` as terminal empty sets. The state that carries the mark is named in a source comment — `candidate` is *\"the one state with no path into a delivered context\"*, while `observed` is a lifecycle a memory can be recalled at — and the transition map makes the ladder real: a candidate may only become `shadow`, `advisory` or `rejected`, never `active` directly, and an `active` memory may only fall to `stale`, `deprecated` or `superseded`, never step back to advisory except through `stale`. Promotion is bounded by authority rather than by the ladder alone: `PROMOTION_LADDER` indices are compared against an authorised tier, so nothing promotes past what the authority permits, and risk runs `R0`-`R5` beside it with `automation` as `auto`, `review`, `pr-only` or `forbidden`. The history is verified rather than asserted — a receipt's `lifecycle` must equal its own `transition.to` and the previous receipt's `lifecycle` must equal its `transition.from`, or the store raises *memory trust history is discontinuous* | once a memory is `active`, a change must be a `delta` or a `structured-merge` rather than a wholesale rewrite, so the content under an approved state cannot be quietly replaced"
  ...
  human_review: "promotion — review is a state the automation cannot leave, not a screen | schemas/promotion/decision.schema.json:32, :55-58, schemas/promotion/review-material.schema.json, lib/memory-review-material.mjs, lib/memory-candidates.mjs:300-302, :340-341 | `automation` is a required field of every decision and its `review` and `pr-only` values are reachable only by a person; the schema forces R4 and R5 away from `auto` regardless of configuration, and a decision that is `auto` while naming a blocker is rejected as *\"self-contradictory, and it is the exact shape a bug in the resolver would produce\"*. The chain under it is derivation rather than assertion: `promotionRisk` maps a validated `change_kind` and `logical_type` onto the risk — `governance_policy` to R5, `repository_artifact` to R4, `normative` to R4 *\"never promoted automatically\"* — throwing on an unknown kind and on a `retrieval_metadata` change that names a field outside the metadata-only whitelist, because such a field *\"changes what a memory asserts or who may act on it\"*. And where the chain does rest on something declared, the system says so instead of hiding it: `review-material.schema.json:70-73` requires a `logical_type_source` of `declared-type`, `declared-authority`, `caller-supplied` or `not-applicable`, with the description naming the failure it exists to expose — *\"Never a regex over the body: grading a lesson as procedural because its prose contains the word `run` is the drift this field exists to make visible\"*. Candidates are emitted with `requires_human_review: true` and `can_authorize_actions: false`, and rejecting one throws without a written reason. `review-material.schema.json` types what a reviewer is shown as `correction-candidate` or `proposed-change` | schema validation; the suite was not run"
...
**2026-09-19** — [`14f4edec52ebc5d1dcf7794f451d3e6d2c64e969`](https://github.com/grpcer/ownmem/commit/14f4edec52ebc5d1dcf7794f451d3e6d2c64e969) — `trust_state` re-tested at the unchanged pin. The record quoted the right sentence — `candidate` is *"the one state with no path into a delivered context"* — and anchored it to a JSON schema, where the machinery is not. Three files carry it. `memory-lifecycle.mjs` holds the frozen transition map: nine states, three terminal ones with empty successor sets, and a ladder that means something, since a candidate may only reach `shadow`, `advisory` or `rejected` and never `active` directly, while an `active` memory may only fall and can return to advisory only by way of `stale`. `memory-promotion-policy.mjs` bounds promotion by an authorised tier rather than by the ladder alone, comparing indices so nothing climbs past what the authority permits. And `memory-trust-store.mjs` makes the history checkable instead of asserted: a receipt's `lifecycle` must equal its own `transition.to`, the previous receipt's `lifecycle` must equal this one's `transition.from`, and a break raises *"memory trust history is discontinuous"*. The same file closes the gap that lets an approved state drift away from what was approved — once a memory is `active`, a change must be a `delta` or a `structured-merge`, not a wholesale rewrite. One aside worth keeping: `R0` has no producer and the comment above it says why it is kept rather than pruned, because it grades a promotion candidate whose blast radius is metadata alone and *"removing it would delete the one level the promotion design starts from"* — a dead enum member with a written defence, which is the rarer thing to find. Nothing was installed and no suite was run.

**2026-09-18** — [`14f4edec52ebc5d1dcf7794f451d3e6d2c64e969`](https://github.com/grpcer/ownmem/commit/14f4edec52ebc5d1dcf7794f451d3e6d2c64e969) — re-read at the same commit; `main` has not moved. The `human_review` mark was re-tested against the [narrowed rubric](../../methodology/atlas-rubric/#human-review-surface), which nineteen of twenty-four marks failed elsewhere in the corpus, and it holds on the strongest ground available: the constraint is in the schema, and the schema says why it is there rather than only in the policy — R4 and R5 *"can never be encoded as automatic, whatever the evidence or the repository configuration says… written here as well as in the policy so that a producer building a decision by hand is bound by it too."*

What the record gains is the chain beneath that. The risk is derived, not declared: `promotionRisk` maps a validated change kind and logical type onto R0–R5, throws on an unknown kind, and refuses a `retrieval_metadata` change that names a field outside the metadata-only whitelist because such a field *"changes what a memory asserts or who may act on it"*. The one input that can still be supplied rather than computed is `logical_type` — and rather than leave that implicit, `review-material.schema.json` requires a `logical_type_source` beside it whose enum includes `caller-supplied`, under a description that names the exact drift it exists to expose: *"Never a regex over the body: grading a lesson as procedural because its prose contains the word `run`."* A system that records the provenance of its own classification is answering the question this atlas usually has to ask from outside.
