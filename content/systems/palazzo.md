...
capability_evidence:
  audit_log: "every mutation, with the destructive half refusing to proceed when its own entry cannot be written | src/wal.rs:42-65, :68-82, src/mcp.rs:1044-1061, :1243 | one 129-line module with two deliberately different methods. `log` is best-effort for non-destructive operations — an unconfigured path is *\"a deliberate no-op\"* and a write failure only warns. `log_strict` returns an error when the entry cannot be appended **including when no WAL path is configured at all**, and its doc comment states the rule: destructive operations *\"call this and abort before touching Qdrant: the WAL is their only audit trail, so a delete that can't be logged must not happen.\"* Both call sites are deletes and both propagate with `?`, the first carrying the context *\"WAL append failed — refusing to delete without an audit trail\"*, per id, inside the loop and before the Qdrant call. The entry is a JSONL line of timestamp, operation and params, and for a delete the params keep wing, room, hall, category, a text preview, session and the caller's stated reason. One word in that doc comment overstates it: `append` is `OpenOptions::new().create(true).append(true).open(path)` followed by `writeln!`, with no `sync_all` or `sync_data` anywhere in the module, so the entry is *ordered* before the delete but not durable against a machine that loses power between the two — what the mark rests on is the ordering and the refusal, not the word `durably` | src/wal.rs:86-128 — three inline tests: `log_strict_errors_without_path` asserts the unconfigured case errors with *not configured*, `log_strict_appends_jsonl` asserts two calls produce two lines with the operation, the params and a `Z`-suffixed timestamp, and `log_without_path_is_silent_noop` pins that the best-effort path does not panic"
...
**2026-09-18** — [`07a788ac1f4c06343d697a149857e4df9a621a8a`](https://github.com/calibrae/palazzo/commit/07a788ac1f4c06343d697a149857e4df9a621a8a) — re-read at the same commit; `main` has not moved, so what is here is this report's. The mark holds and its record was thin — no anchors, and a test field reading *unknown* when `src/wal.rs` carries three inline tests, one per branch of the design. The record now names the split that makes the mark: `log` is best-effort and warns, `log_strict` errors when the entry cannot be appended *or when no WAL path is configured at all*, and both delete call sites propagate it with `?` before Qdrant is touched.

One correction, to a word in the project's own comment rather than to the mechanism. `log_strict` is documented as erroring when an entry cannot be *durably* appended, and `append` opens the file, `writeln!`s a line and drops it — there is no `sync_all` or `sync_data` in the module. The entry is therefore ordered ahead of the delete and its failure does refuse the delete, which is the whole substance of the mark; what it is not is durable against a host that loses power between the write reaching the page cache and the page cache reaching disk. An audit trail whose only copy is on the same machine as the store it audits is worth knowing the flush semantics of.
