...
capability_evidence:
  ...
  trust_state: "a five-value epistemic state that filters both search paths when a caller names one, and ranks when nobody does | src/models.rs:201-207, src/db.rs:277-291, :309-312, :9903, :9943-9946, :33889-33950, src/projection.rs:97, src/mcp.rs:1433 | `epistemic_state` is `candidate`, `verified`, `corroborated`, `rejected` or `defensively_recalled` on the entity, schema v27. Its write-side polarity is right: a value outside the vocabulary is coerced to `candidate`, the least-trusted member, rather than to anything permissive, and an enriched memory is forced back to `candidate` unless it already sits in one of the two terminal states. The read filter lives in `retain_metadata_filters` beside the category, type, workspace and agent predicates, and is exposed to an agent as an enum on the MCP tool. What a reader needs to know is that it is opt-in: the retain runs only `if let Some(ref es) = params.epistemic_state`, so a recall that names no state returns every state, `rejected` included, and the same field separately feeds a `trust_rank` projection. That is the weaker of the two polarities this mark sees — naming a state narrows a default that admits everything, rather than a flag widening a default that excludes | `recall_filters_by_epistemic_state_on_all_paths` is a genuine two-way control: candidates are asserted absent from a verified-only query *and* verified entities absent from a candidate query, on the FTS and dense arms alike"
