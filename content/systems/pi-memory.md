...
revision: 0f09c6b4b79c81f1c68a5a53238830d49d792d5c
revision_url: https://github.com/prjct-app/pi-memory/commit/0f09c6b4b79c81f1c68a5a53238830d49d792d5c
analyzed_at: 2026-09-19
capabilities: "trust_state, bitemporal"
capability_evidence:
  trust_state: "a five-value standing read through an allowlist at the storage layer, with a transition guard that refuses to move a closed fact back to a live one and a feedback path that enqueues a review rather than writing the state itself | src/contracts/memory.ts:4, src/storage/projection.ts:722 (the allowlist), :862 and :864 (retention), src/engine.ts:370-379 (`resolveFact`), :381-392 (`feedback`), src/curation/pipeline.ts:231-233 (the drain), src/curation/publish.ts:278, :341, :346 | `MemoryStanding` is candidate / supported / needs_review / contradicted / superseded; `activeFacts` selects `WHERE scope_id=? AND standing IN (supported, needs_review, candidate)`, an allowlist that excludes the two closed values rather than a denylist that has to remember them, and the publication path skips superseded and contradicted facts at three further points. The same standing drives retention: the eviction query treats superseded and contradicted rows as collectable, and candidate or needs_review rows with no recorded use past a cutoff. The producer is `resolveFact(factId, standing, rationale, replacementId?)`, which requires a rationale, redacts it before commit, validates that a replacement is a different existing fact, and refuses a reopening outright — a closed interval cannot be reopened without losing history, record a new fact instead. A `wrong` or `stale` feedback signal does not write a standing; it enqueues a review job, and the curation pipeline drains that action rather than leaving it to accumulate | tests/curation-safety.test.ts"
