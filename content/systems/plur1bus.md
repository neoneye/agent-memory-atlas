...
description: "Per-agent LanceDB memory for OpenClaw where every correction is versioned, evidenced and logged — and where the one gate that would refuse a bad correction is switched off by its only caller."
root: ../..
page_kind: system
source_name: "Cyb3rb1ade/openclaw-plur1bus-memory"
source_url: https://github.com/Cyb3rb1ade/openclaw-plur1bus-memory
revision: 241aac282e20c40819bdfffef0f9ce7115abd936
revision_url: https://github.com/Cyb3rb1ade/openclaw-plur1bus-memory/commit/241aac282e20c40819bdfffef0f9ce7115abd936
...
matrix:
  ...
  trust: "Seven-state record status and a six-level trust ladder, plus an append-only reconsolidation event log"
  ...
  risks: "Forty-seven config groups over one careful core; the semantic-drift gate is disabled by the only caller that uses it"
---

## 1. Executive Summary

PLUR1BUS is an MIT-licensed memory plugin for OpenClaw — around 9,300 lines in `index.js`, 55,000 across `lib/`, and a further 70,000 in `tests/` and `test/`, which is the first unusual thing about it: the test suite is larger than the implementation it covers, across 312 test files.
...
Where it is weakest: the system can express doubt and cannot act on it. A record carries a status from a seven-value set and a trust level from a six-value ladder, and `conflict`, `demoted` and `untrusted` are all wired into the ranking arithmetic as penalties of 0.3, 0.35 and 0.3. A memory the system has flagged as contradicting another memory is ranked slightly lower and then injected into the prompt.
...
So the states that withhold a memory are the deletion states, and the states that express doubt are ranking adjustments. The mechanism for filtering on status exists, is used, and is pointed at the wrong end of the vocabulary — a record the system has marked `conflict` outranks a `candidate` record about anything sufficiently on-topic.
...
- **`lib/`** (206 files, 54,682 lines) — `recall-pipeline.js` (1,903), `neo-arch.js` (2,356), `obsidian-control-room.js` (3,918) and `obsidian-bridge.js` (2,035) dominate; `safe-update.js`, `acl-middleware.js`, `memory-history.js` and `contradiction-detector.js` carry the correction path.
...
One directory in the tree is not part of the system. `plur1bus/` holds a 4,080-line `index.js` and a partial `lib/`, added on 11 June 2026 in the commit *"plur1bus 5.0.0: generalized plugin for distribution"*, referenced by nothing and excluded from the published `files` list. Reading it produces answers about a version two majors old.

## 4. Essential Implementation Paths

### Capture — `index.js:6723`
...
### The drift gate's only caller — `index.js:6412`

`/correct <old> to <new>` runs a confirmation token exchange, then calls `safeUpdate` with `updateSource: "telegram:/correct"`, an evidence string, and `skipDriftGate: true`. Every other reference to `skipDriftGate` in the tree is in a test. So at this commit the gate does not run in production: no in-tree caller reaches it with the gate on.

Both halves of that are defensible on their own. A user typing an explicit correction has more authority than a cosine distance, and a gate that refuses "I moved from Berlin to Tokyo" for being too different from the memory it corrects would be worse than useless. But then the gate protects nothing, because the automated bulk paths that *would* benefit from it — consolidation, dreaming, conflict resolution — do not call `safeUpdate` at all.
...
- **Doubt does not withhold.** `conflict` and `demoted` and `untrusted` are ranking penalties. The system can record that it does not believe something and will still put it in the prompt.
- **The drift gate is off** wherever it could fire.
...
**I ran nothing.** The screen flagged both `package.json` and `package-lock.json` as changed within the seven-day cooldown, so the tree was read and not installed. Everything below is read from committed test code.

312 test files under `tests/` and `test/`, 69,535 lines — larger than the implementation. `npm test` is `node --check` on three modules followed by `node --test` over both directories, so there is no framework dependency to install.
...
- **A safety gate whose callers all disable it.** Either the default is wrong for the real caller and should change, or the caller is wrong. A gate that never fires is a comment.
...
- **Leaving a superseded copy of your own source in the tree.** A two-major-old duplicate that nothing imports is a trap for every reader and every agent that greps.
...
- Would the drift gate fire on real corrections if it were enabled, or was `skipDriftGate: true` added because it fired on ordinary ones? The commit history for `index.js:6426` would answer this and reading it was outside this review.
