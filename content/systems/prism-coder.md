...
revision: 5e509052d400cafb96479dfc3fee905dc7c7c279
revision_url: https://github.com/dcostenco/prism-coder/commit/5e509052d400cafb96479dfc3fee905dc7c7c279
analyzed_at: 2026-09-17
capabilities: "scope_enforced, negative_eval"
capability_evidence:
  scope_enforced: "the session ledger — every read of the memory store | src/storage/sqlite.ts:1742-1782, src/config.ts:170, src/tools/ledgerHandlers.ts:890, :1084, :1178,:950,:968,:1047 | `searchMemory` builds its WHERE from four conditions before any caller option is considered, and `l.user_id = ?` is unconditional among them, beside `l.archived_at IS NULL` and `l.deleted_at IS NULL`; `project` and `role` are appended only when the caller supplies them, so the user is the fence and the project is a narrowing. The bound value is not a tool argument: `PRISM_USER_ID` is a module constant read once from the `PRISM_USER_ID` environment variable, falling back to the literal `default`, and every handler passes that constant, so a model calling `session_search_memory` cannot name a tenant. 53 `user_id = ?` predicates and 29 `archived_at IS NULL` filters across the storage layer apply the same pair to the listing, handoff, rollup and health paths. The limit is the default: an operator who never sets the variable has one tenant called `default`, and the config comment scopes the field to \"multi-user Supabase instances\" | tests/load/hivemind.test.ts:336-418, which drives ten concurrent workflows into one store and asserts each project's ledger holds exactly its own single entry"
...
**2026-09-17** — [`5e509052d400cafb96479dfc3fee905dc7c7c279`](https://github.com/dcostenco/prism-coder/commit/5e509052d400cafb96479dfc3fee905dc7c7c279) — re-read after 17 commits. Four of the six anchored files are byte-identical, including the SQLite store and all three cited test files, so `negative_eval` rests on unchanged code. `config.ts` and `ledgerHandlers.ts` moved and both cited lines were read directly: `PRISM_USER_ID` is defined identically, one line lower, and the ledger still stamps `user_id: PRISM_USER_ID` on the write — now at three call sites rather than one, each of which was checked. Both marks hold. Nothing was installed, built or run.

**2026-09-12** — [`5e509052d400cafb96479dfc3fee905dc7c7c279`](https://github.com/dcostenco/prism-coder/commit/5e509052d400cafb96479dfc3fee905dc7c7c279) — first reading, at the default branch's head, a commit dated 10 September 2026. Screened before reading: five auto-run surfaces, all read first — a Claude Code plugin marketplace manifest, a `.claude/settings.json` whose `PostToolUse` hook invokes a script absent from the tree, an **empty** `.gitmodules`, and MCP and Smithery manifests declaring the published npm package as the start command; three build-time execution paths (an npm `postinstall`, a `prepublishOnly` chain, a Python `adapters/python/setup.py`); four dependency surfaces changed the day before the pin, inside the seven-day cooldown; three unpinned surfaces including 33 floating ranges above a present lockfile; one uninstalled git-hook payload under `scripts/knowledge-ingest/`; and `GEMINI.md` read as data. Nothing was installed, built or run, and no test in this repository was executed. Licence is Apache-2.0 per `LICENSE` and `package.json`.
