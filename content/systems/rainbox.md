...
capability_evidence:
  ...
  human_review: "AssistantWriteIntent, a confirm-tier queue over assistant memory writes, whose executor the assistant's own dispatch cannot reach | source/db/models.py:1212-1221, source/agents/assistant.py:6838-6848, source/agents/assistant_writes.py:50-85, source/webapp/chat_api.py:830-840 | a confirm-tier write is proposed and must not execute until an operator approves it, and the payload is bound by `payload_hash` so \"a confirmed intent executes exactly what was previewed — the assistant cannot mutate it after confirmation\". The state machine is `proposed -> confirmed -> executing -> completed | failed` with `rejected` and `undone` as terminal states. What answers the producer test is the call graph: `_propose_write` records the intent instead of dispatching, under a docstring saying the write runs only through `execute_write_intent` *\"after the operator approves — so a confirm-tier write can never execute inline, by code, not prompt discipline\"*, and `execute_write_intent` has exactly one caller, the HTTP confirm route, which resolves the approver with `db.get_human_user()` server-side rather than taking an identity from the request. The executor also refuses an intent not in `proposed` and one whose payload hash no longer matches | source/agents/test_assistant_writes.py"
