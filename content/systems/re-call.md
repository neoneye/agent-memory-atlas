...
revision: 1994fd256820df7970eac1f1d588a1961da16edf
revision_url: https://github.com/GiulioDER/RE-call/commit/1994fd256820df7970eac1f1d588a1961da16edf
...
capability_evidence:
  ...
  human_review: "a review gate the type system enforces, refusing without a reviewer, a timestamp and a note | recall/promotion.py:99-110, :200-210, recall/cli.py:623-626, recall/cli_commands/extract_rewrite.py:398, recall/rewrite.py:26 | an extracted proposal moves through `proposed → reviewed → accepted → promoted`, and `review_proposal` calls `_require_review_fields`, which raises unless a non-blank reviewer identity, a review timestamp and a non-blank audit note are all present. The gate is structural rather than conventional: downstream functions cannot accept an unreviewed proposal because they take a `PromotedFact`, and \"only `promote_accepted_proposal` produces one\". Both call sites are CLI commands a person runs with `--reviewer` and `--note`, so a model-extracted claim reaches the corpus through a person who has to say who they are and why. The limit worth stating is that `reviewer_id` is a supplied string: it records who claims to have reviewed, not who authenticated | recall/rewrite.py:26 states the type argument, and the review fields are required rather than defaulted, so a caller passing nothing fails rather than writing a blank reviewer"
