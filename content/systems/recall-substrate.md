...
capability_evidence:
  trust_state: "the cell lifecycle behind a named `active()` accessor, not the verification field the previous record named — that one caps a confidence number | src/types.ts:162, :255, src/programs.ts:173, :228, :247, src/guidance.ts:94, :161, src/subgraph.ts:74, :92, src/evals.ts:187, src/migrate.ts:71-72, src/firewall.ts:68-100, src/types.ts:112 | a cell's `status` is `active`, `superseded` or `annexed`, and the store interface declares `active(): Cell[]` with the predicate written in a comment beside it, so the admission rule is part of the contract rather than a filter each caller remembers. Guidance assembly, program selection, subgraph candidates, the eval harness and the CLI all read through it, and the subgraph path notes that a SQLite fast path and the app-side filter are the same rule expressed twice. `annexed` absorbs the older `archived` on migration. The field the previous record cited is a different axis doing a different job: `verification` is `unverified`, `checked`, `tested` or `external`, and its only consumer is `hasSupportEvidence`, which feeds `attenuateConfidence` — an unsupported claim asserting more than 0.7 is capped at 0.7 with a warning and an attenuation record. That is a ceiling on a number, not an admission decision; the claim is still stored and still retrieved | `lifecycle` in this codebase is a facet tag array a caller may filter a subgraph on, not a state machine — worth knowing before grepping for it"
...
**2026-09-19** — [`b448f24e85309d3a3adc56bc1ad1aaca5d920d89`](https://github.com/H-XX-D/recall-memory-substrate/commit/b448f24e85309d3a3adc56bc1ad1aaca5d920d89) — `trust_state` re-tested at the unchanged pin and **moved to a different field**. The record rested on `verification`, and following it to its only consumer settles what it is: `hasSupportEvidence` treats `checked`, `tested` or `external` as support, and `attenuateConfidence` caps an unsupported claim that asserts more than 0.7 back to 0.7, with a warning and an attenuation record. That is a ceiling on a number — the claim is still stored, still retrieved, and still usable at 0.70 — which is the confidence axis rather than the admission one. The record's second clause named a *lifecycle axis filtering the read path*, and `lifecycle` in this codebase is a facet tag array (`handoff`, `session`) that a caller may filter a subgraph on, not a state machine. What does withhold is `Cell.status` — `active`, `superseded` or `annexed` — reached through `active(): Cell[]`, declared on the store interface with its predicate in a comment beside it and read by guidance assembly, program selection, subgraph candidates, the eval harness and the CLI. The subgraph path records that the SQLite fast path and the app-side filter are the same rule written twice, and migration folds the older `archived` into `annexed`. The mark stands on that; the attenuation is worth keeping in the record beside it, because a ceiling on a credential is a good mechanism that answers a different question. Nothing was installed and no suite was run.
