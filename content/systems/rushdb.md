...
**2026-09-18** — [`67214ab66769feb4ff05ae3885ef4696bcf9a233`](https://github.com/rush-db/rushdb/commit/67214ab66769feb4ff05ae3885ef4696bcf9a233) — re-read at the same commit; `main` has not moved, so the correction is this report's. **Human review withdrawn.** The queue itself is exactly as described — `analyze` writes rows with `origin: 'llm'`, a confidence, a rationale and `status: 'suggested'`, and nothing MERGEs an edge until a row reaches `approved`. What the record did not check is where the approving verb lives. `packages/mcp-server/tools.ts:933` publishes `approveRelationshipPattern` as an MCP tool, annotated `WRITE`, taking the suggestion's `id` — and it sits in the same forty-two-tool list as `analyzeRelationshipPatterns`, which creates the suggestions, and `deleteRelationshipPattern`. The model proposes and the model approves.

Its description asks for better: *"Call listRelationshipPatterns first so the user can review the inferred source, target, direction, type, mode, and confidence."* That is the right instinct written in the one place that cannot enforce it — a sentence in a tool description is a request to the model, not a constraint on it, and this atlas does not treat prose as a producer test. The dashboard page at `suggested-relationships.tsx` is a second door onto the same verb rather than a gate in front of it. `scope_enforced` and `negative_eval` hold.
