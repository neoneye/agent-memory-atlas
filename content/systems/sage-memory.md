...
revision: 43dcd61ede09a72b4087dc663d90e59227e315c7
revision_url: https://github.com/l33tdawg/sage/commit/43dcd61ede09a72b4087dc663d90e59227e315c7
...
capability_evidence:
  ...
  human_review: "an operator adjudicates a memory from the dashboard behind a live on-chain Admin check, and on a domain with two modify-verb holders the verdict only parks it | web/handler.go:3560-3590, web/appv23_access_handler.go:555-602, internal/abci/app.go:598-610, :656-659 | the CEREBRUM operator selects memories and confirms a delete, which builds a challenge transaction carrying the reason `deprecated by user in CEREBRUM` and broadcasts it. Two checks stand in front of it. The route is loopback-restricted behind a passphrase, refusing with *\"Open CEREBRUM through localhost on this machine, unlock its encrypted vault when configured, or use the exact local node-operator identity\"*; and once app-v23 is active `requireAppV23ControlActor` resolves the caller against live committed state on every request — an active enrollment of the current root generation, a role of Admin, and a policy validation — so a retired root or a historical transport key fails closed. Then the verdict itself is not final where it matters: with at least two holders of a modify verb on the domain, the app-v17 rule parks the memory as `challenged` until a SECOND distinct holder confirms or reinstates, rather than deprecating it outright. The stated limit is the other side of that rule: with one holder or none, the legacy one-strike deprecate still applies | e2e Playwright specs (203, not wired to a runner), web tests (710)"
...
**2026-09-19** — re-pinned to [`43dcd61ede09a72b4087dc663d90e59227e315c7`](https://github.com/l33tdawg/sage/commit/43dcd61ede09a72b4087dc663d90e59227e315c7), 49 commits and 94 files on, at v11.23.1. **All six marks hold**, and each one's core mechanism was re-read at its current address rather than assumed: `FindByContentHash` still selects `WHERE content_hash = ? AND memory_id != ? AND status != 'proposed'` (`internal/store/sqlite.go:5101`), the live-status predicate is still `status IN ('committed','challenged')` (`:521`, `:1760`), and the domain filter is still `AND domain_tag = ?` (`:2109`). The change worth the space is on the review path, and it strengthens the mark: the operator forget route now layers a second check over the loopback-and-vault one. When app-v23 is active, `requireAppV23ControlActor` (`web/appv23_access_handler.go:555-602`) resolves the caller against live committed state on every request — an active enrollment of the current root generation, a role of Admin, and a policy validation — so a retired root or a historical transport key fails closed rather than passing on a cached credential. The app-v17 two-phase rule underneath is unchanged: with at least two modify-verb holders on the domain a challenge parks the memory as `challenged` until a second distinct holder confirms or reinstates, and with one holder or none the one-strike deprecate still applies. The evidence record is rewritten around both. Screened again at this commit; nothing was installed, built or run, and the machine still carries no Go toolchain.
