...
revision: 7bd02dabaebee41e419f72fffc45d26a823715b5
revision_url: https://github.com/rahilp/second-brain-cloudflare/commit/7bd02dabaebee41e419f72fffc45d26a823715b5
analyzed_at: 2026-09-19
capabilities: "trust_state, scope_enforced, negative_eval"
capability_evidence:
  trust_state: "a three-value status carried as a tag rather than a column, excluded from recall and from graph traversal, and used on the write path to protect a canonical row from being overwritten | src/memory/status.ts:1-15, src/recall/search.ts:416 and :583, src/graph/traverse.ts:109-128, src/recall/math.ts:34, src/capture/entry.ts:88, :110-119, src/capture/lifecycle.ts:67, :83 | the vocabulary is canonical, draft or deprecated, and it lives in the `tags[]` array behind a `status:` prefix rather than in a field of its own. `getStatus` returns null for a value outside the three, and `withStatus` strips any existing status tag before adding one, so a row cannot carry two. Recall excludes deprecated twice — in the SQL as a `tags NOT LIKE` and again on the hydrated rows — and graph traversal returns readable and deprecated as separate sets rather than dropping the second silently. The status also reaches the write path: a capsule-defining entry with no status is stored as draft, and a canonical target is protected from being overwritten by a newcomer, which is stored as a duplicate candidate instead — under a comment naming the defect that produced the branch, where the route reported success for a row it never inserted. `canonical` additionally raises the recency floor, so this status both filters and, in that one place, ranks | tests"
...
**2026-09-19** — [`7bd02dabaebee41e419f72fffc45d26a823715b5`](https://github.com/rahilp/second-brain-cloudflare/commit/7bd02dabaebee41e419f72fffc45d26a823715b5) — `trust_state` re-tested. The record was a single line naming three anchors; the mechanism is larger than that and the shape of it matters. The status is not a column — it lives in the `tags[]` array behind a `status:` prefix (`src/memory/status.ts:1-15`), parsed by a helper that returns null for anything outside the three values and written by one that strips any existing status tag first, so a row cannot carry two. That is a reasonable way to keep a state in a general tag namespace, and it is worth saying out loud because the exclusion it powers is a substring match: recall's SQL filters `tags NOT LIKE '%"status:deprecated"%'` (`src/recall/search.ts:416`) and then repeats the test on the hydrated rows (`:583`). Two uses the record did not carry. Graph traversal returns `readable` and `deprecated` as two sets rather than dropping the second silently (`src/graph/traverse.ts:109-128`), so a caller can see what was withheld. And the status reaches the write path: a capsule-defining entry with no status is stored as draft (`src/capture/entry.ts:88`), and a canonical target is protected from being overwritten — the newcomer is stored as a duplicate candidate instead (`:110-119`) — under a comment naming the defect that produced the branch, where the route returned an id for a row it had never inserted. `canonical` also raises the recency floor (`src/recall/math.ts:34`), so this status filters in three places and ranks in one, which the record now says. Screened again first; nothing was installed and no suite was run.

**2026-09-16** — [`7bd02dabaebee41e419f72fffc45d26a823715b5`](https://github.com/rahilp/second-brain-cloudflare/commit/7bd02dabaebee41e419f72fffc45d26a823715b5) — re-read after 39 commits, at app and Worker 3.3.0. `src/memory/status.ts`, `src/recall/math.ts` and `src/capture/duplicate.ts` are byte-identical, so the trust-state ladder and the duplicate scope predicate are exact; `graph/traverse.ts`, `capture/share.ts` and `staleness/pass.ts` moved and were re-derived. All three marks hold. The finding added to section 9 is a dashboard success guard that enumerated known failure shapes and so reported an unenumerated one as success — a drain that moved nothing and errored on every page rendered a green checkmark. Re-screened at this commit: one build-time execution path, two floating versions, four manifests inside the cooldown. Nothing was installed, built or run.
