...
**2026-09-19** — re-read at the same pin [`d0e40251d9de61b3340b8d0d7d83150669f1885a`](https://github.com/mindverse/Second-Me/commit/d0e40251d9de61b3340b8d0d7d83150669f1885a), still the tip. **The mark holds.** The first reading was made on 2026-09-17, a day before the `human_review` wording narrowed, so it was re-tested against the requirement that a memory wait in a state until a person resolves it. It does, and the reason is one the previous record implied rather than showed: embedding is not a post-upload trigger. `generate_document_embeddings` is a step of the training process, iterating the document list when a person starts training, so an uploaded file sits as an L0 row with no vector in either Chroma collection and nothing retrieves it. The seven-step cascade behind the memories page removes it before that — the `memories` row, the file on disk, the document embedding, the chunk embeddings, the `ChunkModel` rows and the `Document` row. Both limits already on the record were re-verified and stand: each Chroma deletion sits in its own `try/except` that logs and continues, so a partial delete leaves only a log line; and the cascade reaches neither the versioned L1 biography nor the trained weights, so the review is pre-hoc only with respect to the layer that cannot be edited. Screened again first; nothing installed or run.
