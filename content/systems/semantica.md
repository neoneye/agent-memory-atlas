...
revision: de2e200e74b16c74c6c9c1529ce37735b758561c
revision_url: https://github.com/semantica-agi/semantica/commit/de2e200e74b16c74c6c9c1529ce37735b758561c
...
**2026-09-18** — [`de2e200e74b16c74c6c9c1529ce37735b758561c`](https://github.com/semantica-agi/semantica/commit/de2e200e74b16c74c6c9c1529ce37735b758561c) — re-pinned from `7057387`; 125 files and +28,054 lines, re-screened at the new pin. **Human review withdrawn**, on the mechanism the record cited and on the better one it did not.

The cited mechanism is the Explorer's Markdown editor: a person opens a stored item through `GET /api/markdown/{kind}/{id}` and writes it back through `PUT`, sending the revision they read, and `apply_item_markdown` takes the memory lock and compares it. That is optimistic concurrency over an editor — careful, and a write rather than an adjudication. Nothing waits in a state for anyone; the model's write has already landed and a person may change it afterwards.

The better candidate is one the record missed, and it fails for a different reason. `semantica/explorer/routes/ontology.py:3396-3408` does define `POST /proposals/{id}/approve`, it is an Explorer route rather than an MCP tool, and the agent's surface carries no proposal verb — which is the shape that passes. But the handler sets `proposal.state = "approved"` on an object from `_get_proposals`, which is `request.app.state.ontology_proposals`, a dictionary created lazily on the web app and living in the process. Nothing anywhere reads `state == "approved"`. The queue does not survive a restart and the approval reaches no store, so it is scaffolding rather than a gate. `bitemporal`, `audit_log` and `negative_eval` hold.
