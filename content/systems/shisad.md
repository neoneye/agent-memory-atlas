...
capability_evidence:
  trust_state: "a three-value trust band derived from a validated triple, and an equality against the top band that gates the identity surface before anything is ranked | src/shisad/memory/trust.py:8-36, :88-101, :170-185, src/shisad/memory/surfaces/identity.py:40-58, src/shisad/memory/ingestion.py:130, :216-221 | `_VALID_TRUST_MATRIX` maps `(source_origin, channel_trust, confirmation_status)` to a `TrustRule` carrying a `TrustBand` of `elevated`, `observed` or `untrusted` plus a confidence and a confidence mode, so the band is derived from a validated triple rather than asserted by a writer. `build_identity_pack` is where it bites, and the shape is the clean one: a generator expression applies four admission conditions — the entry type is an identity type, `superseded_by is None`, the band is `elevated`, and the source origin is not on the blocked list — and only the survivors are sorted, by `importance_weight * decay_score` and creation time. So the band is an equality against the top of three, not a weight in the ordering, and the ordering runs on a set the band has already decided. The two guards are independent: a source origin can be barred from identity even when its band is elevated | the confidence rides beside the band as a separate field with its own mode, which is the axis the sort uses and the admission does not"
...
**2026-09-19** — [`e4e33e5980ef5e47b40b3fbfce44bd45e7faec0b`](https://github.com/shisa-ai/shisad/commit/e4e33e5980ef5e47b40b3fbfce44bd45e7faec0b) — `trust_state` re-tested at the unchanged pin; the mark holds and `build_identity_pack` is worth quoting rather than summarising. Its docstring is *"Filter and budget elevated identity entries"*, and the code is a generator expression carrying four admission conditions — identity entry type, `superseded_by is None`, `trust_band == "elevated"`, and a source origin not on the blocked list — wrapped in a `sorted()` whose key is `importance_weight * decay_score` and creation time. Filter first, rank after, in one expression, with the band tested by equality against the top of three rather than contributing a weight. Two of those four conditions are about trust and they are independent: a source origin can be barred from the identity surface even when the triple it produced earns an elevated band, so a single compromised channel cannot buy its way in through the matrix. And the confidence that the trust rule also carries is the axis the sort uses and the admission does not, which is this mark's distinction laid out in six lines. Nothing was installed and no suite was run.
