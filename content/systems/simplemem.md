...
capability_evidence:
  ...
  trust_state: "a three-value memory status filtered by the listings, written by the consolidator and the merge path rather than declared | simplemem/evolver/models.py:21-24, simplemem/evolver/store.py:469, :490, :605, :1447-1459, simplemem/evolver/manager.py:1096, :2380 | `MemoryStatus` is `active`, `superseded` or `archived`, and the listings read `WHERE scope_id = ? AND status = 'active'` — the predicate appears eight times across the store — so neither of the other two states can reach a caller. Both are written by live code rather than being a declared vocabulary: `store.supersede` sets `superseded` from three consolidator call sites and the merge path, and the archival updates set `archived`. The status sits beside the scope predicate in the same `WHERE`, which is the arrangement worth noting — one clause answers *whose memory is this* and the other answers *may it still be believed*, and neither substitutes for the other | the two withheld states are reachable by nothing on the read path, so supersession here is exclusion rather than redirection"
...
**2026-09-19** — [`db80b6a7c591e0ea730a058e9f5fc4eb06572299`](https://github.com/aiming-lab/SimpleMem/commit/db80b6a7c591e0ea730a058e9f5fc4eb06572299) — `trust_state` re-tested at the unchanged pin; the mark holds and the count is now on the record — the active predicate appears eight times across `store.py`, not on a single read. What is worth drawing out is the arrangement rather than the count: `WHERE scope_id = ? AND status = 'active'` puts the scope clause and the status clause side by side in the same query, one answering whose memory this is and the other whether it may still be believed, and the two marks this report carries rest on the two halves of that line. Both non-active states have live writers — `store.supersede` from three consolidator sites and the merge path, and the archival updates — so the vocabulary is exercised rather than declared. Unlike the systems in this corpus that redirect a superseded row to its replacement, supersession here is plain exclusion: the successor is found by the ordinary query because it is active, and the predecessor is simply not returned. Nothing was installed and no suite was run.
