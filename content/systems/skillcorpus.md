...
matrix:
  ...
  write: "Writes are a batch build, not an agent action: crawl, parse, regex safety block, structural thresholds, exact-hash duplicate check, LLM near-duplicate judgement, embed, copy the bundle, insert. Nothing an agent does at answer time writes back: no path in this tree persists a usage or outcome signal. The consumer plugin does emit one — it returns the injected skill ids in the segment metadata so the host can correlate skills shown against skills used — and nothing here consumes it. Retrieval latency is a rewrite call, a fan-out and a gate call, each with its own deadline, and a failure injects nothing rather than raising"
...
**Nothing an agent does ever writes back.** There is no usage counter, no outcome column and no promotion of a skill that worked: the corpus flows one way, from public repositories through a batch build to a read-only artifact.

The qualification matters, because a feedback *signal* does exist and stops one step short of the corpus. `skillcorpus_plugin/plugin-raven/skillsearch_raven/__init__.py:101-110` returns `injected_skill_ids` and `skill_hits_by_source` in the segment metadata under a comment stating the purpose — *"Raven's after-turn feedback correlates skills shown against skills used, which needs the ids that went in"* — and the engine carries an after-turn feedback dispatcher and its routing key (`engine-python/skillsearch/types.py:29,:89`). What is absent is a consumer: no `INSERT`, `UPDATE` or commit anywhere in the engine is tied to feedback. The system is built to hand the signal to its host and to keep it out of the library. The data model says so in its own header — `SkillRecord` is adapted from another project's and "drops the 4 counters / lineage / evolution fields (no execution tracking or evolution)" (`skillcorpus/core/models.py:1-6`). Compared with the atlas's reference for this shape, Voyager, the verification gate that makes a skill library trustworthy has been moved off the agent's execution and onto an LLM reviewer at build time.
...
| --- | --- | --- |
| ~~No feedback, usage or outcome signal anywhere in the tree~~ — **imprecise, corrected 2026-09-20** | `grep -rni "feedback\|usage_count\|outcome" --include='*.py' --include='*.ts' . \| grep -vE '(^\|/)tests?/'` | An after-turn feedback dispatcher and its routing key in the engine, and a plugin returning `injected_skill_ids` for the host to correlate |
| Nothing persists a feedback signal | `grep -rn "feedback" --include='*.py' skillcorpus_plugin/engine-python \| grep -iE "insert\|update\|write\|commit\|conn\."` | Nothing — the signal is emitted and never stored |
| The data model dropped the counters deliberately | read `skillcorpus/core/models.py:1-6` | *"drops the 4 counters / lineage / evolution fields (no execution tracking or evolution)"* |

## History

**2026-09-20** — audited at the same pin. The matrix write field and section 4 both said there is no feedback, usage or outcome signal anywhere in the tree. There is one: the Raven plugin returns the injected skill ids in its segment metadata so the host can correlate skills shown against skills used, and the engine carries an after-turn feedback dispatcher and routing key. What is absent — and what the finding rests on — is a consumer: nothing in this tree persists the signal, and the data model's own header records that the counters were dropped on purpose. Both sentences now say that, and the report gains a Recorded Searches appendix; it had none.
