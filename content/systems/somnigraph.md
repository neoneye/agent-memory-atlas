...
capability_evidence:
  trust_state: "an auto-captured memory is written `pending` and every retrieval filters to `active`, so it is absent from recall rather than ranked low until a person confirms it | src/memory/db.py:123, :143-146, src/memory/tools.py:71, :768, :953, :1712, :1533-1589 | `memories.status` defaults to `active` and `_insert_memory` writes `pending` when the caller asks for it, which is how an automatic capture enters — the agent proposes, the person confirms. The partial index `idx_memories_pending ON memories(created_at) WHERE status = 'pending'` exists so the review queue is cheap, which is the database agreeing about which value is the exception. The active predicate appears nine times across `tools.py`, covering the search, the listing and the context surfaces, so a pending memory reaches none of them | the review surface reads the pending set directly, which is the only path that sees one"
...
**2026-09-19** — [`6dc4d3497adb63df888ccf9874d33a166f65b09c`](https://github.com/AlexisOlson/somnigraph/commit/6dc4d3497adb63df888ccf9874d33a166f65b09c) — `trust_state` re-tested at the unchanged pin. Every element of the record checks: the column defaults to `active`, the partial index on the pending set is where the record said, and the active predicate appears nine times across `tools.py` rather than on one path. The detail worth drawing out is what the partial index means — an index built `WHERE status = 'pending'` is the schema stating that pending is the small exceptional set and active is the norm, and it exists to make the *review queue* cheap rather than the recall path, which is the right way round: the common read needs no help because it is a plain equality on the default. The shape is the one this atlas keeps recommending — an automatic capture proposes, a person confirms, and until they do the memory is absent from search, listing and context rather than present with a lower score. Nothing was installed and no suite was run.
