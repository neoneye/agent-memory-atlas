...
capability_evidence:
  trust_state: "the lesson — a quarantine decision computed per call from durable outcome records and applied as an exclusion before ranking, never as a weight | retriever.py:242 and :247 (computed, then passed as exclude_ids), :309 (`band_loss_rate`), :357 (`usage_stats_with_attribution`), :372 (`_attribution`), :390-418 (`lesson_quarantine` and its three gates) | a lesson is active, quarantined or on probation, and the transition is computed rather than set: `quarantined_lessons` runs on the usage statistics and its result is handed to the ranker as `exclude_ids`, so a quarantined lesson is dropped before scoring rather than pushed down it. Three gates must all agree, and the third is the one worth copying. A raw floor: five losses spanning two task formulations, or six identical-task failures, with mean reward at or below -0.5. Blame-adjusted volume, so a cluster of lessons that always fail together cannot each claim the same failures as independent evidence. And a reference class: the run of losses must be improbable for the base loss rate of the lesson own retrieval-frequency band, so a rarely-retrieved lesson is not punished for the unusual tasks that retrieval sends it. Evidence is scoped to the epoch since the last grounded success, so a lesson can recover and can also relapse, and probation gives it a production path back to a win | tests/test_retriever.py:343 `test_positive_outcome_rehabilitates_quarantined_lesson`, and :274 for the lexical-fallback interaction"
