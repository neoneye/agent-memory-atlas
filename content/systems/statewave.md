...
revision: 8d4d1e264cc9ea6487ffc572d3f60d4a1375f88f
revision_url: https://github.com/smaramwbc/statewave/commit/8d4d1e264cc9ea6487ffc572d3f60d4a1375f88f
...
capability_evidence:
  ...
  trust_state: "a three-value status on the memory row, applied as an equality filter at every read and as the guard on its own sweep, kept separate from the confidence float | server/domain/models.py:25-34 (`MemoryStatus`), server/db/repositories.py:296, :333, :348, :374, :429, :463, :545, :567, server/services/conflicts.py, server/services/memory_ttl.py:75-81 | `MemoryStatus` is active, superseded or tombstoned, stored on the row rather than derived. Conflict resolution marks an older memory superseded when a newer one restates it; the TTL sweep moves a memory past its `valid_to` to tombstoned — and the sweep is itself gated on `status == active`, so it can neither resurrect a superseded row nor tombstone one twice. Reads select `status == active` at eight sites, and the episode partition uses the same test to sort ids into alive and dead, so both non-default states withhold a memory from assembly rather than reordering it, while the separate `confidence: float` is what ranking uses. The enum comment records that the third value was renamed from `deleted` — an aspirational hard-delete state that was never wired up — to match the vocabulary the receipt supersession_status field surfaces | tests/test_conflicts.py:118-133 (the older memory is in the supersede set and the newer one is not, asserted as a pair), tests/test_memory_ttl.py, tests/test_replay.py"
...
**2026-09-19** — [`8d4d1e264cc9ea6487ffc572d3f60d4a1375f88f`](https://github.com/smaramwbc/statewave/commit/8d4d1e264cc9ea6487ffc572d3f60d4a1375f88f) — `trust_state` re-tested. The vocabulary and its rename comment are unchanged at `server/domain/models.py:25-34`. The read anchor was stale: the filter is not at `repositories.py:43` but at eight sites through that file (`:296`, `:333`, `:348`, `:374`, `:463`, `:545`, `:567`), with a ninth use at `:429` partitioning episode ids into alive and dead by the same test. One detail is worth adding because it closes a gap this sweep looks for. The TTL sweep (`server/services/memory_ttl.py:75-81`) does not simply set `tombstoned` on anything past its `valid_to` — it is gated on `status == 'active'` first, so it cannot resurrect a superseded row into a tombstoned one or tombstone the same row twice. A maintenance pass that constrains the state it transitions *from*, not only the one it transitions to, is the version of this that does not corrupt a lifecycle over time. Screened again first; nothing was installed and no suite was run.

**2026-09-16** — [`8d4d1e264cc9ea6487ffc572d3f60d4a1375f88f`](https://github.com/smaramwbc/statewave/commit/8d4d1e264cc9ea6487ffc572d3f60d4a1375f88f) — re-pinned after 3 commits. The whole diff is three lines of `pyproject.toml`: requirement bumps for litellm, ruff and alembic. No source file, test or migration changed, so every anchor, line number and quotation in this report is exact at the new pin and all four marks stand unexamined-but-unchanged. Nothing was installed, built or run.
