...
revision: c644d25b6f7e7f6e246f355c10506f735abac9a6
revision_url: https://github.com/code-ministry-ltd/the-librarian/commit/c644d25b6f7e7f6e246f355c10506f735abac9a6
analyzed_at: 2026-09-19
capabilities: "human_review, negative_eval"
capability_evidence:
  human_review: "one function produces every apply, propose or skip verdict, keyed on the operation type, and no verb on the agent's tool surface resolves a proposal | packages/core/src/curator-apply-policy.ts:1-15, packages/mcp-server/src/mcp/tools/index.ts:14-22, packages/core/src/store/librarian-store.ts:1276-1298, packages/mcp-server/src/trpc/context.ts:1-20 | the header states the contract — \"Every apply/propose/skip verdict in the system — intake apply and grooming apply — is produced HERE and nowhere else\" — and \"[t]he rule is enforced by OPERATION TYPE, never by model-self-reported risk\", the earlier `risk_level` policy levels having been removed. Archive and split, \"the only two operations that destroy or restructure information — ALWAYS propose, regardless of confidence\"; any operation on a `requires_approval` memory proposes; and a submission-level `forceProposal` hint is an override nothing auto-applies past. What answers the producer test is the other end: the MCP registry is seven tools — recall, remember, flag_memory, store_handoff, list_handoffs, claim_handoff, search_references — and none of them approves anything, while `proposalCoreForPrincipal` throws \"proposal moderation requires an admin principal\" before an approval can touch a shelf. The limit worth stating is what that principal is on the default build: the tRPC API is served only on an internal listener and the default provider resolves the trusted admin principal there without a bearer, so the boundary is network isolation rather than a credential, and a substitute auth provider is the seam for making it one | packages/core/src/curator-apply-policy.ts:7-14"
...
**2026-09-19** — re-pinned to [`c644d25b6f7e7f6e246f355c10506f735abac9a6`](https://github.com/code-ministry-ltd/the-librarian/commit/c644d25b6f7e7f6e246f355c10506f735abac9a6), two commits and five files on. **Both marks hold.** `human_review` was re-tested against the producer question rather than re-read, and the evidence record is rewritten around the answer. The apply policy is unchanged and is still the right half of it — one function, keyed on operation type and explicitly *"never by model-self-reported risk"*, with archive and split always proposing. What the old record did not say is where the proposal is resolved: the MCP registry is seven tools and none of them approves anything, and `proposalCoreForPrincipal` refuses with *"proposal moderation requires an admin principal"* before an approval reaches a shelf. The stated limit is new and matters: on the default build the tRPC API is served only on an internal listener whose default provider resolves the trusted admin principal without a bearer, so the boundary is network isolation rather than a credential — a substitute auth provider is the documented seam for making it one. The record also carried two literal pipes inside a quoted phrase, which split it into six fields instead of four; the phrase is rewritten. Screened again first; nothing installed or run.
