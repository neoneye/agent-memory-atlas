...
revision: e5f3abe1a44099d5198aa5aa74c99dab05dc9922
revision_url: https://github.com/theurian/theurian/commit/e5f3abe1a44099d5198aa5aa74c99dab05dc9922
analyzed_at: 2026-09-16
capabilities: "trust_state, scope_enforced, bitemporal, audit_log, negative_eval"
capability_evidence:
  trust_state: "a stored status and sensitivity per revision, applied before ranking rather than after, so a withheld row never occupies a candidate slot | packages/theurian-core/src/theurian/infrastructure/sqlite/schema.py:242-265, packages/theurian-core/src/theurian/application/visibility.py:1-45, packages/theurian-core/src/theurian/infrastructure/sqlite/store.py:530-545 | `knowledge_revisions` carries `status`, `trust_level` and `sensitivity` as NOT NULL columns, and `list_items_by_status(statuses, sensitivities)` is the store's own read. The visibility module states where the question must be asked and why: \"FR-R1 says filter before ranking, and the index can only filter on the status it recorded when it was built; the half of the filter that knows what is approved **now** is this one. Asking it late is what made a withheld document able to occupy a candidate slot, and every number computed from those slots — ``count``, ``usedTokens``, ``fusedScore``, ``droppedForBudget`` — move with it.\" A separate content-identity gate withholds a row whose body hash drifted from the one its revision recorded | packages/theurian-core/tests/unit/test_content_identity_gate.py:190-210"
