...
capability_evidence:
  trust_state: "a five-value status and a four-level trust, both constrained by database CHECKs — the status filtering the reads, the trust ceilinged by the calling credential rather than by the request | src/core/migrations.ts:78 and :80 (the CHECKs), :86 (the scope index), src/core/auth.ts:210-213 (`assertTrustCeiling`), src/core/claims.ts:189, src/core/federation.ts:426, :493 (the three call sites), src/core/federation.ts:287, src/core/governance.ts:425, :889-891, src/core/atlas.ts:204 (the read filters) | `status TEXT NOT NULL CHECK (status IN (active, disputed, superseded, expired, revoked))` and `trust TEXT NOT NULL CHECK (trust IN (unverified, asserted, verified, policy_approved))`, so both vocabularies are closed by the database. The status is read on: organization-visible federation admits `status IN (active, disputed)` — surfacing a disputed claim rather than suppressing it, the same choice mnemora makes — and the governance and atlas reads narrow to active. The trust level is the interesting half: `assertTrustCeiling` throws a forbidden when the requested level ranks above `principal.maxTrust`, so the ceiling is a property of the credential presented and not a field in the request. A caller cannot assert policy_approved unless its own key permits it, and three write paths call the check | src/core/claims.ts:289"
...
**2026-09-19** — [`a68ce06302f3dfc3aef04e83306ad1c35e2b058c`](https://github.com/RamaAditya49/titen/commit/a68ce06302f3dfc3aef04e83306ad1c35e2b058c) — `trust_state` re-tested at an unchanged pin. Both CHECK constraints held at `src/core/migrations.ts:78` and `:80`, and the scope index at `:86`. Two things the record asserted are now anchored, and one is sharper than it was written. The status is read on, not merely indexed: organization-visible federation admits `status IN ('active', 'disputed')` (`src/core/federation.ts:287`) — surfacing a disputed claim rather than suppressing it, the same deliberate choice mnemora makes with contested — while the governance and atlas reads narrow to active (`src/core/governance.ts:425`, `:889-891`, `src/core/atlas.ts:204`). And the trust ceiling is a credential property rather than a request field: `assertTrustCeiling` (`src/core/auth.ts:210-213`) throws a forbidden when the requested level ranks above `principal.maxTrust`, so a caller cannot assert `policy_approved` unless its own key permits it. Three write paths call it — `src/core/claims.ts:189` and `src/core/federation.ts:426`, `:493`. That is the passing shape this atlas looks for on the producer side: the level is bounded by what was presented, not by what was claimed. Screened again first; nothing was installed and no suite was run.
