...
capability_evidence:
  scope_enforced: "a project key resolved from the API key and applied as the first predicate on every ordinary read — with one resolver that omits it, and the fork path is the caller | apps/api/src/middleware/auth.ts:46-77, packages/core/src/storage.ts:61-63, packages/core/src/ops/get-context.ts:51, packages/core/src/ops/create-context.ts:82, apps/api/src/routes/contexts.ts:26-33 | The bearer token hashes to a `key_prefix` and `key_hash`, resolves to `{apiKeyId, projectId}`, and `projectId` is read from the request context by every route. The storage interface then splits in two: `findRootContext(projectId, publicId)`, `listRootContexts(projectId, …)` and the delete helpers take the key, and `getContext` resolves through the first of those, so a foreign project gets `not_found`. But `findRootContextByPublicId(publicId)` sits one line below it in the same interface and takes no project, and its single caller is the fork source resolution in `createContext` — where `projectId` is used only to stamp the new rows. A caller holding a key for one project can pass `from:` a context id belonging to another and receive a full copy of it, chain walked by `getVersions`, `findHead` and `getOrderedNodes`, none of which take the key either. It is reachable over the service, not only through the library: the `POST /contexts` route reads `projectId` from the auth context, parses the body, and passes both to `createContext` with no check that `body.from` belongs to that project. The barrier is possession of a `ctx_` id, which is twelve `crypto.getRandomValues` bytes — not guessable, and not a secret either once it appears in a log line, a bug report or a teammate's message, which is the circulation this product exists to create | packages/core/src/ops/get-context.test.ts:76 pins the read path; nothing pins the fork path"
...
where `safeRaw = redact(normalized.raw)`. It does so twice, in two places that
would have to be fixed together: `daemon.mjs:810-814` on the single-event append
and `:902-906` inside the bulk path's `sessionEvents.map`, each constructing the
same object literal with the same asymmetry. The raw transcript entry is masked
...
into a prompt is `sk-***` in `raw` and intact in `message`. The asymmetry runs
the other way too and is worth knowing before trusting either field: `redact`
truncates every string it walks to 8,000 characters, so `raw` is both masked and
clipped while `message` is neither — the two copies of one event differ in
length as well as in safety. No test in the tree mentions the redactor, and the
check has to be run against `*.mjs`: the daemon, the redactor and their tests
are ESM JavaScript, not TypeScript, so a search scoped to `*.ts` reports the
module as absent rather than untested.
...
**2026-09-18** — [`736b471125b727a9b6473688b18ef5a22727d48c`](https://github.com/ultracontext/ultracontext/commit/736b471125b727a9b6473688b18ef5a22727d48c) — re-read at the same commit; `main` has not moved since 1 June 2026 and the lockfile is 108 days old, so anything found here is this report's rather than the project's. Every claim was re-tested and every one held: `findRootContextByPublicId` still takes no project and `create`'s `from` resolution is still its only caller; `AGENT_COMPAT` is still imported by `roundtrip.test.mjs` and nothing else; the writer is still `target === "codex" ? writeCodexSession : writeClaudeSession`; and no test mentions the redactor. Three anchors sharpened. The cross-project fork is reachable over the service and not only through the library — `POST /contexts` reads `projectId` from the auth context, parses the body and hands both to `createContext` without checking that `body.from` belongs to that project. The redactor asymmetry exists at two call sites rather than one, `daemon.mjs:810-814` and `:902-906`, single-append and bulk, which is what a fix would have to cover. And `redact` truncates every string it walks to 8,000 characters, so `raw` is clipped as well as masked while `message` is neither.
