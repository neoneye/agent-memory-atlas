...
capability_evidence:
  ...
  trust_state: "a candidate ladder where the restrictive value is also the fallback, and durable memory is reachable only through the promote verb | src/pipeline/candidates.js:176-206, :266, :287, migrations/0007_candidate_review.sql | a proposal enters as `pending` and reaches durable memory only through `promoteCandidate`; `rejectCandidate` moves it to `rejected` and the suppress path to `suppressed`. The listing defaults to `status = \"pending\"` and filters `COALESCE(status, 'pending') = ?`, so a row whose status is missing reads as pending — the *restrictive* value is the one an absent field falls back to, which is the polarity that survives a migration. `AND suppressed_at IS NULL` is applied unconditionally beside it, and `status === \"all\"` is the explicit widening a caller must name. A pending candidate is not memory the recall path can return — it is a proposal waiting on a person — so the state filters rather than ranks | test/candidates.spec.js, which asserts the class"
...
**2026-09-19** — [`b17c5486553634b66b3aa70777a007928dab54d7`](https://github.com/12ziyad/universal-memory-engine/commit/b17c5486553634b66b3aa70777a007928dab54d7) — `trust_state` re-tested at the unchanged pin. The ladder is where the record said, and two details about the *listing* are worth carrying because they are the parts a later change could break. The status filter is written `COALESCE(status, 'pending') = ?`, so a row with a missing status reads as `pending` rather than as anything permissive — the fallback is the restrictive value, which is the polarity that survives a schema migration adding rows without the column. And `AND suppressed_at IS NULL` sits beside it unconditionally, outside the status branch, so suppression is not something the `status = "all"` widening can reach: a caller asking to see every state still does not see suppressed proposals. The widening itself is an explicit token rather than a boolean, which makes it visible at the call site. Nothing was installed and no suite was run.
