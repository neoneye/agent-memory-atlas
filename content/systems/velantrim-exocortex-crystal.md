...
capability_evidence:
  ...
  human_review: "the curator queue — approve, reject, resolve a conflict, in sessions — on a surface the agent's own MCP server does not carry | core/review.py:84-660, core/review_decision_store.py:165-430, core/mcp_server.py:1-2, :109-190 | `pending` lists `Observed` facts with a diagnosis, `approve` promotes a clean one or overrides a blocked gate only with a named actor and a reason, refuses a `conflict` verdict until `resolve_conflict` decides it, `reject` moves the fact to `Collapsed`, every decision is staged with a compare-and-swap on the fact's revision and projected to the canon through an outbox, and review sessions resume. What answers the producer test is the shipped MCP server, whose second line calls it *\"minimal, READ-ONLY\"* and whose registry is six tools — `search`, `memory_report`, `get_fact`, `fact_history`, `find_conflicts`, `verify_receipt` — none of which writes anything, so the agent can see the queue and cannot act on it. The limit worth stating is on the other side of that boundary: on the Python and HTTP paths `actor` is a caller-supplied string and `force=True` overrides a blocked gate, so the record of who approved is only as good as the caller | tests/test_review.py (40 cases), tests/test_review_decision_hardening.py, tests/test_review_decision_outbox.py"
...
**The gate.** `core/truth_gate.py:22-92`: no facts is a refusal; a fact
