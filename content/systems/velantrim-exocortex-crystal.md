...
description: "A local-first evidence and memory kernel where a fact enters as Observed, passes a truth gate that never lets model output become a world fact, is projected to canon only when VERIFIED and unrestricted, is adjudicated by a curator in a review queue, and every compliance event lands in a hash-chained audit log; erasure is physical, with a cascade over derived facts and a content-free tombstone."
root: ../..
page_kind: system
source_name: "velantrian/velantrim-exocortex-crystal"
source_url: https://github.com/velantrian/velantrim-exocortex-crystal
revision: 7509be14c274cdc83e8e00287f46a78f8ee33696
revision_url: https://github.com/velantrian/velantrim-exocortex-crystal/commit/7509be14c274cdc83e8e00287f46a78f8ee33696
...
matrix:
  ...
  write: "`ingest` classifies a claim, stores it as `Observed`, and admits it through the truth gate — a source is required, model output can never be a world fact, and a confidence floor applies to world facts and interpretations — into `Validated` and the canon; a blocked fact stays `Observed` for the curator"
  update_delete: "`transition_esm` moves a fact along a declared state matrix with a revision compare-and-swap; `erase_fact` physically removes a fact from L1, the graph, evidence and sessions, cascades over `DERIVED_FROM` edges when asked, refuses Ring Zero, and writes a content-free tombstone with a content hash that nothing consults on a later write"
  ...
  trust: "Eight epistemic states and six truth statuses on the fact, a source status that names model output as such, a Ring Zero of immutable facts, and a restricted bit that wins over every read"
  ...
  risks: "No scope key inside a store; the erasure tombstone carries a content hash that no write path consults, so an erased claim re-ingested from the same source is a new fact; the semantic reader, ANN index and NLI filter are declared not implemented, so retrieval is the bounded lexical and cosine path; the audit detail is content-free by convention"
---
...
Four mechanisms earn four marks. **A fact has a state and a status.** It
...
Two marks are withheld on definitions. Erasure (`core/erasure.py`) is
physical — L1, graph, evidence, sessions — cascades over `DERIVED_FROM` edges,
refuses Ring Zero, is idempotent, and writes a content-free tombstone with a
`content_hash`; nothing on the ingest path consults that hash, so an erased
claim arriving again from a source is a new fact. That is a deletion receipt,
not a rejected-value tombstone. And there is no validity interval and no scope
key: a `restricted` bit and a session-wide restriction implement a processing
restriction, not a partition.
...
```mermaid
%% caption: a fact enters as Observed, passes the truth gate into Validated and the canon or waits for a curator, who approves, rejects to Collapsed or resolves a conflict; only a VERIFIED, unrestricted, canonical fact may ground an answer, and erasure removes it physically with a content-free tombstone
flowchart TB
    IN["ingest: classify claim, require source"] --> O["Observed (L1)"]
    O --> G{"truth gate"}
    G -- "LLM_OUTPUT as WORLD_FACT: never" --> B["blocked, stays Observed"]
    ...
    ANY["any state"] -- "erase_fact (cascade over DERIVED_FROM)"--> T["gone; content-free tombstone; audit event"]
    style G fill:#f4e2bd,stroke:#b8860b
```

The highlighted node is the whole argument: the gate does not weigh relevance
and it cannot be configured to trust the model. Everything downstream — the
...
```mermaid
%% caption: sources pass through the Reader into propositions, the ingest path stores a fact in L1 and admits it through the truth gate into the L3 canon, the curator works the queue, and the read-only query path answers from strict canon or refuses; audit, provenance and erasure logs sit beside the store
flowchart LR
    ...
    REV --> AUD
    ER["erasure.py: cascade, tombstone"] --> L1
    ER --> L3
    ER --> AUD
```
...
confidence, build a facts pack, call `truth_gate`, and on pass transition to
`Validated` and merge into L3; on refusal the fact stays `Observed` with the
reason.
...
(`ready`, `blocked`, `conflict`), `approve` with the rules in section 1,
...
**Tombstone:** the erasure record carries `content_hash`, and
`rg -n 'get_tombstone|get_tombstones|content_hash' core/ingest.py core/pipeline.py`
finds nothing; the readers of the tombstone are evidence, provenance, the
projection and the compliance report. `tombstone` withheld.
...
by the caller; nothing summarises or consolidates. The gate is the write
mechanic. Two things it does that this atlas rarely sees: it lets a feeling
...
can re-verify, and reads `/evidence/{fact_id}`; a curator — human or a
`curator_runtime` — works `/review/queue`, `/review/approve`, `/review/reject`
and the UI. The agent-facing documentation (`docs/ai/README.md`) is a router
...
over derived facts and refuses Ring Zero; a force approval demands an actor
and a reason and is audited (`tests/test_force_override_audit.py`).

**What is withheld, and why.** `tombstone`: the erasure hash is written and
never consulted on ingest. `bitemporal`: no validity interval. `scope_enforced`:
restriction is per fact and per session, not per principal.
...
`test_truth_gate.py` (17), `test_evidence.py` (30),
...
- **A deletion receipt mistaken for a tombstone.** Write the content hash and
  consult it, or say the erased claim may return.
...
be: there is no extraction, no session, no scope, and the Reader is bounded
on purpose.

## 12. Open Questions

- **Will the erasure hash reach the ingest path?** The tombstone carries what
  a rejected-value check would need.
- **What does the curator runtime decide on its own**, and is a machine
  curator's approval distinguished from a person's in the audit event?
