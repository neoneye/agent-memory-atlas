...
revision: 1d33f0197696ba36903a5f1f3ca9d204e6016fc3
revision_url: https://github.com/veracium-ai/Veracium/commit/1d33f0197696ba36903a5f1f3ca9d204e6016fc3
...
capability_evidence:
  ...
  human_review: "confirmation on a call path the model cannot reach, with the actor label explicitly granting nothing | src/veracium/schema.py:94-116, src/veracium/mcp_server.py:387-471, src/veracium/__init__.py:1813-1835 | `confirm()` is the only path that clears `needs_confirmation`, and its metadata is a closed surface: `ConfirmationCallPath` has one value, `HOST_API`, documented as *\"the host API, never model-reachable\"*, and `ConfirmationActor` is `USER` or `HOST`, closed because the field used to be free-form and *\"a host could smuggle prose past the constraints on the other fields.\"* The sentence that settles the producer test is the header's own: *\"Both fields are audit metadata and grant NOTHING — authority comes from the protected `confirm()` call path, not from any label.\"* Checked against the surface rather than taken on trust: the MCP server declares six tools — `remember`, `recall`, `answer`, `maintain`, `record_procedure`, `describe_procedures` — and no confirm verb among them. The request digest is built from the caller's own inputs, so two date-less retries are the same request | tests/"
