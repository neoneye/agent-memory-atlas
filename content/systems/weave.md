...
capability_evidence:
  trust_state: "five CHECK-constrained states beside a separate confidence column, with a widening flag that reaches exactly one of the four withheld values | mcp/migrations/007_claims.sql:20-21, :36, mcp/src/store.rs:752-775, mcp/src/ingest.rs:257, :323-324, :481-482, mcp/src/claims.rs:200 | `status` is CHECK-constrained over `active`, `contradicted`, `superseded`, `rejected` and `quarantined`, indexed, and defaulting to `active`; `confidence` is a separate REAL column, which is the split the rubric asks for. The recall query selects both and filters on the first: the default clause is `AND c.status = 'active'`, and `include_contradicted` widens it to `IN ('active', 'contradicted')` — and only to that, so `superseded`, `rejected` and `quarantined` are unreachable through the flag rather than merely off by default. The ordering is the vector distance, so nothing about the state or the confidence enters the rank. All five values have writers: ingest sets `active` or `quarantined` depending on whether the claim's evidence was supported, and a verification verdict maps to reject or quarantine | the confidence is carried into the result alongside the status, so a caller sees how sure the store is about a claim it has already decided may be shown"
