...
revision: b82fd6c0f9939e8497fa7474999dd1bde5d0d88d
revision_url: https://github.com/Tencent/WeKnora/commit/b82fd6c0f9939e8497fa7474999dd1bde5d0d88d
analyzed_at: 2026-09-19
capabilities: "tombstone, trust_state, scope_enforced, human_review, negative_eval"
capability_evidence:
  tombstone: "the memory write path — a deleted statement is refused when distillation re-derives it | internal/application/service/memory/service.go:809-813 (write) and :342-357 (read) | `DeleteItem` calls `AddTombstone(scope, topic, MemoryFingerprint(content), sourceMessageID)` before removing the row, under a comment naming the failure it prevents: deleting a memory distillation is about to re-derive `is how a user ends up deleting the same thing twice and stops trusting the feature`. Every `Remember` then checks `HasTombstone(scope, MemoryFingerprint(content))` first and returns `ErrPreviouslyForgotten`. The key is a fingerprint of the value, not a row id. A second, looser check on the source message is gated to `MemoryOriginExtracted` only, because `an explicit remember this is the user asking again, and must always win`. Three writers reach the ledger, not one: `:809` on an item delete, `tombstoneTopic` at `:698` on a topic delete — which records a fingerprint for the topic label *and* each of its aliases, so re-deriving the same topic under a different name is refused too — and `:876` on a clear, which tombstones every item it removes | internal/application/service/memory/retrieval_test.go:88-104, service_test.go:455-472, quality_test.go:156"
  ...
  human_review: "the admission gate — a person confirms or rejects an inferred memory before it can be used | internal/router/routes_memory.go:30-31, internal/handler/memory.go:338-364 | `POST /memory/items/:id/confirm` and `POST /memory/items/:id/reject` are the adjudication surface, beside update, delete and topic promote. This is not post-hoc CRUD over memories already in use: a `pending` item is excluded from both injection queries, so confirmation is what admits it. Rejection routes to `DeleteItem`, which records the tombstone, so declining a guess is durable. The producer cannot stand in for the confirmer, checked against the tool registries rather than the docs: the agent's only memory tool is `NewSearchMemoryTool`, a read (`internal/agent/tools/search_memory.go:79`); the shipped MCP server declares twenty-nine tools and not one of them touches memory; `web_fetch` is a page reader with no POST; and `shell_exec` is documented as running in a session-scoped MicroVM that `never runs on the WeKnora host` | internal/application/service/memory/retrieval_test.go:88-104 exercises reject-then-re-extract"
...
**2026-09-19** — re-pinned to [`b82fd6c0f9939e8497fa7474999dd1bde5d0d88d`](https://github.com/Tencent/WeKnora/commit/b82fd6c0f9939e8497fa7474999dd1bde5d0d88d), 67 commits on. The compare API truncated its file list at 300, so the memory subsystem was compared by tree and blob instead: `internal/application/repository/memory.go`, `internal/types/memory.go`, `internal/handler/memory.go`, `internal/router/routes_memory.go` and `internal/agent/tools/search_memory.go` are byte-identical to the previous pin, and inside `internal/application/service/memory/` the only object that changed is `persistence_consistency_test.go`. Every anchor in the five evidence records was re-verified at the new pin and every one is exact. Two records gained what a first reading had missed. The tombstone ledger has three writers, not one — an item delete, a clear, and `tombstoneTopic`, which records a fingerprint for a deleted topic *and each of its aliases*. And `human_review` now carries its producer test: the agent's single memory tool is a read, the shipped MCP server's twenty-nine tools include no memory verb at all, `web_fetch` cannot POST, and `shell_exec` runs in a session MicroVM that never touches the host — so nothing on the producing side can reach `POST /memory/items/:id/confirm`. Screened again first; a dependency surface was inside the cooldown, so nothing was installed and no suite was run.
