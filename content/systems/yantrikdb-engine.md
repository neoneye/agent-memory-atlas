...
revision: 49c7aac1b78b74b296f7708a95d227073b4a666b
revision_url: https://github.com/yantrikos/yantrikdb/commit/49c7aac1b78b74b296f7708a95d227073b4a666b
analyzed_at: 2026-09-19
capabilities: "trust_state, bitemporal, audit_log, negative_eval"
capability_evidence:
  trust_state: "two stored discrete statuses on every memory, both typed beside the encrypted metadata so recall can fail closed without decrypting, and a tombstone excluded twice over | crates/yantrikdb-core/src/base/schema.rs:50 (`consolidation_status`), :121-134 (the synthesis CHECK), crates/yantrikdb-core/src/engine/thread.rs:34-42 (the stated read predicate), :604, :936, :967, crates/yantrikdb-core/src/engine/audit.rs:27-28, :87-88, :113-114, :136-137, :146-147, crates/yantrikdb-core/src/engine/lifecycle.rs:627-700 (the tombstone writers) | `consolidation_status` is active / consolidated / tombstoned and `synthesis_state` is constrained by a CHECK to verified / invalidated / unverified / superseded; the synthesis columns are declared to remain typed beside encrypted metadata so recall can fail closed without decrypting or joining the evidence graph on its hot path. The read predicate is stated in the thread module rather than left to be inferred, and it is three conditions rather than two: `consolidation_status = active` by default, the synthesis lifecycle gate written `synthesis_state IS NULL OR synthesis_state = verified` so a memory that was never synthesised is not caught by a gate that does not apply to it, and — under the v0.10 status read policy — not being the target of a selected active supersedes link. The same doc names the belt-and-braces: tombstoned rows are excluded twice over, by the status predicate and by `tombstone_inner` deleting their entity rows | crates/yantrikdb-core/src/engine/thread.rs:1609-1651"
...
**2026-09-19** — [`49c7aac1b78b74b296f7708a95d227073b4a666b`](https://github.com/yantrikos/yantrikdb/commit/49c7aac1b78b74b296f7708a95d227073b4a666b) — `trust_state` re-tested. Both columns are where they were and the typed-beside-encrypted-metadata reason still stands. Two corrections to how the record described the predicate. It is not a flat `synthesis_state = 'verified'` everywhere: the gate is written `synthesis_state IS NULL OR synthesis_state = 'verified'`, so a memory that was never synthesised is not caught by a gate that does not apply to it — the null branch is a deliberate not-applicable case rather than an oversight, and it appears at `engine/thread.rs:604`, `:936` and in the module's own statement of the rule. And the predicate is three conditions rather than two: the module doc (`thread.rs:34-42`) lists `consolidation_status = 'active'` as recall's default, the synthesis lifecycle gate, and — under the v0.10 status read policy — not being the target of a selected active `supersedes` link. The same paragraph names something this sweep has been rewarding elsewhere: tombstoned rows are *excluded twice over*, by the status predicate and by `tombstone_inner` deleting their `memory_entities` rows, so a read path that lost the predicate would still not find them through the entity index. Twenty-one sites mention the verified state in all; the record now points at the statement of the rule rather than only at the audit queries. Screened again first; nothing was installed and no suite was run.

**2026-09-16** — [`49c7aac1b78b74b296f7708a95d227073b4a666b`](https://github.com/yantrikos/yantrikdb/commit/49c7aac1b78b74b296f7708a95d227073b4a666b) — first reading of the engine repository in its own right, at a commit dated 14 September 2026. The atlas had previously read these crates only as a pinned dependency of [yantrikdb-server](../yantrikdb/) at tag `v0.22.0`, and in process behind the [Hermes plugin](../yantrikdb-hermes-plugin/). Screened before opening, from a shallow clone; a dependency surface had changed inside the seven-day cooldown. Nothing was installed, built or run.
