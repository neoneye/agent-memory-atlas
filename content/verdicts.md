...
**This page covers all 299 reports.**
...
- Biggest risk: neo `conflict` is still arithmetic. `scoreNeoRecallItem` returns `-Infinity` for `pruned`, `tombstoned` and `demoted`, but `conflict` is a 0.3 penalty, so a memory recorded as contradicting another ranks lower and can still reach the prompt, labelled. Epistemic `invalidated` withholds on a separate axis. `/plur1bus curation resolve` and `drop-injected` are the authorized exits; they do not auto-clear the herd.
...
- Maturity impression: MIT, release 7.4.0, ~69,000 lines of JavaScript with ~80,000 more across 340-plus test files — a test suite larger than the implementation, and the denials rather than the permissions are what it asserts. Against that, forty-seven configuration groups, fifteen background jobs, and a `postinstall` that patches the host OpenClaw's shipped code and is contractually unable to fail (`PLUR1BUS_SKIP_HOST_PATCH=1` skips apply; the patch remains).
...
- Do not copy when: you need multi-tenant guarantees on *legacy* derived rows. New dreams, episodes, edges and patterns stamp `visibility` and their readers filter on the requester; an unstamped row is visible only to the owning agent. There is no backfill.
