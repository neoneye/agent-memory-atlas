...
- Maturity impression: ~15,900 lines of TypeScript, 24 commits, seven packages, 204 tests, local Ollama, and a README that says it is not production-ready. Three capability marks — `trust_state`, `scope_enforced` and `negative_eval`, the last of them awarded only at the second reading. At the first, the single negative retrieval test asserted `results.every(m => m.status !== ARCHIVED)` against a database seeded with one archived row and nothing else, so it passed on an empty result — the vacuity this atlas keeps naming, with its own positive control sitting two tests above it in the same file. The repair was one line, and it landed.
...
- Do not copy when: you need a memory whose *contents* can be judged. It carries `audit_log` and `negative_eval` — a per-writer append-only event log over a content-addressed blob store, and committed cases that particular content must not be archived — and neither of the two marks is about belief. There is no status field, no scope key inside any record, and no human gate on memory content, so the archive can prove what happened to a blob and cannot say whether the blob is true.
...
- Maturity impression: 3,888 lines of Python, 46 commits since 5 July 2026, 1,435 lines of tests, none touching the playbook. No marks. Its scorecard records 183/183 levels and 25/25 games won at $654 — the same levels [Tycho](../systems/tycho/) solved for $2,986, with the 0.14 RHAE gap falling on action efficiency in two games out of twenty-five. Its README names Tycho as scoring higher and links a comparison document warning that *"cost methods and run-selection rules differ."*
...
- Fourth risk: **two node names reach Cypher unescaped.** `deprecate_relationship` builds `WHERE a['name'] = '{subject.name}'` by f-string on the same two lines where it passes the *labels* through a twelve-replacement sanitiser that strips the single quote. Names come from LLM extraction over user text, and fourteen other methods in the same file parameterise properly.
- Maturity impression: BUSL-1.1 with **Additional Use Grant: None** and a Change Date of 2030-08-13, so no production use is granted before then. 426 commits since 19 October 2025, 45,741 lines of Python under `src/` against 17,404 lines of tests in 585 cases, six datastores, an MCP server, a React console and a TUI installer. Three marks. Fourteen test skips, every one a `skipUnless` on an unavailable service rather than a behaviour excused.
...
- Biggest risk: **a status spent as a score.** `verification_status` is `unverified / verified / contested / retracted`, held apart from a `confidence` float and widened from a boolean because the boolean could not say *contested* — and the read path adds `-0.3` for retracted, `-0.15` for contested, then clamps. A grain a person retracted ranks lower and still surfaces. The one place it bites is the training export, which labels it `"quality": "rejected", "loss_weight": 0.0` and keeps the record. `trust_state` is withheld on exactly that, and the distance to the mark is one predicate.
- Second risk: **the key a refusal needs is computed and not consulted.** The store is content-addressed, so the hash is a function of the content and every `OP_FORGET` row already holds one. Nothing checks it on write, and `forget`'s own comment anticipates the case — *"a forget + re-add of identical content can move this hash to a new seq"* — handling the race rather than refusing the re-assertion. `tombstone` withheld one lookup short.
- Maturity impression: MIT OR Apache-2.0, 181,665 lines of Rust across seventeen crates, 242 commits since 16 August 2026, 2,522 test functions, a fuzz target and a `deny.toml`. Five marks. The conformance kit is the artifact to copy: every case takes `b: &dyn Backend` and reports `b.name()`, so the same assertions run against the real store, Postgres and an in-memory reference substrate — *"engine CI runs the full suite against it with zero Areev, so the portability claim stays testable."*
...
- Do not copy when: you read five marks as a summary. What is governed extremely well is the *process* by which memory changes — who proposed, who approved, on what evidence, how to undo. The two missing marks are the two about belief, and the gap is that nothing stops a retracted memory reaching the model on the next turn.
