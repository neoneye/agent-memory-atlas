...
- Maturity impression: MIT, release 7.2.6, ~64,000 lines of JavaScript with ~70,000 more across 317 test files — a test suite larger than the implementation, and the denials rather than the permissions are what it asserts. Its regression tests pin arithmetic to numbers on both sides of an admission threshold, and its own commit records verifying that they fail without the fix. Against that, forty-seven configuration groups, fifteen background jobs, and a `postinstall` that patches the host OpenClaw's shipped code and is contractually unable to fail.
...
- Maturity impression: AGPL-3.0 with a CLA held for possible commercial dual-licensing, around 1,760 lines across eight files, no test suite, self-described as a research prototype that "isn't all that yet". It publishes four scores under 31% on its README with a diagnosis of each — and they describe a prior version, since three of their inputs changed at v0.2.3 while the README did not. Correction reaches five named functional predicates, of which one is ever produced by the extractor's normaliser. `talon_engine.py`, absent from the README's file list, disables a HuggingFace `torch.load` safety check to load a remote checkpoint.
