...
- Second risk: **six of twenty-one architecture boxes ship code.** `coverage.py` prints the split itself and adds *"ADDRESSED IS NOT IMPLEMENTED"* — thirteen rows are build prompts, and one of them is stage 10, the reinforce / supersede / archive lifecycle. Every correction mechanism this atlas measures is a prompt here.
...
- Do not copy when: you need memory to be corrected rather than edited. What you get is a folder of notes with a good search over it and no lifecycle at all.
