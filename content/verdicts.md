...
**This page covers all 295 reports.**
...
- Maturity impression: MIT, release v0.3.0 (PyPI `my-agent-mesh`), 24,653 lines of Python with **no third-party dependencies**, eight commits from a curated publish, and now a four-test public contract suite with CI where v0.2.0 had none. v0.3.0 fixed all three of the report's former central findings — the empty verification apparatus (now writable for three of five fields), the poison-event store corruption (now validated at append), and the `shell=True` runner (now argv/`shell=False`).
...
- Do not copy when: you need memory an agent actually receives. Nothing is retrieved automatically, `decisions search` is an unranked substring scan that never reads the decision body, and while verification is now writable the `assumptions` and `evidence` fields still have no producer.
...
- Most reusable component: the fix for its own scope bug, as a worked example. This report previously found `handleCapture` reading `args.session_key ?? defaultSessionKey()` while `handleRecall`, in the same file, read `args.session_key as string | undefined` — so a recall that omitted the argument searched every project, while a committed two-project test passed because the test defined its own `recall` helper that supplied the default the real handler omitted. The handler now defaults it, and a third test asserts the property **against the real handler**: *"recall without session_key does NOT leak across projects (real handler)"*. Two tests of the same property, one of which could never have failed.
- Maturity impression: MIT, ~22,600 lines of TypeScript over SQLite with FTS5, `sqlite-vec` and a local MiniLM, 503 test cases across 30 files, schema at version 8. The project renamed itself from `tdai-memory-mcp`, and the old name was rebuilt from scratch as a three-commit stub that redirects users — the two repositories share no common ancestor, so the old URL is a live repository that is not this project. Four marks, up from one at the previous reading.
