...
**This page covers all 283 reports.**
...
- Do not copy when: you cannot produce a trustworthy outcome signal — the design degrades to self-grading if every outcome is `machine`. Also walk away if you need contradiction *resolved* rather than flagged, or a rejected-value tombstone that keeps a bad lesson gone; the near-duplicate dedup and quarantine both check stored lessons, not rejected ones, so continuous re-distillation can resurrect a suppressed lesson.
